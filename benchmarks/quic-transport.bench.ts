...
class BenchmarkRunner {
    ...
    for (const agentCount of config.agentCounts) {
      for (const messageSize of config.messageSizes) {
        for (const latency of config.latencies) {
          ...
          // Run QUIC benchmark
          await this.runProtocolBenchmark(
            'QUIC',
            agentCount,
            messageSize,
            latency,
  ...
  private async runProtocolBenchmark(
    ...
    try {
      ...
      const connTime = await LatencySimulator.simulate(latency, () =>
        benchmark.benchmarkConnectionEstablishment(agentCount)
      );

      // Message throughput
      const throughput = await LatencySimulator.simulate(latency, () =>
  ...
  private printResults(): void {
    ...
    for (const [scenario, results] of Object.entries(grouped)) {
      ...
      if (quic && http2) {
        const improvement =
          ((http2.metrics.connectionTime - quic.metrics.connectionTime) /
            http2.metrics.connectionTime) *
          100;
      ...
      if (quic && ws) {
        const improvement =
          ((ws.metrics.connectionTime - quic.metrics.connectionTime) / ws.metrics.connectionTime) *
          100;
  ...
  private async exportResults(): Promise<void> {
    ...
    await fs.writeFile(
      '/workspaces/agent-control-plane/docs/benchmarks/results.json',
...
export { BenchmarkRunner, HTTP2Benchmark, QUICBenchmark, WebSocketBenchmark };
