 ...
 *
 * Spawns concurrent documentation agents using agent-control-plane Task tool
...
npx gendev@alpha hooks session-restore --session-id "${CONFIG.sessionId}"
...
npx gendev@alpha hooks post-edit --file "${spec.file}" --memory-key "swarm/${agentType}/progress"

# Notify other agents
npx gendev@alpha hooks notify --message "${agentType}: Completed section X"
\`\`\`

### After Completion:
\`\`\`bash
# Store final document info
npx gendev@alpha hooks post-edit --file "${spec.file}" --memory-key "swarm/${agentType}/complete"

# Mark task complete
npx gendev@alpha hooks post-task --task-id "${CONFIG.sessionId}-${agentType}"
\`\`\`
...
## Output Location
Write to: /home/user/agent-control-plane/nova-medicina/docs/${spec.file}
...
OPTION 2: Automated Execution (Requires agent-control-plane runtime)
--------------------------------------------------------------
If you have agent-control-plane runtime installed:

  npx agent-control-plane execute --manifest .swarm-memory/manifest.json
...
async function main() {
  ...
  executeCommand(
    `npx gendev@alpha hooks pre-task --description "Documentation swarm initialization"`,
    'Pre-task hook'
  );
