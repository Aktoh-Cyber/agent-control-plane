...
```bash
npx gendev@alpha hooks session-restore --session-id "docswarm-xxx"
```

### During Work

```bash
npx gendev@alpha hooks post-edit --file "filename.md" --memory-key "swarm/agent/progress"
npx gendev@alpha hooks notify --message "agent: Completed section X"
```

### After Completion

```bash
npx gendev@alpha hooks post-task --task-id "docswarm-xxx-agent"
```
...
All documentation will be generated in `/home/user/agent-control-plane/nova-medicina/docs/`:
...
**Framework**: agent-control-plane with gendev hooks
