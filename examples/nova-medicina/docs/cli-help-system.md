...
**Main Implementation:** `/home/user/agent-control-plane/nova-medicina/src/cli/help-system.ts`
...
program.command('analyze').addHelpText('beforeAll', () => {
  helpSystem.showAnalyzeHelp();
  return '';
});
```
...
```bash
npx gendev@alpha hooks pre-task --description "cli help system"
```

### Post-Task Hook

```bash
npx gendev@alpha hooks post-task --task-id "help-001"
```
...
npx gendev@alpha hooks post-edit \
  --file "/home/user/agent-control-plane/nova-medicina/src/cli/help-system.ts" \
