  ...
  try {
    ...
    const complexCase = {
      ...
      patientContext: {
        age: 58,
        gender: 'male',
        medicalHistory: ['hypertension', 'type 2 diabetes', 'COPD', 'previous MI (2 years ago)'],
        medications: [
        ...
        familyHistory: ['father: heart disease, diabetes', 'mother: autoimmune disorder'],
        vitalSigns: {
          temperature: 38.2,
          heartRate: 108,
...
async function example2_ProviderEscalation() {
  ...
  try {
    ...
    const escalationCriteria = {
      lowConfidence: analysis.confidenceScore.overall < 0.7,
      multipleDifferentials: analysis.differentialDiagnoses.length > 3,
      seriousCondition: analysis.diagnosis.severity === 'serious',
      rapidProgression:
        uncertainCase.patientContext.progression === 'rapid' ||
        uncertainCase.patientContext.progression === 'gradual worsening',
      neurologicalSymptoms: uncertainCase.symptoms.some(
        (s) => s.includes('weakness') || s.includes('numbness') || s.includes('vision')
      ),
    };
    ...
    if (urgencyLevel >= 4) {
      escalationPath = {
        ...
        message:
          'Urgent neurological consultation needed - possible progressive neuromuscular disorder',
      };
      ...
      escalationPath.specialists.map(async (specialization) => {
        ...
        if (specialist) {
          ...
          const notification = await providerService.notifyProvider({
            ...
            metadata: {
              escalationReason: Object.keys(escalationCriteria).filter(
                (k) => escalationCriteria[k]
              ),
...
async function example3_LearningFromFeedback() {
  ...
  try {
    ...
    console.log(
      `  Approved: ${learningMetrics.approved} (${((learningMetrics.approved / learningMetrics.totalCases) * 100).toFixed(1)}%)`
    );
    console.log(
      `  Modified: ${learningMetrics.modified} (${((learningMetrics.modified / learningMetrics.totalCases) * 100).toFixed(1)}%)`
    );
    console.log(
      `  Rejected: ${learningMetrics.rejected} (${((learningMetrics.rejected / learningMetrics.totalCases) * 100).toFixed(1)}%)`
    );
    console.log();
    ...
    console.log(
      `  Provider approval rate: ${(updatedMetrics.providerReviews.approvalRate * 100).toFixed(1)}%`
    );
