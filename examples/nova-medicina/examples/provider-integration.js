...
async function example1_ProviderDashboard() {
  ...
  try {
    ...
    console.log(
      ...
      registeredProvider.notificationPreferences.channels.join(', ')
    );
    console.log();
...
async function example2_NotificationConfig() {
  ...
  try {
    ...
    const testCases = [
      ...
      { confidence: 0.8, priority: 'high', expected: 'ROUTE TO PROVIDER' },
      { confidence: 0.65, priority: 'medium', expected: 'FILTERED OUT' },
      { confidence: 0.4, priority: 'urgent', expected: 'ROUTE TO PROVIDER (URGENT)' },
    ];
...
async function example3_ReviewWorkflow() {
  ...
  try {
    ...
    console.log(
      '  New confidence:',
      (modifiedReview.modifications.diagnosis.confidence * 100).toFixed(1) + '%'
    );
    console.log(
      '  Additional recommendations:',
      modifiedReview.modifications.recommendations.length
    );
    console.log();
  ...
  try {
    ...
    if (reviewStatus.consensusReached) {
      ...
      reviewStatus.consensusRecommendations.forEach((rec, idx) => {
        ...
        console.log(
          `     Agreed by: ${rec.agreementCount}/${reviewStatus.totalProviders} providers`
        );
      });
    }
...
async function example5_ProviderAnalytics() {
  ...
  try {
    ...
    console.log(
      '  Modified:',
      analytics.modified,
      `(${(analytics.modificationRate * 100).toFixed(1)}%)`
    );
    console.log(
      '  Rejected:',
      analytics.rejected,
