...
async function example2_verificationPipeline() {
  ...
  const goodInput = {
    claim:
      'Evidence from multiple randomized controlled trials suggests that statins may reduce cardiovascular events in high-risk patients',
    citations: [
      {
        id: 'cit1',
        type: 'meta-analysis',
  ...
  const badInput = {
    claim:
      'This treatment always cures cancer permanently with 150% success rate and absolutely zero side effects',
    citations: [],
  };
...
async function example4_logicalPatterns() {
  ...
  const circularText =
    'A is effective because it works well. It works well because it is effective. Therefore, A is effective.';
  ...
  const contradictoryText =
    'The medication always works in all patients. The medication never works in any patient.';
...
async function example6_completeSystem() {
  ...
  const input = {
    claim:
      'Recent systematic reviews and meta-analyses suggest that aspirin therapy may provide cardiovascular benefit in selected high-risk populations',
    citations: [
      {
        id: 'cit1',
        type: 'meta-analysis',
