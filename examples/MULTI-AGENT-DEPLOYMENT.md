...
This example demonstrates advanced GenDev MCP capabilities for deploying and orchestrating multi-agent swarms with shared memory coordination.
...
- **Store**: `mcp__gendev__memory_usage` with `action: 'store'`
- **Retrieve**: `mcp__gendev__memory_usage` with `action: 'retrieve'`
- **List**: `mcp__gendev__memory_usage` with `action: 'list'`
- **Search**: `mcp__gendev__memory_search` with pattern matching
...
const status = await client.messages.create({
  ...
  tools: [{ name: 'mcp__gendev__swarm_status' }],
});
...
const metrics = await client.messages.create({
  ...
  tools: [{ name: 'mcp__gendev__agent_metrics' }],
});
```
...
### Method 2: Using agent-control-plane CLI
...
| ------------------------------- | ------------------------- | -------------------------------------------- |
| `mcp__gendev__swarm_init`       | Initialize swarm topology | `topology`, `maxAgents`, `strategy`          |
| `mcp__gendev__agent_spawn`      | Create specialized agents | `type`, `capabilities`, `name`               |
| `mcp__gendev__memory_usage`     | Store/retrieve memory     | `action`, `namespace`, `key`, `value`, `ttl` |
| `mcp__gendev__task_orchestrate` | Coordinate task execution | `task`, `strategy`, `priority`               |
| `mcp__gendev__swarm_status`     | Monitor swarm health      | `swarmId`                                    |
| `mcp__gendev__agent_metrics`    | Get performance data      | `agentId`                                    |
...
npx agent-control-plane mcp status gendev

# Restart if needed
npx agent-control-plane mcp start gendev
```
...
4. **Monitor**: Set up persistent logging with `mcp__gendev__performance_report`
5. **Automate**: Create workflow templates with `mcp__gendev__workflow_create`
...
**Built with** agent-control-plane v1.2.7 | GenDev MCP | Claude Agent SDK
