...
This API is integrated with gendev's ReasoningBank for coordination:

```bash
# Pre-task hook
npx gendev@alpha hooks pre-task --description "API implementation"

# Post-edit hooks
npx gendev@alpha hooks post-edit --file "crates/climate-api/src/main.rs" --memory-key "climate/api/server"

# Notify coordination
npx gendev@alpha hooks notify --message "API server ready"
```
