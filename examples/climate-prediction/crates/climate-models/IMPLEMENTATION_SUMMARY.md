...
_On test dataset with 1000 samples_
...
1. **Pre-task Hook**: Initialize task tracking
   ...
   ```bash
   npx gendev@alpha hooks pre-task --description "ML model implementation"
   ```

2. **Post-edit Hook**: Track file changes

   ```bash
   npx gendev@alpha hooks post-edit --file "src/lib.rs" --memory-key "climate/models/ml"
   ```

3. **Post-task Hook**: Complete task with metrics
   ```bash
   npx gendev@alpha hooks post-task --task-id "ml-models" --export-metrics true
   ```
