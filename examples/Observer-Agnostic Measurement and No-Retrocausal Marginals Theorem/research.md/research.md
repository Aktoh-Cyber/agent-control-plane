...
> _Does who or what observes a quantum system change the outcome?_
...
- A bipartite delayed-choice or eraser setup uses (S) and an idler (I) with joint state (\rho\_{SI}). Measurements on (I) use POVMs ({F^{(\mu)}\_z}) with control (\mu).
...
2. **No-Retrocausal Marginals in Delayed-Choice/Eraser.**
   For any (\rho\_{SI}) and any measurement families ({E^{(\lambda)}\_y}), ({F^{(\mu)}_z}), the **singles** distribution on (S)
   [
   p(y \mid \lambda) ;=; \sum_{z}\mathrm{Tr}!\left((E^{(\lambda)}\_y \otimes F^{(\mu)}_z),\rho_{SI}\right)
   ]
...
Linearity and the Born rule give 1): (p(y\mid\lambda)) depends only on (\rho*S) and (E^{(\lambda)}\_y). The apparatus state (\rho_O) factors out when the control map (\lambda) is fixed at the classical level. For 2): trace over (I) to get (\rho_S=\mathrm{Tr}\_I(\rho*{SI})). Since (\sum_z F^{(\mu)}\_z = I_I) for every (\mu), the singles distribution on (S) reduces to (\mathrm{Tr}!\left(E^{(\lambda)}\_y \rho_S\right)), which is (\mu)-independent. Both statements hold regardless of whether (O) is a human, robot, RNG, or timer.
