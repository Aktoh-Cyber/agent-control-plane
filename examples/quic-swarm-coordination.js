 ...
 *
 * QUIC approach with agent-control-plane:
...
async function liveStateSynchronization() {
  ...
  agents.forEach((agent) => {
    console.log(
      `   ${agent.id}: ${agent.progress.toFixed(0)}% complete, ${agent.conflicts.length} conflicts`
    );
  });
...
async function main() {
  ...
  try {
    ...
    console.log('   Start QUIC server first: npx agent-control-plane quic');
  }
}
