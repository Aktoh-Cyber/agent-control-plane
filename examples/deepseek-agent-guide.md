...
npx gendev agent run --agent coder --model deepseek/deepseek-chat --task "Task 1" &
npx gendev agent run --agent tester --model deepseek/deepseek-chat --task "Task 2" &
wait
```
...
CMD ["sh", "-c", "npx gendev proxy start --daemon && \
     npx gendev agent run --agent coder --model $MODEL --task \"$TASK\""]
```
...
services:
  proxy:
    ...
    command: npx gendev proxy start
    environment:
  ...
  agent:
    ...
    command: npx gendev agent run --agent coder --model deepseek/deepseek-chat
```
...
npx gendev models --provider openrouter
...
- **Agent List**: `npx gendev agent list`
...
1. Check logs: `npx gendev proxy logs`
2. Verify configuration: `npx gendev proxy config`
3. Test connectivity: `npx gendev proxy status`
4. GitHub Issues: https://github.com/Aktoh-Cyber/agent-control-plane/issues
