...
export async function executeGoalBasedResearch(goal, options = {}) {
  ...
  console.log(
    chalk.bold.cyan('║          GOAL-ORIENTED RESEARCH (GOALIE + SWARM)                   ║')
  );
  console.log(
    chalk.bold.cyan('╚═══════════════════════════════════════════════════════════════════╝\n')
  );
  ...
  console.log(
    chalk.bold.cyan('║                GOAL-ORIENTED RESEARCH COMPLETE                     ║')
  );
  console.log(
    chalk.bold.cyan('╚═══════════════════════════════════════════════════════════════════╝\n')
  );
  ...
  console.log(
    chalk.dim(`   Sub-goals completed: ${results.successfulSubGoals}/${subGoals.length}`)
  );
  console.log(chalk.dim(`   Total agents deployed: ${results.totalAgents}`));
  console.log(
    chalk.dim(
      `   Success rate: ${((results.successfulSubGoals / subGoals.length) * 100).toFixed(1)}%\n`
    )
  );
...
export async function explainGoalPlan(goal) {
  return new Promise((resolve, reject) => {
    const child = spawn('npx', ['goalie', 'explain', goal], { stdio: 'pipe' });
