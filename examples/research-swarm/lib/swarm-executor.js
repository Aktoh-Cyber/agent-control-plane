...
export async function executeSwarm(agents, options = {}) {
  const { maxConcurrent = 4, verbose = false } = options;

  console.log(
    chalk.bold.cyan('\n╔═══════════════════════════════════════════════════════════════════╗')
  );
  console.log(
    chalk.bold.cyan('║                    RESEARCH SWARM ACTIVATED                        ║')
  );
  console.log(
    chalk.bold.cyan('╚═══════════════════════════════════════════════════════════════════╝\n')
  );
  ...
  agents.forEach((agent, idx) => {
    ...
    console.log(
      chalk.gray(
        `   ${roleEmoji} ${idx + 1}. ${agent.config.role.toUpperCase().replace('-', ' ')} (Priority ${agent.config.priority})`
      )
    );
  });
  console.log();
  ...
  try {
    ...
    console.log(
      chalk.bold(
        `📊 Results: ${chalk.green(results.successful)}/${results.total} successful (${successRate}%)\n`
      )
    );
...
function executeAgent(agent, verbose) {
  return new Promise((resolve) => {
    ...
    child.on('close', (code) => {
      ...
      if (code === 0) {
        spinner.succeed(
          chalk.green(`${roleEmoji} ${agent.config.role}: Complete (${jobId?.substring(0, 8)})`)
        );
        resolve({
          agent: agent.agent,
          role: agent.config.role,
