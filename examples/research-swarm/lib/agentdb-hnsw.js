 ...
 * - Uses ReasoningBank WASM bindings from agent-control-plane
 */
 ...
 * 2. HybridReasoningBank requires full agent-control-plane framework integration
...
export async function initializeHNSWIndex(options = {}) {
  const { M = 16, efConstruction = 200, maxLayers = 5 } = options;
...
export async function buildHNSWGraph(options = {}) {
  ...
  return new Promise((resolve, reject) => {
    const args = [
      ...
      'hnsw-build',
      '--db-path',
      '/workspaces/agent-control-plane/examples/research-swarm/data/research-jobs.db',
      '--batch-size',
      batchSize.toString(),
    ];
