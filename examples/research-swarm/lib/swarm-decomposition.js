  ...
  if (actualSwarmSize >= 3) {
    agents.push({
      ...
      config: {
        ...
        timeMinutes: Math.floor(timePerAgent * 0.2),
        focus: 'broad',
        enableED2551,
        antiHallucination,
        role: 'explorer',
  ...
  if (actualSwarmSize >= 3) {
    agents.push({
      ...
      config: {
        ...
        timeMinutes: Math.floor(timePerAgent * 0.3),
        focus: 'narrow',
        enableED2551,
        antiHallucination,
        role: 'depth-analyst',
  ...
  if (actualSwarmSize >= 7) {
    agents.push({
      ...
      config: {
        ...
        timeMinutes: Math.floor(timePerAgent * 0.1),
        focus: 'narrow',
        enableED2551,
        antiHallucination: 'high',
        role: 'critic',
...
export function validateSwarmConfig(agents) {
  ...
  if (totalTime > 600) {
    warnings.push(
      `Total time budget (${totalTime}min) is high. Consider reducing depth or swarm size.`
    );
  }
  ...
  if (!priorities.includes(3)) {
    warnings.push(
      'No agents with priority 3 (synthesis phase). Results may not be properly combined.'
    );
  }
