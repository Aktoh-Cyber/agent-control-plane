...
async function createJobRecord() {
  ...
  try {
    const { error: insertError } = await supabase.from('permit_research_jobs').insert({
      id: JOB_ID,
      agent: agentName,
      task: task,
...
async function markComplete(exitCode, executionLog = '', reportPath = null) {
  ...
  try {
    ...
    if (reportPath && fs.existsSync(reportPath)) {
      ...
      console.log(
        `✅ Report cleaned: ${fullReport.length} characters (removed ${rawReport.length - fullReport.length} chars of metadata)`
      );
    } else {
      ...
      if (firstIndex === -1) {
        ...
        if (secondIndex === -1) {
          ...
          console.log(
            `✅ Extracted clean report between separators: ${fullReport.length} characters`
          );
        }
      }
...
async function main() {
  ...
  try {
    ...
    const originalScript = path.join(__dirname, 'run-researcher-agent-control-plane-optimized.js');
    ...
    // The agent script (run-researcher-agent-control-plane-optimized.js) now handles ALL progress updates
    ...
    agentProcess.stderr.on('data', async (data) => {
      ...
      // Parse tool calls from agent-control-plane v1.8.11+ streaming output
      ...
      if (toolMatch && hasSupabase) {
        ...
        if (estimatedProgress > lastWrittenProgress) {
          ...
          await updateProgress(estimatedProgress, `Executing ${toolName.trim()}`, {
    ...
    if (fs.existsSync(OUTPUT_DIR)) {
      ...
      if (descriptiveFiles.length > 0) {
        ...
        console.log(
          `✅ Full report file found (descriptive, NOT JOB_ID): ${descriptiveFiles[0].name} (${descriptiveFiles[0].size} bytes)`
        );
      }
    }
    ...
    if (!reportPath) {
      ...
      if (fs.existsSync(reportsDir)) {
        ...
        if (files.length > 0) {
          ...
          console.log(
            `✅ Full report file found (descriptive, NOT JOB_ID): ${files[0].name} (${files[0].size} bytes)`
          );
        }
      }
    }
    ...
    if (!reportPath && fs.existsSync(OUTPUT_DIR)) {
      console.log(
        `   Fallback: Looking for ${agentName} files created after ${new Date(startTime).toISOString()}`
      );
      const agentFiles = fs
        .readdirSync(OUTPUT_DIR)
        .filter((f) => {
      ...
      if (agentFiles.length > 0) {
        ...
        console.log(
          `✅ Full report file found by agent name: ${agentFiles[0].name} (${agentFiles[0].size} bytes)`
        );
      }
    }
    ...
    if (!reportPath) {
      ...
      if (fs.existsSync(reportsDir)) {
        ...
        if (jobIdFiles.length > 0) {
          ...
          console.log(
            `⚠️  Using markdown JOB_ID file (may need extra cleaning): ${jobIdFiles[0].name} (${jobIdFiles[0].size} bytes)`
          );
        }
      }
    }
