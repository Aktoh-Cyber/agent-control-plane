 ...
 *
 * Usage:
 *   node agents/scripts/run-researcher-agent-control-plane-optimized.js co2-beverage-permit-researcher "address"
...
function buildOptimizedEnvironment(options) {
  const env = {
    ...
    VALIDATION_MODELS:
      process.env.VALIDATION_MODELS || 'claude-sonnet-4-20250514,gemini-2.0-flash-exp',
...
async function prestartMCPServers(env) {
  ...
  const prestartServers = mcpConfig.prestart?.servers || ['gendev', 'agentic-cloud'];
  ...
  console.log(
    `✅ MCP servers ready: ${geminiReady ? 'Gemini' : 'WebSearch'}, ${otherServers.join(', ')}\n`
  );
...
/**
 * Execute agent-control-plane CLI with full optimization
 */
  ...
  try {
    return await new Promise((resolve, reject) => {
      ...
      const cliArgs = [
        ...
        options.provider,
        '--output',
        'json', // CRITICAL: Force JSON output for clean extraction
      ];
      ...
      // Progress is now handled by the agent script itself (run-researcher-agent-control-plane-optimized.js lines 797-827)
      ...
      child.on('close', (code) => {
        ...
        if (code === 0) {
          ...
          reject(new Error(`agent-control-plane exited with code ${code}\n${errorOutput}`));
        }
      });

      // Handle errors
      child.on('error', (error) => {
        reject(new Error(`Failed to spawn agent-control-plane: ${error.message}`));
      });
    });
  } catch (error) {
...
async function markComplete(exitCode, files, resultData) {
  ...
  try {
    ...
    const uploadProcess = spawn(
      'node',
      [path.join(__dirname, 'upload-report.js'), files.markdown],
      {
        env: process.env,
        stdio: 'inherit',
      }
...
async function main() {
  ...
  const options = {
    ...
    provider:
      args.find((a) => a.startsWith('--provider='))?.split('=')[1] || DEFAULT_CONFIG.provider,
...
USAGE:
  node agents/scripts/run-researcher-agent-control-plane-optimized.js <agent-name> "<task>" [options]
...
5. Check MCP servers: npx gendev@alpha mcp start
...
function extractMCPTools(output) {
  const tools = [];
  if (output.includes('gendev') || output.includes('Claude Flow')) tools.push('gendev');
  if (output.includes('agentic-cloud') || output.includes('Flow Nexus'))
    tools.push('agentic-cloud');
  if (output.includes('gemini-search') || output.includes('Gemini Search'))
    tools.push('gemini-search');
