...
async function executeGoalie(goal) {
  return new Promise((resolve, reject) => {
    const child = spawn('npx', ['goalie', 'search', goal, '--output', 'json'], { stdio: 'pipe' });
  ...
  return new Promise((resolve) => {
    const child = spawn(
      ...
      [
        ...
        subGoal,
        '--swarm-size',
        '3', // Use minimal swarm for sub-goals
      ],
      { stdio: 'inherit' }
    );
...
- Research-Swarm Issues: https://github.com/Aktoh-Cyber/agent-control-plane/issues
