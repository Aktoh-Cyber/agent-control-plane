...
Instead of creating custom Supabase adapters, **leverage the existing agent-control-plane Supabase integration** that's already built into the system. This provides:
...
### From agent-control-plane@1.9.1:

1. **`SupabaseFederationAdapter`** (`/agent-control-plane/src/federation/integrations/supabase-adapter.ts`)
...
2. **Federation System** (`/agent-control-plane/src/federation/`)
   - ✅ Multi-tenant support
   - ✅ Agent coordination
   - ✅ Cross-session memory sharing

3. **AgentDB** (`/agent-control-plane/src/agentdb/`)
...
Create `/workspaces/agent-control-plane/examples/research-swarm/lib/permit-platform-adapter.js`:

```javascript
/**
 * Permit Platform Integration Adapter
 *
 * Uses agent-control-plane's built-in Supabase federation adapter
 * to sync research-swarm jobs with permit platform database.
 */

import { SupabaseFederationAdapter } from 'agent-control-plane/dist/federation/integrations/supabase-adapter.js';
...
export class PermitPlatformAdapter {
  constructor(config) {
    ...
    // Use agent-control-plane's built-in Supabase adapter
  ...
  async ensurePermitTables() {
    ...
    const { error } = await this.supabase.client.from('permit_research_jobs').select('id').limit(1);
  ...
  async createJob(jobData) {
    const { id, agent, task, config, userId, agentType } = jobData;
    ...
    try {
      const { error } = await this.supabase.client.from('permit_research_jobs').insert({
        id: localJobId,
      ...
      await this.supabase.registerSession(localJobId, this.tenantId, agent, {
        jobType: 'research',
        permitPlatform: true,
        config,
      });
  ...
  async syncPendingUpdates() {
    ...
    for (const job of recentJobs) {
      await this.updateProgress(job.id, job.progress, job.current_message, {
        status: job.status,
  ...
  async findSimilarJobs(taskEmbedding, limit = 5) {
    try {
      const memories = await this.supabase.semanticSearch(taskEmbedding, this.tenantId, limit);
...
Create `/workspaces/agent-control-plane/examples/research-swarm/schema/supabase/permit_tables.sql`:
...
-- Permit research jobs table (extends agent-control-plane federation schema)
...
Create `/workspaces/agent-control-plane/examples/research-swarm/run-permit-researcher.js`:
 ...
 *
 * Uses agent-control-plane's built-in Supabase integration
...
`);
  ...
  try {
    ...
    if (CONFIG.enableGoalie) {
      ...
      await adapter.updateProgress(jobId, 15, `Spawned ${swarmAgents.length}-agent swarm`);
...
```bash
# Supabase (uses agent-control-plane built-in adapter)
...
1. **No Custom Code** - Leverage agent-control-plane's existing adapter
...
6. **Tested & Maintained** - Part of core agent-control-plane
...
Instead of building custom adapters, this approach **leverages agent-control-plane's built-in Supabase integration**, providing:

- ✅ **AgentDB + Supabase Hybrid** - SQLite for speed, Supabase for persistence
- ✅ **Federation Support** - Multi-tenant architecture built-in
- ✅ **Real-time Subscriptions** - Automatic frontend updates
- ✅ **Vector Search** - pgvector semantic search
- ✅ **Zero Custom Maintenance** - Uses core agent-control-plane components
...
| **Testing**          | You test adapter          | Already tested in agent-control-plane |
...
# Verify agent-control-plane federation tables exist
...
# If missing, run agent-control-plane federation schema:
psql $DATABASE_URL -f node_modules/agent-control-plane/schema/federation-schema.sql
```
...
✅ **Use agent-control-plane's built-in Supabase adapter** - Proven, tested, maintained
