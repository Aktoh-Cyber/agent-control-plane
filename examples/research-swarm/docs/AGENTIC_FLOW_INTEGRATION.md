...
**Version**: 1.1.0+
**Status**: Supported
**Integration Type**: Hybrid - Mix research-swarm agents with agent-control-plane agents

---

## 🎯 Overview

Research-swarm can leverage the full ecosystem of **66+ agent-control-plane agents** to enhance research capabilities with specialized expertise.
...
Add agent-control-plane agents as custom roles in swarm decomposition.
...
export function decomposeTaskWithAgenticFlow(task, options = {}) {
  ...
  // Add agent-control-plane specialized agents
  const customAgents = [
    {
      id: 'backend-specialist',
      agent: 'backend-dev', // agent-control-plane agent
    ...
    {
      id: 'security-auditor',
      agent: 'reviewer', // agent-control-plane security reviewer
      task: `Security analysis for: ${task}`,
      config: {
        depth: options.depth,
        timeMinutes: Math.floor(options.timeMinutes * 0.1),
        focus: 'narrow',
    ...
    {
      id: 'ml-specialist',
      agent: 'ml-developer', // agent-control-plane ML agent
...
Use agent-control-plane MCP tools alongside research-swarm.
...
// Step 2: Code generation phase (agent-control-plane)
// Use MCP tool: mcp__agent-control-plane__agentic_flow_agent
const codeTask = {
  agent: 'coder',
...
Modify the swarm executor to support agent-control-plane agents directly.
...
export async function executeSwarmWithAgenticFlow(agents, options = {}) {
  // Separate research-swarm and agent-control-plane agents
  ...
  // Execute agent-control-plane agents
  const agenticFlowResults = await executeAgenticFlowAgents(agenticFlowAgents, options);
...
async function executeAgenticFlowAgent(agent, options) {
  return new Promise((resolve) => {
    // Use agent-control-plane CLI or MCP tool
    const args = [
      'npx',
      'agent-control-plane',
      'agent',
...
- ✅ agent-control-plane installed: `npm install -g agent-control-plane`
...
2. **Identify agents needed**:
   ...
   ```bash
   # List all agent-control-plane agents
...
4. **Test integration**:
   - Run with single agent-control-plane agent first
...
**Example** (5 research + 3 agent-control-plane agents):
...
# + 1-2 agent-control-plane agents for domain expertise
...
async function hybridResearch(task) {
  ...
  console.log('\n💻 Phase 2: Code generation with agent-control-plane...\n');
...
// 30-minute budget example
{
  ...
  expertise: 30% (9min),  // agent-control-plane domain agents
  implementation: 20% (6min), // agent-control-plane code generation
...
- Start with minimal swarm (3 research + 1-2 agent-control-plane)
...
- Main repo: https://github.com/Aktoh-Cyber/agent-control-plane
- Agent list: `npx agent-control-plane agent list`
- MCP documentation: See agent-control-plane README

---

## ✅ Summary

**Yes, you can integrate agent-control-plane agents!**
...
2. Add agent-control-plane agents for specialized expertise
...
**Questions?** Open an issue: https://github.com/Aktoh-Cyber/agent-control-plane/issues
