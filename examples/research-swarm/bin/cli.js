...
import { getDatabase, getJobs, getJobStatus, initDatabase } from '../lib/db-utils.js';
    ...
    try {
      ...
      if (jobs.length === 0) {
        ...
        jobs.forEach((job) => {
          ...
          console.log(
            `${statusIcon} ${chalk.dim(job.id.substring(0, 8))} | ${chalk.cyan(job.agent)} | ${chalk.bold(job.progress + '%')} | ${statusColor(job.status)}`
          );
...
program
  ...
  .action((jobId) => {
    try {
      ...
      console.log(
        chalk.bold('📊 Status:     ') +
          (job.status === 'completed' ? chalk.green(job.status) : chalk.yellow(job.status))
      );
      ...
      console.log(
        chalk.dim(`   Duration:   ${job.duration_seconds ? job.duration_seconds + 's' : 'N/A'}`)
      );
...
program
  ...
  .action(async () => {
    try {
      ...
      console.log(
        chalk.bold.cyan('║                    AGENTDB LEARNING STATS                          ║')
      );
      console.log(
        chalk.bold.cyan('╚═══════════════════════════════════════════════════════════════════╝\n')
      );
      ...
      console.log(
        `   Avg Confidence: ${chalk.yellow(stats.avg_confidence?.toFixed(3) || 'N/A')}\n`
      );
...
program
  ...
  .action(async (tasks, options) => {
    ...
    console.log(
      chalk.bold.cyan('║                    PARALLEL RESEARCH SWARM                         ║')
    );
    console.log(
      chalk.bold.cyan('╚═══════════════════════════════════════════════════════════════════╝\n')
    );
    ...
    console.log(
      chalk.cyan(
        `🐝 Spawning ${tasks.length} research agents (max ${options.concurrent} concurrent)...\n`
      )
    );
    ...
    for (const task of tasks) {
      ...
      const jobPromise = new Promise((resolve) => {
        ...
        const child = spawn(
          'node',
          [path.join(__dirname, '../run-researcher-local.js'), options.agent, task],
          {
            stdio: 'pipe',
          }
        );
        ...
        child.on('close', (code) => {
          ...
          if (code === 0) {
            console.log(
              chalk.green(
                `   ✅ Completed: ${task.substring(0, 50)}... (${jobId?.substring(0, 8)})`
              )
            );
          } else {
    ...
    console.log(
      chalk.bold.cyan('║                    SWARM COMPLETE                                  ║')
    );
    console.log(
      chalk.bold.cyan('╚═══════════════════════════════════════════════════════════════════╝\n')
    );
  ...
  .action(async (query, options) => {
    ...
    try {
      ...
      results.forEach((result, i) => {
        ...
        console.log(
          chalk.dim(`   Source: ${result.source_type} | ID: ${result.source_id?.substring(0, 8)}\n`)
        );
      });
    } catch (error) {
      console.error(chalk.red(`Error: ${error.message}`));
...
program
  ...
  .action(async () => {
    ...
    try {
      ...
      console.log(
        chalk.bold.cyan('║                    HNSW GRAPH STATISTICS                           ║')
      );
      console.log(
        chalk.bold.cyan('╚═══════════════════════════════════════════════════════════════════╝\n')
      );
      ...
      console.log(
        `   Status:          ${stats.status === 'active' ? chalk.green(stats.status) : chalk.yellow(stats.status)}\n`
      );
...
program
  ...
  .action(async (goal, options) => {
    try {
      ...
      console.log(
        chalk.bold.cyan('║                    GOAP RESEARCH PLAN                              ║')
      );
      console.log(
        chalk.bold.cyan('╚═══════════════════════════════════════════════════════════════════╝\n')
      );
      ...
      plan.subGoals.forEach((subGoal, idx) => {
        ...
        console.log(
          chalk.dim(`      Complexity: ${subGoal.complexity} | Priority: ${subGoal.priority}\n`)
        );
      });
...
program
  ...
  .action(async (goal) => {
    try {
      ...
      console.log(
        chalk.bold.cyan('║                    GOAP EXPLANATION                                ║')
      );
      console.log(
        chalk.bold.cyan('╚═══════════════════════════════════════════════════════════════════╝\n')
      );
