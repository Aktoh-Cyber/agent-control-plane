...
const ITERATIONS = parseInt(
  process.argv.find((arg) => arg.startsWith('--iterations='))?.split('=')[1] || '10'
);
...
for (let i = 0; i < ITERATIONS; i++) {
  ...
  `
    ...
    'completed',
    100,
    `This is a test report for benchmark iteration ${i + 1}`,
    0.85,
    30,
    5000,
    128
...
console.log(
  `   Total Time:        ${chalk.cyan(allStats.reduce((a, b) => a + b, 0).toFixed(2))}ms`
);
