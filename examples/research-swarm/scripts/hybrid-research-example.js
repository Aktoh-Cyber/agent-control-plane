 ...
 * agent-control-plane agents (for domain expertise and implementation).
 *
 * Usage:
...
async function hybridResearch(task, options = {}) {
  ...
  console.log(
    chalk.bold.cyan('║          HYBRID RESEARCH: Research-Swarm + Agentic-Flow           ║')
  );
  console.log(
    chalk.bold.cyan('╚═══════════════════════════════════════════════════════════════════╝\n')
  );
  ...
  // Phase 2: Domain expertise with agent-control-plane (optional)
  if (options.addDomainExpert) {
    console.log(chalk.bold.yellow('🎓 PHASE 2: Domain Expert Analysis\n'));
    console.log(
      chalk.dim(`Using agent-control-plane agent: ${options.expertAgent || 'backend-dev'}\n`)
    );
  ...
  // Phase 3: Implementation with agent-control-plane (optional)
  if (options.generateCode) {
    console.log(chalk.bold.yellow('💻 PHASE 3: Code Generation\n'));
    console.log(chalk.dim('Using agent-control-plane coder agent...\n'));
  ...
  // Phase 4: Code review with agent-control-plane (optional)
  if (options.reviewCode) {
    console.log(chalk.bold.yellow('🔍 PHASE 4: Code Review\n'));
    console.log(chalk.dim('Using agent-control-plane reviewer agent...\n'));
  ...
  console.log(
    chalk.bold.cyan('║                    HYBRID RESEARCH COMPLETE                        ║')
  );
  console.log(
    chalk.bold.cyan('╚═══════════════════════════════════════════════════════════════════╝\n')
  );
  ...
  console.log(
    chalk.dim(`   ✅ Research agents: ${researchResults.successful}/${researchResults.total}`)
  );
...
async function executeAgenticFlowAgent(agentType, task) {
  return new Promise((resolve, reject) => {
    ...
    child.on('error', (error) => {
      console.error(chalk.red(`\n❌ Error executing agent-control-plane agent: ${error.message}`));
      resolve(); // Don't fail entire workflow
    });
  });
}
...
const options = {
  ...
  researchTime: args.includes('--research-time')
    ? parseInt(args[args.indexOf('--research-time') + 1])
    : 20,
  addDomainExpert: args.includes('--add-expert'),
  expertAgent: args.includes('--expert-agent')
    ? args[args.indexOf('--expert-agent') + 1]
    : 'backend-dev',
...
if (args.includes('--help') || args.includes('-h')) {
  ...
  console.log(
    chalk.dim('  --add-expert                  Add domain expert analysis (agent-control-plane)')
  );
  console.log(
    chalk.dim('  --expert-agent <agent>        Expert agent type (default: backend-dev)')
  );
  console.log(
    chalk.dim('  --generate-code               Generate implementation code (agent-control-plane)')
  );
  console.log(
    chalk.dim('  --review-code                 Review generated code (agent-control-plane)')
  );
  ...
  console.log(
    chalk.cyan(
      '  node scripts/hybrid-research-example.js "ML pipeline" --add-expert --expert-agent ml-developer\n'
    )
  );
  console.log(chalk.dim('  # Full workflow: research + code + review'));
  console.log(
    chalk.cyan(
      '  node scripts/hybrid-research-example.js "REST API" --add-expert --generate-code --review-code\n'
    )
  );
