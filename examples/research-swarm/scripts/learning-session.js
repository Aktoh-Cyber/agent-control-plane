...
const MIN_PATTERNS = parseInt(
  process.argv.find((arg) => arg.startsWith('--min-patterns='))?.split('=')[1] || '2'
);
...
for (let i = 0; i < patterns.length; i++) {
  for (let j = i + 1; j < patterns.length; j++) {
    ...
    if (similarity > 0.3) {
      // Only store meaningful associations
...
function calculateConfidence(patterns) {
  ...
  const consistencyScore =
    1 - (Math.max(...patterns.map((p) => p.reward)) - Math.min(...patterns.map((p) => p.reward)));
