...
try {
  ...
  optimizations.push({
    name: 'WAL Mode',
    status: walResult.journal_mode === 'wal' ? 'enabled' : 'already active',
  });
  ...
  optimizations.push({
    ...
    status: syncMode.synchronous === 1 ? 'NORMAL' : 'FULL',
  });
  console.log(
    chalk.green(`   ✅ Synchronous mode: ${syncMode.synchronous === 1 ? 'NORMAL' : 'FULL'}\n`)
  );
  ...
  console.log(
    integrityOk
      ? chalk.green('   ✅ Integrity check passed\n')
