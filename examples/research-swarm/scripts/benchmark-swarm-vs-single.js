...
console.log(
  chalk.bold.cyan('║            SWARM vs SINGLE-AGENT BENCHMARK                         ║')
);
console.log(
  chalk.bold.cyan('╚═══════════════════════════════════════════════════════════════════╝\n')
);
...
async function runResearch(config) {
  ...
  return new Promise((resolve) => {
    ...
    child.on('close', (code) => {
      ...
      // Calculate metrics
      const totalReportLength = jobs.reduce(
        (sum, job) => sum + (job.report_content?.length || 0),
        0
      );
...
function compareResults(single, swarm, task) {
  ...
  if (swarm.duration < single.duration) {
    console.log(
      chalk.green(
        `   ✅ Swarm is ${speedup.toFixed(2)}x FASTER (${single.duration.toFixed(1)}s → ${swarm.duration.toFixed(1)}s)`
      )
    );
  } else {
    console.log(
      chalk.yellow(
        `   ⚠️  Swarm is ${(1 / speedup).toFixed(2)}x slower (${single.duration.toFixed(1)}s → ${swarm.duration.toFixed(1)}s)`
      )
    );
  }
  ...
  console.log(
    chalk.cyan(
      `   Single: ${single.agentCount} agent | Swarm: ${swarm.agentCount} agents (${agentRatio.toFixed(1)}x)`
    )
  );
  console.log(
    chalk.dim(`   Expected: ${task.expectedAgents.swarm} agents for depth ${task.depth}`)
  );
  ...
  console.log(
    chalk.cyan(
      `   Swarm: ${swarm.totalReportLength} chars (${reportRatio.toFixed(2)}x more detailed)`
    )
  );
...
function displayFinalSummary() {
  ...
  console.log(
    chalk.bold.cyan('║                    FINAL BENCHMARK SUMMARY                         ║')
  );
  console.log(
    chalk.bold.cyan('╚═══════════════════════════════════════════════════════════════════╝\n')
  );
  ...
  const avgSingleDuration =
    results.single.reduce((sum, r) => sum + r.duration, 0) / results.single.length;
  const avgSwarmDuration =
    results.swarm.reduce((sum, r) => sum + r.duration, 0) / results.swarm.length;

  const avgSingleReportLength =
    results.single.reduce((sum, r) => sum + r.totalReportLength, 0) / results.single.length;
  const avgSwarmReportLength =
    results.swarm.reduce((sum, r) => sum + r.totalReportLength, 0) / results.swarm.length;
  ...
  console.log(
    chalk.dim(
      `   Single: ${singleSuccess}/${results.single.length} (${((singleSuccess / results.single.length) * 100).toFixed(1)}%)`
    )
  );
  console.log(
    chalk.dim(
      `   Swarm: ${swarmSuccess}/${results.swarm.length} (${((swarmSuccess / results.swarm.length) * 100).toFixed(1)}%)`
    )
  );
  ...
  console.log(
    chalk.green(
      `   ✅ Swarm reports are ${(avgSwarmReportLength / avgSingleReportLength).toFixed(2)}x more detailed`
    )
  );
  ...
  console.log(
    chalk.bold.cyan('║                    BENCHMARK COMPLETE                              ║')
  );
  console.log(
    chalk.bold.cyan('╚═══════════════════════════════════════════════════════════════════╝\n')
  );
