...
const NUM_TASKS = parseInt(
  process.argv.find((arg) => arg.startsWith('--tasks='))?.split('=')[1] || '5'
);
...
function runResearchTask(task, enableLearning) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(
      'node',
      [path.join(__dirname, '../run-researcher-local.js'), 'researcher', task],
      {
        env,
...
async function runPhase1() {
  ...
  console.log(
    chalk.bold.yellow('║   PHASE 1: WITHOUT SELF-LEARNING (Baseline)                       ║')
  );
  console.log(
    chalk.bold.yellow('╚═══════════════════════════════════════════════════════════════════╝\n')
  );
  ...
  for (let i = 0; i < tasks.length; i++) {
    ...
    if (result.success && result.jobId) {
      ...
      console.log(
        chalk.green(
          `   ✅ Completed in ${result.duration.toFixed(1)}s | Quality: ${metrics.quality.toFixed(0)}%\n`
        )
      );
    } else {
...
async function runPhase2() {
  ...
  console.log(
    chalk.bold.green('║   PHASE 2: WITH SELF-LEARNING (Enhanced)                          ║')
  );
  console.log(
    chalk.bold.green('╚═══════════════════════════════════════════════════════════════════╝\n')
  );
...
`)
  ...
  metrics.forEach((metric) => {
    ...
    if (isImprovement) {
      console.log(
        chalk.green(
          `   📈 Improvement: ${improvementStr}% ${metric.lowerIsBetter ? 'faster' : 'better'}\n`
        )
      );
  ...
  const qualityImprovement =
    ((calculateStats(results.withLearning, 'quality').avg -
      calculateStats(results.withoutLearning, 'quality').avg) /
      calculateStats(results.withoutLearning, 'quality').avg) *
    100;

  const timeImprovement =
    ((calculateStats(results.withoutLearning, 'durationSeconds').avg -
...
`)
  ...
  console.log(
    `   Quality Improvement:     ${chalk.green(qualityImprovement > 0 ? '+' : '')}${chalk.green(qualityImprovement.toFixed(1) + '%')}`
  );
  console.log(
    `   Speed Improvement:       ${chalk.green(timeImprovement > 0 ? '+' : '')}${chalk.green(timeImprovement.toFixed(1) + '%')}`
  );
  ...
  if (totalImprovement > 10) {
    console.log(
      chalk.bold.green('✅ SIGNIFICANT IMPROVEMENT: Self-learning provides measurable benefits\n')
    );
  } else if (totalImprovement > 5) {
    console.log(
      chalk.bold.yellow('⚠️  MODERATE IMPROVEMENT: Benefits observable but not dramatic\n')
    );
  ...
  console.log(
    chalk.dim(`   Average Reward:      ${learningStats.avg_reward?.toFixed(3) || 'N/A'}`)
  );
  console.log('');
}
  ...
  const report = {
    ...
    summary: {
      ...
      speedImprovement:
        ...
        calculateStats(results.withLearning, 'durationSeconds').avg,
      tokenSavings:
        calculateStats(results.withoutLearning, 'tokensUsed').avg -
        calculateStats(results.withLearning, 'tokensUsed').avg,
    },
  };
