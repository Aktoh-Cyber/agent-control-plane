 ...
 * Demonstrates how research-swarm and agent-control-plane agents work together.
 */
...
console.log(
  chalk.bold.cyan('║          HYBRID WORKFLOW SIMULATION TEST                           ║')
);
console.log(
  chalk.bold.cyan('╚═══════════════════════════════════════════════════════════════════╝\n')
);
...
async function simulateHybridWorkflow(workflow) {
    ...
    try {
      ...
      if (phase.system === 'research-swarm') {
        ...
        agents.forEach((agent, idx) => {
          ...
          console.log(
            chalk.dim(
              `      ${idx + 1}. ${emoji} ${agent.config.role} (P${agent.config.priority}, ${agent.config.timeMinutes}min)`
            )
          );
        });
      ...
      } else if (phase.system === 'agent-control-plane') {
        // Simulate agent-control-plane agent
        ...
        const estimatedTime = 15; // Assume 15 min per agent-control-plane agent
  ...
  const agenticFlowAgents = results.phases.filter((p) => p.system === 'agent-control-plane').length;
...
async function runAllSimulations() {
  ...
  console.log(
    chalk.bold.cyan('║                SIMULATION SUMMARY                                  ║')
  );
  console.log(
    chalk.bold.cyan('╚═══════════════════════════════════════════════════════════════════╝\n')
  );
  ...
  allResults.forEach((result, idx) => {
    ...
    console.log(
      chalk.dim(
        `   Phases: ${result.phases.length} | Agents: ${result.totalAgents} | Time: ${result.totalTime}min`
      )
    );
  });
  ...
  const avgTime = Math.round(
    allResults.reduce((sum, r) => sum + r.totalTime, 0) / allResults.length
  );
