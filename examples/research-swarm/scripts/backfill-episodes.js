...
try {
  ...
  for (const [patternId, patternEpisodes] of Object.entries(byPattern)) {
    ...
    patternEpisodes.forEach((episode, index) => {
      ...
      const explorationRate =
        episode.exploration_rate || Math.min(1, (episode.duration_seconds || 0) / 3600);
  ...
  console.log(
    chalk.dim(`   Avg Episode Number:      ${stats.avg_episode_number?.toFixed(2) || 'N/A'}`)
  );
  console.log(
    chalk.dim(`   Avg Improvement Rate:    ${stats.avg_improvement_rate?.toFixed(3) || 'N/A'}`)
  );
  console.log(
    chalk.dim(`   Avg Exploration Rate:    ${stats.avg_exploration_rate?.toFixed(3) || 'N/A'}`)
  );
  console.log(
    chalk.dim(`   Avg Exploitation Rate:   ${stats.avg_exploitation_rate?.toFixed(3) || 'N/A'}`)
  );
  console.log(
    chalk.dim(`   Avg Learning Rate:       ${stats.avg_learning_rate?.toFixed(3) || 'N/A'}`)
  );
  console.log('');
} catch (error) {
