...
patterns.forEach((pattern, idx) => {
  try {
    execSync(
      \`npx gendev@alpha memory store "\${pattern.key}" "\${pattern.value}" --namespace \${namespace} --reasoningbank\`,
      { stdio: 'pipe' }
    );
    success++;
...
function cachedQuery(query, namespace = 'default') {
  ...
  const result = execSync(
    \`npx gendev@alpha memory query "\${query}" --namespace \${namespace} --reasoningbank\`,
    { encoding: 'utf8' }
  );
