...
Research findings for integrating Microsoft's Phi-4-mini-instruct-onnx model with agent-control-plane using onnxruntime-node for CPU inference.
...
// Load model
const session = await ort.InferenceSession.create('./models/phi-4/model.onnx', {
...
**onnxruntime-node is the correct choice** for implementing Phi-4 inference in agent-control-plane:
