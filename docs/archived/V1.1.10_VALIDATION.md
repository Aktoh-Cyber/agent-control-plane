...
### Test 5: MCP Integration (gendev-sdk)

```bash
export ENABLE_CLAUDE_FLOW_SDK=true
node dist/cli-proxy.js --agent coder --task "Use the gendev-sdk MCP to store 'test-validation'='all systems working' in memory, then retrieve it" --provider anthropic --max-tokens 250
```
...
if (provider === 'gemini' && process.env.GOOGLE_GEMINI_API_KEY) {
  ...
  envOverrides.ANTHROPIC_BASE_URL =
    process.env.ANTHROPIC_BASE_URL || process.env.GEMINI_PROXY_URL || 'http://localhost:3000'; // ✅ Correct
}
```
...
```bash
npm install -g agent-control-plane@1.1.10
# or
npx agent-control-plane@1.1.10 --agent coder --task "..."
```
...
npx agent-control-plane@1.1.10 --agent coder --task "Create a REST API" --provider gemini
```
...
npx agent-control-plane@1.1.10 --agent coder --task "Write tests" --provider openrouter --model "openai/gpt-4o-mini"
```
...
npx agent-control-plane@1.1.10 --agent coder --task "Refactor code"
```
...
export ENABLE_CLAUDE_FLOW_SDK=true
export ANTHROPIC_API_KEY="sk-ant-..."
npx agent-control-plane@1.1.10 --agent coder --task "Store config in MCP memory"
```
...
- **NPM**: https://www.npmjs.com/package/agent-control-plane/v/1.1.10
