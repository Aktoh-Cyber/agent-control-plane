...
**agent-control-plane v1.1.6** is **CONFIRMED** to properly use the Claude Agent SDK with agents from `.claude/agents/` directory.
...
```bash
npx agent-control-plane --agent coder --task "Create a hello world function"
...
```bash
npx agent-control-plane --agent reviewer --task "Review this code"
...
```bash
npx agent-control-plane --agent my-agent --task "Do something"
...
npx agent-control-plane --agent coder --task "..." --provider openrouter
...
- `agentic-cloud-neural` - Neural network training
...
```bash
npx agent-control-plane --agent [any-agent] --task "..."
```
...
# Gemini (speed)
npx agent-control-plane --agent coder --task "..." --provider gemini

# ONNX (free local)
npx agent-control-plane --agent coder --task "..." --provider onnx
```
...
```bash
npx agent-control-plane --agent coder --task "Create a function"
```
...
**✅ CONFIRMED**: agent-control-plane v1.1.6 properly integrates Claude Agent SDK with agents from `.claude/agents/` directory
