...
- **CLI `--agent` mode**: `npx agent-control-plane --agent coder` doesn't use Agent Booster
...
```
User: npx agent-control-plane --agent coder --task "convert var to const"
  ↓
...
```
User: npx agent-control-plane --agent coder --task "convert var to const" --agent-booster
  ↓
CLI Wrapper
...
npx agent-control-plane --agent coder --pattern var-to-const --file utils.js
# Result: 85ms

# Vague task (uses LLM)
npx agent-control-plane --agent coder --task "improve error handling" --file utils.js
# Result: 26s
```
