...
npx agent-control-plane --agent coder --task "Process PII data" --privacy high

# Complex tasks use OpenRouter (cheap)
npx agent-control-plane --agent coder --task "Design distributed system" --complexity high

# Simple tasks default to ONNX (free)
npx agent-control-plane --agent coder --task "Hello world function"
```
...
npx agent-control-plane --agent coder --task "Build API" --model "meta-llama/llama-3.1-8b-instruct"
```
...
```javascript
import { modelDownloader } from 'agent-control-plane/utils/model-downloader';
...
```javascript
import { ensurePhi4Model } from 'agent-control-plane/utils/model-downloader';
