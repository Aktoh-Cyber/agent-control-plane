...
export class InputValidator {
  ...
  static validateNumber(value: number, min: number, max: number, paramName: string): void {
...
describe('Security Tests', () => {
  describe('SQL Injection Protection', () => {
    test('should reject malicious session_id in delete', async () => {
      ...
      await expect(server.callTool('agentdb_delete', maliciousInput)).rejects.toThrow();
    });
  ...
  describe('Input Validation', () => {
    test('should reject oversized batch insert', async () => {
      ...
      await expect(server.callTool('agentdb_insert_batch', oversizedBatch)).rejects.toThrow(
        'exceeds maximum'
      );
    });
