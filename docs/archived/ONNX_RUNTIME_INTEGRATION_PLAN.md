...
export class ONNXProvider implements LLMProvider {
  ...
  private async isCUDAAvailable(): Promise<boolean> {
    try {
      ...
      const testSession = await ort.InferenceSession.create('path/to/test.onnx', {
...
- Local model cache (~/.agent-control-plane/onnx-models/)
...
export class ONNXModelManager {
  private cacheDir = join(homedir(), '.agent-control-plane', 'onnx-models');
  ...
  async downloadModel(modelId: string): Promise<string> {
    ...
    await this.downloadFromHuggingFace(modelInfo.huggingface, modelInfo.files, modelPath);
...
npx agent-control-plane onnx models

# Download a model
npx agent-control-plane onnx download phi-3-mini-4k-cpu

# List downloaded models
npx agent-control-plane onnx list

# Test ONNX inference
npx agent-control-plane onnx test --model phi-3-mini-4k-cpu

# Benchmark performance
npx agent-control-plane onnx benchmark --model phi-3-mini-4k-cpu

# Check hardware capabilities
npx agent-control-plane onnx info

# Use ONNX provider for inference
npx agent-control-plane --provider onnx --model phi-3-mini-4k-cpu --task "Hello world"

# Use ONNX with GPU
npx agent-control-plane --provider onnx --model phi-3-mini-4k-gpu --execution-provider cuda --task "Complex task"
```
...
```bash
# CPU Benchmark
npx agent-control-plane onnx benchmark \
  --model phi-3-mini-4k-cpu \
  --provider cpu \
  --iterations 100
