...
if (conflicts.length > 0) {
  ...
  await jj_wrapper.resolve_conflict(conflicts[0].id, consensus_result.winning_resolution);

  // Reward winning proposer
  await qudag.transfer_ruv(consensus_result.proposer, consensus_result.reward_amount);
}
```
...
```typescript
async function runQualityOracles(vertex: QUDAGVertex): Promise<QualityReport> {
...
async function hireOptimalAgent(task: Task, budget: number): Promise<AgentID> {
