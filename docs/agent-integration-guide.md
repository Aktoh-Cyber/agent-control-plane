...
**Version:** 2.0.0
**Date:** 2025-11-02
**Purpose:** Guide for integrating self-learning swarm optimization into agent-control-plane agents
...
npx agent-control-plane --agent agent-name --task "subtask-1" --output reasoningbank:swarm/task-id/agent-1
npx agent-control-plane --agent agent-name --task "subtask-2" --output reasoningbank:swarm/task-id/agent-2
```
...
const results = await Promise.all([
  exec('npx agent-control-plane --agent worker --task "task-1"'),
  exec('npx agent-control-plane --agent worker --task "task-2"'),
  exec('npx agent-control-plane --agent worker --task "task-3"')
]);
\`\`\`

See /agent-control-plane/src/prompts/parallel-execution-guide.md for complete patterns.
...
npx agent-control-plane --agent coder --task "Implement simple REST API"

# Test with parallel-friendly task
npx agent-control-plane --agent coder --task "Refactor 50 modules in /src directory"
```
...
npx agent-control-plane --agent <name> --task "Simple task"

# 2. Test parallel execution
npx agent-control-plane --agent <name> --task "Complex parallel-friendly task"
...
- **Parallel Execution Guide**: `/agent-control-plane/src/prompts/parallel-execution-guide.md`
- **Optimization Report**: `/docs/swarm-optimization-report.md`
- **Implementation Report**: `/docs/parallel-execution-implementation.md`
- **GitHub Issue**: [#43](https://github.com/Aktoh-Cyber/agent-control-plane/issues/43)
