...
Starting with v1.9.1, agent-control-plane provides **graceful fallback** for QUIC transport, ensuring compatibility with all Node.js versions including 22+.
...
agent-control-plane now uses an **intelligent transport loader** that:
...
```typescript
import { getTransportCapabilities } from 'agent-control-plane/transport/quic-loader';
...
FROM node:22-slim
COPY --from=build /usr/local/lib/node_modules/agent-control-plane /usr/local/lib/node_modules/agent-control-plane
CMD ["npx", "agent-control-plane"]
```
...
npm install -g agent-control-plane@latest --include=optional
...
- [#45: Asset errors on macOS 15.7.1](https://github.com/Aktoh-Cyber/agent-control-plane/issues/45)
- [#44: Release v1.9.0](https://github.com/Aktoh-Cyber/agent-control-plane/issues/44)
...
**Questions?** Open an [issue](https://github.com/Aktoh-Cyber/agent-control-plane/issues) or [discussion](https://github.com/Aktoh-Cyber/agent-control-plane/discussions)!
