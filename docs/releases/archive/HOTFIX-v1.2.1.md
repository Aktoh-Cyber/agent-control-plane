...
```bash
npx agent-control-plane mcp add    # ❌ Started MCP server instead
npx agent-control-plane mcp list   # ❌ Started MCP server instead
```
...
```bash
npx agent-control-plane mcp list   # ✅ Now shows configured servers
npx agent-control-plane mcp add    # ✅ Now adds servers to config
```
...
```bash
npx agent-control-plane --agent coder --task "..." --optimize
...
```bash
$ npx agent-control-plane --agent coder --task "Create calculator" --optimize
...
```bash
# Update globally
npm install -g agent-control-plane@1.2.1
...
npx agent-control-plane --agent coder --task "test" --optimize
