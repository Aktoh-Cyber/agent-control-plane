...
npx agent-control-plane quic --port 4433 --cert ./certs/cert.pem --key ./certs/key.pem
```
...
- **CLI Integration** - `npx agent-control-plane quic` and `--transport quic` flag
...
npx agent-control-plane --agent reviewer --task "Review PR #128 for security and style" --optimize

# Optimize for cost
npx agent-control-plane --agent reviewer --task "Light style review only" --optimize --priority cost

# Set a strict budget per task
npx agent-control-plane --agent coder --task "Refactor utility functions" --optimize --max-cost 0.001
```
...
npx agent-control-plane agent info mesh-coordinator
...
docker run --rm -e ANTHROPIC_API_KEY=$ANTHROPIC_API_KEY agent-control-plane \
  --agent reviewer --task "Security posture review for service X" --transport quic
```

Agentic Cloud sandboxes at scale:

```bash
# Example pattern when using your Agentic Cloud setup
