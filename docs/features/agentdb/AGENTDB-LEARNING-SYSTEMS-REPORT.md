...
if (feedback.nextState) {
  const nextActions = Object.keys(policy.qValues).filter((k) =>
    k.startsWith(feedback.nextState + '|')
  );
...
**TypeScript Controllers** (`/workspaces/agent-control-plane/packages/agentdb/src/controllers/`):
...
**Rust/WASM** (`/workspaces/agent-control-plane/reasoningbank/crates/`):
...
**MCP Integration** (`/workspaces/agent-control-plane/packages/agentdb/src/mcp/`):

- `agentdb-mcp-server.ts` - MCP server implementation
- `learning-tools-handlers.ts` - Learning tool handlers

**WASM Artifacts** (`/workspaces/agent-control-plane/agent-control-plane/wasm/reasoningbank/`):
