...
function validateNumericRange(value: number, min: number, max: number, paramName: string): void {
...
Error: Failed to insert episode
  at ReflexionMemory.storeEpisode (/workspaces/agent-control-plane/packages/agentdb/dist/controllers/ReflexionMemory.js:145:13)
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (!rateLimiter.checkLimit(clientId, 100, 60000)) {
    // 100 req/min
...
describe('Security Tests', () => {
  test('SQL Injection Protection', async () => {
    ...
    await expect(mcpServer.callTool('agentdb_delete', maliciousInput)).rejects.toThrow(); // Should reject, not execute
  });
  ...
  test('Batch Size Limit', async () => {
    ...
    await expect(mcpServer.callTool('agentdb_insert_batch', oversizedBatch)).rejects.toThrow(
      'exceeds maximum'
    );
  });
  ...
  test('Rate Limiting', async () => {
    ...
    const requests = Array(101)
      .fill(null)
      .map(() => mcpServer.callTool('db_stats', {}));
...
- **GitHub Security Advisories:** https://github.com/Aktoh-Cyber/agent-control-plane/security/advisories
