...
**Goal**: Transform agent-control-plane's embedded AgentDB copy (400KB, 6,955 lines) into a proper dependency relationship with optimized memory and ReasoningBank capabilities.

**Current State**:

- ❌ Code duplication (embedded copy in `agent-control-plane/src/agentdb/`)
...
**File**: `/workspaces/agent-control-plane/agent-control-plane/package.json`
...
```typescript
// agent-control-plane/src/reasoningbank/index.ts
...
```bash
# Backup first
cp -r agent-control-plane/src/agentdb agent-control-plane/src/agentdb.backup

# Remove 400KB of duplicated code
rm -rf agent-control-plane/src/agentdb/controllers
rm -rf agent-control-plane/src/agentdb/optimizations
rm -rf agent-control-plane/src/agentdb/benchmarks
rm -rf agent-control-plane/src/agentdb/tests

# Keep only CLI wrapper if needed
# Keep agent-control-plane/src/agentdb/cli/agentdb-cli.ts as wrapper
```
...
Create unified memory coordinator that both agent-control-plane and agentdb share:

```typescript
// agent-control-plane/src/memory/SharedMemoryPool.ts
...
```typescript
// agent-control-plane/src/reasoningbank/AgentDBBackend.ts
...
```typescript
// agent-control-plane/src/reasoningbank/HybridBackend.ts
...
export class HybridReasoningBank {
  ...
  async retrievePatterns(query: string, k: number = 5) {
    if (this.useWasm) {
      ...
      return Promise.all(wasmResults.map((id) => this.agentdb.reflexion.getEpisode(id)));
    } else {
...
```typescript
// agent-control-plane/src/reasoningbank/AdvancedMemory.ts
...
```typescript
// agent-control-plane/src/reasoningbank/BatchProcessor.ts
...
```typescript
// agent-control-plane/src/memory/MemoryMonitor.ts
...
- [ ] Add `agentdb@^1.3.9` to agent-control-plane dependencies
...
2. **Import Paths**: No breaking changes (re-exports maintained)
   ...
   import { HybridReasoningBank, AdvancedMemorySystem } from 'agent-control-plane/reasoningbank';
   ```
...
- [Quick Start Guide](../agent-control-plane/README.md)
- [ReasoningBank Tutorial](../agent-control-plane/docs/REASONINGBANK.md)
- [Memory Optimization Tips](../agent-control-plane/docs/MEMORY_OPTIMIZATION.md)
