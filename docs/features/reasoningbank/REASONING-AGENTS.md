...
From `agent-control-plane/package.json` line 148-158:
...
npx agent-control-plane --agent adaptive-learner --task "Implement JWT authentication"

# Pattern matching for solution reuse
npx agent-control-plane --agent pattern-matcher --task "Design pagination system"

# Context synthesis for complex tasks
npx agent-control-plane --agent context-synthesizer --task "Architect microservices system"

# Experience curation for quality assurance
npx agent-control-plane --agent experience-curator --task "Review recent executions"

# Memory optimization for maintenance
npx agent-control-plane --agent memory-optimizer --task "Consolidate memory system"
```
...
npx agent-control-plane --agent reasoning-optimized --task "Build authentication system"
...
npx agent-control-plane --agent coder --task "Implement rate limiting"
...
```
User runs: npx agent-control-plane --agent coder --task "..."
                                    ↓
...
1. **ReasoningBank Implementation**: `/agent-control-plane/src/reasoningbank/`
...
npx agent-control-plane --agent reasoning-optimized --task "Build authentication"

# Individual reasoning agents
npx agent-control-plane --agent adaptive-learner --task "Implement feature"
```
````
