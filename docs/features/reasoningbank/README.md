...
- [Integration Plan](REASONINGBANK_INTEGRATION_PLAN.md) - Integration with agent-control-plane
...
npx gendev reasoningbank store \
  --task "Solve authentication bug" \
  --reward 0.95 \
  --success true

# Search for similar patterns
npx gendev reasoningbank search \
  --task "Fix login issue" \
  --k 5

# Get pattern statistics
npx gendev reasoningbank stats \
  --task "authentication"
```
