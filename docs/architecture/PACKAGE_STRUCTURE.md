# agent-control-plane Package Structure

## Overview

The `agent-control-plane` npm package includes all necessary files for agent execution, including 76 pre-built agent definitions in the `.claude/agents/` directory.

## Package Contents

When you install `agent-control-plane` via npm, you get:
...
│       ├── agentic-cloud/      # Agentic Cloud cloud agents
...
```
node_modules/agent-control-plane/.claude/agents/
```

These are automatically loaded when you run `npx agent-control-plane`.
...
1. **Package agents**: Load from `node_modules/agent-control-plane/.claude/agents/`
...
cd /tmp/test-install
npm install /path/to/agent-control-plane-1.1.2.tgz

# Verify agents loaded
./node_modules/.bin/agent-control-plane --list
```
...
npx agent-control-plane --agent coder --task "test" --provider gemini
...
```json
{
  "files": ["dist", "docs", ".claude", "README.md", "LICENSE"]
}
```
...
- Runtime state directories (`.gendev/`, `.swarm/`, `memory/`)
...
- **Agentic Cloud** (9): Cloud sandboxes, neural networks, workflows
