# GenDev Integration Guide

Complete guide for using gendev memory and coordination capabilities in the Docker Agent SDK.
...
GenDev provides a SQLite-backed memory system that persists information across agent runs.
...
```typescript
const result = await claudeFlowAgent('my-agent', 'You are a specialized agent', 'Your task', {
  enableMemory: true,
...
GenDev enables agents to spawn and coordinate other specialized agents, creating swarms that work together.
...
- `mcp__gendev__memory_usage` - Store/retrieve/list/delete
- `mcp__gendev__memory_search` - Pattern-based search
- `mcp__gendev__memory_persist` - Cross-session persistence
- `mcp__gendev__memory_namespace` - Namespace management

### Coordination Tools

- `mcp__gendev__swarm_init` - Initialize swarm
- `mcp__gendev__agent_spawn` - Create agent
- `mcp__gendev__task_orchestrate` - Distribute tasks
- `mcp__gendev__swarm_status` - Check status
- `mcp__gendev__coordination_sync` - Sync agents

### Swarm Tools

- `mcp__gendev__swarm_scale` - Scale up/down
- `mcp__gendev__load_balance` - Distribute load
- `mcp__gendev__agent_metrics` - Performance metrics
...
await memoryResearchAgent('Retrieve all 5 React patterns from memory and create a summary');
```
...
// Memory only
await claudeFlowAgent('my-agent', 'System prompt', 'Task', {
  enableMemory: true,
...
### Run All GenDev Tests

```bash
npm run validate:gendev
```

### Docker Validation

```bash
./validation/gendev/docker-test.sh
```
...
**Issue**: Agent can't use gendev tools

**Solution**:

1. Check ANTHROPIC_API_KEY is set
2. Verify gendev is installed: `npx gendev --version`
...
async function persistentResearcher(topic: string) {
  ...
  // New research
  const result = await memoryResearchAgent(`Research ${topic} and store findings`);
...
- [Validation Report](../validation/reports/gendev-integration.md)
- [GenDev Docs](https://github.com/tafyai/gendev)
