...
# Start proxy server
npx agent-control-plane@1.1.11 proxy --provider openrouter --model "openai/gpt-4o-mini"
```
...
npx agent-control-plane proxy --provider openrouter --model "anthropic/claude-3.5-sonnet"
...
```bash
export OPENROUTER_API_KEY=sk-or-v1-...
npx agent-control-plane proxy --provider openrouter --model "deepseek/deepseek-chat-v3.1"
...
```bash
npx agent-control-plane proxy  # Reads .env automatically
```
...
nohup npx agent-control-plane proxy > proxy.log 2>&1 &

# Check if running
ps aux | grep agent-control-plane

# Stop
pkill -f agent-control-plane
```
...
```bash
docker build -t agent-control-plane-proxy .
docker run -d -p 3000:3000 -e GOOGLE_GEMINI_API_KEY=your-key agent-control-plane-proxy
```
...
npx agent-control-plane proxy --port 3000

# Terminal 2: OpenRouter on 3001
npx agent-control-plane proxy --provider openrouter --port 3001
```
