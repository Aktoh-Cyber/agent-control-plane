...
kind: Deployment
metadata:
  name: agent-control-plane-openrouter
spec:
  replicas: 3
  template:
    spec:
      containers:
        - name: agent
          image: agent-control-plane:openrouter
          env:
...
```json
{
  "family": "agent-control-plane-openrouter",
...
```bash
# Build and push
gcloud builds submit --tag gcr.io/PROJECT/agent-control-plane:openrouter

# Deploy
gcloud run deploy agent-control-plane-openrouter \
  --image gcr.io/PROJECT/agent-control-plane:openrouter \
...
_Note: OpenRouter adds ~50-100ms overhead for API routing_
