...
This guide shows you how to integrate custom MCP (Model Context Protocol) servers into agent-control-plane.
...
2. **gendev** (NPM) - 101 tools
3. **agentic-cloud** (NPM) - 96 tools
...
npx agent-control-plane --agent coder --task "Calculate 42 + 58 using the calculate tool"
```
...
# Run agent
npx agent-control-plane --agent researcher --task "Search for 'AI trends' using custom_search"
```
...
npx agent-control-plane --agent coder --task "Use tools from my-mcp-package"
```
...
export ENABLE_CLAUDE_FLOW_MCP=true        # gendev (101 tools)
export ENABLE_FLOW_NEXUS_MCP=true         # agentic-cloud (96 tools)
...
export ENABLE_MY_EXTERNAL_MCP=true
npx agent-control-plane --agent coder --task "List all available tools"
```
...
npx agent-control-plane --agent coder --task "Test my custom tools" --verbose
...
### External Example: agentic-cloud
...
5. **Test Standalone First**
   ...
   - Then integrate with agent-control-plane
...
**Need help?** Open an issue on [GitHub](https://github.com/Aktoh-Cyber/agent-control-plane/issues) or check the [MCP docs](https://modelcontextprotocol.io).
