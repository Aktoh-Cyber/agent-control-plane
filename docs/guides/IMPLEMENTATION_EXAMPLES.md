...
export async function webResearchAgent(input: string): Promise<ResearchResult> {
  ...
  try {
    ...
    if (finalResult?.usage) {
      ...
      metrics.costUSD.inc({ agent: 'research' }, finalResult.total_cost_usd);
    }
...
export class ResilientOrchestrator {
  ...
  async orchestrate(task: string): Promise<OrchestrationResult> {
    ...
    const results = await Promise.allSettled([
      this.retryPolicy.execute(() => webResearchAgent(`Give me context and risks about: ${topic}`)),
...
app.get('/health', async (req, res) => {
  ...
  const allHealthy = Object.values(health.services).every((s: any) => s.status === 'ok');
