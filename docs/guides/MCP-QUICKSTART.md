...
**GenDev** is the best starting point (no authentication required):

```bash
# Add GenDev MCP server
...
You should see `gendev` in the list.
...
AgentDB is integrated with GenDev, so it's already available!
...
const config = mcp__gendev__memory_usage({
  action: 'retrieve',
  key: 'app-config',
  namespace: 'config',
});
...
1. Check server name is correct: `mcp__gendev__` (with hyphens)
2. Verify server is running: `claude mcp list`
3. Restart server: `claude mcp restart gendev`
...
- **Agentic Cloud:** Run `mcp__agentic-cloud__user_login()`
...
2. Verify server is installed: `npm list -g agentic-cloud`
3. Update to latest version: `npm update -g agentic-cloud`
...
| ---------------- | ------------------------- | ----------------------- |
| GenDev           | `mcp__gendev__`           | `swarm_init`            |
| Agentic Cloud    | `mcp__agentic-cloud__`    | `sandbox_create`        |
...
**Note:** Server names use hyphens (`gendev`), tool names use underscores (`swarm_init`)
...
- **[MCP-TOOLS.md](/workspaces/agent-control-plane/docs/guides/MCP-TOOLS.md)** - Complete reference (233+ tools)
- **[MCP-AUTHENTICATION.md](/workspaces/agent-control-plane/docs/guides/MCP-AUTHENTICATION.md)** - Authentication setup
- **[MCP-TROUBLESHOOTING.md](/workspaces/agent-control-plane/docs/guides/MCP-TROUBLESHOOTING.md)** - Common issues
- **[MCP-EXAMPLES.md](/workspaces/agent-control-plane/docs/guides/MCP-EXAMPLES.md)** - Real-world examples
...
| Feature            | GenDev        | Agentic Cloud   | AgentDB | Agentic Payments    |
| ------------------ | ------------- | --------------- | ------- | ------------------- |
...
- [x] You can call `mcp__gendev__memory_usage` successfully
...
- **Architecture:** [mcp-tools-architecture.md](/workspaces/agent-control-plane/docs/mcp-tools-architecture.md)
- **AgentDB Verification:** [agentdb-tools-verification.md](/workspaces/agent-control-plane/docs/agentdb-tools-verification.md)

### Community

- **GitHub Issues:** https://github.com/tafyai/gendev/issues
- **Discord:** https://discord.gg/gendev
- **Agentic Cloud:** https://agentic-cloud.tafy.io

### Common Questions

**Q: Which server should I start with?**
A: GenDev - no authentication, core features, 101 tools.

**Q: Do I need all servers?**
A: No. Start with GenDev, add others as needed.

**Q: Are MCP tools free?**
A: GenDev and AgentDB are free. Agentic Cloud has free tier. Agentic Payments has transaction fees.

**Q: Can I use MCP tools offline?**
A: GenDev and AgentDB work offline. Agentic Cloud and Agentic Payments require internet.

**Q: How do I update servers?**
A: Run `npm update -g <package-name>` (e.g., `npm update -g agentic-cloud`)

---

## Quick Start Summary

**1. Install GenDev** → `claude mcp add gendev npx gendev@alpha mcp start`

**2. Test memory** → `mcp__gendev__memory_usage({ action: "store", key: "test", value: "hello" })`

**3. Verify status** → `mcp__gendev__swarm_status()`

**4. Read full docs** → [MCP-TOOLS.md](/workspaces/agent-control-plane/docs/guides/MCP-TOOLS.md)
