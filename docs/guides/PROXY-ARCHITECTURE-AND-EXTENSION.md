...
export class AnthropicToMistralProxy {
  ...
  constructor(config: { mistralApiKey: string; mistralBaseUrl?: string; defaultModel?: string }) {
    this.app = express();
...
export class AnthropicToOllamaProxy {
  ...
  constructor(config: { ollamaBaseUrl?: string; defaultModel?: string }) {
    this.app = express();
...
Need help adding a provider? Open an issue: [GitHub Issues](https://github.com/Aktoh-Cyber/agent-control-plane/issues)
