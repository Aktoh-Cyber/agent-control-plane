...
npx agent-control-plane --agent <agent> --task "<task>" --provider onnx

# Examples
npx agent-control-plane --agent coder --task "Write Python hello world" --provider onnx
npx agent-control-plane --agent researcher --task "Analyze AI trends" --provider onnx
npx agent-control-plane --agent reviewer --task "Review code quality" --provider onnx
```
...
See full list: `npx agent-control-plane --list`
...
1. **Offline Development**
   ...
   export PROVIDER=onnx
   export ONNX_OPTIMIZED=true
   npx agent-control-plane --agent coder --task "Build feature"
   ```
...
2. **Privacy-Sensitive Data**
   ...
   export PROVIDER=onnx
   export ONNX_OPTIMIZED=true
   npx agent-control-plane --agent coder --task "Process medical records"
   ```
...
3. **Cost Optimization**
   ...
   for task in task1 task2 task3; do
     npx agent-control-plane --agent coder --task "$task"
   done
   ```
...
5. **GPU-Accelerated Development**
   ...
   npx agent-control-plane --agent coder --task "Complex feature"
   ```
...
npx agent-control-plane --agent coder --task "Hello world" --provider onnx

# Complex tasks - cheap OpenRouter
npx agent-control-plane --agent coder --task "Design distributed system" \
...
export USE_ONNX=true
npx agent-control-plane --agent coder --task "Process PII"

# Non-sensitive - OpenRouter (cheaper)
unset USE_ONNX
export OPENROUTER_API_KEY=sk-or-v1-...
npx agent-control-plane --agent coder --task "Public API"
```
...
npx agent-control-plane --agent coder --task "small task" --provider onnx
```
...
2. **Explore All Agents:** `npx agent-control-plane --list`
