...
```typescript
import { createBillingSystem, SubscriptionTier, BillingCycle } from 'agent-control-plane/billing';
...
```typescript
import { UsageMetric } from 'agent-control-plane/billing';
...
```typescript
import { PricingManager, SubscriptionTier } from 'agent-control-plane/billing';
...
// Compare tiers
const comparison = pricing.compareFeatures(SubscriptionTier.Starter, SubscriptionTier.Pro);
...
```typescript
import { MeteringEngine, UsageMetric } from 'agent-control-plane/billing';
...
```typescript
import { SubscriptionManager, SubscriptionTier, BillingCycle } from 'agent-control-plane/billing';
...
```typescript
import { CouponManager, CouponType, SubscriptionTier } from 'agent-control-plane/billing';
...
```typescript
import { PaymentProcessor, PaymentProvider } from 'agent-control-plane/billing';
...
import {
  ...
  BillingCycle,
  UsageMetric,
} from 'agent-control-plane/billing';
...
```typescript
import { BillingSystem, UsageMetric } from 'agent-control-plane/billing';
...
class AgentService {
  ...
  async executeAgent(subscriptionId: string, hours: number) {
    ...
    const allowed = await this.billing.checkQuota(subscriptionId, UsageMetric.AgentHours);
...
```typescript
import { createBillingSystem } from 'agent-control-plane/billing';
...
The billing CLI is included in the agent-control-plane package:

```bash
npx agent-control-plane billing --help
```
...
```bash
# Create subscription
npx agent-control-plane billing subscription:create user_123 pro monthly pm_card_123

# Upgrade subscription
npx agent-control-plane billing subscription:upgrade sub_123 enterprise

# Cancel subscription
npx agent-control-plane billing subscription:cancel sub_123 false

# Get subscription status
npx agent-control-plane billing subscription:status sub_123
```
...
```bash
# Record usage
npx agent-control-plane billing usage:record sub_123 agent_hours 50

# Get usage summary
npx agent-control-plane billing usage:summary sub_123

# Check quota
npx agent-control-plane billing usage:check sub_123 agent_hours
```
...
```bash
# List all tiers
npx agent-control-plane billing pricing:tiers

# Compare tiers
npx agent-control-plane billing pricing:compare starter pro
```
...
```bash
# Create coupon
npx agent-control-plane billing coupon:create SAVE20 percentage 20

# Validate coupon
npx agent-control-plane billing coupon:validate SAVE20 pro 99

# List coupons
npx agent-control-plane billing coupon:list
```
...
```typescript
import { createBillingSystem, createBillingMCPTools } from 'agent-control-plane/billing';
...
```typescript
import { StorageAdapter } from 'agent-control-plane/billing';
...
- **Documentation**: [https://github.com/Aktoh-Cyber/agent-control-plane](https://github.com/Aktoh-Cyber/agent-control-plane)
- **Issues**: [https://github.com/Aktoh-Cyber/agent-control-plane/issues](https://github.com/Aktoh-Cyber/agent-control-plane/issues)
- **Discord**: [Join our community](https://discord.gg/agent-control-plane)
