...
```bash
npx gendev hook pre-task -d "Continue API development" --load-memory
```

### Manual agent control

```bash
npx gendev hook pre-task -d "Debug issue #123" --auto-spawn-agents false
```

### Full optimization

```bash
npx gendev hook pre-task -d "Refactor codebase" --optimize-topology --estimate-complexity
```
...
npx gendev hook pre-task --description "Your task here"
```
