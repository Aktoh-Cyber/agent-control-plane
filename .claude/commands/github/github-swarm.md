...
```bash
npx gendev github swarm [options]
```
...
```bash
npx gendev github swarm --repository owner/repo
```

### Maintenance-focused swarm

```bash
npx gendev github swarm -r owner/repo -f maintenance --issue-labels
```

### Development swarm with PR automation

```bash
npx gendev github swarm -r owner/repo -f development --auto-pr --code-review
```

### Full-featured triage swarm

```bash
npx gendev github swarm -r owner/repo -a 8 -f triage --issue-labels --auto-pr
```
