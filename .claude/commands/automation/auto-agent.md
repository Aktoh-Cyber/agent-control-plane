...
```bash
npx gendev auto agent --task "Build a REST API with authentication"
```

### Constrained spawning

```bash
npx gendev auto agent -t "Debug performance issue" --max-agents 3
```

### Analysis only

```bash
npx gendev auto agent -t "Refactor codebase" --no-spawn
```

### Minimal strategy

```bash
npx gendev auto agent -t "Fix bug in login" -s minimal
```
