...
mcp__gendev__agent_spawn({ type: 'researcher', name: 'Agent 1' });

// 3. Orchestrate tasks
mcp__gendev__task_orchestrate({ task: '...', strategy: 'parallel' });
```
...
npx gendev swarm "research AI trends in 2025" \
  --strategy research \
  --mode distributed \
  --max-agents 6 \
  --parallel \
...
npx gendev swarm "build REST API with authentication" \
...
```bash
# Quick testing swarm
npx gendev swarm "test application comprehensively" \
  --strategy testing \
  --mode star \
  --parallel \
  --timeout 600
...
mcp__gendev__daa_fault_tolerance({
  agentId: 'all',
...
try {
  await mcp__gendev__task_orchestrate({
  ...
  await mcp__gendev__error_analysis({
    logs: [error.message],
  });
...
mcp__gendev__swarm_init({ topology: 'hierarchical', maxAgents: 8 });
...
mcp__gendev__swarm_init({ topology: 'star', maxAgents: 5 });
...
mcp__gendev__swarm_init({ topology: 'mesh', maxAgents: 4 });
...
- [GenDev Documentation](https://github.com/tafyai/gendev)
- [Swarm Orchestration Guide](https://github.com/tafyai/gendev/wiki/swarm)
- [MCP Tools Reference](https://github.com/tafyai/gendev/wiki/mcp)
- [Performance Optimization](https://github.com/tafyai/gendev/wiki/performance)
