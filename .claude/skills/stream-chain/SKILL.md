...
```bash
gendev stream-chain pipeline analysis
```
...
```bash
gendev stream-chain run <prompt1> <prompt2> [...] [options]
```
...
```bash
gendev stream-chain pipeline <type> [options]
```
...
```bash
gendev stream-chain pipeline refactor
```
...
```bash
gendev stream-chain pipeline test
```
...
```bash
gendev stream-chain pipeline optimize
```
...
```bash
gendev stream-chain pipeline refactor --timeout 60 --verbose
```

#### Debug Test Generation

```bash
gendev stream-chain pipeline test --debug
```

#### Comprehensive Optimization

```bash
gendev stream-chain pipeline optimize --timeout 90 --verbose
```
...
Define reusable pipelines in `.gendev/config.json`:
...
```bash
gendev stream-chain pipeline security
gendev stream-chain pipeline documentation
```
...
gendev swarm init --topology mesh
...
# Results stored in .gendev/memory/stream-chain/
```
...
gendev stream-chain pipeline optimize --debug
...
```bash
gendev stream-chain run "complex task" --timeout 120
```
...
```bash
gendev stream-chain run "step 1" "step 2" --debug
```
...
cat .gendev/config.json | grep -A 10 "streamChain"
```
