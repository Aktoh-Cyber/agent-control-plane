...
```bash
gendev pair --start --mode navigator
```
...
gendev pair --start --mode switch

# 5-minute intervals (rapid)
gendev pair --start --mode switch --interval 5m

# 15-minute intervals (deep focus)
gendev pair --start --mode switch --interval 15m
```
...
Create `.gendev/pair-config.json`:
...
```bash
# Set configuration
gendev pair config set defaultMode switch
gendev pair config set verification.threshold 0.98

# Get configuration
gendev pair config get
gendev pair config get defaultMode

# Export/Import
gendev pair config export > config.json
gendev pair config import config.json

# Reset
gendev pair config reset
```
...
```bash
# Create profile
gendev pair profile create refactoring \
  --mode driver \
  --verify true \
  --threshold 0.98 \
  --focus refactor
...
# Use profile
gendev pair --start --profile refactoring

# List profiles
gendev pair profile list
```
...
```bash
# Save session
gendev pair --save [--name <name>]

# Load session
gendev pair --load <session-id>

# Export session
gendev pair --export <session-id> [--format json|md]

# Generate report
gendev pair --report <session-id>
```
...
```bash
# Start in background
gendev pair --start --background

# Monitor background session
gendev pair --monitor

# Attach to background session
gendev pair --attach <session-id>

# End background session
gendev pair --end <session-id>
```
...
gendev pair --start --record

# Replay session
gendev pair --replay <session-id>

# Session analytics
gendev pair --analytics <session-id>
```
...
```bash
gendev pair --start --git --auto-commit
```

**With CI/CD:**

```bash
gendev pair --start --ci --non-interactive
```

**With IDE:**

```bash
gendev pair --start --ide vscode
```
...
- Run `gendev pair config validate`
...
- `gendev pair --help` - Show help
- `gendev pair config` - Manage configuration
- `gendev pair profile` - Manage profiles
- `gendev pair templates` - List templates
- `gendev pair agents` - List available agents
