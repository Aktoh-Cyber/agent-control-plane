...
description: 'Orchestrate multi-agent swarms with agent-control-plane for parallel task execution, dynamic topology, and intelligent coordination. Use when scaling beyond single agents, implementing complex workflows, or building distributed AI systems.'
---
...
Orchestrates multi-agent swarms using agent-control-plane's advanced coordination system. Supports mesh, hierarchical, and adaptive topologies with automatic task distribution, load balancing, and fault tolerance.
...
```bash
# Initialize swarm
npx agent-control-plane hooks swarm-init --topology mesh --max-agents 5

# Spawn agents
npx agent-control-plane hooks agent-spawn --type coder
npx agent-control-plane hooks agent-spawn --type tester
npx agent-control-plane hooks agent-spawn --type reviewer

# Orchestrate task
npx agent-control-plane hooks task-orchestrate \
...
npx agent-control-plane hooks pre-task --description "Build API"

# Post-task synchronization
npx agent-control-plane hooks post-task --task-id "task-123"

# Session restore
npx agent-control-plane hooks session-restore --session-id "swarm-001"
```
