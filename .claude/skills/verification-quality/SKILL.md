...
- GenDev installed (`npx gendev@alpha`)
...
npx gendev@alpha truth

# Run verification check
npx gendev@alpha verify check

# Verify specific file with custom threshold
npx gendev@alpha verify check --file src/app.js --threshold 0.98

# Rollback last failed verification
npx gendev@alpha verify rollback --last-good
```
...
npx gendev@alpha truth --period 7d

# View scores for specific agent
npx gendev@alpha truth --agent coder --period 24h

# Find files/tasks below threshold
npx gendev@alpha truth --threshold 0.8
```
...
npx gendev@alpha truth --format table

# JSON for programmatic access
npx gendev@alpha truth --format json

# CSV for spreadsheet analysis
npx gendev@alpha truth --format csv

# HTML report with visualizations
npx gendev@alpha truth --format html --export report.html
```
...
npx gendev@alpha truth --watch

# Export metrics automatically
npx gendev@alpha truth --export .gendev/metrics/truth-$(date +%Y%m%d).json
```
...
```bash
# Verify single file
npx gendev@alpha verify check --file src/app.js

# Verify directory recursively
npx gendev@alpha verify check --directory src/

# Verify with auto-fix enabled
npx gendev@alpha verify check --file src/utils.js --auto-fix
...
npx gendev@alpha verify check --task task-123

# Verify with custom threshold
npx gendev@alpha verify check --task task-456 --threshold 0.99

# Verbose output for debugging
npx gendev@alpha verify check --task task-789 --verbose
```
...
npx gendev@alpha verify batch --files "*.js" --parallel

# Verify with pattern matching
npx gendev@alpha verify batch --pattern "src/**/*.ts"

# Integration test suite
npx gendev@alpha verify integration --test-suite full
```
...
npx gendev@alpha verify check --json > verification.json
...
npx gendev@alpha verify rollback --to-commit abc123

# Interactive rollback with preview
npx gendev@alpha verify rollback --interactive
```
...
npx gendev@alpha verify rollback --selective

# Rollback with automatic backup
npx gendev@alpha verify rollback --backup-first

# Dry-run mode (preview without executing)
npx gendev@alpha verify rollback --dry-run
```
...
```bash
# JSON report
npx gendev@alpha verify report --format json

# HTML report with charts
npx gendev@alpha verify report --export metrics.html --format html

# CSV for data analysis
npx gendev@alpha verify report --format csv --export metrics.csv

# Markdown summary
npx gendev@alpha verify report --format markdown
```
...
```bash
# Last 24 hours
npx gendev@alpha verify report --period 24h

# Last 7 days
npx gendev@alpha verify report --period 7d

# Last 30 days with trends
npx gendev@alpha verify report --period 30d --include-trends

# Custom date range
npx gendev@alpha verify report --from 2025-01-01 --to 2025-01-31
```
...
npx gendev@alpha verify dashboard

# Custom port
npx gendev@alpha verify dashboard --port 8080

# Export dashboard data
npx gendev@alpha verify dashboard --export

# Dashboard with auto-refresh
npx gendev@alpha verify dashboard --refresh 5s
```
...
Set verification preferences in `.gendev/config.json`:
...
```json
{
  ...
  "truth": {
    ...
    "autoExport": {
      "enabled": true,
      "path": ".gendev/metrics/truth-daily.json"
    }
  }
}
```
...
npx gendev@alpha verify check --threshold 0.99

# Lenient mode (90% acceptable)
npx gendev@alpha verify check --threshold 0.90

# Set default threshold
npx gendev@alpha config set verification.threshold 0.98
```
...
verify:
  stage: test
  script:
    - npx gendev@alpha verify check --threshold 0.95 --json > verification.json
    - |
      score=$(jq '.overallScore' verification.json)
...
npx gendev@alpha swarm --verify --threshold 0.98

# Hive Mind with auto-rollback
npx gendev@alpha hive-mind --verify --rollback-on-fail

# Training pipeline with verification
npx gendev@alpha train --verify --threshold 0.99
```
...
npx gendev@alpha pair --verify --real-time

# Pair with custom threshold
npx gendev@alpha pair --verify --threshold 0.97 --auto-fix
```
...
npx gendev@alpha verify watch --directory src/

# Watch with auto-fix
npx gendev@alpha verify watch --directory src/ --auto-fix

# Watch with notifications
npx gendev@alpha verify watch --notify --threshold 0.95
```
...
npx gendev@alpha truth --format json | \
  curl -X POST https://pushgateway.example.com/metrics/job/gendev \
  -d @-
...
npx gendev@alpha verify install-hook --pre-commit

# .git/hooks/pre-commit example:
#!/bin/bash
npx gendev@alpha verify check --threshold 0.95 --json > /tmp/verify.json

score=$(jq '.overallScore' /tmp/verify.json)
if (( $(echo "$score < 0.95" | bc -l) )); then
  echo "❌ Verification failed with score: $score"
  echo "Run 'npx gendev@alpha verify check --verbose' for details"
  exit 1
fi
...
npx gendev@alpha truth --verbose --threshold 0.0

# Check specific criteria
npx gendev@alpha verify check --verbose

# View agent-specific issues
npx gendev@alpha truth --agent <agent-name> --format json
```
...
npx gendev@alpha verify rollback --history
...
```bash
# Increase timeout
npx gendev@alpha verify check --timeout 60s

# Verify in batches
npx gendev@alpha verify batch --batch-size 10
```
...
- `npx gendev@alpha pair` - Collaborative development with verification
- `npx gendev@alpha train` - Training with verification feedback
- `npx gendev@alpha swarm` - Multi-agent coordination with quality checks
- `npx gendev@alpha report` - Generate comprehensive project reports
