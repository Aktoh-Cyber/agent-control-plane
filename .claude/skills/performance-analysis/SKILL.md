...
version: 1.0.0
description: Comprehensive performance analysis, bottleneck detection, and optimization recommendations for GenDev swarms
category: monitoring
...
```bash
npx gendev bottleneck detect
```

### Generate Performance Report

```bash
npx gendev analysis performance-report --format html --include-metrics
```

### Analyze and Auto-Fix

```bash
npx gendev bottleneck detect --fix --threshold 15
```
...
```bash
npx gendev bottleneck detect [options]
```
...
npx gendev bottleneck detect --swarm-id swarm-123 -t 24h

# Export detailed analysis
npx gendev bottleneck detect -t 24h -e bottlenecks.json

# Auto-fix detected issues
npx gendev bottleneck detect --fix --threshold 15

# Low threshold for sensitive detection
npx gendev bottleneck detect --threshold 10 --export critical-issues.json
```
...
mcp__gendev__bottleneck_detect({
  timeRange: '1h',
  threshold: 20,
  autoFix: false,
});
...
npx gendev analysis performance-report --compare swarm-123 --format markdown

# Custom output with specific sections
npx gendev analysis performance-report \
...
npx gendev swarm monitor --interval 5
...
async function analyzePerformance() {
  ...
  const bottlenecks = await runCommand('npx gendev bottleneck detect --format json');

  // Generate performance report
  const report = await runCommand('npx gendev analysis performance-report --format json');
  ...
  fs.writeFileSync('analysis/combined-report.json', JSON.stringify(analysis, null, 2));
...
npx gendev bottleneck detect --threshold 10

# Check cache performance
npx gendev cache manage --action stats

# Review memory metrics
npx gendev memory usage
```
...
```bash
# Identify slow tasks
npx gendev task status --detailed

# Analyze coordination overhead
npx gendev bottleneck detect --time-range 1h
...
npx gendev analysis performance-report --sections metrics

# Review cache strategy
npx gendev cache manage --action analyze

# Enable cache warming
npx gendev bottleneck detect --fix
```
...
- `npx gendev swarm monitor` - Real-time monitoring
- `npx gendev token usage` - Token optimization analysis
- `npx gendev cache manage` - Cache optimization
- `npx gendev agent metrics` - Agent performance metrics
- `npx gendev task status` - Task execution analysis
