...
- GenDev CLI installed (`npm install -g gendev@alpha`)
- Claude Code with hooks enabled
- `.claude/settings.json` with hook configurations

**Optional:**

- MCP servers configured (gendev, ruv-swarm, agentic-cloud)
...
npx gendev hook pre-task --description "Implement authentication"

# Post-edit hook (auto-formats and stores in memory)
npx gendev hook post-edit --file "src/auth.js" --memory-key "auth/login"

# Session end hook (saves state and metrics)
npx gendev hook session-end --session-id "dev-session" --export-metrics
```
...
```bash
npx gendev hook pre-edit [options]
...
Examples:
  npx gendev hook pre-edit --file "src/auth/login.js"
  npx gendev hook pre-edit -f "config/db.js" --validate-syntax
  npx gendev hook pre-edit -f "production.env" --backup-file --check-conflicts
```
...
```bash
npx gendev hook pre-bash --command <cmd>
...
Examples:
  npx gendev hook pre-bash -c "rm -rf /tmp/cache"
  npx gendev hook pre-bash --command "docker build ." --estimate-resources
```
...
```bash
npx gendev hook pre-task [options]
...
Examples:
  npx gendev hook pre-task --description "Implement user authentication"
  npx gendev hook pre-task -d "Continue API dev" --load-memory
  npx gendev hook pre-task -d "Refactor codebase" --optimize-topology
```
...
```bash
npx gendev hook pre-search --query <query>
...
Examples:
  npx gendev hook pre-search -q "authentication middleware"
```
...
```bash
npx gendev hook post-edit [options]
...
Examples:
  npx gendev hook post-edit --file "src/components/Button.jsx"
  npx gendev hook post-edit -f "api/auth.js" --memory-key "auth/login"
  npx gendev hook post-edit -f "utils/helpers.ts" --train-patterns
```
...
```bash
npx gendev hook post-bash --command <cmd>
...
Examples:
  npx gendev hook post-bash -c "npm test" --update-metrics
```
...
```bash
npx gendev hook post-task [options]
...
Examples:
  npx gendev hook post-task --task-id "auth-implementation"
  npx gendev hook post-task -t "api-refactor" --analyze-performance
  npx gendev hook post-task -t "bug-fix-123" --store-decisions
```
...
```bash
npx gendev hook post-search --query <query> --results <path>
...
Examples:
  npx gendev hook post-search -q "auth" -r "results.json" --train-patterns
```
...
```bash
npx gendev hook mcp-initialized --swarm-id <id>
...
```bash
npx gendev hook agent-spawned --agent-id <id> --type <type>
...
```bash
npx gendev hook task-orchestrated --task-id <id>
...
```bash
npx gendev hook neural-trained --pattern <name>
...
```bash
npx gendev hook memory-sync --namespace <ns>
...
```bash
npx gendev hook session-start --session-id <id>
...
```bash
npx gendev hook session-restore --session-id <id>
...
Examples:
  npx gendev hook session-restore --session-id "swarm-20241019"
  npx gendev hook session-restore -s "feature-auth" --restore-memory
```
...
```bash
npx gendev hook session-end [options]
...
Examples:
  npx gendev hook session-end --session-id "dev-session-2024"
  npx gendev hook session-end -s "feature-auth" --export-metrics --generate-summary
  npx gendev hook session-end -s "quick-fix" --cleanup-temp
```
...
```bash
npx gendev hook notify --message <msg>
...
Examples:
  npx gendev hook notify -m "Task completed" --level info
  npx gendev hook notify -m "Critical error" --level error --broadcast
```
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx gendev hook pre-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/current'"
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx gendev hook pre-bash --command '${tool.params.command}'"
          }
        ]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx gendev hook post-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/complete' --auto-format --train-patterns"
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx gendev hook post-bash --command '${tool.params.command}' --update-metrics"
          }
        ]
      }
    ]
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx gendev hook pre-edit --file '${tool.params.file_path}' --auto-assign-agent --validate-syntax",
            "timeout": 3000,
      ...
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx gendev hook pre-task --description '${tool.params.task}' --auto-spawn-agents --load-memory",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx gendev hook pre-search --query '${tool.params.pattern}' --check-cache"
          }
        ]
      }
    ],
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx gendev hook post-edit --file '${tool.params.file_path}' --memory-key 'edits/${tool.params.file_path}' --auto-format --train-patterns",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx gendev hook post-task --task-id '${result.task_id}' --analyze-performance --store-decisions --export-learnings",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx gendev hook post-search --query '${tool.params.pattern}' --cache-results --train-patterns"
          }
        ]
      }
    ],
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "npx gendev hook session-start --session-id '${session.id}' --load-context"
          }
        ]
      }
    ],
    ...
    "SessionEnd": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "npx gendev hook session-end --session-id '${session.id}' --export-metrics --generate-summary --cleanup-temp"
          }
        ]
      }
    ]
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx gendev hook check-protected --file '${tool.params.file_path}'"
          }
        ]
      }
    ]
...
```javascript
// Hook command
npx gendev hook pre-task --description "Build REST API"
...
```javascript
// Hook command
npx gendev hook post-edit --file "api/auth.js"
...
```javascript
// Hook command
npx gendev hook session-end --session-id "dev-2024"
...
for FILE in $FILES; do
  ...
  npx gendev hook pre-edit --file "$FILE" --validate-syntax
  ...
  npx gendev hook post-edit --file "$FILE" --auto-format
done
...
# Run quality checks
npx gendev hook session-end \
  --generate-report \
  --export-metrics

# Verify quality thresholds
TRUTH_SCORE=$(npx gendev metrics score --format json | jq -r '.truth_score')
...
npx gendev hook pre-task \
  --description "Implement user authentication API" \
  --auto-spawn-agents \
  --load-memory

# STEP 2: Work begins - pre-edit validation
npx gendev hook pre-edit \
  --file "api/auth.js" \
  --auto-assign-agent \
  --validate-syntax
...
npx gendev hook session-restore \
...
npx gendev hook session-start --session-id "fullstack-feature"
...
# Backend work
npx gendev hook pre-edit --file "api/profile.js"
# ... implement backend ...
npx gendev hook post-edit \
  --file "api/profile.js" \
...
npx gendev hook pre-edit --file "components/Profile.jsx"
# ... implement frontend ...
npx gendev hook post-edit \
...
npx gendev hook session-start --session-id "debug-memory-leak"
...
npx gendev hook pre-search --query "EventEmitter"
# ... search executes ...
npx gendev hook post-search \
  --query "EventEmitter" \
  --cache-results
...
npx gendev hook pre-task --description "Analyze code complexity"
# ... analysis ...
npx gendev hook post-task \
  --task-id "analysis" \
  --store-decisions
...
for file in src/**/*.js; do
  npx gendev hook pre-edit --file "$file" --backup-file
  # ... refactor ...
  npx gendev hook post-edit \
    --file "$file" \
...
npx gendev hook pre-edit --file "test.js" --debug

# Check hook execution logs
cat .gendev/logs/hooks-$(date +%Y-%m-%d).log

# Validate configuration
npx gendev hook validate-config
```
...
- `npx gendev init --hooks` - Initialize hooks system
- `npx gendev hook --list` - List available hooks
- `npx gendev hook --test <hook>` - Test specific hook
- `npx gendev memory usage` - Manage memory
- `npx gendev agent spawn` - Spawn agents
- `npx gendev swarm init` - Initialize swarm
