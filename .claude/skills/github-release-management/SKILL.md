...
npx gendev github release-create \
  --version "2.0.0" \
  --build-artifacts \
...
npx gendev swarm init --topology hierarchical

# Execute complete release pipeline
npx gendev sparc pipeline "Release v2.0.0 with full validation"
```
...
[Single Message - Swarm Initialization]:
  ...
  mcp__gendev__agent_spawn { type: "coordinator", name: "Release Director" }
  mcp__gendev__agent_spawn { type: "coder", name: "Version Manager" }
  mcp__gendev__agent_spawn { type: "tester", name: "QA Engineer" }
  mcp__gendev__agent_spawn { type: "reviewer", name: "Release Reviewer" }
  mcp__gendev__agent_spawn { type: "analyst", name: "Deployment Analyst" }
  mcp__gendev__agent_spawn { type: "researcher", name: "Compatibility Checker" }
```
...
npx gendev github version-suggest \
  --current v1.2.3 \
  --analyze-commits \
  --check-compatibility \
  --suggest-pre-release
...
```bash
# Multi-platform build coordination
npx gendev github release-build \
...
npx gendev github release-test \
...
npx gendev github release-deploy \
...
[Single Message - Multi-Package Release]:
  ...
  mcp__gendev__swarm_init { topology: "mesh", maxAgents: 8 }

  // Spawn package-specific agents
  Task("Package A Manager", "Coordinate gendev package release v1.0.72", "coder")
  ...
  Write("packages/gendev/package.json", "[v1.0.72 content]")
  ...
  Bash("cd packages/gendev && npm install && npm test")
  Bash("cd packages/ruv-swarm && npm install && npm test")
  Bash("npm run test:integration")

  // Create unified release PR
  Bash(`gh pr create \
    --title "Release: gendev v1.0.72, ruv-swarm v1.0.12" \
...
npx gendev github multi-release \
...
[Single Message - Cross-Repo Release]:
  ...
  mcp__gendev__swarm_init { topology: "star", maxAgents: 6 }
  ...
  mcp__gendev__swarm_monitor { interval: 5, duration: 300 }
```
...
npx gendev github emergency-release \
  --issue 789 \
  --severity critical \
...
npx gendev github release-validate \
  --checks "
    version-conflicts,
...
npx gendev github performance-test \
  --baseline v1.9.0 \
  --candidate v2.0.0 \
...
npx gendev github release-monitor \
  --version v2.0.0 \
...
npx gendev github release-analytics \
  --version v2.0.0 \
  --compare-with v1.9.0 \
...
npx gendev github rollback-config \
  --triggers '{
    "error-rate": ">5%",
    "latency-p99": ">1000ms",
    "availability": "<99.9%",
...
npx gendev github release-security \
  --scan-dependencies \
  --check-secrets \
  --audit-permissions \
  --sign-artifacts \
...
npx gendev github release-compliance \
    ...
    steps:
      ...
      - name: Initialize Release Swarm
        run: |
          ...
          npx gendev@alpha swarm init --topology hierarchical
      ...
      - name: Generate Release Changelog
        run: |
          ...
          CHANGELOG=$(npx gendev@alpha github changelog \
      ...
      - name: Build Release Artifacts
        run: |
          ...
          npx gendev@alpha github release-build \
            --platforms "linux,macos,windows" \
            --architectures "x64,arm64" \
            --parallel
      ...
      - name: Security Scan
        run: |
          ...
          npx gendev@alpha github release-security \
            --scan-dependencies \
            --check-secrets \
            --sign-artifacts
      ...
      - name: Post-Release Validation
        run: |
          ...
          # Validate deployment
          npx gendev@alpha github release-validate \
      ...
      - name: Monitor Release
        run: |
          ...
          npx gendev@alpha github release-monitor \
            --version ${{ github.ref_name }} \
            --duration 1h \
            --alert-on-errors &
```
  ...
  emergency-hotfix:
    ...
    steps:
      ...
      - name: Emergency Release
        run: |
          npx gendev@alpha github emergency-release \
...
npx gendev@alpha diagnostic-run \
  --component build \
  --verbose
...
npx gendev@alpha github compat-test \
...
npx gendev@alpha github rollback \
  --to-version v1.9.9 \
...
npx gendev@alpha github release-analytics \
  --version v2.0.0 \
  --identify-issues
```
...
npx gendev@alpha github version-sync \
...
- [GenDev SPARC Guide](../../docs/sparc-methodology.md)
...
- Discussions: https://github.com/tafyai/gendev/discussions
- Documentation: https://gendev.dev/docs
