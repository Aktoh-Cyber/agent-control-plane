...
tags:
  [hive-mind, swarm, queen-worker, consensus, collective-intelligence, multi-agent, coordination]
author: GenDev Team
---
...
The Hive Mind system represents the pinnacle of multi-agent coordination in GenDev, implementing a queen-led hierarchical architecture where a strategic queen coordinator directs specialized worker agents through collective decision-making and shared memory.
...
npx gendev hive-mind init

# Force reinitialize
npx gendev hive-mind init --force

# Custom configuration
npx gendev hive-mind init --config hive-config.json
```
...
npx gendev hive-mind spawn "Build microservices architecture"

# Strategic queen type
npx gendev hive-mind spawn "Research AI patterns" --queen-type strategic

# Tactical queen with max workers
npx gendev hive-mind spawn "Implement API" --queen-type tactical --max-workers 12

# Adaptive queen with consensus
npx gendev hive-mind spawn "Optimize system" --queen-type adaptive --consensus byzantine

# Generate Claude Code commands
npx gendev hive-mind spawn "Build full-stack app" --claude
```
...
npx gendev hive-mind status

# Get detailed metrics
npx gendev hive-mind metrics

# Monitor collective memory
npx gendev hive-mind memory
```
...
npx gendev hive-mind sessions

# Pause a session
npx gendev hive-mind pause <session-id>

# Resume a paused session
npx gendev hive-mind resume <session-id>

# Stop a running session
npx gendev hive-mind stop <session-id>
```
...
const decision = await hiveMind.buildConsensus('Architecture pattern selection', [
  'microservices',
  'monolith',
  'serverless',
]);
...
```bash
npx gendev hive-mind spawn "Build REST API" --claude
```
...
npx gendev sparc tdd "User authentication" --hive-mind
...
npx gendev hive-mind spawn "Analyze repo quality" --objective "owner/repo"

# PR review coordination
npx gendev hive-mind spawn "Review PR #123" --queen-type tactical
```
...
Hive Mind integrates with GenDev hooks for automation:
...
```bash
npx gendev hive-mind spawn "Research ML frameworks" --queen-type strategic
```

**Tactical Queens** - For implementation and execution

```bash
npx gendev hive-mind spawn "Build authentication" --queen-type tactical
```

**Adaptive Queens** - For optimization and dynamic tasks

```bash
npx gendev hive-mind spawn "Optimize performance" --queen-type adaptive
```
...
npx gendev hive-mind memory --gc

# Optimize database
npx gendev hive-mind memory --optimize

# Export and clear
npx gendev hive-mind memory --export --clear
```
...
npx gendev hive-mind spawn "..." --consensus weighted

# Or use simple majority
npx gendev hive-mind spawn "..." --consensus majority
```
...
```bash
# Frontend hive
npx gendev hive-mind spawn "Build UI" --name frontend-hive

# Backend hive
npx gendev hive-mind spawn "Build API" --name backend-hive
...
npx gendev hive-mind export <session-id> --output backup.json

# Import session
npx gendev hive-mind import backup.json
```
...
npx gendev hive-mind spawn "Build e-commerce platform" \
...
```bash
# Spawn research hive
npx gendev hive-mind spawn "Research GraphQL vs REST" \
  --queen-type adaptive \
  --consensus byzantine
...
```bash
# Review coordination
npx gendev hive-mind spawn "Review PR #456" \
  --queen-type tactical \
  --max-workers 6
...
- [Hive Mind Documentation](https://github.com/tafyai/gendev/docs/hive-mind)
- [Collective Intelligence Patterns](https://github.com/tafyai/gendev/docs/patterns)
- [Byzantine Consensus](https://github.com/tafyai/gendev/docs/consensus)
- [Memory Optimization](https://github.com/tafyai/gendev/docs/memory)

---

**Skill Version**: 1.0.0
**Last Updated**: 2025-10-19
**Maintained By**: GenDev Team
**License**: MIT
