...
hooks:
  pre: |
    ...
    npx agent-control-plane@latest reasoningbank retrieve "$TASK" --domain adaptive-learning
  post: |
    echo "✨ Learning from execution..."
    # Store execution results for future learning
    npx agent-control-plane@latest reasoningbank distill --task-id "$TASK_ID" --agent adaptive-learner
---
