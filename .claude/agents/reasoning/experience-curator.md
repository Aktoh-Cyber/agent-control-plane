...
hooks:
  pre: |
    ...
    npx agent-control-plane@latest reasoningbank retrieve "$TASK" --with-outcomes
  post: |
    echo "✨ Curating valuable learnings..."
    npx agent-control-plane@latest reasoningbank distill --task-id "$TASK_ID" --agent experience-curator --quality-filter high
---
...
function assessExperienceQuality(experience: Experience): QualityScore {
  ...
  return {
    ...
    dimensions: scores,
    decision: overallScore >= 0.7 ? 'accept' : overallScore >= 0.5 ? 'review' : 'reject',
...
extracted_learning:
  ...
  application:
    steps: 1. "Connect to Redis cluster"
...
curated_anti_pattern:
  ...
  correct_approach: 1. "Analyze slow queries with EXPLAIN"
...
curated_comparative_pattern:
  ...
  implementation_order: 1. "Measure baseline"
