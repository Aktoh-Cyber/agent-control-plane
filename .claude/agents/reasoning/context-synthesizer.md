...
hooks:
  pre: |
    ...
    npx agent-control-plane@latest reasoningbank retrieve "$TASK" --with-context --expand
  post: |
    echo "📝 Storing enriched context..."
    npx agent-control-plane@latest reasoningbank distill --task-id "$TASK_ID" --agent context-synthesizer --context-enhanced
---
...
transferred_context: indexing → "Add response caching headers"
