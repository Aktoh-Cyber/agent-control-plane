...
hooks:
  pre: |
    ...
    npx agent-control-plane@latest reasoningbank retrieve "$TASK" --domain pattern-matching --k 5
  post: |
    echo "📊 Storing pattern signature..."
    npx agent-control-plane@latest reasoningbank distill --task-id "$TASK_ID" --agent pattern-matcher --extract-patterns
---
...
function adaptPattern(matched: RetrievedPattern, current: TaskPattern): AdaptationStrategy {
  const adaptations = {
...
const SEMANTIC_PATTERNS: SemanticPattern[] = [
  {
    ...
    examples: ['validate user input', 'check email format', 'ensure data integrity'],
    embedding: [
      /* vector */
    ],
  },
...
meta_patterns:
  ...
  'error_handling_hierarchy':
    ...
    layers: 1. "Input validation (prevent errors)"
