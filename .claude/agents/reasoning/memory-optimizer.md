...
hooks:
  pre: |
    ...
    npx agent-control-plane@latest reasoningbank status
  post: |
    echo "🧹 Running memory consolidation if needed..."
    npx agent-control-plane@latest reasoningbank consolidate --auto
---
...
```typescript
async function consolidateSimilarPatterns(threshold: number = 0.9): Promise<ConsolidationResult> {
...
const HEALTH_CHECKS: HealthCheck[] = [
  ...
  },
  {
    ...
    check: async () => countSimilarPatterns(0.9),
    threshold: 20,
...
const candidates = await requestFromAgent({
  ...
  params: { threshold: 0.9, domain: 'all' },
});
