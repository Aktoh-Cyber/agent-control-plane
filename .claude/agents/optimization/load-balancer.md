...
npx gendev agent spawn load-balancer --type coordinator

# Start load balancing
npx gendev load-balance --swarm-id <id> --strategy adaptive

# Monitor load distribution
npx gendev agent-metrics --type load-balancer

# Adjust balancing parameters
npx gendev config-manage --action update --config '{"stealThreshold": 5, "agingBoost": 10}'
```
...
npx gendev performance-report --format detailed

# Bottleneck analysis
npx gendev bottleneck-analyze --component swarm-coordination

# Resource utilization tracking
npx gendev metrics-collect --components ["load-balancer", "task-queue"]
```
