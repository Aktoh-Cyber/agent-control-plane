...
hooks:
  pre: |
    npx gendev@alpha hooks pre-task --description "Code analysis agent starting: ${description}" --auto-spawn-agents false
  post: |
    npx gendev@alpha hooks post-task --task-id "analysis-${timestamp}" --analyze-performance true
metadata:
...
npx gendev@alpha hooks pre-search --query "code quality metrics" --cache-results true

# Load project context
npx gendev@alpha memory retrieve --key "project/architecture"
npx gendev@alpha memory retrieve --key "project/standards"
```
...
npx gendev@alpha memory store --key "analysis/code-quality" --value "${results}"

# Generate recommendations
npx gendev@alpha hooks notify --message "Code analysis complete: ${summary}"
```
