...
import type { ConfidenceMetrics, MCPToolResponse, MedicalAnalysis } from '../types';
...
export class ConfidenceScoreTool {
  ...
  async execute(args: {
    ...
    try {
      ...
      const response = {
        ...
        issues,
        interpretation,
        breakdown: args.detailedBreakdown
          ? this.generateDetailedBreakdown(metrics, args.analysis)
          : undefined,
      };
   ...
   */
  private generateDetailedBreakdown(metrics: ConfidenceMetrics, analysis: MedicalAnalysis): any {
    return {
      overall: {
  ...
  private assessCitationQuality(analysis: MedicalAnalysis): number {
    ...
    const verifiedRatio =
      analysis.citations.filter((c) => c.verified).length / analysis.citations.length;
    const avgRelevance =
      analysis.citations.reduce((sum, c) => sum + c.relevanceScore, 0) / analysis.citations.length;
