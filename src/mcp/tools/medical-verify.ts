...
import type { MCPToolResponse, MedicalAnalysis } from '../types';
...
export class MedicalVerifyTool {
   ...
   */
  private async verifyAnalysis(analysis: MedicalAnalysis, strictMode: boolean): Promise<any> {
  ...
  private performConsistencyChecks(analysis: MedicalAnalysis): Array<{
    ...
    checks.push({
      ...
      message: recommendationsAligned ? 'Recommendations provided' : 'No recommendations provided',
    });
  ...
  private performSafetyChecks(analysis: MedicalAnalysis): Array<{
    ...
    checks.push({
      ...
      level: 'critical',
      message:
        shouldRequireReview && !requiresReview
