...
import type { Citation, MCPToolResponse, MedicalAnalysis, MedicalCondition } from '../types';
...
export class MedicalAnalyzeTool {
    ...
    try {
      ...
      const response = {
        analysis: {
          ...
          recommendations:
            args.includeRecommendations !== false ? analysis.recommendations : undefined,
  ...
  private identifyConditions(
    ...
    if (symptomText.includes('chest pain') || symptomText.includes('palpitations')) {
      conditions.push({
        ...
        confidence: 0.65,
        symptoms: symptoms.filter((s) =>
          ['chest pain', 'palpitations', 'shortness of breath'].some((k) =>
    ...
    if (symptomText.includes('abdominal pain') || symptomText.includes('nausea')) {
      conditions.push({
        ...
        confidence: 0.7,
        symptoms: symptoms.filter((s) =>
          ['abdominal pain', 'nausea', 'vomiting', 'diarrhea'].some((k) =>
            s.toLowerCase().includes(k)
          )
        ),
  ...
  private determineSeverity(
    ...
    // Moderate thresholds
    if ((temp && temp > 101) || (hr && hr > 100) || (bp && bp > 140)) {
      return 'moderate';
    }
   ...
   */
  private async findSupportingCitations(conditions: MedicalCondition[]): Promise<Citation[]> {
