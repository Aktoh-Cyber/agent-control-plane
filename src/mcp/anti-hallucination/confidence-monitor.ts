...
import type { ConfidenceMetrics, MedicalAnalysis, ValidationIssue } from '../types';

export class ConfidenceMonitor {
  private readonly confidenceThreshold: number;
  private readonly criticalThreshold: number;

  constructor(confidenceThreshold: number = 0.8, criticalThreshold: number = 0.6) {
  ...
  private calculateDiagnosisConfidence(analysis: MedicalAnalysis): number {
    ...
    const avgConfidence =
      analysis.conditions.reduce((sum, condition) => sum + condition.confidence, 0) /
    ...
    const differentialPenalty = analysis.conditions.some(
      (c) => c.differential && c.differential.length > 3
    )
      ? 0.95
      : 1.0;
