...
import { ConfidenceScore, ConfidenceScorer, MedicalCitation } from '../core/confidence-scorer';
...
export class VerificationPipeline {
  ...
  async preOutputVerification(input: VerificationInput): Promise<VerificationResult> {
    ...
    const verified =
      this.confidenceScorer.isConfident(confidence) &&
  ...
  private detectLogicalInconsistencies(text: string): HallucinationDetection[] {
    ...
    const hasPositive = sentences.some(
      (s) => !negationWords.some((n) => s.toLowerCase().includes(n))
    );
    const hasNegative = sentences.some((s) =>
      negationWords.some((n) => s.toLowerCase().includes(n))
    );
  ...
  private checkOutputFidelity(
    input: string,
    output: string
  ): { faithful: boolean; reason: string } {
    ...
    if (outputNumbers > inputNumbers * 2) {
      return {
        faithful: false,
        reason:
          'Output contains many more numbers than input - potential quantitative hallucination',
      };
    }
