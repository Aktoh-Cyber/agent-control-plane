...
export class StrangeLoopsDetector {
  ...
  private extractClaims(text: string): CausalNode[] {
    ...
    sentences.forEach((sentence, index) => {
      ...
      const hasCausalLanguage = /\b(because|therefore|thus|hence|consequently|as a result)\b/i.test(
        sentence
      );
  ...
  private detectRelationship(
    ...
    if (
      /\b(cause|lead to|result in)\b/i.test(text1) ||
      /\b(cause|lead to|result in)\b/i.test(text2)
    ) {
      type = 'causes';
    } else if (
      /\b(enable|allow|permit)\b/i.test(text1) ||
      /\b(enable|allow|permit)\b/i.test(text2)
    ) {
      type = 'enables';
    } else if (
      /\b(prevent|block|inhibit)\b/i.test(text1) ||
      /\b(prevent|block|inhibit)\b/i.test(text2)
    ) {
      type = 'prevents';
    }
   ...
   */
  private findCycles(graph: { nodes: CausalNode[]; edges: CausalEdge[] }): CausalNode[][] {
   ...
   */
  private checkContradiction(claim1: CausalNode, claim2: CausalNode): ChainContradiction | null {
    const text1 = claim1.claim.toLowerCase();
    const text2 = claim2.claim.toLowerCase();
  ...
  async validateCausalChain(
    ...
    const valid =
      chain.cycles.length === 0 && chain.contradictions.length === 0 && avgEdgeStrength >= 0.5;
  ...
  async detectRecursivePatterns(
    ...
    for (const ref of selfReferences) {
      patterns.push({
        ...
        reason: ref.depth > maxDepth ? 'Recursion depth exceeds safe limit' : undefined,
      });
    }
  ...
  getStatistics(patterns: LogicalPattern[]): {
    ...
    const mostCommon = Object.entries(byType).sort((a, b) => b[1] - a[1])[0]?.[0] || 'none';
