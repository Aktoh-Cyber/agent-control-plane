...
export class ConfidenceScorer {
  ...
  private calculateCitationStrength(citations: MedicalCitation[]): number {
    ...
    const totalScore = citations.reduce((sum, citation) => {
      ...
      const impactScore = citation.impactFactor ? Math.min(1, citation.impactFactor / 10) : 0.5;
  ...
  private calculateMedicalAgreement(citations: MedicalCitation[]): number {
    ...
    const evidenceLevels = citations.reduce(
      (acc, citation) => {
  ...
  private calculateExpertConsensus(citations: MedicalCitation[]): number {
    ...
    const qualityWeight =
      consensusSources.reduce((sum, citation) => {
  ...
  private calculateOverallScore(
    ...
    const baseScore =
      statistical * 0.3 + citationStrength * 0.25 + medicalAgreement * 0.25 + expertConsensus * 0.2;
