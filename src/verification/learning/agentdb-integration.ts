...
export class AgentDBIntegration {
  ...
  private async updateSourceReliability(record: LearningRecord): Promise<void> {
    ...
    for (const category of categories) {
      ...
      source.successRate =
        (source.successRate * (source.sampleSize - 1) + success) / source.sampleSize;
  ...
  getPatternStatistics(): {
    ...
    const reliablePatterns = Array.from(this.patterns.values()).filter(
      (p) => p.confidence >= this.PATTERN_THRESHOLD
    ).length;
    ...
    const averageReliability =
      totalPatterns > 0
        ? Array.from(this.patterns.values()).reduce((sum, p) => sum + p.reliability, 0) /
          totalPatterns
        : 0.5;
