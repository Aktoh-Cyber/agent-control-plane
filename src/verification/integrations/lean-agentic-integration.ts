...
export class LeanAgenticIntegration {
  ...
  async validateCausalInference(
    ...
    const significant =
      pValue < this.SIGNIFICANCE_LEVEL &&
  ...
  private hasCycles(model: CausalModel): boolean {
    ...
    const dfs = (node: string): boolean => {
      ...
      const outgoing = model.relationships.filter((r) => r.from === node).map((r) => r.to);
   ...
   */
  private identifyBiasThreats(model: CausalModel, data: Record<string, any>): BiasThreats[] {
    ...
    // Confounding bias
    const unmeasuredConfounders = model.variables.filter(
      (v) => v.type === 'confounder' && !v.observed
    );
  ...
  private assessModelValidity(diagnostics: ModelDiagnostics): boolean {
    return (
      !diagnostics.multicollinearity.problematic &&
