...
async function testControllerBuild(sandbox: Sandbox): Promise<TestResult> {
  ...
  try {
    ...
    const buildResult = await sandbox.commands.run('cd /workspace/controller && make build');
...
async function testCRDGeneration(sandbox: Sandbox): Promise<TestResult> {
  ...
  try {
    ...
    const result = await sandbox.commands.run('cd /workspace/controller && make manifests');
...
async function testJujutsuClient(sandbox: Sandbox): Promise<TestResult> {
  ...
  try {
    ...
    await sandbox.commands.run('mkdir -p /tmp/test-repo && cd /tmp/test-repo && jj init --git');
    ...
    const listResult = await sandbox.commands.run('cd /tmp/test-repo && jj log --no-graph');
    logs.push(`Changes:\n${listResult.stdout}`);
...
async function testKubernetesDeployment(sandbox: Sandbox): Promise<TestResult> {
  ...
  try {
    ...
    const statusResult = await sandbox.commands.run('kubectl get deployments -n ajj-test');
...
async function testHelmChart(sandbox: Sandbox): Promise<TestResult> {
  ...
  try {
    ...
    const expectedResources = ['Deployment', 'ServiceAccount', 'ClusterRole', 'ClusterRoleBinding'];
