...
export class EmergencyEscalationService {
  ...
  async createAlert(
    ...
    console.log(
      `[EMERGENCY] ${severity.toUpperCase()} alert created: ${alert.id} for query ${query.id}`
    );
  ...
  setEscalationChain(specialization: string, providerIds: string[]): void {
    ...
    console.log(
      `[EMERGENCY] Escalation chain configured for ${specialization}: ${providerIds.length} providers`
    );
  }
  ...
  getAlertsForProvider(providerId: string): EmergencyAlert[] {
    return Array.from(this.alerts.values()).filter(
      (alert) => alert.assignedProviders.includes(providerId) && this.activeAlerts.has(alert.id)
    );
  }
  ...
  getStats(): {
    ...
    return {
      ...
      bySeverity,
      averageResponseTime:
        acknowledgedCount > 0 ? totalResponseTime / acknowledgedCount / 60000 : 0, // minutes
