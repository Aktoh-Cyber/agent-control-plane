...
import type { AnalysisRequest, AnalysisResponse, WebSocketMessage } from '../types/medical.types';
...
export class MedicalAnalysisAPI {
    ...
    router.post('/analyze', async (req: Request, res: Response): Promise<any> => {
      try {
        ...
        if (!request.symptoms || request.symptoms.length === 0) {
          return res
            .status(400)
            .json(this.createErrorResponse('INVALID_REQUEST', 'Symptoms are required', requestId));
        }
        ...
        result.requiresProviderReview =
          this.antiHallucinationService.requiresProviderReview(confidenceScore);
    ...
    router.get('/analysis/:id', async (req: Request, res: Response): Promise<any> => {
      try {
        ...
        if (!result) {
          return res
            .status(404)
            .json(this.createErrorResponse('NOT_FOUND', 'Analysis not found', uuidv4()));
        }
    ...
    router.post('/provider/notify', async (req: Request, res: Response): Promise<any> => {
      try {
        ...
        if (!analysisId) {
          return res
            .status(400)
            .json(this.createErrorResponse('INVALID_REQUEST', 'Analysis ID is required', uuidv4()));
        }
  ...
  private setupErrorHandling(): void {
    this.app.use((err: Error, _req: Request, res: Response, _next: NextFunction) => {
      ...
      res
        .status(500)
        .json(this.createErrorResponse('INTERNAL_ERROR', 'An unexpected error occurred', uuidv4()));
    });
  }
