...
import { DataRestriction, DataSharingPolicy } from './types';
...
export class DataSharingControls {
  ...
  async addAllowedProvider(patientId: string, providerId: string): Promise<boolean> {
    ...
    if (!policy.allowedProviders.includes(providerId)) {
      ...
      console.log(
        `[DATA_SHARING] Added provider ${providerId} to allowed list for patient ${patientId}`
      );
    }
  ...
  async removeAllowedProvider(patientId: string, providerId: string): Promise<boolean> {
    ...
    if (index > -1) {
      ...
      console.log(
        `[DATA_SHARING] Removed provider ${providerId} from allowed list for patient ${patientId}`
      );
    }
  ...
  async addDataCategory(patientId: string, category: string): Promise<boolean> {
    ...
    if (!policy.dataCategories.includes(category)) {
      ...
      console.log(
        `[DATA_SHARING] Added data category ${category} to policy for patient ${patientId}`
      );
    }
  ...
  async addRestriction(patientId: string, restriction: DataRestriction): Promise<boolean> {
    ...
    console.log(
      `[DATA_SHARING] Added restriction to policy for patient ${patientId}: ${restriction.description}`
    );
  ...
  async setPolicyActiveStatus(patientId: string, active: boolean): Promise<boolean> {
    ...
    console.log(
      `[DATA_SHARING] Policy for patient ${patientId} ${active ? 'activated' : 'deactivated'}`
    );
