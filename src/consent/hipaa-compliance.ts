...
import { AccessLogEntry, HIPAACompliance } from './types';
...
export class HIPAAComplianceService {
  ...
  async logAccess(entry: AccessLogEntry): Promise<void> {
    ...
    if (!entry.authorized) {
      console.warn(
        `[HIPAA] Unauthorized access attempt: ${entry.userId} -> Patient ${entry.patientId}`
      );
  ...
  async addAuthorizedRepresentative(patientId: string, representativeId: string): Promise<boolean> {
    ...
    if (!compliance.authorizedRepresentatives.includes(representativeId)) {
      ...
      console.log(
        `[HIPAA] Added authorized representative ${representativeId} for patient ${patientId}`
      );
    }
  ...
  async performAudit(patientId: string): Promise<{
    ...
    (compliance as any).compliant = issues.length === 0;
