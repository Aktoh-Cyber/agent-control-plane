import { Brain, DollarSign, Layers, Shield, TrendingUp, Zap } from 'lucide-react';
...
const features = [
  {
    ...
    title: 'Agent Booster',
    description:
      '352x faster code operations via Rust/WASM. Transform code at blazing speed with zero cost.',
    metric: '352ms → 1ms',
    color: 'secondary',
  },
  {
    ...
    title: 'ReasoningBank',
    description:
      'Persistent learning memory system. Agents remember patterns and improve with every task.',
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map((feature, index) => {
            ...
            return (
              ...
              >
                <div
                  className={`w-14 h-14 rounded-xl flex items-center justify-center mb-6 ${colorClass} border`}
                >
                ...
                <div
                  className={`inline-block px-4 py-2 rounded-full text-sm font-semibold ${colorClass} border`}
                >
                  {feature.metric}
                </div>
              </div>
