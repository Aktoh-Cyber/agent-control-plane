import { GitBranch, Package, Rocket, Terminal } from 'lucide-react';

const installSteps = [
  {
    icon: Package,
    title: 'Install Package',
    command: 'npm install -g agent-control-plane',
  ...
  },
  {
    ...
    command: 'npx agent-control-plane --agent researcher --task "Analyze trends"',
    description: 'Execute agents with full MCP tool access',
  },
  {
    icon: GitBranch,
    title: 'Or Use npx',
    command: 'npx agent-control-plane --help',
    ...
    <section className="py-24 px-6 bg-background-light/50">
      <div className="container mx-auto max-w-7xl">
        ...
        <div className="space-y-8">
          <h3 className="text-3xl font-bold text-center mb-8 text-foreground">Common Use Cases</h3>
            ...
            >
              <div className="flex flex-col lg:flex-row gap-6">
                <div className="lg:w-1/3">
                  <h4 className="text-xl font-bold mb-3 text-foreground">{example.title}</h4>
                  <p className="text-muted-foreground">{example.description}</p>
                </div>

                <div className="lg:w-2/3">
                  <pre className="bg-background rounded-xl p-6 overflow-x-auto border border-border-light">
                    <code className="text-sm text-primary font-mono">{example.code}</code>
                  </pre>
                </div>
              </div>
            </div>
        ...
        <div className="mt-16 bg-gradient-primary rounded-2xl p-8">
          <h3 className="text-2xl font-bold mb-6 text-center text-foreground">Programmatic API</h3>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-background/10 rounded-xl p-6 backdrop-blur-sm">
              <div className="text-sm font-semibold mb-3 text-foreground/80">Import Components</div>
...
import * as reasoningbank from 'agent-control-plane/reasoningbank';`}
            ...
            <div className="bg-background/10 rounded-xl p-6 backdrop-blur-sm">
              <div className="text-sm font-semibold mb-3 text-foreground/80">Use in Code</div>
              <pre className="bg-background/20 rounded-lg p-4 overflow-x-auto">
                <code className="text-sm text-foreground font-mono">
