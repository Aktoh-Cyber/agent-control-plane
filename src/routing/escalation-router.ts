...
import { PatientQuery, Provider } from '../providers/types';
...
import { EmergencyType, RoutingDecision, SeverityLevel } from './types';
...
export class EscalationRouter {
   ...
   */
  async route(query: PatientQuery, availableProviders: Provider[]): Promise<RoutingDecision> {
  ...
  async escalateToNextLevel(
    ...
    console.log(
      `[ESCALATION_ROUTER] Escalating query ${query.id} from provider ${currentProviderId}: ${reason}`
    );
