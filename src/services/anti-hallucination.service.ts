...
export class AntiHallucinationService {
  ...
  public calculateConfidenceScore(analysis: AnalysisResult): ConfidenceScore {
    ...
    factors.push({
      ...
      impact:
        diagnosisConfidence > 0.8 ? 'positive' : diagnosisConfidence < 0.5 ? 'negative' : 'neutral',
      weight: 0.3,
    ...
    factors.push({
      ...
      impact:
        knowledgeBaseScore > 0.8 ? 'positive' : knowledgeBaseScore < 0.6 ? 'negative' : 'neutral',
      weight: 0.2,
    ...
    factors.push({
      ...
      impact:
        contradictionScore > 0.9 ? 'positive' : contradictionScore < 0.7 ? 'negative' : 'neutral',
      weight: 0.15,
  ...
  private assessDiagnosisConfidence(diagnoses: Diagnosis[]): number {
    ...
    const evidenceStrength =
      primaryDiagnosis.supportingEvidence.reduce((sum, ev) => sum + ev.reliability, 0) /
