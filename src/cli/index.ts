...
program
  ...
  .action(async (condition, options) => {
    try {
      ...
      const patterns = await learningService.recognizePatterns(analysisRequest.symptoms, {
...
program
  ...
  .action(async (analysisId, options) => {
    try {
      ...
      console.log(
        `Overall Confidence: ${getConfidenceColor(confidenceScore.overall)}${(confidenceScore.overall * 100).toFixed(1)}%${chalk.reset}`
      );

      if (options.detailed) {
        console.log(chalk.bold('\nBreakdown:'));
        console.log(
          `  Diagnosis Confidence:      ${(confidenceScore.breakdown.diagnosisConfidence * 100).toFixed(1)}%`
        );
        console.log(
          `  Citation Verification:     ${(confidenceScore.breakdown.citationVerification * 100).toFixed(1)}%`
        );
        console.log(
          `  Knowledge Base Validation: ${(confidenceScore.breakdown.knowledgeBaseValidation * 100).toFixed(1)}%`
        );
        console.log(
          `  Contradiction Check:       ${(confidenceScore.breakdown.contradictionCheck * 100).toFixed(1)}%`
        );
        console.log(
          `  Provider Alignment:        ${(confidenceScore.breakdown.providerAlignment * 100).toFixed(1)}%`
        );
      }
      ...
      if (warnings.length > 0) {
        ...
        warnings.forEach((warning) => {
          const icon =
            warning.severity === 'critical' ? '🚨' : warning.severity === 'error' ? '❌' : '⚠️';
 ...
 */
const configCmd = program.command('config').description('Manage MedAI configuration');
...
function displayAnalysisResults(result: any, patterns: any): void {
  ...
  if (result.diagnosis && result.diagnosis.length > 0) {
    ...
    result.diagnosis.forEach((diag: any, index: number) => {
      ...
      console.log(
        `   Confidence: ${getConfidenceColor(diag.confidence)}${(diag.confidence * 100).toFixed(1)}%${chalk.reset}`
      );
      ...
      if (diag.differentialDiagnoses && diag.differentialDiagnoses.length > 0) {
        ...
        diag.differentialDiagnoses.forEach((diff: any) => {
          console.log(
            chalk.gray(`     - ${diff.condition} (${(diff.probability * 100).toFixed(1)}%)`)
          );
        });
      }
    });
  ...
  console.log(chalk.bold('\n\n📊 Confidence Score:'));
  console.log(
    `Overall: ${getConfidenceColor(result.confidenceScore.overall)}${(result.confidenceScore.overall * 100).toFixed(1)}%${chalk.reset}`
  );
...
function getConfidenceColor(confidence: number): string {
  ...
  if (confidence >= 0.7) return chalk.yellow.bold('');
