...
async function main() {
  ...
  const critiques = await agentdb.reflexionMemory.getCritiqueSummary('debugging memory issues', 5);
  ...
  const relevantPatterns = await agentdb.reasoningBank.searchPatterns(newTask, {
    k: 2,
    threshold: 0.6,
  });
  ...
  const relevantEpisodes = await agentdb.reflexionMemory.retrieveRelevant(newTask, {
    k: 2,
    onlySuccesses: true,
  });
  ...
  const newEpisodeId = await agentdb.reflexionMemory.storeEpisode({
    ...
    critique:
      'Profiling showed table scans. Learned from past episode about indexing. Very effective approach.',
    latencyMs: 1500,
    tokensUsed: 400,
  });
