...
async function main() {
  ...
  try {
    ...
    results.push(
      await benchmark('Single Embedding Generation', 100, async () => {
    ...
    results.push(
      await benchmark('Cached Embedding Access', 1000, async () => {
    ...
    results.push(
      await benchmark('Batch Embedding (10 items)', 50, async () => {
        const texts = Array.from({ length: 10 }, (_, i) => `Batch text ${i} ${Math.random()}`);
    ...
    results.push(
      await benchmark('Batch Embedding (100 items)', 10, async () => {
        const texts = Array.from(
          { length: 100 },
          (_, i) => `Large batch text ${i} ${Math.random()}`
        );
        await agentdb.embedder.embedBatch(texts);
      })
    );
    ...
    results.push(
      await benchmark('Pattern Storage (Single)', 100, async () => {
    ...
    results.push(
      await benchmark('Pattern Storage (Batch of 10)', 20, async () => {
    ...
    results.push(
      await benchmark('Pattern Search (k=10)', 100, async () => {
    ...
    results.push(
      await benchmark('Episode Storage (Single)', 100, async () => {
    ...
    results.push(
      await benchmark('Episode Storage (Batch of 10)', 20, async () => {
    ...
    results.push(
      await benchmark('Episode Retrieval (k=10)', 100, async () => {
    ...
    console.log(
      chalk.white(`   Cache hit rate:     ${(stats.embedder.cache.hitRate * 100).toFixed(1)}%`)
    );
    console.log(
      chalk.white(
        `   Cache size:         ${stats.embedder.cache.size}/${stats.embedder.cache.maxSize}`
      )
    );
