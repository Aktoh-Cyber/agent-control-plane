...
program.name('agentdb-onnx').description('AgentDB with optimized ONNX embeddings').version('1.0.0');
    ...
    try {
      ...
      results.forEach((r, i) => {
        ...
        if (r.critique) {
          console.log(
            chalk.gray(
              `   Critique:   ${r.critique.substring(0, 80)}${r.critique.length > 80 ? '...' : ''}`
            )
          );
        }
        console.log();
...
program
  ...
  .action(async (dbPath) => {
    try {
      ...
      console.log(
        chalk.gray(`  Cache hit rate: ${(stats.embedder.cache.hitRate * 100).toFixed(1)}%`)
      );
      console.log(
        chalk.gray(`  Cache size:     ${stats.embedder.cache.size}/${stats.embedder.cache.maxSize}`)
      );
