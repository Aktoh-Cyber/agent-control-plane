...
const patterns = await agentdb.reasoningBank.searchPatterns('how to debug', { k: 5 });
```
...
const similar = await agentdb.reflexionMemory.retrieveRelevant('performance bug', {
  onlySuccesses: true,
});
```
