...
Agent-Booster is accessible via agent-control-plane MCP tools:

```typescript
mcp__agent - control - plane__agent_booster_edit_file;
mcp__agent - control - plane__agent_booster_batch_edit;
mcp__agent - control - plane__agent_booster_parse_markdown;
```
...
Can be used through agent-control-plane CLI:

```bash
npx agent-control-plane agent coder "task description"
```
...
```bash
# These will fail - use agent-control-plane instead:
...
# Use this instead:
npx agent-control-plane agent coder "add TypeScript types to file.js"
```
