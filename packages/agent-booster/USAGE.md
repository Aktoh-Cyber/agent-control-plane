...
For anything requiring reasoning, use agent-control-plane instead:

```bash
# Instead of Agent Booster for vague tasks:
npx agent-control-plane agent coder "convert var to const in src/utils.js"
...
The agent-control-plane MCP server includes 3 Agent Booster tools with automatic LLM fallback:
...
| Task                          | Use Agent Booster | Use LLM (agent-control-plane) |
| ----------------------------- | ----------------- | ----------------------------- |
