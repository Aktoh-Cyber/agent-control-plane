  ...
  async apply(request: MorphApplyRequest): Promise<MorphApplyResponse> {
    ...
    try {
      ...
      if (!request.edit || request.edit.trim().length === 0) {
        throw new Error(
          'Edit instruction cannot be empty. Provide specific code snippet or transformation.'
        );
      }
      ...
      if (isVague) {
        throw new Error(
          `Vague instruction detected: "${request.edit}". Agent Booster requires specific code snippets, not high-level instructions. Use an LLM for vague tasks.`
        );
      }
