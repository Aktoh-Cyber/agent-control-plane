...
async function runSimulatedBenchmark() {
  ...
  for (const sample of dataset) {
    ...
    results.samples.push({
      ...
      strategy: selectStrategy(0.85 + Math.random() * 0.1),
    });
  ...
  results.aggregate.successRate =
    (results.samples.filter((s) => s.success).length / results.samples.length) * 100;
...
function selectStrategy(confidence) {
  ...
  if (confidence >= 0.5) return 'insert_before';
  return 'append';
}
...
runSimulatedBenchmark().then((results) => {
  ...
  const morphCost =
    baseline.usage && baseline.usage.applies_used ? baseline.usage.applies_used * 0.01 : 0.12;

  console.log(
    `│ Avg Latency             │ ${morphAvg.toFixed(0).padStart(10)}ms    │ ${agentAvg.toFixed(0).padStart(10)}ms    │ ${speedup}x faster │`
  );
  console.log(
    `│ p50 Latency             │ ${morphP50.toFixed(0).padStart(10)}ms    │ ${results.aggregate.p50Latency.toFixed(0).padStart(10)}ms    │ ${(morphP50 / results.aggregate.p50Latency).toFixed(1)}x faster │`
  );
  console.log(
    `│ p95 Latency             │ ${morphP95.toFixed(0).padStart(10)}ms    │ ${results.aggregate.p95Latency.toFixed(0).padStart(10)}ms    │ ${(morphP95 / results.aggregate.p95Latency).toFixed(1)}x faster │`
  );
  console.log(
    `│ Success Rate            │ ${morphSuccess.toFixed(1).padStart(10)}%     │ ${results.aggregate.successRate.toFixed(1).padStart(10)}%     │ ${(results.aggregate.successRate - morphSuccess).toFixed(1)}pp      │`
  );
  console.log(
    `│ Total Cost (12 edits)   │ ${morphCost.toFixed(3).padStart(10)}     │ ${results.aggregate.totalCost.toFixed(3).padStart(10)}     │ 100% free  │`
  );
  console.log('└─────────────────────────┴─────────────────┴─────────────────┴─────────────┘\n');

  console.log('💰 Cost Savings\n');
  const morphTotalCost =
    (baseline.aggregate && baseline.aggregate.totalCost) || baseline.total_cost || 0.12;
