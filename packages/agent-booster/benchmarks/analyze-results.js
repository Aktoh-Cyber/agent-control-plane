...
console.log('-'.repeat(70));
console.log(
  `Success Rate: ${results.successful_tests}/${results.total_tests} (${((results.successful_tests / results.total_tests) * 100).toFixed(1)}%)`
);
...
console.log('-'.repeat(70));
console.log(
  `Applies: ${results.usage.applies_used}/500 (${((results.usage.applies_used / 500) * 100).toFixed(1)}%)`
);
console.log(
  `Input Tokens: ${results.usage.input_tokens}/300000 (${((results.usage.input_tokens / 300000) * 100).toFixed(2)}%)`
);
console.log(
  `Output Tokens: ${results.usage.output_tokens}/100000 (${((results.usage.output_tokens / 100000) * 100).toFixed(2)}%)`
);
console.log(
  `Avg Input/Test: ${(results.usage.input_tokens / results.total_tests).toFixed(1)} tokens`
);
console.log(
  `Avg Output/Test: ${(results.usage.output_tokens / results.total_tests).toFixed(1)} tokens`
);
...
Object.entries(categories)
  ...
  .forEach(([category, data]) => {
    ...
    console.log(
      `${category.padEnd(25)} | ${data.tests.length} tests | ${avgLatency}ms avg | ${avgInput}+${avgOutput} tokens`
    );
  });
...
const sortedByTokens = [...results.results].sort(
  (a, b) => a.tokens.input + a.tokens.output - (b.tokens.input + b.tokens.output)
);
...
console.log(
  `Projected Input Tokens: ${inputTokensPerMonth.toFixed(0)}/300000 (${((inputTokensPerMonth / 300000) * 100).toFixed(1)}%)`
);
console.log(
  `Projected Output Tokens: ${outputTokensPerMonth.toFixed(0)}/100000 (${((outputTokensPerMonth / 100000) * 100).toFixed(1)}%)`
);
...
console.log(
  `Tests remaining: ${Math.floor(results.usage.remaining_budget.applies / (results.usage.applies_used / results.total_tests))}`
);
console.log(
  `Days until depletion (at 10 tests/day): ${Math.floor(results.usage.remaining_budget.applies / 10)} days`
);
...
console.log(
  `Latency Reduction: ${(results.performance.avg_latency_ms - estimatedAgentBoosterLatency).toFixed(0)}ms saved per test`
);
console.log(`\nCost Savings:`);
