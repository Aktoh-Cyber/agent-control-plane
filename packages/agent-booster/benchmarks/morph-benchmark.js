...
if (!MORPH_API_KEY) {
  console.error(
    'Error: MORPH_API_KEY environment variable is required. See .env.example for setup.'
  );
  process.exit(1);
}
      ...
      messages: [
        {
          role: 'system',
          content:
            'You are an expert code assistant. Analyze the code transformation request and provide the improved code. Return ONLY the transformed code without explanations.',
        },
        {
          role: 'user',
          content: prompt,
...
async function runTest(testCase) {
  ...
  if (result.success) {
    console.log(
      `✅ Success - Latency: ${result.latency}ms, Tokens: ${result.tokens.input}+${result.tokens.output}`
    );
