...
async function runTemplateTests() {
  ...
  console.log(
    '│ Test ID  │ Description                         │ Status  │ Confidence │ Strategy   │'
  );
  console.log(
    '├──────────┼─────────────────────────────────────┼─────────┼────────────┼────────────┤'
  );
  ...
  for (const testId of failedTestIds) {
    ...
    if (!testCase) {
      console.log(
        `│ ${testId.padEnd(8)} │ NOT FOUND                           │ ❌      │ N/A        │ N/A        │`
      );
      continue;
    }
...
runTemplateTests()
  .then((results) => {
    ...
    console.log(
      `Still Failing:      ${failedTests} (${((failedTests / totalTests) * 100).toFixed(0)}%)\n`
    );
    ...
    console.log(
      `Improvement:                   +${passedTests} tests (+${improvementRate.toFixed(0)}%)\n`
    );
    ...
    if (passedTests > 0) {
      ...
      results
        ...
        .forEach((r) => {
          console.log(
            `   • ${r.testId}: ${r.description} (${(r.confidence * 100).toFixed(1)}% confidence)`
          );
        });
      console.log('');
    }
    ...
    console.log(
      `Min Confidence:     ${passedTests > 0 ? (Math.min(...results.filter((r) => r.success).map((r) => r.confidence)) * 100).toFixed(1) + '%' : 'N/A'}`
    );
    console.log(
      `Max Confidence:     ${passedTests > 0 ? (Math.max(...results.filter((r) => r.success).map((r) => r.confidence)) * 100).toFixed(1) + '%' : 'N/A'}\n`
    );
    ...
    console.log(
      `Improvement:          +${newPassingTests} wins (+${(projectedWinRate - originalWinRate).toFixed(0)}%)\n`
    );
