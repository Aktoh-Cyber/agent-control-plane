...
```bash
cd /workspaces/agent-control-plane/agent-booster/benchmarks
...
**API Key:** [REDACTED - use MORPH_API_KEY environment variable]
...
```bash
npx gendev@alpha memory retrieve --namespace agent-booster-swarm --key morphllm-usage
npx gendev@alpha memory retrieve --namespace agent-booster-swarm --key benchmark-progress
```
