  ...
  for (const sample of dataset) {
    ...
    try {
      ...
      if (runtime === 'wasm') {
        ...
        result = booster.apply_edit(sample.input, sample.expected_output, 'javascript');
      } else {
        // Native addon
...
runRealBenchmark()
  .then((results) => {
    ...
    console.log(
      `│ p50 Latency             │ ${morphAvg.toFixed(0).padStart(10)}ms    │ ${results.aggregate.p50Latency.toFixed(0).padStart(10)}ms    │ ${(morphAvg / results.aggregate.p50Latency).toFixed(1)}x faster │`
    );
    console.log(
      `│ p95 Latency             │ ${(morphAvg * 1.4).toFixed(0).padStart(10)}ms    │ ${results.aggregate.p95Latency.toFixed(0).padStart(10)}ms    │ ${((morphAvg * 1.4) / results.aggregate.p95Latency).toFixed(1)}x faster │`
    );
    console.log(
