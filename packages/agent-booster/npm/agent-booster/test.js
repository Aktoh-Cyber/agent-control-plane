  ...
  await test(
    ...
    () => {
      try {
        ...
        assert(
          typeof info.runtime === 'string' || info.runtime === null,
          'Runtime should be string or null'
        );
      } catch (error) {
        // Expected if no runtime available
        if (!error.message.includes('No runtime available')) {
...
async function testAPI() {
  ...
  await test(
    'analyze returns analysis object',
    async () => {
      try {
    ...
    async () => {
      ...
      try {
        ...
        if (error.message.includes('No runtime available')) {
          ...
          assert(error.message.includes('npm install'), 'Error should suggest installation');
        }
      }
    },
    TESTS_ENABLED.errorHandling
