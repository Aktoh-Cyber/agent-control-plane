  ...
  try {
    ...
    if (testType === 'correct') {
      ...
      if (parsed.success && parsed.confidence >= test.expectedMinConfidence) {
        ...
        console.log(
          `   Confidence: ${(parsed.confidence * 100).toFixed(1)}% | Strategy: ${parsed.strategy} | Latency: ${parsed.latency}ms\n`
        );
        return { passed: true, result: parsed };
      } else {
        console.log(`⚠️  ${test.name}`);
        console.log(
          `   Low confidence: ${(parsed.confidence * 100).toFixed(1)}% (expected ≥${(test.expectedMinConfidence * 100).toFixed(0)}%)\n`
        );
      ...
      if (!parsed.success || parsed.confidence < 0.5) {
        ...
        console.log(
          `   Incorrectly accepted with ${(parsed.confidence * 100).toFixed(1)}% confidence\n`
        );
        return { passed: false, result: parsed };
      }
    }
...
console.log(
  `✅ Incorrect Usage Tests: ${incorrectPassed}/${incorrectTests.length} correctly rejected`
);
console.log(
...
console.log('💡 For vague instructions, use LLM with agent-control-plane:\n');
console.log('   npx agent-control-plane agent coder "convert var to const in file.js"\n');
