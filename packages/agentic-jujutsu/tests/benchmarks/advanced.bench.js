...
class BenchmarkRunner {
  ...
  compareResults(result1, result2) {
    ...
    if (speedup > 1) {
      ...
      console.log(
        `  ${colors.red}${result2.name} is ${(1 / speedup).toFixed(2)}x faster${colors.reset}`
      );
...
async function runBenchmarks() {
  ...
  console.log(
    `  Heap Total:    ${((memAfter.heapTotal - memBefore.heapTotal) / 1024 / 1024).toFixed(2)}MB`
  );
  console.log(
    `  External:      ${((memAfter.external - memBefore.external) / 1024 / 1024).toFixed(2)}MB`
  );
  console.log(
    `  Array Buffers: ${((memAfter.arrayBuffers - memBefore.arrayBuffers) / 1024 / 1024).toFixed(2)}MB`
  );
  ...
  const concurrentResult = await runner.benchmarkAsync(
    'Concurrent Promises (100 ops)',
    100,
    async () => {
      const promises = Array(10)
        .fill(0)
        .map((_, i) => Promise.resolve({ id: i, value: i * 2 }));
