...
describe('ML-DSA Signing', function () {
  ...
  describe('Keypair Generation', function () {
    ...
    it('should generate unique keypairs', async function () {
      ...
      assert.notStrictEqual(keypair1.secretKey, keypair2.secretKey, 'Secret keys should be unique');
    });
  ...
  describe('Signature Verification', function () {
    ...
    it('should verify batch of signatures', async function () {
      ...
      const verifications = await Promise.all(
        messages.map((msg, i) => jj.verifySignature(msg, signatures[i], keypair.publicKey))
      );
      const duration = performance.now() - start;
  ...
  describe('Commit Signing Workflow', function () {
    ...
    it('should handle multi-agent commit workflow', async function () {
      ...
      // Register all agents
      await Promise.all(agents.map((a) => jj.registerQuantumAgent(a.id, ['sign'], a.keypair)));
      ...
      // Verify all commits
      const verifications = await Promise.all(
        signedCommits.map((commit, i) => jj.verifyCommit(commit, agents[i].keypair.publicKey))
      );

      assert.ok(
        verifications.every((v) => v === true),
        'All commits should verify'
      );
  ...
  describe('Security Properties', function () {
    ...
    it('should maintain quantum resistance', async function () {
      ...
      console.log(
        `  ✓ Quantum attack simulation: ${successfulForgeries}/${attackAttempts} forgeries succeeded`
      );
