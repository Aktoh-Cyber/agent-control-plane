...
import { createHooksIntegration } from '../../typescript/hooks-integration';
...
async function main() {
  ...
  await Promise.all(agentTasks.map((task) => executeAgent(config, sessionId, task)));
  ...
  const integration = await createHooksIntegration(config, sessionId, 'conflict-detector', true);
