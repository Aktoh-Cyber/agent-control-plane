...
class MockCoordination {
  ...
  getStats() {
    return {
      ...
      activeAgents: Array.from(this.agents.values()).filter((a) => new Date() - a.lastSeen < 60000)
        .length,
  ...
  async handleConflict(agentId, conflict) {
    if (conflict.severity === 1) {
      ...
      log(
        colors.yellow,
        '⏱️',
        `${agentId}: Sequential execution (waiting for ${conflict.operationB})`
      );
      await this.wait(300);
    } else {
...
async function scenario1_NoConflicts() {
  ...
  console.log(
    `${colors.bright}${colors.cyan}  Scenario 1: No Conflicts (Different Files)${colors.reset}`
  );
  console.log(
    `${colors.bright}${colors.cyan}═══════════════════════════════════════${colors.reset}\n`
  );
  ...
  await Promise.all([
    orchestrator.executeWithCoordination(
      'coder-1',
      {
        id: 'op1',
        type: 'edit',
...
async function scenario2_MinorConflict() {
  ...
  console.log(
    `${colors.bright}${colors.yellow}  Scenario 2: Minor Conflict (Same Branch)${colors.reset}`
  );
  console.log(
    `${colors.bright}${colors.yellow}═══════════════════════════════════════${colors.reset}\n`
  );
  ...
  await orchestrator.executeWithCoordination(
    'coder-1',
    {
      id: 'op1',
      type: 'branch-create',
...
async function scenario3_SevereConflict() {
  ...
  console.log(
    `${colors.bright}${colors.red}  Scenario 3: Severe Conflict (Same Files)${colors.reset}`
  );
  console.log(
    `${colors.bright}${colors.red}═══════════════════════════════════════${colors.reset}\n`
  );
...
async function scenario4_ComplexWorkflow() {
  ...
  console.log(
    `${colors.bright}${colors.magenta}  Scenario 4: Complex Workflow (Multiple Agents)${colors.reset}`
  );
  console.log(
    `${colors.bright}${colors.magenta}═══════════════════════════════════════${colors.reset}\n`
  );
