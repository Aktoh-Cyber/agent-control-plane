...
async function main() {
  ...
  const coderVertexId = await bridge.registerOperation('op-coder-1', coderOp, ['src/main.js']);
  ...
  const reviewerConflicts = await bridge.checkConflicts('op-reviewer-1', 'review', ['src/main.js']);
  ...
  const testerVertexId = await bridge.registerOperation('op-tester-1', testerOp, [
    'test/main.test.js',
  ]);
  ...
  const architectConflicts = await bridge.checkConflicts('op-architect-1', 'refactor', [
    'src/main.js',
  ]);
  ...
  for (const agent of agentStats) {
    ...
    console.log(
      `     Operations: ${agent.operationsCount}, Reputation: ${agent.reputation.toFixed(2)}`
    );
  }
  console.log('');
