...
// 2. Sign a commit
const signature = QuantumSigner.signCommit('commit-id', keypair.secretKey);

// 3. Verify signature
const isValid = QuantumSigner.verifyCommit('commit-id', signature, keypair.publicKey);
...
```javascript
const signature = QuantumSigner.signCommit('abc123', keypair.secretKey, {
  author: 'Alice',
  repo: 'my-project',
});
```
...
exec('jj log -r @ -T "commit_id"', (error, stdout) => {
  ...
  fs.writeFileSync(`${sigDir}/${commitId}.json`, JSON.stringify(signature, null, 2));
...
**Need help?** Open an issue at [github.com/Aktoh-Cyber/agent-control-plane/issues](https://github.com/Aktoh-Cyber/agent-control-plane/issues)
