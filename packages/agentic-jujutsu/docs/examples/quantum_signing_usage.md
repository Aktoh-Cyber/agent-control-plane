...
const isValid = QuantumSigner.verifyCommit('abc123', signature, keypair.publicKey);
...
const signature = QuantumSigner.signCommit('commit-id-here', keypair.secretKey, metadata);
...
async function signCurrentCommit() {
  ...
  // Sign the commit
  const signature = QuantumSigner.signCommit(currentCommit.id, keypair.secretKey, {
...
async function verifyCommitChain(jj, startCommit, publicKey) {
  ...
  for (const commit of commits) {
    ...
    const isValid = QuantumSigner.verifyCommit(commit.id, signature, publicKey);
...
class KeyManager {
  ...
  generateNewKeypair() {
    ...
    fs.writeFileSync(path.join(this.keyDir, filename), JSON.stringify(keypair, null, 2), {
      mode: 0o600,
    });
  ...
  getCurrentKeypair() {
    ...
    return JSON.parse(fs.readFileSync(path.join(this.keyDir, latest), 'utf8'));
  }
...
class KeyBackup {
  backupKeypair(keypair, backupLocations) {
    const encrypted = encryptSecretKey(keypair.secretKey, process.env.MASTER_PASSWORD);
