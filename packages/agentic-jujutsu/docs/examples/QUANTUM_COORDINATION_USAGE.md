...
const vertexId = await bridge.registerOperation('op-1', op1, ['src/file.js']);

// Agent 2 checks for conflicts
const conflicts = await bridge.checkConflicts('op-2', 'review', ['src/file.js']);
...
for (const op of operations) {
  const conflicts = await bridge.checkConflicts(op.id, op.type, op.files);
...
const conflicts = await bridge.checkConflicts('deploy-op', 'deploy', ['dist/*']);
