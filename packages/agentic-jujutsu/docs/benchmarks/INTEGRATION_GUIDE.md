...
This guide describes how to integrate the benchmarking system with the existing agentic-jujutsu codebase, GenDev coordination, and AgentDB learning infrastructure.
...
**Location:** `/workspaces/agent-control-plane/packages/agentic-jujutsu/src/hooks.rs`
...
export class BenchmarkHooks {
  ...
  async preTask(description: string): Promise<void> {
    execSync(`npx gendev@alpha hooks pre-task --description "${description}"`, {
      stdio: 'inherit',
    });
  }

  async postEdit(file: string, memoryKey: string): Promise<void> {
    execSync(`npx gendev@alpha hooks post-edit --file "${file}" --memory-key "${memoryKey}"`, {
      stdio: 'inherit',
    });
  }

  async postTask(taskId: string): Promise<void> {
    execSync(`npx gendev@alpha hooks post-task --task-id "${taskId}"`, { stdio: 'inherit' });
  }

  async sessionRestore(): Promise<void> {
    execSync(`npx gendev@alpha hooks session-restore --session-id "${this.sessionId}"`, {
      stdio: 'inherit',
    });
  }

  async sessionEnd(exportMetrics: boolean = true): Promise<void> {
    execSync(`npx gendev@alpha hooks session-end --export-metrics ${exportMetrics}`, {
      stdio: 'inherit',
    });
  }

  async notify(message: string): Promise<void> {
    execSync(`npx gendev@alpha hooks notify --message "${message}"`, { stdio: 'inherit' });
  }
}
```
...
export class BenchmarkRunner {
  ...
  async run(config: BenchmarkConfig): Promise<void> {
    ...
    // Pre-task hook
    await this.hooks.preTask(`benchmark-suite: ${config.tests.suites.join(', ')}`);
...
**Location:** `/workspaces/agent-control-plane/packages/agentic-jujutsu/src/agentdb_sync.rs`
...
export class AgentDBLearning {
  ...
  private generateCritique(result: ProcessedResult): string {
    ...
    if (result.comparison.jjVsGit.speedupFactor > 2) {
      parts.push(
        `Excellent performance: ${result.comparison.jjVsGit.speedupFactor.toFixed(1)}x faster than Git.`
      );
    } else if (result.comparison.jjVsGit.speedupFactor > 1) {
      parts.push(
        `Good performance: ${result.comparison.jjVsGit.speedupFactor.toFixed(1)}x faster than Git.`
      );
    } else {
      parts.push(
        `Slower than Git: ${(1 / result.comparison.jjVsGit.speedupFactor).toFixed(1)}x slower.`
      );
    }
    ...
    if (result.comparison.jjVsGit.memoryRatio < 0.8) {
      parts.push(
        `Memory efficient: ${(result.comparison.jjVsGit.memoryRatio * 100).toFixed(0)}% of Git's usage.`
      );
    }
  ...
  private calculateIOEfficiency(ioOps: IOMetrics): number {
    ...
    return Math.max(
      0,
      Math.min(1, (bytesPerOp - avgBytesPerOp) / (goodBytesPerOp - avgBytesPerOp))
    );
  }
}
```
...
**Location:** `/workspaces/agent-control-plane/packages/agentic-jujutsu/src/wasm.rs`
...
**Location:** `/workspaces/agent-control-plane/packages/agentic-jujutsu/tests/`
...
export class SwarmCoordination {
  ...
  async orchestrateBenchmark(swarmId: string, suite: TestSuite): Promise<void> {
...
services:
  ...
  metrics-collector:
    ...
    volumes:
      - ./results:/results
      - ../../.gendev/metrics:/metrics # Existing metrics dir
    depends_on:
      - jujutsu-bench
      - git-bench
...
program.name('benchmark').description('Agentic Jujutsu Benchmarking CLI').version('1.0.0');
...
program
  ...
  .action(async (options) => {
    const results = JSON.parse(await readFile(options.results, 'utf-8'));
