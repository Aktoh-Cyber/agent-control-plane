...
class TestExecutor {
  ...
  async runParallel(tests: Test[]): Promise<RawMetric[]> {
    ...
    return Promise.all(tests.map((test, i) => this.executeTest(test, pools[i])));
  }
}
```
...
class MetricsCollector {
  async attach(container: Container): Promise<ActiveCollector> {
    ...
    await Promise.all([cpuCollector.start(), memoryCollector.start(), ioCollector.start()]);
...
class StatisticalEngine {
  ...
  private performInference(jjData: number[], gitData: number[]): InferentialStats {
...
class SpeedOptimizer {
  async analyzeBottlenecks(metrics: ProcessedResult): Promise<Bottleneck[]> {
  ...
  async recommend(bottlenecks: Bottleneck[]): Promise<Recommendation[]> {
    return bottlenecks.map((bn) => {
      // Match bottleneck to optimization strategy
      const strategy = this.strategies.find((s) => s.applicableTo(bn));
...
class HotPathDetector {
  async identify(trace: ExecutionTrace): Promise<HotPath[]> {
    ...
    const criticalPaths = cumulativeTimes.filter((path) => path.percentage > 0.1);
...
class ReportGenerator {
  ...
  private async generateMarkdown(results: ProcessedResult[]): Promise<string> {
    const sections = [
...
async function withRetry<T>(fn: () => Promise<T>, config: RetryConfig): Promise<T> {
  for (let attempt = 0; attempt < config.maxAttempts; attempt++) {
    try {
      ...
      if (attempt === config.maxAttempts - 1 || !config.retryableErrors.includes(error.code)) {
        throw error;
      }
...
class ParallelExecutor {
  async runParallel(tests: Test[], maxConcurrency: number = 4): Promise<RawMetric[]> {
    const queue = [...tests];
