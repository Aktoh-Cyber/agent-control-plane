...
class DynamicResourceAllocator {
  async adjustResources(container: Container, metrics: ResourceMetrics): Promise<void> {
...
class ParallelAnalyzer {
  ...
  async analyzeParallel(results: RawMetric[]): Promise<ProcessedResult[]> {
    // Split into chunks
...
class ResultCache {
  ...
  async set(testId: string, repoHash: string, result: RawMetric): Promise<void> {
    const key = `${testId}:${repoHash}`;
    this.cache.set(key, {
      result,
      timestamp: Date.now(),
...
class AutoScaler {
  async evaluateScaling(metrics: SystemMetrics): Promise<ScalingDecision> {
    ...
    // - Queue empty
    // - No tests running
    if (metrics.cpu.used < 0.3 && metrics.queue.pending === 0 && metrics.queue.running < 2) {
      return {
        action: 'scale-down',
