...
export declare class JjWrapper {
  ...
  /** Check for conflicts before executing operation */
  checkAgentConflicts(operationId: string, affectedFiles: Array<string>): Promise<string>;
...
async function basicCoordination() {
  ...
  const vertexId = await jj.registerAgentOperation('agent-1', op1.id, ['src/auth.js']);
...
class AgentOrchestrator {
  ...
  async executeWithCoordination(agentId, operation, files) {
    ...
    try {
      // Check for conflicts
      const conflictsJson = await this.jj.checkAgentConflicts(operation.id, files);
      const conflicts = JSON.parse(conflictsJson);
      ...
      // Register in coordination
      await this.jj.registerAgentOperation(agentId, operation.id, files);
...
module.exports = {
  coordination: {
    ...
    conflictDetection: {
      ...
      autoResolve: ['minor'],
      exclusivePatterns: ['.git/**', 'package-lock.json', 'Cargo.lock'],
    },
    performance: {
      maxDAGSize: 50000,
