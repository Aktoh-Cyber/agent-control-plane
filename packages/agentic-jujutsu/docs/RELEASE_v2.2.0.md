...
const conflicts = await wrapper.checkAgentConflicts('edit-operation', 'edit', ['src/main.rs']);
...
const vertexId = await bridge.registerOperation(operation.id, operation, ['src/file.rs']);

// Check for DAG-based conflicts
const dagConflicts = await bridge.checkConflicts('new-op-id', 'edit', ['src/file.rs']);
...
**Special Thanks**: GenDev orchestration system for parallel agent coordination.
