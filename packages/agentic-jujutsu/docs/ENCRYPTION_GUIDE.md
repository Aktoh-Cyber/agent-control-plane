...
wrapper.enableEncryption(process.env.REASONING_BANK_KEY, process.env.REASONING_BANK_PUBLIC_KEY);
```
...
```javascript
const encrypted = encryptTrajectory(JSON.stringify(trajectoryData), publicKey);
```
...
async function batchEncrypt(trajectories, publicKey) {
  return Promise.all(trajectories.map((t) => encryptTrajectory(JSON.stringify(t), publicKey)));
}
```
...
class LazyTrajectory {
  ...
  async getData() {
    if (!this._decrypted) {
      this._decrypted = await decryptWrapperTrajectory(this._wrapper, this._id, this._secretKey);
    }
