...
for (const op of operations) {
  if (op.quantumFingerprint) {
    const isValid = await wrapper.verifyOperationFingerprint(op.id, op.quantumFingerprint);
    if (!isValid) {
...
const isValid = publicKey.verify(operationData, Buffer.from(operation.signature, 'hex'));
```
...
await Promise.all(operations.map((op) => wrapper.generateOperationFingerprint(op.id)));
```
