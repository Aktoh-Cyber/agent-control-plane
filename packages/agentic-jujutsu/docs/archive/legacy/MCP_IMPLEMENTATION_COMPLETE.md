...
transport.connect("npx", &["gendev@alpha", "mcp", "start"]).await?;
...
```bash
# Connect to agent-control-plane MCP server via stdio
...
```typescript
import { AgentDBSync, MCPClientConfig } from '@agent-control-plane/jujutsu';
...
3. Test with real agent-control-plane MCP server
