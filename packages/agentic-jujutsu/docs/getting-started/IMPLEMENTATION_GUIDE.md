...
cd agent-control-plane/packages/agentic-jujutsu
...
import { JJWrapper, JJConfig, CommandOutput } from '@agent-control-plane/jujutsu';
...
import { AgentDBSync } from '@agent-control-plane/jujutsu/agentdb';
...
import { JJWrapper } from '@agent-control-plane/jujutsu';
import { HooksClient } from '@agent-control-plane/jujutsu/hooks';
...
```typescript
// examples/integration/gendev-integration.ts
import { JJWrapper } from '@agent-control-plane/jujutsu';
import { SwarmCoordination } from '@agent-control-plane/jujutsu/swarm';
...
class JJWrapperPool {
  ...
  static async get(workingDir: string): Promise<JJWrapper> {
    if (!this.instances.has(workingDir)) {
      this.instances.set(workingDir, await JJWrapper.new({ workingDir }));
    }
...
**Support:** https://github.com/Aktoh-Cyber/agent-control-plane/issues
