...
```typescript
import { JJWrapper } from 'https://unpkg.com/@agent-control-plane/jujutsu/pkg/deno/index.js';
...
```javascript
import { JJWrapper } from '@agent-control-plane/jujutsu/bundler';
...
<script type="module">
  import init, {
    JJWrapper,
    JJConfig,
  } from './node_modules/@agent-control-plane/jujutsu/pkg/web/agentic_jujutsu.js';
...
```typescript
import { JJWrapper, JJConfig } from '@agent-control-plane/jujutsu/deno';

const config = new JJConfig().with_verbose(true).with_timeout(30000);
...
const config = new JJConfig().with_max_log_entries(100); // Only keep 100 most recent
...
```javascript
const config = new JJConfig().with_jj_path('/usr/local/bin/jj'); // Absolute path
...
{
  "compilerOptions": {
    "types": ["@agent-control-plane/jujutsu"]
  }
}
```
...
```javascript
const config = new JJConfig().with_timeout(120000); // Increase to 2 minutes
...
```javascript
const config = new JJConfig().with_agentdb_sync(true); // Enable AgentDB sync
...
const instances = await Promise.all(
  repos.map((path) => JJWrapper.new(new JJConfig().with_repo_path(path)))
);

// Operate on all repositories
const statuses = await Promise.all(instances.map((jj) => jj.status()));
```
