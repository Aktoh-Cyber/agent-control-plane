...
for (const expert of experts) {
  ...
  const expertId = await attention.moe.addExpert(expert.name, expert.specialization, vectors);
...
async function autoCreateExperts(
  ...
  for (const [category, docs] of byCategory) {
    ...
    await attention.moe.addExpert(expertName, category.toLowerCase().replace(/\s+/g, '_'), vectors);
...
for (const level of hierarchy) {
  ...
  for (const child of level.children) {
    ...
    await attention.moe.addExpert(child.name, child.specialization, vectors);
...
class AdaptiveExpertSystem {
  ...
  async recordQuery(query: string, expertName: string, feedback: 'positive' | 'negative') {
...
await adaptiveSystem.recordQuery('React hooks example', 'React Developer', 'positive');
await adaptiveSystem.recordQuery(
  'Vue composition API',
  'React Developer', // Wrong expert!
  'negative'
);
await adaptiveSystem.recordQuery('Angular services', 'Angular Developer', 'positive');
...
async function addDynamicExpert(name: string, specialization: string, documents: string[]) {
  const vectors = documents.map((doc) => embed(doc));

  const expertId = await attention.moe.addExpert(name, specialization, vectors);
...
// Example usage
const newExpertId = await addDynamicExpert('Go Developer', 'golang', [
...
async function mergeExperts(expert1Id: number, expert2Id: number, newName: string) {
  ...
  const mergedId = await attention.moe.addExpert(newName, 'merged', allVectors);
...
stats.forEach((expert) => {
  ...
  console.log(
    `  Queries: ${expert.queryCount}, Avg confidence: ${expert.avgConfidence.toFixed(3)}`
  );
});

// Check for imbalance
const avgLoad = totalQueries / stats.length;
const imbalanced = stats.filter(
  (s) => s.queryCount > avgLoad * 1.5 || s.queryCount < avgLoad * 0.5
);
...
const trainingData = ['React', 'React is good', 'React tutorial'];
```
