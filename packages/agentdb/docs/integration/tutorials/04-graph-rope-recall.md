...
await attention.graphRoPE.addEdge(5, 1, 0.9); // TypeScript -> JavaScript (strong)
await attention.graphRoPE.addEdge(1, 2, 0.3); // JavaScript -> Python (weak)
...
for (const entry of knowledge) {
  ...
  for (const related of entry.relatedConcepts) {
    ...
    if (targetId) {
      await attention.graphRoPE.addEdge(sourceId, targetId, related.strength);

      console.log(
        `${entry.concept} --[${related.relationship}]--> ${related.concept} (${related.strength})`
      );
    }
  }
}
...
const conversation: Message[] = [
  ...
  { id: 3, speaker: 'user', text: 'How do I create components?', replyTo: 2, topic: 'react' },
  {
    id: 4,
    speaker: 'assistant',
    text: 'You can create components using functions or classes',
    replyTo: 3,
    topic: 'react',
  },
  { id: 5, speaker: 'user', text: 'What about state management?', replyTo: 4, topic: 'state' },
  {
    id: 6,
    speaker: 'assistant',
    text: 'React provides useState hook for state',
    replyTo: 5,
    topic: 'state',
  },
];
...
for (let i = 0; i < conversation.length; i++) {
  for (let j = i + 1; j < conversation.length; j++) {
    if (conversation[i].topic === conversation[j].topic) {
      ...
      await attention.graphRoPE.addEdge(conversation[j].id, conversation[i].id, strength);
    }
  }
}
...
const concepts: Concept[] = [
  ...
  // Level 2: Concepts
  { id: 4, name: 'Frontend Frameworks', level: 2, parent: 2, relatedTo: [{ id: 7, type: 'uses' }] },
  { id: 5, name: 'Backend Frameworks', level: 2, parent: 2 },
  { id: 6, name: 'Machine Learning', level: 2, parent: 3 },

  // Level 3: Details
  { id: 7, name: 'React', level: 3, parent: 4, relatedTo: [{ id: 8, type: 'competes-with' }] },
  { id: 8, name: 'Vue', level: 3, parent: 4, relatedTo: [{ id: 7, type: 'competes-with' }] },
...
async function exploreConcept(conceptId: number, hops: number = 2) {
  ...
  for (const [level, items] of byLevel) {
    ...
    items.forEach((item) => {
      console.log(
        `    - ${item.metadata.name} (${item.pathLength} hops, ${item.ropeScore.toFixed(3)})`
      );
    });
  }
}
...
async function findPath(fromId: number, toId: number, maxHops: number = 5): Promise<number[]> {
  ...
  while (queue.length > 0) {
    ...
    // Get connected nodes
    const edges = await db.all('SELECT target_id FROM graph_edges WHERE source_id = ?', id);
...
async function exploreCommunity(startId: number, visited: Set<number>): Promise<number[]> {
  const community = [];
  const queue = [startId];
...
await attention.graphRoPE.addEdge(id1, id3, 0.7); // Mentioned in text
await attention.graphRoPE.addEdge(id1, id4, 0.4); // Weak association
```
