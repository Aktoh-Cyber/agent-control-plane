...
async function consolidateDocument(document: string, chunkSize: number = 512) {
  ...
  const consolidated = await attention.flash.consolidateMemories(vectors, windowSize);

  console.log(
    `Consolidated ${chunks.length} chunks into ${consolidated.consolidatedCount} segments`
  );
...
async function consolidateByTimeWindow(memories: TimestampedMemory[], windowHours: number = 24) {
  // Sort by timestamp
  ...
  for (let i = 0; i < windows.length; i++) {
    ...
    console.log(
      `Window ${i + 1}: ${window.length} memories -> ${result.consolidatedCount} segments`
    );
  }
...
const logs: TimestampedMemory[] = [
  ...
  {
    timestamp: new Date('2024-01-01T09:15'),
    text: 'User viewed dashboard',
    vector: embed('dashboard'),
  },
  // ... more logs
...
async function consolidateMultiSource(memories: SourcedMemory[]) {
  ...
  for (const [source, sourceMemories] of bySource) {
    ...
    console.log(
      `${source}: ${sourceMemories.length} -> ${consolidated.consolidatedCount} (${consolidated.compressionRatio.toFixed(2)}x)`
    );
  }
...
class IncrementalConsolidator {
  ...
  async flush() {
    ...
    const consolidated = await this.attention.flash.consolidateMemories(this.buffer);
...
for await (const event of eventStream) {
  await consolidator.addMemory(embed(event.text), event.metadata);
}
...
// Query
const results = await consolidator.query(embed('What events occurred?'), 10);
...
async function batchConsolidate(vectors: Float32Array[], batchSize: number = 1000) {
  const batches = [];
...
console.log(
  `Saved: ${(saved / 1024 / 1024).toFixed(2)} MB (${((saved / originalSize) * 100).toFixed(1)}%)`
);
```
