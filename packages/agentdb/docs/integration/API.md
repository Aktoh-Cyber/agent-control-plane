...
class FlashConsolidation {
  ...
  consolidateMemories(vectors: Float32Array[], windowSize?: number): Promise<ConsolidatedMemory>;

  queryConsolidated(query: Float32Array, k: number): Promise<FlashResult[]>;
...
class GraphRoPERecall {
  ...
  graphAwareSearch(query: Float32Array, k: number, hops?: number): Promise<GraphResult[]>;
...
class MoERouting {
  ...
  routeQuery(query: Float32Array, k: number, expertCount?: number): Promise<MoEResult[]>;

  addExpert(name: string, specialization: string, vectors: Float32Array[]): Promise<number>;
...
await moe.addExpert('Code Expert', 'code_snippets', codeVectors);
```
