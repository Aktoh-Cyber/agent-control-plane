...
createRelationship(agentNode1, agentNode2, 'COLLABORATED_ON', { taskId, outcome, timestamp });
```
...
const queryEmbedding = await embedder.embed('Find agents best suited for debugging memory leaks');
...
// Retrieve results
mcp__gendev__agentdb_get_results({
...
1. **MCP Tools**: Framework ready for GenDev integration
