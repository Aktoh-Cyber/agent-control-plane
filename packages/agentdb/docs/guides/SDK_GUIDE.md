...
dryRunResults.forEach((p) => {
  console.log(
    `  ${p.cause} → ${p.effect} (uplift: ${p.uplift.toFixed(3)}, p=${p.pValue.toFixed(4)})`
  );
});
...
const results = await Promise.all(tasks.map((task) => agent.reflexion.retrieve({ task, k: 5 })));
```
...
- **Discussions**: [Ask questions](https://github.com/anthropics/agent-control-plane/discussions)
