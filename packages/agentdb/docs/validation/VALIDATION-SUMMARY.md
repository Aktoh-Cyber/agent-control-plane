...
**Root Cause:**
The MCP server existed (`/workspaces/agent-control-plane/packages/agentdb/src/mcp/agentdb-mcp-server.ts`) but was not integrated into the CLI entry point.

**Solution:**
Added `handleMcpCommand()` function in `/workspaces/agent-control-plane/packages/agentdb/src/cli/agentdb-cli.ts:699-726` that dynamically imports and starts the MCP server.
...
### 1. `/workspaces/agent-control-plane/packages/agentdb/src/cli/agentdb-cli.ts`
...
### 2. `/workspaces/agent-control-plane/packages/agentdb/package.json`
...
Location: `/workspaces/agent-control-plane/packages/agentdb/tests/cli-test-suite.sh`
