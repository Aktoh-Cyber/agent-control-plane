...
// Generate delta
function generateDelta(db: AgentDB, baseVersion: number): DeltaSync {
  const operations = db
    .queryChanges('SELECT * FROM change_log WHERE version > ?', baseVersion)
    .map((change) => ({
...
function validateSyncMessage(
  ...
  const expectedSignature = crypto.createHmac('sha256', sharedSecret).update(data).digest('hex');

  return crypto.timingSafeEqual(Buffer.from(msg.signature), Buffer.from(expectedSignature));
}
```
