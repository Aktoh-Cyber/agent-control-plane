...
export class SGDOptimizer {
  constructor(learningRate: number, momentum?: number, dampening?: number, nesterov?: boolean);
...
export class HardNegativeMiner {
  constructor(strategy: 'semi_hard' | 'hard' | 'distance_weighted', margin?: number);
...
export class BatchProcessor {
  constructor(config: { batchSize: number; numWorkers?: number; prefetch?: boolean });
...
**From `.gendev/metrics/performance.json`** (found in package):
