...
**Location**: `/workspaces/agent-control-plane/packages/agentdb/src/controllers/SkillLibrary.ts:150`
...
const embedding = db
  .prepare('SELECT embedding FROM skill_embeddings WHERE skill_id = ?')
  .get(skillId) as any;
```
...
class SkillLibrary {
  ...
  constructor(db: Database.Database, embedder: EmbeddingService) {
    ...
    this.stmtCache.set('selectSkillById', this.db.prepare('SELECT * FROM skills WHERE id = ?'));
  }
...
**Effort**: 15 minutes
**File**: `/workspaces/agent-control-plane/packages/agentdb/src/controllers/SkillLibrary.ts`
