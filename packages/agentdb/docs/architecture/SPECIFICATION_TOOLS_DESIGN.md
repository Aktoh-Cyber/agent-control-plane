...
export async function agentdb_init(args: any) {
  ...
  if (args.enable_hnsw !== false) {
    db.exec(
      `CREATE INDEX IF NOT EXISTS idx_hnsw_vectors ON vectors USING hnsw (embedding vector_cosine_ops)`
    );
  }
...
export async function agentdb_search(args: any, context: MCPContext) {
  ...
  if (args.filters?.tags && args.filters.tags.length > 0) {
    const tagConditions = args.filters.tags
      .map((tag: string) => `json_extract(tags, '$') LIKE '%${tag}%'`)
      .join(' OR ');
