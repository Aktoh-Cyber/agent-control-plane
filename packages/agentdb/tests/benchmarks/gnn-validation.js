...
async function validateGNN() {
  ...
  console.log(
    `  GNN Available: ${detection.features.gnn ? COLORS.green + '✅ YES' : COLORS.red + '❌ NO'}${COLORS.reset}`
  );
  console.log(
    `  Native Bindings: ${detection.native ? COLORS.green + '✅ YES' : COLORS.yellow + '⚠️  WASM Fallback'}${COLORS.reset}`
  );
  ...
  console.log(
    `  Changed Dimensions: ${changedDims}/${queryEmbedding.length} (${((changedDims / queryEmbedding.length) * 100).toFixed(1)}%)`
  );
  ...
  console.log('\n📊 Performance Summary:');
  console.log(
    `  Enhancement Throughput: ${COLORS.green}${enhanceThroughput.toFixed(1)} queries/sec${COLORS.reset}`
  );
  console.log(
    `  Search Throughput: ${COLORS.green}${searchThroughput.toFixed(1)} searches/sec${COLORS.reset}`
  );
  console.log(
    `  Avg Enhancement Latency: ${COLORS.green}${(enhanceBenchDuration / 100).toFixed(2)}ms${COLORS.reset}`
  );
  console.log(
    `  Avg Search Latency: ${COLORS.green}${(searchBenchDuration / 50).toFixed(2)}ms${COLORS.reset}`
  );
