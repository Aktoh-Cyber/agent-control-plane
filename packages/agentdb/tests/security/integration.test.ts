...
describe('Integration Security Tests', () => {
  ...
  describe('BatchOperations.bulkDelete Security', () => {
    ...
    it('should reject SQL injection in table name', () => {
      expect(() => batchOps.bulkDelete("episodes'; DROP TABLE episodes--", { id: 1 })).toThrow(
        ValidationError
      );
  ...
  describe('BatchOperations.bulkUpdate Security', () => {
    ...
    it('should reject SQL injection in table name', () => {
      expect(() =>
        batchOps.bulkUpdate("episodes'; DROP TABLE episodes--", { reward: 1.0 }, { id: 1 })
      ).toThrow(ValidationError);
    });

    it('should reject SQL injection in SET clause', () => {
      expect(() =>
        batchOps.bulkUpdate('episodes', { 'reward = 1; DROP TABLE episodes--': 0.5 }, { id: 1 })
      ).toThrow(ValidationError);
    ...
    it('should reject SQL injection in WHERE clause', () => {
      expect(() =>
        batchOps.bulkUpdate('episodes', { reward: 1.0 }, { "id' OR '1'='1": 1 })
      ).toThrow(ValidationError);
  ...
  describe('Parameterized Query Verification', () => {
    ...
    it('should use parameterized queries for UPDATE', () => {
      ...
      const updated = batchOps.bulkUpdate('episodes', { task: maliciousValue }, { id: 1 });
