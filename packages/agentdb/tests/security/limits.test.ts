...
describe('AgentDB v2 Security: Query Timeouts', () => {
  describe('withTimeout', () => {
    ...
    it('should timeout slow operations', async () => {
      ...
      await expect(withTimeout(slowPromise, 100, 'slow operation')).rejects.toThrow(SecurityError);
    ...
    it('should use default timeout', async () => {
      ...
      await expect(withTimeout(slowPromise)).rejects.toThrow(/timeout/);
    });
...
describe('AgentDB v2 Security: Rate Limiting', () => {
  ...
  describe('RateLimiter', () => {
    ...
    it('should throw error when consuming with enforcement', () => {
      ...
      expect(() => limiter.consume(1, 'test operation')).toThrow(SecurityError);
      expect(() => limiter.consume(1, 'test operation')).toThrow(/Rate limit exceeded/);
    });
...
describe('AgentDB v2 Security: Circuit Breaker', () => {
  ...
  describe('CircuitBreaker', () => {
    it('should allow operations when closed', async () => {
      const result = await breaker.execute(async () => 'success', 'test operation');
    ...
    it('should open after max failures', async () => {
      ...
      // Cause failures
      for (let i = 0; i < 3; i++) {
        await expect(breaker.execute(failingOp, 'test')).rejects.toThrow('Operation failed');
      }
    ...
    it('should block requests when open', async () => {
      ...
      await expect(breaker.execute(async () => 'success', 'test')).rejects.toThrow(SecurityError);

      await expect(breaker.execute(async () => 'success', 'test')).rejects.toThrow(
