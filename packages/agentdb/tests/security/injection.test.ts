...
describe('AgentDB v2 Security: Vector Validation', () => {
  describe('validateVector', () => {
    ...
    it('should reject oversized vectors', () => {
      ...
      expect(() => validateVector(new Float32Array(dimension), dimension)).toThrow(
        /must be between/
      );
    });
...
describe('AgentDB v2 Security: Search Options Validation', () => {
  describe('validateSearchOptions', () => {
    ...
    it('should reject invalid k values', () => {
      ...
      expect(() => validateSearchOptions({ k: SECURITY_LIMITS.MAX_K + 1 })).toThrow(
        /must be between/
      );
    ...
    it('should reject invalid threshold values', () => {
      expect(() => validateSearchOptions({ threshold: -0.1 })).toThrow(/must be between 0 and 1/);
      expect(() => validateSearchOptions({ threshold: 1.5 })).toThrow(/must be between 0 and 1/);
      expect(() => validateSearchOptions({ threshold: NaN })).toThrow(/must be a finite number/);
      expect(() => validateSearchOptions({ threshold: Infinity })).toThrow(
        /must be a finite number/
      );
    });

    it('should reject invalid efSearch values', () => {
      expect(() => validateSearchOptions({ efSearch: 0 })).toThrow(/must be between/);
      expect(() => validateSearchOptions({ efSearch: SECURITY_LIMITS.MAX_EF_SEARCH + 1 })).toThrow(
        /must be between/
      );
      expect(() => validateSearchOptions({ efSearch: 3.5 as any })).toThrow(/must be an integer/);
    });
  ...
  describe('validateHNSWParams', () => {
    ...
    it('should reject invalid M values', () => {
      ...
      expect(() => validateHNSWParams({ M: SECURITY_LIMITS.MAX_M + 1 })).toThrow(/must be between/);
      expect(() => validateHNSWParams({ M: 3.14 as any })).toThrow(/must be an integer/);
    });

    it('should reject invalid efConstruction values', () => {
      expect(() => validateHNSWParams({ efConstruction: 3 })).toThrow(/must be between/);
      expect(() =>
        validateHNSWParams({ efConstruction: SECURITY_LIMITS.MAX_EF_CONSTRUCTION + 1 })
...
describe('AgentDB v2 Security: Graph Query Security', () => {
  describe('validateCypherParams', () => {
    ...
    it('should reject invalid parameter names', () => {
      expect(() => validateCypherParams({ 'invalid-name': 'value' })).toThrow(
        /must be alphanumeric/
      );
      expect(() => validateCypherParams({ '123invalid': 'value' })).toThrow(/must be alphanumeric/);
      expect(() => validateCypherParams({ 'name;DROP': 'value' })).toThrow(/must be alphanumeric/);
      expect(() => validateCypherParams({ "name' OR '1'='1": 'value' })).toThrow(
        /must be alphanumeric/
      );
    });
    ...
    it('should reject oversized parameter values', () => {
      ...
      expect(() => validateCypherParams({ name: longValue })).toThrow(/too long/);
    });

    it('should reject null bytes in parameters', () => {
      expect(() => validateCypherParams({ name: 'test\x00malicious' })).toThrow(/null bytes/);
    });
    ...
    it('should reject too many parameters', () => {
      ...
      expect(() => validateCypherParams(params)).toThrow(/Too many Cypher parameters/);
    });
  });
...
describe('AgentDB v2 Security: Metadata Sanitization', () => {
  describe('sanitizeMetadata', () => {
    ...
    it('should reject oversized metadata', () => {
      ...
      expect(() => sanitizeMetadata(largeMetadata)).toThrow(/exceeds maximum size/);
    });
    ...
    it('should reject overly long property keys', () => {
      ...
      expect(() => sanitizeMetadata(metadata)).toThrow(/property key exceeds maximum length/);
    });
...
describe('AgentDB v2 Security: Batch Operations', () => {
  describe('validateBatchSize', () => {
    it('should accept valid batch sizes', () => {
      ...
      expect(validateBatchSize(SECURITY_LIMITS.MAX_BATCH_SIZE)).toBe(
    ...
    it('should reject invalid batch sizes', () => {
      ...
      expect(() => validateBatchSize(SECURITY_LIMITS.MAX_BATCH_SIZE + 1)).toThrow(
...
describe('AgentDB v2 Security: Real-World Attack Scenarios', () => {
  ...
  it('should prevent DoS via oversized vectors', () => {
    ...
    expect(() => validateVector(oversized, oversized.length)).toThrow(/must be between/);
  });

  it('should prevent DoS via excessive batch size', () => {
    // Attacker tries to overload system
    expect(() => validateBatchSize(SECURITY_LIMITS.MAX_BATCH_SIZE * 10)).toThrow(/must be between/);
  });
