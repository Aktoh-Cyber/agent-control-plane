...
describe('QUICServer', () => {
  ...
  describe('Connection Management', () => {
    ...
    it('should throw error when accepting connection on stopped server', async () => {
      await server.stop();
      await expect(server.acceptConnection('client-1', 'valid-token-123')).rejects.toThrow(
  ...
  describe('Sync Request Handling', () => {
    ...
    it('should throw error for unknown sync type', async () => {
      ...
      await expect(server.handleSyncRequest('client-1', request)).rejects.toThrow(
        'Unknown sync type: unknown_type'
      );
    });
    ...
    it('should throw error for sync request from unconnected client', async () => {
      ...
      await expect(server.handleSyncRequest('non-existent', request)).rejects.toThrow(
