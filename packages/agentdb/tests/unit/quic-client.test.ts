...
class MockQUICClient {
  ...
  async pushUpdates(data: any): Promise<any> {
    ...
    const totalItems =
      (data.episodes?.length || 0) + (data.skills?.length || 0) + (data.edges?.length || 0);
...
describe('QUICClient', () => {
  ...
  describe('Episode Synchronization', () => {
    ...
    it('should throw error when syncing while disconnected', async () => {
      ...
      await expect(client.syncEpisodes([{ id: '1', content: 'test' }])).rejects.toThrow(
  ...
  describe('Skill Synchronization', () => {
    ...
    it('should throw error when syncing while disconnected', async () => {
      ...
      await expect(client.syncSkills([{ id: '1', name: 'test' }])).rejects.toThrow(
        'Not connected to server'
      );
    });
  });
  ...
  describe('Causal Edge Synchronization', () => {
    ...
    it('should throw error when syncing while disconnected', async () => {
      ...
      await expect(client.syncCausalEdges([{ from: '1', to: '2' }])).rejects.toThrow(
        'Not connected to server'
      );
    });
  });
  ...
  describe('Pull Operations', () => {
    ...
    it('should throw error when pulling while disconnected', async () => {
      ...
      await expect(client.pullUpdates(Date.now())).rejects.toThrow('Not connected to server');
    });
  });
  ...
  describe('Push Operations', () => {
    ...
    it('should throw error when pushing while disconnected', async () => {
      ...
      await expect(client.pushUpdates({ episodes: [] })).rejects.toThrow('Not connected to server');
    });
  });
  ...
  describe('Error Handling', () => {
    it('should handle network timeouts gracefully', async () => {
      ...
      await expect(disconnectedClient.syncEpisodes([{ id: '1', content: 'test' }])).rejects.toThrow(
        'Not connected to server'
      );
    });
