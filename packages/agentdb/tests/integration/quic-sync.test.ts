...
describe('QUIC Synchronization Integration', () => {
  ...
  describe('Basic Client-Server Sync', () => {
    it('should complete full sync workflow', async () => {
      const client = new IntegrationQUICClient(
        'quic://localhost:4433',
        'valid-token-123',
        'client-1'
      );
  ...
  describe('Multi-Client Synchronization', () => {
    it('should handle multiple simultaneous clients', async () => {
      ...
      // Connect all clients
      const connections = await Promise.all(clients.map((client) => client.connect(server)));
    ...
    it('should sync data from multiple clients', async () => {
      const client1 = new IntegrationQUICClient(
        'quic://localhost:4433',
        'valid-token-123',
        'sync-1'
      );
      const client2 = new IntegrationQUICClient(
        'quic://localhost:4433',
        'valid-token-123',
        'sync-2'
      );
  ...
  describe('Network Failure Recovery', () => {
    ...
    it('should retry failed sync operations', async () => {
      ...
      const items = [{ id: 'retry-item', content: 'Test content', timestamp: Date.now() }];
  ...
  describe('Episode-Skill-Causal Sync', () => {
    it('should sync all data types together', async () => {
      ...
      const skills = [{ id: 'skill-sync-1', type: 'skill', name: 'Skill', timestamp: Date.now() }];
  ...
  describe('Performance Characteristics', () => {
    it('should maintain performance under load', async () => {
      ...
      const clients = Array.from(
        { length: numClients },
        (_, i) => new IntegrationQUICClient('quic://localhost:4433', 'valid-token-123', `perf-${i}`)
      );
    ...
    it('should handle rapid successive syncs', async () => {
      ...
      const syncOperations = Array.from({ length: 50 }, (_, i) =>
        client.sync(server, [{ id: `rapid-${i}`, content: `Item ${i}`, timestamp: Date.now() }])
      );
