...
test('Test Suite 2: Bundle Content Analysis', () => {
  ...
  assert(bundleContent.includes('AgentDB Browser Bundle'), 'Bundle includes header comment');
  ...
  assert(bundleContent.includes('initSqlJs'), 'Bundle includes sql.js WASM loader');
...
test('Test Suite 3: v1 API Backward Compatibility', () => {
  ...
  v1Methods.forEach((method) => {
    assert(bundleContent.includes(method), `v1 method "${method}" is present`);
  });
});
...
test('Test Suite 4: v2 Enhanced API Features', () => {
  ...
  v2Methods.forEach((method) => {
    assert(bundleContent.includes(method), `v2 method "${method}" is present`);
  });
});
...
test('Test Suite 6: Embedding & Search Features', () => {
  ...
  assert(bundleContent.includes('cosineSimilarity'), 'Cosine similarity function exists');

  assert(bundleContent.includes('Float32Array'), 'Float32Array used for embeddings');

  assert(bundleContent.includes('384'), '384-dimensional embeddings configured');
});
...
test('Test Suite 7: Configuration Options', () => {
  ...
  configOptions.forEach((option) => {
    assert(bundleContent.includes(option), `Configuration option "${option}" available`);
  });

  assert(bundleContent.includes('indexeddb'), 'IndexedDB storage option available');

  assert(bundleContent.includes('BroadcastChannel'), 'Cross-tab sync with BroadcastChannel');
});
...
test('Test Suite 8: Namespace & Module Exports', () => {
  ...
  assert(bundleContent.includes('var AgentDB'), 'AgentDB namespace is defined');

  assert(bundleContent.includes('AgentDB.Database'), 'AgentDB.Database is exported');
  ...
  assert(bundleContent.includes('global.AgentDB'), 'AgentDB is attached to global object');

  assert(bundleContent.includes('module.exports'), 'CommonJS module.exports is available');

  assert(bundleContent.includes('define.amd'), 'AMD (RequireJS) support is available');
});
...
test('Test Suite 9: Error Handling', () => {
  ...
  assert(bundleContent.includes('throw new Error'), 'Error throwing for invalid states');

  assert(bundleContent.includes('[AgentDB]'), 'Consistent error message prefixes');
});
...
test('Test Suite 10: Performance Features', () => {
  ...
  assert(bundleContent.includes('performance.now'), 'Performance timing available (if supported)');

  assert(bundleContent.includes('Promise'), 'Promise support for async operations');
