...
describe('AgentDB Browser Bundle', () => {
  ...
  describe('Search and Query Operations', () => {
    beforeAll(() => {
      ...
      db.run('INSERT INTO vectors (text, metadata) VALUES (?, ?)', [
        'Campaign 1',
        JSON.stringify({ roas: 2.5 }),
      ]);
      db.run('INSERT INTO vectors (text, metadata) VALUES (?, ?)', [
        'Campaign 2',
        JSON.stringify({ roas: 3.0 }),
      ]);
      db.run('INSERT INTO vectors (text, metadata) VALUES (?, ?)', [
        'Campaign 3',
        JSON.stringify({ roas: 1.8 }),
      ]);
    });
  ...
  describe('Error Handling', () => {
    ...
    it('should throw error on missing required fields', () => {
      db.run(
        `CREATE TABLE IF NOT EXISTS test (id INTEGER PRIMARY KEY, required_field TEXT NOT NULL)`
      );
