...
async function testCLI() {
  ...
  test(
    'CLI: --version',
    result.stdout.includes('agentdb v') ? 'PASS' : 'FAIL',
    result.stdout.trim()
  );
...
async function testLibraryAPI() {
  ...
  try {
    ...
    test(
      'Library: Required exports',
      hasAllExports ? 'PASS' : 'FAIL',
...
async function testSecurity() {
  ...
  try {
    ...
    test('Security: Safe WHERE clause', whereClause.includes('session_id = ?') ? 'PASS' : 'FAIL');
  } catch (error) {
