...
import { CausalEdge, CausalMemoryGraph } from '../src/controllers/CausalMemoryGraph.js';
...
import { Episode, ReflexionMemory, ReflexionQuery } from '../src/controllers/ReflexionMemory.js';
import { Skill, SkillLibrary, SkillQuery } from '../src/controllers/SkillLibrary.js';
  ...
  describe('db_stats', () => {
    it('should return statistics for all tables', () => {
      ...
      tables.forEach((table) => {
        const result = ctx.db.prepare(`SELECT COUNT(*) as count FROM ${table}`).get() as {
          count: number;
        };
    ...
    it('should return accurate counts after inserts', async () => {
      const beforeCount = ctx.db.prepare('SELECT COUNT(*) as count FROM episodes').get() as {
        count: number;
      };
      ...
      const afterCount = ctx.db.prepare('SELECT COUNT(*) as count FROM episodes').get() as {
        count: number;
      };
