...
class AttentionBenchmark {
  ...
  private generateTestData(
    count: number
  ): Array<{ content: string; embedding: number[]; metadata: any }> {
    const data = [];
  ...
  async benchmarkMechanism(name: string, mechanism: any, workloadSize: number): Promise<void> {
    console.log(`\n🔬 Benchmarking ${name}...`);
  ...
  generateReport(results: BenchmarkResult[]): string {
    ...
    for (const target of targets) {
      ...
      if ('targetUs' in target) {
        const avgLatency =
          mechanismResults.reduce((sum, r) => sum + r.avgLatencyUs, 0) / mechanismResults.length;
        const status = avgLatency < target.targetUs ? '✅' : '❌';
        lines.push(
          `- ${status} **${target.mechanism}**: ${avgLatency.toFixed(2)}µs avg (target: <${target.targetUs}µs)`
        );
      } else if ('targetSpeedup' in target) {
        const avgSpeedup =
          speedups[target.mechanism]?.reduce((a, b) => a + b, 0) /
          (speedups[target.mechanism]?.length || 1);
        const status = avgSpeedup >= target.targetSpeedup ? '✅' : '❌';
        lines.push(
          `- ${status} **${target.mechanism}**: ${avgSpeedup.toFixed(2)}x speedup (target: ${target.targetSpeedup}x for 10K+)`
        );
      }
    }
