...
import { DEFAULT_CONFIG, generateRandomVectors, measureMemory } from './runner.js';
...
describe('Memory Usage Benchmarks - RuVector', () => {
  ...
  test('memory growth rate - incremental inserts', async () => {
    ...
    for (const checkpoint of checkpoints) {
      ...
      console.log(
        `  ${checkpoint.toLocaleString()} vectors: ${(currentMemory - baseMemory).toFixed(2)} MB`
      );
    }

    // Calculate memory per vector
    const memoryPerVector =
      measurements[measurements.length - 1].memoryMB / measurements[measurements.length - 1].count;
...
describe('Memory Profiling - Different Dimensions', () => {
  ...
  for (const dim of DIMENSIONS) {
    test(`memory usage - dimension ${dim}`, async () => {
      ...
      console.log(
        `Dimension ${dim}: ${memory.peakMB.toFixed(2)} MB (${bytesPerVector.toFixed(0)} bytes/vector)`
      );
    });
  }
});
