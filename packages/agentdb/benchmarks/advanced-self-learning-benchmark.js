...
async function benchmarkAdaptiveLearning() {
  ...
  for (let session = 0; session < 10; session++) {
    ...
    console.log(
      `  Session ${session + 1}: Success rate ${(overallSuccessRate * 100).toFixed(1)}%, Avg reward: ${(stats?.avgReward || 0).toFixed(3)}`
    );
  }
  ...
  console.log(
    `  ⏱️  Avg session duration: ${(sessions.reduce((sum, s) => sum + s.duration, 0) / sessions.length).toFixed(2)}ms`
  );
...
async function benchmarkSkillEvolution() {
  ...
  for (const base of baseSkills) {
    ...
    console.log(
      `  ${base.name}: ${base.successRate.toFixed(2)} → ${(bestVersion[0]?.successRate || 0).toFixed(2)}`
    );
  }

  const avgImprovement =
    skillEvolution.reduce((sum, s) => sum + s.improvement, 0) / skillEvolution.length;
...
async function runAdvancedSelfLearningBenchmarks() {
  ...
  console.log(
    '║' + ' '.repeat(8) + 'AgentDB Self-Learning Advanced Benchmark' + ' '.repeat(9) + '║'
  );
  console.log('╚' + '═'.repeat(58) + '╝');
  console.log('');
  ...
  console.log(
    `  Learning improvement: ${((lastSession.successRate - firstSession.successRate) * 100).toFixed(1)}%`
  );

  console.log('\n🎯 Skill Evolution:');
  const avgSkillImprovement =
    results.skillEvolution.reduce((sum, s) => sum + s.improvement, 0) /
    results.skillEvolution.length;
