...
export function formatRegressionReport(report: RegressionReport): string {
  ...
  lines.push('╠══════════════════════════════════════════════════════════════════════╣');
  lines.push(
    `║ Platform: ${report.platform.padEnd(12)} │ Node: ${report.nodeVersion.padEnd(12)} │ Date: ${new Date(report.timestamp).toLocaleDateString().padEnd(10)} ║`
  );
  lines.push('╚══════════════════════════════════════════════════════════════════════╝');
  lines.push('');
  ...
  lines.push('├───────────────────┬─────────────────────────────────────────────────┤');
  lines.push(
    `│ Total Checks      │ ${report.summary.total.toString().padStart(6)}                                       │`
  );
  lines.push(
    `│ ✅ Passed         │ ${report.summary.passed.toString().padStart(6)}                                       │`
  );
  lines.push(
    `│ ⬆️  Improved       │ ${report.summary.improved.toString().padStart(6)}                                       │`
  );
  lines.push(
    `│ ⚠️  Warnings       │ ${report.summary.warnings.toString().padStart(6)}                                       │`
  );
  lines.push(
    `│ ❌ Failed         │ ${report.summary.failed.toString().padStart(6)}                                       │`
  );
  lines.push('└───────────────────┴─────────────────────────────────────────────────┘');
  lines.push('');
  ...
  if (report.regressions.length > 0) {
    ...
    const sorted = [...report.regressions].sort(
    ...
    for (const reg of sorted) {
      ...
      const change =
        `${reg.changePercent >= 0 ? '+' : ''}${reg.changePercent.toFixed(1)}%`.padStart(9);
