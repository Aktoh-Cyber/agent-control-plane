...
async function benchmarkPatternLearning() {
  ...
  for (let cycle = 0; cycle < CONFIG.learning.feedback_cycles; cycle++) {
    ...
    const avgSuccessRate =
      bestPatterns.reduce((sum, p) => sum + p.successRate, 0) / bestPatterns.length;
...
async function runAdvancedBenchmarks() {
  ...
  console.log(
    '║' + ' '.repeat(10) + 'AgentDB ReasoningBank Advanced Benchmark' + ' '.repeat(7) + '║'
  );
  console.log('╚' + '═'.repeat(58) + '╝');
  console.log('');
  ...
  allResults.scalability.forEach((r) => {
    console.log(
      `  ${r.size.padEnd(10)}: ${r.throughput.toFixed(2)} patterns/sec, ${r.memoryUsed}MB`
    );
  });
  ...
  console.log(
    `  Threshold: ${bestThreshold.threshold} (${bestThreshold.avgMatches.toFixed(1)} matches, ${bestThreshold.avgSearchTime.toFixed(2)}ms)`
  );
  ...
  console.log(
    `  Improvement: ${((lastCycle.currentSuccessRate - firstCycle.currentSuccessRate) * 100).toFixed(1)}%`
  );
