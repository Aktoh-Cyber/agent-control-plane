...
class SimpleBenchmark {
  ...
  async benchmarkReflexionMemory(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `   ✅ ${result.name}: ${result.duration.toFixed(2)}ms (${result.operationsPerSecond?.toFixed(0)} ops/sec)`
      );
      console.log(
        `      Episodes: ${episodeCount}, Insert: ${insertDuration.toFixed(2)}ms, Avg Query: ${avgQueryTime.toFixed(2)}ms`
      );
    } catch (error) {
  ...
  async benchmarkSkillLibrary(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `      Skills: ${skillCount}, Insert: ${insertDuration.toFixed(2)}ms, Avg Search: ${avgSearchTime.toFixed(2)}ms`
      );
    } catch (error) {
  ...
  async benchmarkCausalGraph(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `      Nodes: ${nodeCount}, Edges: ${edgeCount}, Avg Query: ${avgQueryTime.toFixed(2)}ms`
      );
    } catch (error) {
  ...
  async generateReport(): Promise<void> {
    ...
    if (successful.length > 0) {
      ...
      for (const result of successful) {
        console.log(
          `   ${result.name}: ${result.duration.toFixed(2)}ms (${result.operationsPerSecond?.toFixed(0) || 'N/A'} ops/sec)`
        );
      }
    }
