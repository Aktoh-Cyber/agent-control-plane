...
async function runBenchmarks() {
  ...
  const v2Storage = results.find(
    (r) => r.version === 'v2' && r.operation === 'storage' && !r.useBackends
  );
  ...
  console.log(
    `  v2 (no backends): ${v2Storage.throughput.toFixed(2)} patterns/sec (${((v2Storage.throughput / v1Storage.throughput) * 100).toFixed(1)}%)`
  );
  console.log(
    `  v2 (with backends): ${v2StorageBackends.throughput.toFixed(2)} patterns/sec (${((v2StorageBackends.throughput / v1Storage.throughput) * 100).toFixed(1)}%)`
  );
  ...
  const v2Search = results.find(
    (r) => r.version === 'v2' && r.operation === 'search' && !r.useBackends
  );
  const v2SearchBackends = results.find(
    (r) => r.version === 'v2' && r.operation === 'search' && r.useBackends
  );
  ...
  console.log(
    `  v2 (no backends): ${v2Search.throughput.toFixed(2)} searches/sec (${((v2Search.throughput / v1Search.throughput) * 100).toFixed(1)}%)`
  );
  console.log(
    `  v2 (with backends): ${v2SearchBackends.throughput.toFixed(2)} searches/sec (${((v2SearchBackends.throughput / v1Search.throughput) * 100).toFixed(1)}%)`
  );
