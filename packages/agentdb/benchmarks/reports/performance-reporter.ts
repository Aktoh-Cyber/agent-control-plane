...
export class PerformanceReporter {
  ...
  async generateMarkdownReport(results: BenchmarkResult[]): Promise<void> {
    ...
    await fs.writeFile(path.join(this.reportsDir, 'reports', 'performance-report.md'), md, 'utf-8');
  }
  ...
  private identifyBottlenecks(results: BenchmarkResult[]): any[] {
    ...
    const sorted = results.filter((r) => r.success).sort((a, b) => b.duration - a.duration);
  ...
  private generateRecommendations(results: BenchmarkResult[]): string[] {
    ...
    if (
      hnswResult &&
      withoutHNSW &&
      hnswResult.operationsPerSecond &&
      withoutHNSW.operationsPerSecond
    ) {
      const speedup = withoutHNSW.duration / hnswResult.duration;
      if (speedup > 10) {
        recommendations.push(
          `✅ HNSW indexing provides ${speedup.toFixed(0)}x speedup - highly recommended for production`
        );
      }
    }

    // Check batch operations
    const batchComparison = results.find(
      (r) => r.name === 'Batch vs Individual Comparison' && r.success
    );
    if (batchComparison && batchComparison.metrics.speedupFactor > 2) {
      recommendations.push(
        `✅ Batch operations are ${batchComparison.metrics.speedupFactor}x faster - use batch inserts for bulk data`
      );
    }
    ...
    if (memoryReduction && memoryReduction.metrics.reduction4BitPercent) {
      ...
      if (reduction > 50) {
        recommendations.push(
          `✅ 4-bit quantization reduces memory by ${reduction.toFixed(0)}% - recommended for large datasets`
        );
      }
    }
    ...
    if (recommendations.length === 0) {
      recommendations.push(
        'No specific optimizations identified - performance is within normal ranges'
      );
    }
