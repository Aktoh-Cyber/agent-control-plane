...
class AttentionBenchmark {
  ...
  async benchmarkSelfAttention(
    ...
    for (let i = 0; i < queryCount; i++) {
      const query = Array(embeddingDim)
        .fill(0)
        .map(() => Math.random());
  ...
  async benchmarkMultiHeadAttention(
    ...
    const controller = this.db.getController(
      'multi-head-attention'
    ) as MultiHeadAttentionController;
    ...
    for (const concurrency of concurrencyLevels) {
      ...
      for (let i = 0; i < iterations / concurrency; i++) {
        ...
        for (let j = 0; j < concurrency; j++) {
          ...
          const queryStart = performance.now();
          const promise = controller.computeAttention(query, { topK: 10 }).then(() => {
