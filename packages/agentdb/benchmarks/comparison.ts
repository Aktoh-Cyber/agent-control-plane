...
import { BaselineData, loadBaseline } from './regression-check.js';
import { BenchmarkResult } from './runner.js';
...
export function compareBenchmarks(
  ...
  for (const ruvResult of ruvectorResults) {
    ...
    const status =
      p50Speedup >= targetSpeedup * 1.1
        ? 'exceeded'
        : p50Speedup >= targetSpeedup * 0.9
          ? 'met'
          : 'missed';
...
export function formatComparisonReport(
  ...
  // Summary Statistics
  const avgSpeedup =
    comparisons.reduce((sum, c) => sum + c.improvement.p50Speedup, 0) / comparisons.length;
  const maxSpeedup = Math.max(...comparisons.map((c) => c.improvement.p50Speedup));
  const avgMemReduction =
    memoryComparisons.reduce((sum, m) => sum + m.reduction, 0) / memoryComparisons.length;

  lines.push('┌────────────────────────────────────────────────────────────────────────┐');
  lines.push('│ Summary Statistics                                                     │');
  lines.push('├────────────────────────────────────────────────────────────────────────┤');
  lines.push(
    `│ Average Search Speedup:        ${avgSpeedup.toFixed(1)}x faster                          │`
  );
  lines.push(
    `│ Maximum Search Speedup:        ${maxSpeedup.toFixed(1)}x faster                          │`
  );
  lines.push(
    `│ Average Memory Reduction:      ${avgMemReduction.toFixed(1)}x less memory                    │`
  );
  lines.push(
    `│ Benchmarks Meeting Target:     ${comparisons.filter((c) => c.status !== 'missed').length}/${comparisons.length}                               │`
  );
  lines.push('└────────────────────────────────────────────────────────────────────────┘');
  lines.push('');
  ...
  // Overall Assessment
  const metTarget =
    comparisons.filter((c) => c.status !== 'missed').length / comparisons.length >= 0.9;
...
export function generateComparisonJSON(
  ...
  return {
    ...
    summary: {
      ...
      successRate:
        (comparisons.filter((c) => c.status !== 'missed').length / comparisons.length) * 100,
    },
