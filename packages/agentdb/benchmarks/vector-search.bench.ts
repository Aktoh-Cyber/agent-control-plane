...
import { DEFAULT_CONFIG, generateRandomVectors } from './runner.js';
...
describe('Batch Insert Benchmarks - RuVector', () => {
  ...
  for (const batchSize of BATCH_SIZES) {
    bench(
      `batch insert ${batchSize} vectors`,
      async () => {
...
describe('Concurrent Search Benchmarks - RuVector', () => {
  ...
  bench(
    'concurrent searches (10 queries)',
    async () => {
      const promises = queryVectors.map((query) => Promise.resolve(backend.search(query, 10)));
