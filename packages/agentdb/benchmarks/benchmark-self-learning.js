...
async function runBenchmarks() {
  ...
  const v2Storage = results.find(
    (r) => r.version === 'v2' && r.operation === 'episode_storage' && !r.useBackends
  );
  const v2StorageBackends = results.find(
    (r) => r.version === 'v2' && r.operation === 'episode_storage' && r.useBackends
  );
  ...
  console.log(
    `  v2 (no backends): ${v2Storage.throughput.toFixed(2)} episodes/sec (${((v2Storage.throughput / v1Storage.throughput) * 100).toFixed(1)}%)`
  );
  console.log(
    `  v2 (with backends): ${v2StorageBackends.throughput.toFixed(2)} episodes/sec (${((v2StorageBackends.throughput / v1Storage.throughput) * 100).toFixed(1)}%)`
  );
  console.log(`  🚀 Speedup: ${(v2StorageBackends.throughput / v1Storage.throughput).toFixed(2)}x`);

  // Retrieval comparison
  const v1Retrieval = results.find(
    (r) => r.version === 'v1' && r.operation === 'episode_retrieval'
  );
  const v2Retrieval = results.find(
    (r) => r.version === 'v2' && r.operation === 'episode_retrieval' && !r.useBackends
  );
  const v2RetrievalBackends = results.find(
    (r) => r.version === 'v2' && r.operation === 'episode_retrieval' && r.useBackends
  );
  ...
  console.log(
    `  v2 (no backends): ${v2Retrieval.throughput.toFixed(2)} retrievals/sec (${((v2Retrieval.throughput / v1Retrieval.throughput) * 100).toFixed(1)}%)`
  );
  console.log(
    `  v2 (with backends): ${v2RetrievalBackends.throughput.toFixed(2)} retrievals/sec (${((v2RetrievalBackends.throughput / v1Retrieval.throughput) * 100).toFixed(1)}%)`
  );
  console.log(
    `  🚀 Speedup: ${(v2RetrievalBackends.throughput / v1Retrieval.throughput).toFixed(2)}x`
  );

  // Task Stats comparison
  const v1Stats = results.find((r) => r.version === 'v1' && r.operation === 'task_stats');
  const v2Stats = results.find(
    (r) => r.version === 'v2' && r.operation === 'task_stats' && !r.useBackends
  );
  const v2StatsBackends = results.find(
    (r) => r.version === 'v2' && r.operation === 'task_stats' && r.useBackends
  );
