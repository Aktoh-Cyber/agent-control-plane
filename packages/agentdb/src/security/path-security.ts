...
export class TempFileManager {
   ...
   */
  async writeTempFile(data: Buffer | string, prefix: string = 'agentdb'): Promise<string> {
 ...
 */
export async function ensureDir(dirPath: string, baseDir: string): Promise<string> {
  const safeDir = validatePath(dirPath, baseDir);
 ...
 */
export async function safeStats(filePath: string, baseDir: string): Promise<fs.Stats> {
  const safePath = validatePath(filePath, baseDir);
 ...
 */
export async function pathExists(filePath: string, baseDir: string): Promise<boolean> {
  try {
    const safePath = validatePath(filePath, baseDir);
    await fs.promises.access(safePath, fs.constants.F_OK);
    return true;
