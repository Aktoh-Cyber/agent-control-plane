 ...
 *
 * Uses agent-control-plane's router SDK to support:
...
export class LLMRouter {
  ...
  private loadEnv(): void {
    ...
    try {
      ...
      const possiblePaths = [
        ...
        path.join(process.cwd(), '..', '..', '.env'),
        '/workspaces/agent-control-plane/.env',
      ];
  ...
  private async callGemini(
    ...
    const tokensUsed =
      ((data.usageMetadata?.promptTokenCount as number) || 0) +
  ...
  private async callAnthropic(
    ...
    const tokensUsed =
      ((data.usage?.input_tokens as number) || 0) + ((data.usage?.output_tokens as number) || 0);
