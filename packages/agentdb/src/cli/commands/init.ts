...
export async function initCommand(options: InitOptions = {}): Promise<void> {
  ...
  try {
    ...
    const embeddingModel =
      model || (dimension === 768 ? 'Xenova/bge-base-en-v1.5' : 'Xenova/all-MiniLM-L6-v2');

    console.log(`\n${colors.bright}${colors.cyan}🚀 Initializing AgentDB${colors.reset}\n`);
    console.log(`  Database:      ${colors.blue}${actualDbPath}${colors.reset}`);
    console.log(
      `  Backend:       ${getBackendColor(selectedBackend)}${selectedBackend}${colors.reset}`
    );
    ...
    for (const schemaFile of schemaFiles) {
      ...
      if (fs.existsSync(schemaPath)) {
        ...
        console.warn(
          `${colors.yellow}⚠ Warning: Schema file not found: ${schemaPath}${colors.reset}`
        );
      }
    }
    ...
    if (selectedBackend === 'ruvector' && detection.features.gnn) {
      console.log(
        `${colors.bright}${colors.magenta}🧠 Bonus:${colors.reset} GNN self-learning available`
      );
      console.log(
        `   Use ${colors.cyan}agentdb train${colors.reset} to enable adaptive patterns\n`
      );
    }

    if (selectedBackend === 'hnswlib') {
      console.log(
        `${colors.yellow}💡 Tip:${colors.reset} Install RuVector for 150x performance boost`
      );
