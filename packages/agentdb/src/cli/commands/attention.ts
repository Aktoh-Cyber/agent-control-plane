...
function createInitCommand(): Command {
  return new Command('init')
    ...
    .option(
      '-m, --mechanism <type>',
      'Attention mechanism (flash, hyperbolic, sparse, linear, performer)',
      'flash'
    )
    ...
    .action(async (options: AttentionInitOptions) => {
      try {
        ...
        const configPath =
          options.config || path.join(process.cwd(), '.agentdb', 'attention-config.json');
        ...
        if (options.json) {
          ...
          console.log(
            `  Enabled Mechanisms: ${Object.entries(config.mechanisms)
    ...
    .action(async (options: AttentionOptimizeOptions) => {
      try {
        ...
        if (options.json) {
          ...
          console.log(
            `  Performance Gain: ${(optimizationResult.performanceGain * 100).toFixed(1)}%`
          );
          console.log(
            `  Memory Reduction: ${(optimizationResult.memoryReduction * 100).toFixed(1)}%`
          );
          console.log(`  Recommended Configuration:`);
          console.log(
            `    ${JSON.stringify(optimizationResult.config, null, 4).split('\n').join('\n    ')}`
          );
          console.log('');
        }
      } catch (error: any) {
...
function displayBenchmarkResults(results: any): void {
  ...
  console.log(
    `  Fastest: ${results.comparison.fastest.mechanism} (${results.comparison.fastest.avgTimeMs.toFixed(3)}ms)`
  );
  console.log(
    `  Slowest: ${results.comparison.slowest.mechanism} (${results.comparison.slowest.avgTimeMs.toFixed(3)}ms)`
  );
