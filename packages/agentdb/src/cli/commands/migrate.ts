...
export async function migrateCommand(options: MigrationOptions): Promise<void> {
  ...
  console.log(
    `  Optimize for GNN: ${optimize ? colors.green + 'Yes' : colors.yellow + 'No'}${colors.reset}`
  );
  ...
  try {
    ...
    const frontierSchemaPath = path.join(
      path.dirname(new URL(import.meta.url).pathname),
      '../../schemas/frontier-schema.sql'
    );
    ...
    stats.performance.totalRecords = Object.values(stats.recordsMigrated).reduce(
      (a, b) => a + b,
      0
    );
    const totalTime = Date.now() - startTime;
    stats.performance.recordsPerSecond = Math.round(
      stats.performance.totalRecords / (totalTime / 1000)
    );
...
function detectSourceType(db: Database.Database): 'v1-agentdb' | 'claude-flow-memory' | 'unknown' {
  ...
  if (
    tables.includes('memory_entries') &&
    tables.includes('patterns') &&
    tables.includes('task_trajectories')
  ) {
...
function analyzeMigration(db: Database.Database, sourceType: string, tables: string[]): any {
  const analysis: any = {
    sourceType,
...
async function migrateClaudeFlowMemory(
  ...
  for (const entry of memoryEntries as any[]) {
    try {
      ...
      if (verbose)
        console.log(
          `    ${colors.yellow}⚠${colors.reset} Failed to migrate entry ${entry.id}: ${(e as Error).message}`
        );
    }
  }
  console.log(
    `  ${colors.green}✅${colors.reset} Migrated ${stats.recordsMigrated.memoryEntries} memory entries to episodes`
  );
  ...
  for (const pattern of patterns as any[]) {
    try {
      ...
      const createdAt = pattern.created_at
        ? Math.floor(new Date(pattern.created_at).getTime() / 1000)
      ...
      if (verbose)
        console.log(
          `    ${colors.yellow}⚠${colors.reset} Failed to migrate pattern ${pattern.id}: ${(e as Error).message}`
        );
    }
  }
  console.log(
    `  ${colors.green}✅${colors.reset} Migrated ${stats.recordsMigrated.patterns} patterns to skills`
  );
  ...
  for (const traj of trajectories as any[]) {
    try {
      ...
      if (verbose)
        console.log(
          `    ${colors.yellow}⚠${colors.reset} Failed to migrate trajectory ${traj.task_id}: ${(e as Error).message}`
        );
    }
  }
  console.log(
    `  ${colors.green}✅${colors.reset} Migrated ${stats.recordsMigrated.trajectories} trajectories to events\n`
  );
}
    ...
    try {
      ...
      for (const row of rows as any[]) {
        try {
          ...
          if (verbose)
            console.log(
              `    ${colors.yellow}⚠${colors.reset} Failed to migrate row: ${(e as Error).message}`
            );
        }
      }

      console.log(
        `  ${colors.green}✅${colors.reset} Migrated ${(stats.recordsMigrated as any)[table]} records from ${table}`
      );
    } catch (e) {
      console.log(
        `  ${colors.yellow}⚠${colors.reset} Error migrating ${table}: ${(e as Error).message}`
      );
    }
  }
  console.log('');
...
async function performGNNOptimization(
  ...
  for (const ep of episodes) {
    try {
      ...
      if (verbose)
        console.log(
          `    ${colors.yellow}⚠${colors.reset} Failed to create embedding for episode ${ep.id}`
        );
    }
  }
  console.log(
    `  ${colors.green}✅${colors.reset} Generated ${stats.gnnOptimization.episodeEmbeddings} episode embeddings`
  );

  // Create causal edges from episode sequence
  if (verbose) console.log(`  ${colors.blue}→${colors.reset} Analyzing causal relationships...`);

  const sessionEpisodes = db
    .prepare(
      `
  ...
  console.log(
    `  ${colors.green}✅${colors.reset} Created ${stats.gnnOptimization.causalEdgesCreated} causal edges`
  );
  ...
  console.log(
    `  ${colors.green}✅${colors.reset} Created ${stats.gnnOptimization.skillLinksCreated} skill links`
  );
...
function printMigrationReport(stats: MigrationStats, totalTime: number): void {
  ...
  for (const [type, count] of Object.entries(stats.recordsMigrated)) {
    if (count > 0) {
      console.log(
        `  ${type.padEnd(20)} ${colors.green}${count.toString().padStart(6)}${colors.reset}`
      );
    }
  }
  ...
  console.log(
    `  ${'Total'.padEnd(20)} ${colors.bright}${colors.green}${totalMigrated.toString().padStart(6)}${colors.reset}\n`
  );

  console.log(`${colors.bright}GNN Optimization Results:${colors.reset}`);
  console.log(
    `  Episode embeddings:    ${colors.blue}${stats.gnnOptimization.episodeEmbeddings}${colors.reset}`
  );
  console.log(
    `  Causal edges created:  ${colors.blue}${stats.gnnOptimization.causalEdgesCreated}${colors.reset}`
  );
  console.log(
    `  Skill links created:   ${colors.blue}${stats.gnnOptimization.skillLinksCreated}${colors.reset}`
  );
  if (stats.gnnOptimization.averageSimilarity > 0) {
    console.log(
      `  Avg similarity score:  ${colors.blue}${stats.gnnOptimization.averageSimilarity.toFixed(3)}${colors.reset}`
    );
    console.log(
      `  Clustering coeff:      ${colors.blue}${stats.gnnOptimization.clusteringCoefficient.toFixed(3)}${colors.reset}`
    );
  }
  console.log('');
  ...
  console.log(
    `  Migration time:        ${colors.blue}${(stats.performance.migrationTime / 1000).toFixed(2)}s${colors.reset}`
  );
  console.log(
    `  Optimization time:     ${colors.blue}${(stats.performance.optimizationTime / 1000).toFixed(2)}s${colors.reset}`
  );
  console.log(
    `  Total time:            ${colors.blue}${(totalTime / 1000).toFixed(2)}s${colors.reset}`
  );
  console.log(
    `  Records/second:        ${colors.blue}${stats.performance.recordsPerSecond}${colors.reset}\n`
  );

  console.log(
    `${colors.bright}${colors.green}✅ Database ready for RuVector GNN training${colors.reset}\n`
  );
}
