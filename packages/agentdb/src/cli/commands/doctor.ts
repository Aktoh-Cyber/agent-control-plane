 ...
 *
 * REFACTORED: Uses dependency injection via ModuleLoader instead of runtime patches
 */
...
import { ModuleLoader, getDefaultLoader } from '../../services/ModuleLoader.js';

interface DoctorOptions {
  dbPath?: string;
  verbose?: boolean;
  moduleLoader?: ModuleLoader; // Dependency injection for testing
}

export async function doctorCommand(options: DoctorOptions = {}): Promise<void> {
  const { dbPath = './agentdb.db', verbose = false, moduleLoader } = options;

  // Use provided module loader or default singleton
  ...
  // Check if optional dependencies are available (using ModuleLoader)
  const transformersResult = await loader.loadModule('@xenova/transformers', { cache: true });
  if (transformersResult.isAvailable) {
  ...
  try {
    ...
    console.log(
      `     Features: GNN=${result.features.gnn ? 'Yes' : 'No'}, Graph=${result.features.graph ? 'Yes' : 'No'}`
    );
  ...
  // Check 7: Core Modules (using ModuleLoader)
  console.log('\n🔧 Core Modules');
  const coreModules = [
    { name: 'fs', module: 'fs' },
  ...
  const moduleResults = await loader.loadModules(
    coreModules.map((m) => m.module),
    { cache: true }
  );

  for (const mod of coreModules) {
    const result = moduleResults.get(mod.module);
  ...
  if (cpuCount >= 8) {
    ...
    recommendations.push(
      `⚠️  Only ${cpuCount} CPU cores detected - parallel operations may be limited.`
    );
  }
  ...
  try {
    ...
    if (result.backend === 'ruvector' && result.features.gnn) {
      ...
      recommendations.push(
        '💡 Consider using --backend ruvector for 150x performance improvement.'
      );
    }
  } catch {}
  ...
  if (dbPath && dbPath !== ':memory:' && fs.existsSync(dbPath)) {
    ...
    if (dbSizeMB > 100) {
      recommendations.push(
        `💡 Large database (${dbSizeMB.toFixed(1)}MB) - consider periodic optimization:`
      );
  ...
  // Embedding optimization (using ModuleLoader)
  const transformersCheck = await loader.isModuleAvailable('@xenova/transformers');
  if (transformersCheck) {
