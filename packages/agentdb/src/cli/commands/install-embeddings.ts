...
export async function installEmbeddingsCommand(
  options: InstallEmbeddingsOptions = {}
): Promise<void> {
  console.log(
  ...
  try {
    ...
    try {
      ...
      console.log('');
      console.log(
        `${colors.cyan}💡 Tip:${colors.reset} Set ${colors.yellow}HUGGINGFACE_API_KEY${colors.reset} for online models`
      );
      console.log('');
    } catch (installError) {
