...
export async function statusCommand(options: StatusOptions = {}): Promise<void> {
  ...
  try {
    ...
    console.log(
      `  Status:        ${dbExists ? colors.green + '✅ Exists' : colors.red + '❌ Not found'}${colors.reset}`
    );
    ...
    try {
      ...
      console.log(
        `  Backend:       ${backend ? getBackendColor(backend) + backend : colors.yellow + 'Not configured'}${colors.reset}`
      );
      ...
      for (const table of tables) {
        try {
          ...
          if (verbose || count > 0) {
            console.log(
              `  ${table.label.padEnd(20)} ${colors.blue}${count.toLocaleString()}${colors.reset}`
            );
          }
        } catch {
          // Table doesn't exist
      ...
      console.log(
        `  ${colors.bright}Total Records${colors.reset}      ${colors.blue}${totalRecords.toLocaleString()}${colors.reset}`
      );
      console.log('');
      ...
      console.log(
        `  Detected:      ${getBackendColor(detection.backend)}${detection.backend}${colors.reset}`
      );
      console.log(
        `  Native:        ${detection.native ? colors.green + '✅ Yes' : colors.yellow + '⚠️  WASM'}${colors.reset}`
      );
      ...
      console.log(
        `  GNN:           ${detection.features.gnn ? colors.green + '✅ Available' : colors.yellow + '⚠️  Not available'}${colors.reset}`
      );
      console.log(
        `  Graph:         ${detection.features.graph ? colors.green + '✅ Available' : colors.yellow + '⚠️  Not available'}${colors.reset}`
      );
      console.log(
        `  Compression:   ${detection.features.compression ? colors.green + '✅ Available' : colors.yellow + '⚠️  Not available'}${colors.reset}`
      );
      console.log('');
      ...
      if (verbose) {
        ...
        if (memoryStats.page_count && pageSize.page_size) {
          const memoryInMB = (
            (memoryStats.page_count * pageSize.page_size) /
            (1024 * 1024)
          ).toFixed(2);
          console.log(`${colors.bright}Memory:${colors.reset}`);
          console.log(
            `  Pages:         ${colors.blue}${memoryStats.page_count.toLocaleString()}${colors.reset}`
          );
