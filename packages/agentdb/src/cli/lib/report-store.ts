...
export class ReportStore {
   ...
   */
  async detectRegressions(scenarioId: string, threshold: number = 0.1): Promise<Regression[]> {
    if (!this.db) throw new Error('Database not initialized');
    ...
    for (const { metric_name } of metricNames) {
      ...
      const baseline = trend.points.slice(0, 3).reduce((sum, p) => sum + p.value, 0) / 3;
 ...
 */
export async function createReportStore(dbPath: string = ':memory:'): Promise<ReportStore> {
