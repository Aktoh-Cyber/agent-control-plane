  ...
  private checkThresholds(metrics: HealthMetrics): Alert[] {
    ...
    if (metrics.memory.heapUsed > this.thresholds.memoryMB) {
      const level: Alert['level'] =
        metrics.memory.heapUsed > this.thresholds.memoryMB * 1.5
          ? 'critical'
          : metrics.memory.heapUsed > this.thresholds.memoryMB * 1.2
            ? 'warning'
            : 'info';

      const action: Alert['action'] =
        level === 'critical' ? 'heal' : level === 'warning' ? 'throttle' : 'log';
