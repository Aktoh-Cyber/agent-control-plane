...
export function validateConfig(config: any): AttentionConfig {
  ...
  for (const mechanismName of validMechanisms) {
    if (!config.mechanisms[mechanismName]) {
      config.mechanisms[mechanismName] =
        DEFAULT_ATTENTION_CONFIG.mechanisms[
          mechanismName as keyof typeof DEFAULT_ATTENTION_CONFIG.mechanisms
        ];
      continue;
    }
    ...
    if (
      !Number.isInteger(mechanism.dimension) ||
      mechanism.dimension < 64 ||
      mechanism.dimension > 2048
    ) {
      throw new Error(
        `Invalid dimension for ${mechanismName}: must be an integer between 64 and 2048`
      );
    }
    ...
    switch (mechanismName) {
      case 'flash':
        if (
          !Number.isInteger(mechanism.blockSize) ||
          mechanism.blockSize < 16 ||
          mechanism.blockSize > 256
        ) {
      ...
      case 'sparse':
        if (
          typeof mechanism.sparsity !== 'number' ||
          mechanism.sparsity < 0 ||
          mechanism.sparsity > 1
        ) {
      ...
      case 'linear':
        if (
          !Number.isInteger(mechanism.kernelSize) ||
          mechanism.kernelSize < 8 ||
          mechanism.kernelSize > 128
        ) {
      ...
      case 'performer':
        if (
          !Number.isInteger(mechanism.randomFeatures) ||
          mechanism.randomFeatures < 64 ||
          mechanism.randomFeatures > 1024
        ) {
          throw new Error(
            'Invalid randomFeatures for performer: must be an integer between 64 and 1024'
          );
        }
        break;
    }
