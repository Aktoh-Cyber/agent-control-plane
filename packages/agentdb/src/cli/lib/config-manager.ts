...
export class ConfigManager {
  ...
  loadWithEnv(baseConfig: AgentDBConfig): AgentDBConfig {
    ...
    if (process.env.AGENTDB_MEMORY_THRESHOLD) {
      config.monitoring.alertThresholds.memoryMB = parseInt(
        process.env.AGENTDB_MEMORY_THRESHOLD,
        10
      );
    }
    if (process.env.AGENTDB_LATENCY_THRESHOLD) {
      config.monitoring.alertThresholds.latencyMs = parseInt(
        process.env.AGENTDB_LATENCY_THRESHOLD,
        10
      );
    }
