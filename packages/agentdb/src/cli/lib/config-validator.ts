...
export class ConfigValidator {
  ...
  static validate(config: SimulationConfig): ValidationResult {
    ...
    if (config.backend) {
      ...
      if (!validBackends.includes(config.backend)) {
        errors.push(
          `Invalid backend: ${config.backend}. Must be one of: ${validBackends.join(', ')}`
        );
      }
    ...
    if (config.attentionHeads !== undefined) {
      ...
      if (!validHeads.includes(config.attentionHeads)) {
        errors.push(
          `Invalid attention heads: ${config.attentionHeads}. Must be one of: ${validHeads.join(', ')}`
        );
      }
    ...
    if (config.searchStrategy) {
      ...
      if (!validStrategies.includes(config.searchStrategy)) {
        errors.push(
          `Invalid search strategy: ${config.searchStrategy}. Must be one of: ${validStrategies.join(', ')}`
        );
      }
    ...
    if (config.clustering) {
      ...
      if (!validClustering.includes(config.clustering)) {
        errors.push(
          `Invalid clustering algorithm: ${config.clustering}. Must be one of: ${validClustering.join(', ')}`
        );
      }
    ...
    if (config.selfHealing && typeof config.selfHealing === 'string') {
      ...
      if (!validSelfHealing.includes(config.selfHealing)) {
        errors.push(
          `Invalid self-healing mode: ${config.selfHealing}. Must be one of: ${validSelfHealing.join(', ')}`
        );
      }
    ...
    if (config.neuralFeatures && Array.isArray(config.neuralFeatures)) {
      ...
      if (invalid.length > 0) {
        errors.push(
          `Invalid neural features: ${invalid.join(', ')}. Valid: ${validFeatures.join(', ')}`
        );
      }
    }
    ...
    if (config.dimensions !== undefined) {
      ...
      if (!validDims.includes(config.dimensions)) {
        warnings.push(
          `Dimension ${config.dimensions} is non-standard. Common: ${validDims.join(', ')}`
        );
      }
    }
  ...
  static isOptimal(config: SimulationConfig): boolean {
    const optimalChecks = [
      ...
      config.selfHealing === 'mpc' ||
        config.selfHealing === true ||
        config.selfHealing === undefined,
    ];
  ...
  static validateCompatibility(config: SimulationConfig): ValidationResult {
    ...
    if (config.nodes && config.nodes > 1000000 && !config.selfHealing) {
      warnings.push(
        'Self-healing recommended for large graphs (>1M nodes) to maintain performance.'
      );
    }
