...
export class HistoryTracker {
   ...
   */
  private calculateRSquared(points: Array<{ timestamp: Date; value: number }>): number {
  ...
  async detectRegressions(
    ...
    for (const metric of metricNames) {
      ...
      const baseline = values.slice(0, windowSize).reduce((a, b) => a + b, 0) / windowSize;
  ...
  async generateTrendReport(scenarioId: string): Promise<string> {
    ...
    if (regressions.length > 0) {
      ...
      for (const regression of regressions) {
        const icon =
          regression.severity === 'critical' ? '🔴' : regression.severity === 'major' ? '🟠' : '🟡';
