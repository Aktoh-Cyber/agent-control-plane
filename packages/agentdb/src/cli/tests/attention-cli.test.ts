...
describe('Attention Configuration', () => {
  ...
  describe('updateMechanismConfig', () => {
    it('should update mechanism configuration', async () => {
      ...
      const updated = await updateMechanismConfig('flash', { heads: 16 }, TEST_CONFIG_PATH);
    ...
    it('should throw on unknown mechanism', async () => {
      ...
      await expect(updateMechanismConfig('unknown', {}, TEST_CONFIG_PATH)).rejects.toThrow(
        'Unknown mechanism'
      );
    });
    ...
    it('should validate after update', async () => {
      ...
      await expect(updateMechanismConfig('flash', { heads: 0 }, TEST_CONFIG_PATH)).rejects.toThrow(
        'Invalid heads'
      );
    });
  });
  ...
  describe('setDefaultMechanism', () => {
    ...
    it('should throw on invalid mechanism', async () => {
      ...
      await expect(setDefaultMechanism('invalid', TEST_CONFIG_PATH)).rejects.toThrow(
        'Invalid mechanism'
      );
    });
  });
...
describe('Error Handling', () => {
  it('should handle invalid mechanism', async () => {
    ...
    await expect(updateMechanismConfig('invalid', {}, TEST_CONFIG_PATH)).rejects.toThrow();
  });

  it('should handle invalid configuration values', async () => {
    await saveAttentionConfig(DEFAULT_ATTENTION_CONFIG, TEST_CONFIG_PATH);

    await expect(updateMechanismConfig('flash', { heads: -1 }, TEST_CONFIG_PATH)).rejects.toThrow();
  });

  it('should handle file system errors gracefully', async () => {
    const invalidPath = '/invalid/path/config.json';

    await expect(saveAttentionConfig(DEFAULT_ATTENTION_CONFIG, invalidPath)).rejects.toThrow();
  });
});
