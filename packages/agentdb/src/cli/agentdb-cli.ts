...
import { Episode, ReflexionMemory } from '../controllers/ReflexionMemory.js';
...
class AgentDBCLI {
  ...
  async causalExperimentCalculate(experimentId: number): Promise<void> {
    ...
    const experiment = this.db!.prepare('SELECT * FROM causal_experiments WHERE id = ?').get(
      experimentId
    ) as any;
    if (!experiment) {
    ...
    if (result?.confidenceInterval && result.confidenceInterval.length === 2) {
      log.info(
        `95% CI: [${result.confidenceInterval[0]?.toFixed(3) || 'N/A'}, ${result.confidenceInterval[1]?.toFixed(3) || 'N/A'}]`
      );
    }
    ...
    const counts = this.db!.prepare(
      'SELECT COUNT(*) as total, SUM(is_treatment) as treatment FROM causal_observations WHERE experiment_id = ?'
    ).get(experimentId) as any;
    if (!counts) {
      throw new Error(`Failed to get observation counts for experiment ${experimentId}`);
    }
    log.info(
      `Sample Sizes: ${counts.treatment || 0} treatment, ${(counts.total || 0) - (counts.treatment || 0)} control`
    );
  ...
  async causalQuery(params: {
    ...
    edges.slice(0, params.limit || 10).forEach((edge, i) => {
      console.log(
        `${colors.bright}#${i + 1}: ${edge.fromMemoryType} → ${edge.toMemoryType}${colors.reset}`
      );
  ...
  async reflexionRetrieve(params: {
    ...
    episodes.forEach((ep, i) => {
      ...
      console.log(
        `  Similarity: ${colors.cyan}${ep.similarity?.toFixed(3) || 'N/A'}${colors.reset}`
      );
      if (ep.critique) {
    ...
    if (params.synthesizeContext && episodes.length > 0) {
      const context = ContextSynthesizer.synthesize(
        episodes.map((ep) => ({
          task: ep.task,
          reward: ep.reward,
          success: ep.success,
  ...
  async reflexionGetCritiqueSummary(params: { task: string; k?: number }): Promise<void> {
    if (!this.reflexion) throw new Error('Not initialized');
  ...
  async skillSearch(params: { task: string; k?: number; minSuccessRate?: number }): Promise<void> {
    if (!this.skills) throw new Error('Not initialized');
  ...
  async skillConsolidate(params: {
    ...
    log.success(
      `Created ${result.created} new skills, updated ${result.updated} existing skills in ${duration}ms`
    );
  ...
  async quicStatus(): Promise<void> {
    ...
    if (syncMeta.servers.length === 0) {
      ...
      syncMeta.servers.forEach((server: any) => {
        ...
        console.log(
          `    Status: ${server.connected ? colors.green + 'Connected' : colors.red + 'Disconnected'}${colors.reset}`
        );
...
async function main() {
  ...
  if (command === 'migrate') {
    ...
    if (!options.sourceDb) {
      ...
      console.log(
        'Usage: agentdb migrate <source-db> [--target <target-db>] [--no-optimize] [--dry-run] [--verbose]'
      );
      process.exit(1);
    }
...
async function handleMcpCommand(args: string[]) {
  ...
  if (subcommand === 'start' || !subcommand) {
    ...
    // Spawn the MCP server as a child process (like agent-control-plane does)
...
async function handleSyncCommands(cli: AgentDBCLI, subcommand: string, args: string[]) {
  if (subcommand === 'start-server') {
    ...
    if (!server) {
      ...
      log.info(
        'Usage: agentdb sync push --server <host:port> [--incremental] [--filter <pattern>]'
      );
      process.exit(1);
    }
    ...
    if (!server) {
      log.error('Missing required --server parameter');
      log.info(
        'Usage: agentdb sync pull --server <host:port> [--incremental] [--filter <pattern>]'
      );
      process.exit(1);
    }
...
async function handleVectorSearchCommand(args: string[]) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === '-k' && i + 1 < args.length) {
      ...
      try {
        ...
        log.error(
          'Invalid vector format. Use JSON array: "[0.1,0.2,0.3]" or space-separated: "0.1 0.2 0.3"'
        );
        process.exit(1);
      }
    }
  ...
  if (vector.length === 0) {
    ...
    log.info(
      'Usage: agentdb vector-search <db-path> <vector> [-k 10] [-t 0.75] [-m cosine] [-f json] [-v] [--mmr [lambda]]'
    );
    process.exit(1);
  }
...
async function handleImportCommand(args: string[]) {
  ...
  try {
    if (decompress) {
      ...
      log.info(
        `Decompressed ${(compressed.length / 1024).toFixed(2)} KB to ${(decompressed.length / 1024).toFixed(2)} KB`
      );
    } else {
...
async function handleStatsCommand(args: string[]) {
  ...
  const embeddingCount =
    cli.db.prepare('SELECT COUNT(*) as count FROM episode_embeddings').get()?.count || 0;
...
const isMainModule =
  import.meta.url === `file://${process.argv[1]}` || import.meta.url.endsWith('/agentdb-cli.js');
