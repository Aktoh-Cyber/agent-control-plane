...
import type {
  CausalEdge as GraphCausalEdge,
  GraphDatabaseAdapter,
} from '../backends/graph/GraphDatabaseAdapter.js';
...
export class CausalMemoryGraph {
  ...
  async addCausalEdge(edge: CausalEdge): Promise<number> {
    ...
    if (this.graphBackend && 'createCausalEdge' in this.graphBackend) {
      ...
      const mechanismText =
        edge.mechanism || `${edge.fromMemoryType}-${edge.toMemoryType} causal link`;
  ...
  queryCausalEffects(query: CausalQuery): CausalEdge[] {
    ...
    const params: any[] = [interventionMemoryId, interventionMemoryType, minConfidence, minUplift];
  ...
  detectConfounders(edgeId: number): {
    ...
    const potentialConfounders = this.db
      .prepare(
        `
    ...
    const confounders = potentialConfounders
      .map((conf: any) => {
