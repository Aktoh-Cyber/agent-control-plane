...
export class SyncCoordinator {
  ...
  async sync(onProgress?: (progress: SyncProgress) => void): Promise<SyncReport> {
    ...
    try {
      ...
      console.log(
        chalk.gray(
          `  Changes detected: ${changes.episodes.length + changes.skills.length + changes.edges.length} items`
        )
      );
      ...
      if (changes.episodes.length > 0 || changes.skills.length > 0 || changes.edges.length > 0) {
        onProgress?.({
          phase: 'pushing',
          current: 0,
          total: changes.episodes.length + changes.skills.length + changes.edges.length,
        });
      ...
      if (pullResult.conflicts && pullResult.conflicts.length > 0) {
        onProgress?.({
          phase: 'resolving',
          current: 0,
          total: pullResult.conflicts.length,
  ...
  private async detectChanges(): Promise<{
    ...
    const edges = this.db.prepare('SELECT * FROM skill_edges WHERE ts > ?').all(lastEdgeSync);
   ...
   */
  private async pullChanges(onProgress?: (progress: SyncProgress) => void): Promise<{
    itemsPulled: number;
    bytesTransferred: number;
    data: any;
    conflicts?: any[];
  ...
  private loadSyncState(): SyncState {
    try {
      const row = this.db.prepare('SELECT * FROM sync_state WHERE id = 1').get();
