...
export class WASMVectorSearch {
  ...
  private async initializeWASM(): Promise<void> {
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.debug(
          '[WASMVectorSearch] ReasoningBank WASM not available, using optimized JavaScript fallback'
        );
  ...
  buildIndex(vectors: Float32Array[], ids: number[], metadata?: any[]): void {
    if (vectors.length < this.config.indexThreshold) {
      console.log(
        `[WASMVectorSearch] Dataset too small (${vectors.length} < ${this.config.indexThreshold}), skipping index`
      );
      return;
    }
