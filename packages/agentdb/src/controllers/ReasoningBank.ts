...
import type { SearchResult, VectorBackend } from '../backends/VectorBackend.js';
...
export class ReasoningBank {
   ...
   */
  private async searchPatternsV2(
   ...
   */
  private async searchPatternsLegacy(
    query: PatternSearchQuery & { taskEmbedding: Float32Array }
  ): Promise<ReasoningPattern[]> {
    const k = query.k || 10;
    const threshold = query.threshold || 0.0;
  ...
  getPatternStats(): PatternStats {
    ...
    const totalRow = this.db
      .prepare('SELECT COUNT(*) as count FROM reasoning_patterns')
      .get() as any;
