...
export class ContextSynthesizer {
  ...
  private static generateKeyInsights(
    ...
    if (successRate >= 0.8) {
      insights.push(
        `High success rate (${(successRate * 100).toFixed(0)}%) indicates strong pattern match`
      );
    } else if (successRate < 0.5) {
      insights.push(
        `Low success rate (${(successRate * 100).toFixed(0)}%) suggests exploring alternative approaches`
      );
    }
    ...
    if (averageReward >= 0.8) {
      insights.push(
        `High average reward (${averageReward.toFixed(2)}) shows effective past solutions`
      );
    ...
    const topMemories = memories.filter((m) => m.success && (m.reward || 0) >= 0.9).slice(0, 3);
  ...
  private static generateSummary(
    ...
    // Opening
    parts.push(
      `Based on ${memories.length} similar past ${memories.length === 1 ? 'experience' : 'experiences'}`
    );
