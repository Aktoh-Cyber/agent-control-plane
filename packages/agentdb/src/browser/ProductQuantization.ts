...
export class ProductQuantization {
  ...
  constructor(config: PQConfig) {
    ...
    if (this.config.dimension % this.config.numSubvectors !== 0) {
      throw new Error(
        `Dimension ${this.config.dimension} must be divisible by numSubvectors ${this.config.numSubvectors}`
      );
    }
  }
  ...
  async train(vectors: Float32Array[]): Promise<void> {
    ...
    console.log(
      `[PQ] Training ${this.config.numSubvectors} subvectors with ${this.config.numCentroids} centroids each...`
    );
  ...
  getStats(): {
    ...
    const codebookSize = this.codebook
      ? this.config.numSubvectors *
        ...
        (this.config.dimension / this.config.numSubvectors) *
        4
      : 0;
