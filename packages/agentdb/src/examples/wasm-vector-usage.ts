...
async function batchProcessing() {
  ...
  console.log(
    `Average similarity: ${(similarities.reduce((a, b) => a + b) / similarities.length).toFixed(4)}\n`
  );
...
async function enhancedEmbeddings() {
  ...
  console.log(
    `Similarity between "machine learning" and "deep learning": ${similarity.toFixed(4)}\n`
  );
...
async function documentSearch() {
  ...
  const documents = [
    ...
    },
    {
      ...
      content: 'Traditional Italian cooking techniques and pasta recipes...',
    },
  ];
...
export { basicSimilarity, batchProcessing, documentSearch, enhancedEmbeddings, indexSearch };
