  ...
  try {
    ...
    for (let i = 0; i < parseInt(options.iterations); i++) {
      ...
      try {
        ...
        if (verbosity >= 3) {
          console.log(
            `   ✅ Iteration ${i + 1} completed in ${(iterationEnd - iterationStart).toFixed(2)}ms`
          );
    ...
    if (verbosity >= 1) {
      ...
      console.log(
        `Success: ${result.success} (${((result.success / result.iterations) * 100).toFixed(1)}%)`
      );
      console.log(
        `Failures: ${result.failures} (${((result.failures / result.iterations) * 100).toFixed(1)}%)`
      );
      console.log(`\nMetrics:`);
