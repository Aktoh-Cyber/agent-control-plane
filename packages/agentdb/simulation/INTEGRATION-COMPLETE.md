...
Add to `/workspaces/agent-control-plane/.env`:
...
From GenDev MCP tools:

```typescript
// Run simulation via MCP
await mcp__gendev__agentdb_simulate({
...
// List scenarios
const scenarios = await mcp__gendev__agentdb_list_scenarios();

// Get results
const results = await mcp__gendev__agentdb_get_results({
