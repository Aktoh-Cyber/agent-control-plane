...
Located in: `/workspaces/agent-control-plane/packages/agentdb/simulation/tests/latent-space/`
...
Located in: `/workspaces/agent-control-plane/packages/agentdb/src/cli/tests/`
...
```bash
npx gendev@alpha hooks pre-task --description "Swarm 4: Testing - Create comprehensive test suite"
```

### Post-Edit Hook

```bash
npx gendev@alpha hooks post-edit --file "simulation/tests" \
...
```bash
npx gendev@alpha hooks post-task --task-id "swarm-4-testing"
```
