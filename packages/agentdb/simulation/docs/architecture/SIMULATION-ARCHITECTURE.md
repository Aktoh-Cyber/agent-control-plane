...
describe('MySimulation', () => {
  ...
  test('should run simulation', async () => {
    ...
    expect(report.metrics.recall.k10).toBeGreaterThan(0.9);
  });
});
```
...
export class MySearchStrategy implements SearchStrategy {
  ...
  async search(query: Float32Array, graph: HNSWGraph, k: number): Promise<Neighbor[]> {
...
\n\n`;
  ...
  private coherenceLabel(score: number): string {
    ...
    if (score >= 0.9) return 'Acceptable';
...
const SCENARIOS = {
  get hnsw() {
    return require('./hnsw-exploration').HNSWExploration;
  },
  get attention() {
    return require('./attention-analysis').AttentionAnalysis;
  },
};
```
