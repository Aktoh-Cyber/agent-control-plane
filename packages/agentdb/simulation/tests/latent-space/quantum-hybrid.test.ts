...
describe('QuantumHybrid (Theoretical)', () => {
  ...
  describe('2025 Viability (Current)', () => {
    it('should assess near-term viability ~12.4%', () => {
      ...
      expect(nearTerm).toBeLessThan(0.2);
    ...
    it('should have limited qubit count', () => {
      const hardware2025 = quantumHybridScenario.config.hardwareProfiles.find(
        (h) => h.year === 2025
      );
  ...
  describe('2030 Viability (Mid-term)', () => {
    it('should project ~38.2% viability', () => {
      ...
      if (results.length > 0) {
        const avg =
          results.reduce((sum, r) => sum + (r.viability?.current2025Viability || 0), 0) /
          results.length;
  ...
  describe('2040 Viability (Long-term)', () => {
    it('should project ~84.7% viability', () => {
      ...
      expect(longTerm).toBeGreaterThan(0.7);
    });
    ...
    it('should have advanced hardware', () => {
      const hardware2040 = quantumHybridScenario.config.hardwareProfiles.find(
        (h) => h.year === 2040
      );
  ...
  describe('Grover Algorithm', () => {
    ...
    it('should calculate √M speedup', () => {
      const groverResults = (report.detailedResults as any[]).filter(
        (r) => r.algorithm === 'grover'
      );
  ...
  describe('Quantum Walk', () => {
    ...
    it('should calculate speedup', () => {
      const qwalkResults = (report.detailedResults as any[]).filter(
        (r) => r.algorithm === 'quantum-walk'
      );
  ...
  describe('Amplitude Encoding', () => {
    ...
    it('should require log(d) qubits', () => {
      const ampResults = (report.detailedResults as any[]).filter(
        (r) => r.algorithm === 'amplitude-encoding'
      );
  ...
  describe('Hybrid Approach', () => {
    ...
    it('should split workload', () => {
      const hybridResults = (report.detailedResults as any[]).filter(
        (r) => r.algorithm === 'hybrid'
      );
      ...
      hybridResults.forEach((r) => {
        if (r.quantumMetrics) {
          ...
          expect(r.quantumMetrics.classicalFraction + r.quantumMetrics.quantumFraction).toBeCloseTo(
            1.0,
            0.1
          );
        }
  ...
  describe('Classical Baseline', () => {
    ...
    it('should have 1x speedup for classical', () => {
      const classicalResults = (report.detailedResults as any[]).filter(
        (r) => r.algorithm === 'classical'
      );
