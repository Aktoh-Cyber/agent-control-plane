...
describe('AttentionAnalysis', () => {
  ...
  describe('Performance Metrics', () => {
    ...
    it('should have reasonable backward pass time', () => {
      ...
      const avgBackward =
        results.reduce((sum, r) => sum + r.metrics.performance.backwardPassMs, 0) / results.length;
  ...
  describe('Query Enhancement', () => {
    it('should improve recall by >10%', () => {
      const bestResult = report.summary.bestConfiguration;
      expect(bestResult.metrics.queryEnhancement.recallImprovement).toBeGreaterThan(0.1);
    });
  ...
  describe('Learning Convergence', () => {
    ...
    it('should achieve >90% transferability', () => {
      const bestResult = report.summary.bestConfiguration;
      expect(bestResult.metrics.learning.transferability).toBeGreaterThan(0.9);
    });
