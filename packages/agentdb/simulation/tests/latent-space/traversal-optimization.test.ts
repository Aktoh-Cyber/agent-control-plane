...
describe('TraversalOptimization', () => {
  ...
  describe('Dynamic-K Adaptation', () => {
    it('should adapt k in range 5-20', () => {
      const dynamicK = (report.detailedResults as any[]).find((r) => r.strategy === 'dynamic-k');
  ...
  describe('Scalability', () => {
    ...
    it('should maintain performance at scale', () => {
      const large = (report.detailedResults as any[]).filter((r) => r.graphSize === 1000000);

      large.forEach((r) => {
        expect(r.metrics.recall).toBeGreaterThan(0.9);
      });
    });
  });
