...
export default {
  ...
  async run(config: any) {
    ...
    for (let level = 1; level <= depth; level++) {
      ...
      if (verbosity >= 3) {
        console.log(
          `      🔹 Level ${level}: Meta-observation + Adaptation (reward: ${improvedReward.toFixed(2)})`
        );
      }
    ...
    if (verbosity >= 3) {
      console.log(
        `      🔍 Reward Progression: 0.70 → ${previousReward.toFixed(2)} (+${((previousReward - 0.7) * 100).toFixed(1)}%)`
      );
    }
