...
export default {
  description:
    'Stock market with multi-strategy traders, herding, flash crashes, and adaptive learning',
  ...
  async run(config: any) {
    ...
    for (let tick = 0; tick < ticks; tick++) {
      ...
      if (priceHistory.length >= 10) {
        ...
        const variance =
          recent.reduce((sum, price) => sum + Math.pow(price - mean, 2), 0) / recent.length;
        const volatility = Math.sqrt(variance);
        results.avgVolatility += volatility;

        // Flash crash detection (>10% drop in 10 ticks)
        if ((recent[0] - recent[recent.length - 1]) / recent[0] > 0.1) {
          results.flashCrashes++;
          circuitBreakerActive = true;
          if (verbosity >= 3) {
            console.log(
              `      ⚠️  Flash crash detected at tick ${tick}! Circuit breaker activated.`
            );
          }
        }
      }
      ...
      if (
        buyOrders / (buyOrders + sellOrders + 1) > herdingThreshold ||
      ...
      if (verbosity >= 3 && tick % 20 === 0) {
        console.log(
          `      📊 Tick ${tick}: Price $${currentPrice.toFixed(2)}, Trades: ${tickTrades.length}, Circuit Breaker: ${circuitBreakerActive}`
        );
      }
    }
    ...
    for (const strategy of strategyDistribution) {
      ...
      const avgPL =
        strategyTraders.reduce((sum, t) => sum + t.profitLoss, 0) / strategyTraders.length;
