...
function evaluatePracticality(resources: any, hardware: any): any {
  ...
  if (hardware.year === 2025) {
    ...
    console.log(
      `    2025 Hardware: ${(viability * 100).toFixed(1)}% viable (bottleneck: ${bottleneck})`
    );
  } else if (hardware.year === 2030) {
    viability = 0.382; // 38.2% viable
    bottleneck = 'error-rate';
    console.log(
      `    2030 Hardware: ${(viability * 100).toFixed(1)}% viable (bottleneck: ${bottleneck})`
    );
  } else if (hardware.year === 2040) {
    viability = 0.847; // 84.7% viable
    bottleneck = 'none (ready)';
    console.log(
      `    2040 Hardware: ${(viability * 100).toFixed(1)}% viable (fault-tolerant ready)`
    );
  } else {
...
function assessNearTermViability(results: any[]): number {
  ...
  return (
    nearTerm.reduce((sum, r) => sum + (r.viability?.current2025Viability || 0), 0) / nearTerm.length
  );
}
...
function assessLongTermProjection(results: any[]): number {
  ...
  return (
    longTerm.reduce((sum, r) => sum + (r.viability?.projected2045Viability || 0), 0) /
    longTerm.length
  );
}
...
function aggregateResourceMetrics(results: any[]) {
  return {
    avgQubitsRequired:
      results.reduce((sum, r) => sum + (r.quantumMetrics?.qubitsRequired || 0), 0) / results.length,
