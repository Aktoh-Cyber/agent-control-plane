...
function generateSemanticClusters(
  ...
  const centers: number[][] = Array(numClusters)
    .fill(0)
...
function louvainCommunityDetection(graph: any, resolution: number): any {
  ...
  while (improved && iteration < maxIterations) {
    ...
    if (
      previousModularity > 0 &&
      Math.abs(currentModularity - previousModularity) < convergenceThreshold
    ) {
      console.log(
        `    Louvain converged at iteration ${iteration}, Q=${currentModularity.toFixed(3)}`
      );
      break;
    }
  ...
  console.log(
    `    Louvain: ${numCommunities} communities, Q=${finalModularity.toFixed(3)}, ${iteration} iterations`
  );
...
function aggregateCommunityMetrics(results: any[]) {
  return {
    avgNumCommunities:
      results.reduce((sum, r) => sum + r.metrics.numCommunities, 0) / results.length,
    avgModularity: averageModularity(results),
  };
}
...
function aggregateSemanticMetrics(results: any[]) {
  return {
    ...
    avgOverlap:
      results.reduce((sum, r) => sum + r.metrics.embeddingClusterOverlap, 0) / results.length,
  };
}
...
function aggregateAgentMetrics(results: any[]) {
  return {
    avgSpecialization:
      results.reduce((sum, r) => sum + r.metrics.taskSpecialization, 0) / results.length,
  };
}
