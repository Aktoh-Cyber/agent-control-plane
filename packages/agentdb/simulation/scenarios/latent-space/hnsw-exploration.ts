...
function aggregateGraphMetrics(results: HNSWComparisonMetrics[]) {
  return {
    averageSmallWorldIndex:
      results.reduce((sum, r) => sum + r.graphMetrics.smallWorldIndex, 0) / results.length,
    averageClusteringCoeff:
      results.reduce((sum, r) => sum + r.graphMetrics.clusteringCoefficient, 0) / results.length,
  };
}
...
function compareBackends(results: HNSWComparisonMetrics[]) {
  ...
  return backends.map((backend) => ({
    backend,
    avgQPS:
      results.filter((r) => r.backend === backend).reduce((sum, r) => sum + r.qps, 0) /
      results.filter((r) => r.backend === backend).length,
    avgSpeedup:
      results
        .filter((r) => r.backend === backend)
        .reduce((sum, r) => sum + r.speedupVsBaseline, 0) /
      results.filter((r) => r.backend === backend).length,
  }));
}
