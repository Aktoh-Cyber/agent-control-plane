...
export const selfOrganizingHNSWScenario: SimulationScenario = {
  ...
  config: {
    ...
    deletionRates: [0.01, 0.05, 0.1], // % nodes deleted per day
...
async function applyMPCAdaptation(hnsw: any, horizon: number): Promise<void> {
  ...
  const candidates = [currentM - 2, currentM, currentM + 2, currentM + 4].filter(
    (m) => m >= 8 && m <= 64
  );
  let bestM = currentM;
  ...
  if (bestM !== currentM) {
    console.log(
      `    MPC: Adapting M from ${currentM} to ${bestM} (forecast degradation prevented)`
    );
...
function predictDegradation(hnsw: any, horizon: number): number[] {
  ...
  const latencyTrend =
    recentHistory[recentHistory.length - 1].latencyP95 - recentHistory[0].latencyP95;
...
function averageDegradationPrevented(results: any[]): number {
  return (
    results.reduce((sum, r) => sum + Math.max(0, r.improvement.latencyImprovement), 0) /
    results.length
  );
}
...
function aggregateParameterMetrics(results: any[]) {
  return {
    ...
    avgStability:
      results.reduce((sum, r) => sum + r.parameters.parameterStability, 0) / results.length,
  };
}

function aggregateHealingMetrics(results: any[]) {
  return {
    avgFragmentationRate:
      results.reduce((sum, r) => sum + r.healing.fragmentationRate, 0) / results.length,
    avgHealingTime: averageHealingTime(results),
  };
}
