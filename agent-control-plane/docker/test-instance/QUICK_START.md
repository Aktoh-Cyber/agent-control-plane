...
1. **Directly via CLI** (no IDE needed)
   ...
   npx agent-control-plane --agent coder --task "Create API"
   ```
...
3. **In Docker** (completely isolated)
   ...
   ```bash
   docker run agent-control-plane --help
   ```
...
```bash
docker exec agent-control-plane-test node /app/dist/cli-proxy.js agent info coder
```
...
```bash
docker exec agent-control-plane-test node /app/dist/cli-proxy.js config list
```
...
**You can use agent-control-plane with:**
