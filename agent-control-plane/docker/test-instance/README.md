...
Complete Docker setup for testing agent-control-plane in an isolated environment.
...
# Enter the container
docker-compose exec agent-control-plane /bin/bash
```
...
docker volume inspect agent-control-plane_agentdb-data
docker volume inspect agent-control-plane_memory-data

# Backup volumes
docker run --rm -v agent-control-plane_agentdb-data:/data -v $(pwd):/backup \
...
docker-compose exec agent-control-plane env | grep API_KEY
...
docker-compose exec agent-control-plane ls -la /app/wasm/reasoningbank/
...
# Remove images
docker rmi agent-control-plane-test
```
