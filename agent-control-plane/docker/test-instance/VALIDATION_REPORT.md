...
**Date:** 2025-11-01
**Version:** agent-control-plane v1.8.13
...
Image: test-instance-agent-control-plane
...
Status: PASSED
Tools Available: 15 (7 agent-control-plane + 3 agent-booster + 5 agentdb)
```
...
```yaml
Services:
  - agent-control-plane: Main service
Volumes:
...
```bash
docker run test-instance-agent-control-plane cli --list
docker run test-instance-agent-control-plane cli agent info coder
docker run test-instance-agent-control-plane cli config list
```

### 2. Interactive Shell ✅

```bash
docker exec -it agent-control-plane-test /bin/bash
...
```bash
# RESTful proxy mode
docker run -p 3000:3000 test-instance-agent-control-plane proxy

# QUIC transport (ultra-low latency)
docker run -p 4433:4433 test-instance-agent-control-plane quic
```
...
```bash
docker logs agent-control-plane-test
```
...
```bash
docker exec agent-control-plane-test env | grep API_KEY
```
...
**Docker Instance:** test-instance-agent-control-plane
