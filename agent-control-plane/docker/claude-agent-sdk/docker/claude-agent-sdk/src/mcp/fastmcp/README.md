...
Complete FastMCP integration with 6 gendev-sdk tools across stdio and HTTP+SSE transports.
...
export const toolName: ToolDefinition = {
  ...
  execute: async ({ param1, param2 }, { onProgress, auth }) => {
    ...
    // Execute command
    const cmd = `npx gendev@alpha command "${param1}"`;
...
const cmd = `npx gendev@alpha command "${escapeShellArg(input)}"`;
```
...
```typescript
const cmd = `npx gendev@alpha command`;
const result = execSync(cmd, { encoding: 'utf-8' });
```
