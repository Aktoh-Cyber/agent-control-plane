...
```
Client → stdio → FastMCP → Validation → Auth → Tool → execSync → GenDev CLI → Result → Client
   │                                                                                          ▲
   │                                                                                          │
...
```
Client → HTTP POST /mcp → FastMCP → Validation → Auth → Tool → execSync → GenDev CLI
   │                                                                                     │
   │                                                                                     ▼
   │                                                                                  Result
   │                                                                                     │
...
execute: async (params, { onProgress, auth }) => {
  ...
  const result = execSync(`npx gendev@alpha ${cmd}`);
