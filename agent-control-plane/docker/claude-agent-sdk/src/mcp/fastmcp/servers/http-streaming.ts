...
console.error(
  '🔧 Tools: 6 (memory_store, memory_retrieve, memory_search, swarm_init, agent_spawn, task_orchestrate)'
);
...
mcp.addTool({
  ...
  execute: async ({ key, namespace }) => {
    ...
    const result = execSync(cmd, { encoding: 'utf-8', maxBuffer: 10 * 1024 * 1024 });
    return JSON.stringify(
      { success: true, key, namespace, value: result.trim(), timestamp: new Date().toISOString() },
      null,
      2
    );
  },
