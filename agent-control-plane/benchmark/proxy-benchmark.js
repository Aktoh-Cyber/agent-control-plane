...
class BenchmarkRunner {
  ...
  async benchmarkHTTP1(port = 3000, payloadSize = 1000) {
    ...
    for (let i = 0; i < BENCHMARK_CONFIG.benchmarkRequests; i++) {
      const result = await this.measureRequest(() => this.makeHTTP1Request(port, payload));
      if (result.success) {
  ...
  async benchmarkHTTP2(port = 3001, payloadSize = 1000) {
    ...
    // Benchmark
    for (let i = 0; i < BENCHMARK_CONFIG.benchmarkRequests; i++) {
      const result = await this.measureRequest(() => this.makeHTTP2Request(port, payload));
      if (result.success) {
        latencies.push(result.latency);
      }
  ...
  async benchmarkWebSocket(port = 8080, payloadSize = 1000) {
    ...
    // Benchmark
    for (let i = 0; i < BENCHMARK_CONFIG.benchmarkRequests; i++) {
      const result = await this.measureRequest(() => this.makeWebSocketRequest(ws, payload));
      if (result.success) {
        latencies.push(result.latency);
      }
  ...
  async benchmarkConcurrency(protocol, port, connections) {
    ...
    console.log(
      `✅ Concurrency ${connections}: ${stats.mean.toFixed(2)}ms avg, ${stats.throughput} req/s`
    );
...
async function main() {
  ...
  try {
    ...
    for (const size of BENCHMARK_CONFIG.payloadSizes) {
      ...
      const improvement = (((http1Stats.mean - http2Stats.mean) / http1Stats.mean) * 100).toFixed(
        1
      );
    ...
    fs.writeFileSync('/tmp/benchmark-results.json', JSON.stringify(benchmark.results, null, 2));
