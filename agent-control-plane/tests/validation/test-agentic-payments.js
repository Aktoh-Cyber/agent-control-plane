...
async function testAgenticPaymentsIntegration() {
  try {
    ...
    const discoveryResult = query({
      prompt:
        'List all available MCP tools. Group them by server (gendev-sdk, gendev, flow-nexus, agentic-payments). Count tools per server.',
      options: {
...
`;
    ...
    console.log(
      '  ✅ All four servers accessible (gendev-sdk, gendev, flow-nexus, agentic-payments)'
    );
