...
async function testModel(model: (typeof TOP_20_MODELS)[0], rank: number): Promise<TestResult> {
  ...
  try {
    ...
    const nativeToolCalling =
      model.provider === 'anthropic' || (model.provider === 'openai' && model.id.includes('gpt-4'));
...
`;
  ...
  results.forEach((r) => {
    ...
    const notes = r.error
      ? `Error: ${r.error}`
      : r.toolUsesDetected === 0 && r.responseText
        ? `Text only: ${r.responseText.substring(0, 40)}...`
        : 'OK';
...
async function runTests() {
  ...
  console.log(
    `   Need Custom Instructions: ${results.filter((r) => r.success && r.toolUsesDetected === 0).length}`
  );
}
