...
const TEST_DIR = '/tmp/agent-control-plane-validation';
...
const testCases: TestCase[] = [
  ...
  // Test 3: DeepSeek - Code generation
  {
    ...
    checkFn: async (output: string, testDir: string) => {
      ...
      const notTruncated =
        !output.endsWith('<function') && !output.endsWith('<') && output.length > 50;
...
async function runAgentWithProvider(
  ...
  return new Promise((resolve, reject) => {
    ...
    const proc = spawn('node', args, {
      cwd: '/workspaces/agent-control-plane/agent-control-plane',
      env: process.env,
    });
...
async function cleanupTestEnvironment(): Promise<void> {
  // Remove test files
  try {
    const files = [`${TEST_DIR}/hello.py`, `${TEST_DIR}/multiply.py`];
...
async function runTest(testCase: TestCase): Promise<{
  ...
  try {
    const output = await runAgentWithProvider(testCase.provider, testCase.model, testCase.task);
