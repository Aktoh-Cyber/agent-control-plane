...
validate('Retry Utility', () => existsSync('src/utils/retry.ts'), 'Retry utility exists');
...
validate('Logger Utility', () => existsSync('src/utils/logger.ts'), 'Logger utility exists');
...
validate('Health Check', () => existsSync('src/health.ts'), 'Health check module exists');
...
validate(
  ...
  () => {
    ...
    return (
      typeof health.getHealthStatus === 'function' && typeof health.startHealthServer === 'function'
    );
  },
...
validate(
  ...
  () => {
    ...
    return indexContent.includes('logger') && indexContent.includes('startHealthServer');
  },
