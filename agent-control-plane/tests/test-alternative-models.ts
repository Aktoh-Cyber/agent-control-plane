  ...
  const testModels = [
    {
      ...
      task: 'Write a one-line Python function to calculate factorial',
    },
    {
      name: 'DeepSeek Coder',
    ...
    },
    {
      ...
      task: 'Explain what a REST API is in one sentence',
    },
  ];
  ...
  for (const test of testModels) {
    ...
    try {
      ...
      results.push({
        ...
        cost:
          (data.usage?.prompt_tokens || 0) * 0.00001 +
...
async function testONNXModel(): Promise<TestResult> {
  ...
  try {
    ...
    return {
      ...
      success: true,
      response:
        'ONNX Runtime initialized successfully. Model inference requires downloading model files.',
      latency: Date.now() - startTime,
    };
  } catch (error: any) {
...
async function main() {
  ...
  // Final statistics
  const successCount =
    openrouterResults.filter((r) => r.success).length + (onnxResult.success ? 1 : 0);
  const totalTests = openrouterResults.length + 1;
  const avgLatency =
    [...openrouterResults, onnxResult].reduce((sum, r) => sum + (r.latency || 0), 0) / totalTests;
