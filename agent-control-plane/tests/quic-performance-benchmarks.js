...
class QuicBenchmarkSuite {
  ...
  generateReport() {
    ...
    console.log('\n🎯 Summary:');
    console.log(
      `  ✅ Latency Test:       ${this.results.latency.improvement || 'N/A'}% faster than HTTP/2`
    );
    console.log(`  ✅ Throughput:         ${this.results.throughput.throughput || 'N/A'} MB/s`);
    console.log(
      `  ✅ Concurrent Streams: ${this.results.concurrency.successful || 'N/A'}/${this.results.concurrency.targetStreams || 'N/A'} successful`
    );
    console.log(
      `  ✅ 0-RTT Reconnect:    ${this.results.reconnection.improvement || 'N/A'}% faster`
    );
