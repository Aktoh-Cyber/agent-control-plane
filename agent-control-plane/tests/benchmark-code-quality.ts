...
const CODING_TASKS = [
  {
    name: 'binary_search',
    prompt:
      'Write a Python function that implements binary search with type hints, docstrings, and error handling for edge cases',
  },
  {
    name: 'rest_endpoint',
    prompt:
      'Create a FastAPI endpoint that accepts JSON, validates input, handles errors, and returns proper HTTP status codes',
  },
  {
    name: 'async_processing',
    prompt:
      'Write a Python async function that fetches data from multiple URLs concurrently using asyncio, with timeout and error handling',
  },
  {
    name: 'data_processing',
    prompt:
      'Create a Python function that reads a CSV file, processes data with pandas, handles missing values, and exports to JSON with error handling',
  },
];
...
async function testModel(modelId: string, modelName: string, task: any): Promise<BenchmarkResult> {
  ...
  try {
    ...
    const cost =
      (data.usage?.prompt_tokens || 0) * 0.00001 + (data.usage?.completion_tokens || 0) * 0.00003;
...
async function runBenchmark() {
  ...
  for (const task of CODING_TASKS) {
    ...
    for (const model of TEST_MODELS) {
      ...
      // Print result
      const quality =
        '★'.repeat(Math.floor(result.codeQuality)) +
        '☆'.repeat(10 - Math.floor(result.codeQuality));
      console.log(
        `   ${result.compiles ? '✅' : '❌'} ${model.name}: ${result.latency}ms | Quality: ${quality} (${result.codeQuality}/10)`
      );
    }
  }
  ...
  for (const [model, modelResults] of Object.entries(byModel)) {
    const avgQuality =
      modelResults.reduce((sum, r) => sum + r.codeQuality, 0) / modelResults.length;
    ...
    const hasDocsRate =
      (modelResults.filter((r) => r.hasDocumentation).length / modelResults.length) * 100;
    const errorHandlingRate =
      (modelResults.filter((r) => r.errorHandling).length / modelResults.length) * 100;
  ...
  const rankings = Object.entries(byModel)
  ...
  rankings.forEach((r, i) => {
    ...
    console.log(
      `     Quality: ${r.avgQuality.toFixed(1)}/10 | Latency: ${r.avgLatency.toFixed(0)}ms | Cost: $${r.totalCost.toFixed(6)}\n`
    );
  });
