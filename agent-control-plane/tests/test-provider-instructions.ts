...
async function runTests() {
  ...
  successful.forEach((r) => {
    console.log(
      `  - ${r.provider.padEnd(12)} | ${r.modelName.padEnd(25)} | ${r.toolUsesDetected} tools | ${r.responseTime}ms`
    );
  });
  ...
  console.log(
    `   With Tool Usage: ${successful.length}/${results.length} (${((successful.length / results.length) * 100).toFixed(1)}%)`
  );
  console.log(
    `   Avg Response Time: ${Math.round(results.filter((r) => r.success).reduce((sum, r) => sum + r.responseTime, 0) / results.filter((r) => r.success).length)}ms`
  );
