...
async function testMCPConnection() {
  ...
  console.log('Test 1: In-SDK MCP Server (gendev-sdk)');
  try {
    const result = query({
  ...
  console.log('Test 2: External stdio MCP Server (gendev)');
  try {
    const result = query({
      prompt: 'Use the gendev MCP server to store test-key=test-value in memory, then retrieve it.',
      options: {
