...
async function testNeuralTool() {
  ...
  try {
    const result = query({
      prompt:
        'Use the neural_train tool to train a convergent cognitive pattern for 3 iterations. Report the training results.',
      options: {
    ...
    for await (const msg of result) {
      if (msg.type === 'assistant') {
        ...
        console.log(
          `📊 Initialized with ${msg.tools.length} tools, permissionMode: ${msg.permissionMode}\n`
        );
