...
class SupabaseFederationTester {
  ...
  async createTables() {
    ...
    try {
      ...
      const sqlPath = join(
        process.cwd(),
        'docs/supabase/migrations/001_create_federation_tables.sql'
      );
  ...
  async testMemoryStorage(sessionId: string) {
    ...
    try {
      ...
      const { data, error } = await this.client.from('agent_memories').insert(memories).select();
  ...
  async testTaskManagement() {
    ...
    try {
      ...
      const { data, error } = await this.client.from('agent_tasks').insert([task]).select();
  ...
  async testEventLogging() {
    ...
    try {
      ...
      const { data, error } = await this.client.from('agent_events').insert(events).select();
  ...
  async cleanup() {
    ...
    try {
      ...
      for (const table of tables) {
        const { error } = await this.client.from(table).delete().like('tenant_id', `test-%`);
