...
class CollaborativeAgent {
  ...
  async collaborate(iterations: number): Promise<void> {
    for (let i = 0; i < iterations; i++) {
      ...
      console.log(
        `[${this.config.agentType}] Iteration ${this.iteration}: ${episode.task} (reward: ${episode.reward.toFixed(2)}, learned from ${this.sharedMemories.length} agents)`
      );
  ...
  private async querySharedMemories(): Promise<void> {
    try {
      ...
      if (similar.length > 0) {
        console.log(
          `   [${this.config.agentType}] Found ${similar.length} similar patterns from other agents`
        );
      }
    } catch (error: any) {
...
async function runAgentDBCollaborationTest() {
  ...
  console.log(
    `   ${tenant1Patterns.length !== tenant2Patterns.length ? '✓' : '✗'} Tenants properly isolated`
  );
