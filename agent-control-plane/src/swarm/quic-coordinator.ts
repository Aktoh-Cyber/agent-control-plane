...
import { QuicClient, QuicConnection, QuicConnectionPool, QuicStats } from '../transport/quic.js';
    ...
    switch (this.config.topology) {
      ...
      case 'hierarchical':
        ...
        if (agent.role === 'worker') {
          const coordinators = Array.from(this.state.agents.values()).filter(
      ...
      case 'star':
        ...
        if (agent.role === 'coordinator') {
          ...
          const coordinator = Array.from(this.state.agents.values()).find(
            (a) => a.role === 'coordinator'
          );
          if (coordinator) {
  ...
  private resolveRecipients(message: SwarmMessage): string[] {
    ...
    for (const target of to) {
      if (target === '*') {
        ...
        recipients.push(
          ...Array.from(this.state.agents.keys()).filter((id) => id !== message.from)
        );
      } else {
