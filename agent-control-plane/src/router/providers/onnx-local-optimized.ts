...
import type { ChatParams, ChatResponse, Message } from '../types.js';
import { ONNXLocalConfig, ONNXLocalProvider } from './onnx-local.js';
...
export class OptimizedONNXProvider extends ONNXLocalProvider {
  ...
  constructor(config: OptimizedONNXConfig = {}) {
    ...
    this.optimizedConfig = {
      modelPath:
        config.modelPath ||
        './models/phi-4-mini/cpu_and_mobile/cpu-int4-rtn-block-32-acc-level-4/model.onnx',
  ...
  private optimizeContext(messages: Message[]): Message[] {
    ...
    for (let i = messages.length - 1; i >= 0; i--) {
      ...
      if (totalTokens + tokens > maxTokens) {
        console.log(
          `📊 Context pruned: Saved ${messages.length - optimized.length} messages, ~${totalTokens} tokens kept`
        );
        break;
      }
