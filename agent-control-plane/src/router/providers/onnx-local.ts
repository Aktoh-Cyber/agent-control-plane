...
export class ONNXLocalProvider implements LLMProvider {
  ...
  private initializeKVCache(batchSize: number, sequenceLength: number) {
    ...
    for (let i = 0; i < numLayers; i++) {
      ...
      kvCache[`past_key_values.${i}.key`] = new (ort.Tensor as any)('float32', emptyCache, [
        batchSize,
        numKVHeads,
        0,
        headDim,
      ...
      kvCache[`past_key_values.${i}.value`] = new (ort.Tensor as any)('float32', emptyCache, [
        batchSize,
        numKVHeads,
        0,
        headDim,
    ...
    try {
      ...
      for (let step = 0; step < maxNewTokens; step++) {
        ...
        for (let i = 0; i < vocabSize; i++) {
          const logit =
            logits[lastTokenLogitsOffset + i] / (params.temperature || this.config.temperature);
          if (logit > maxVal) {
            maxVal = logit;
            nextToken = i;
          }
