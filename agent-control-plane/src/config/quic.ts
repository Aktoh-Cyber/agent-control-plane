...
export function validateQuicConfig(config: QuicConfigSchema): void {
  ...
  if (config.maxConcurrentStreams! < 1) {
    errors.push(
      `Invalid maxConcurrentStreams: ${config.maxConcurrentStreams}. Must be at least 1.`
    );
  }
  ...
  if (config.initialCongestionWindow! < 1) {
    errors.push(
      `Invalid initialCongestionWindow: ${config.initialCongestionWindow}. Must be at least 1.`
    );
  }

  if (config.maxDatagramSize! < 1200 || config.maxDatagramSize! > 65527) {
    errors.push(
      `Invalid maxDatagramSize: ${config.maxDatagramSize}. Must be between 1200 and 65527.`
    );
  }
