...
npx agent-control-plane --agent researcher --task "Analyze security patterns"
npx agent-control-plane --agent researcher --task "Analyze performance patterns"
npx agent-control-plane --agent researcher --task "Analyze scalability patterns"
npx agent-control-plane --agent coder --task "Implement core features"
npx agent-control-plane --agent tester --task "Create comprehensive tests"
```
...
const results = await Promise.all([
  execAsync('npx agent-control-plane --agent researcher --task "domain1"'),
  execAsync('npx agent-control-plane --agent researcher --task "domain2"'),
  execAsync('npx agent-control-plane --agent researcher --task "domain3"'),
  execAsync('npx agent-control-plane --agent coder --task "implement"'),
  execAsync('npx agent-control-plane --agent tester --task "test"'),
]);
```
...
│     └─ Use: npx agent-control-plane --agent TYPE --task TASK
```
...
async function parallelCodeReview(files: string[]) {
  ...
  const reviewPromises = batches.map((batch, i) =>
    execAsync(
      `npx agent-control-plane --agent code-reviewer ` +
  ...
  const allReviews = await Promise.all(
    batches.map((_, i) => reasoningBank.retrieve(`swarm/${taskId}/batch-${i}`))
  );
...
async function multiDomainResearch(domains: string[]) {
  ...
  const allResearch = await Promise.all(
    domains.map((domain) => reasoningBank.retrieve(`swarm/${taskId}/research/${domain}`))
  );

  // Phase 3: Parallel analysis of research findings
  const analysisPromises = domains.map((domain) =>
    execAsync(
      `npx agent-control-plane --agent analyst ` +
...
async function hierarchicalSwarmDeployment(task: string) {
  ...
  const workerPromises = subtasks.tasks.map((subtask, i) =>
    execAsync(
      `npx agent-control-plane --agent ${subtask.agentType} ` +
...
const results = await Promise.all(
  Array.from({ length: 10 }, (_, i) =>
    execAsync(
      `npx agent-control-plane --agent worker ` +
...
if (taskQueue.length > HIGH_THRESHOLD) {
  ...
  await Promise.all(
    Array.from({ length: newAgents }, (_, i) =>
      execAsync(`npx agent-control-plane --agent worker --task "scale-up-${i}"`)
    )
  );
...
const results = await Promise.allSettled([
  execAsync('npx agent-control-plane --agent researcher --task "task1"'),
  execAsync('npx agent-control-plane --agent coder --task "task2"'),
  execAsync('npx agent-control-plane --agent tester --task "task3"'),
]);
...
const result1 = await exec('npx agent-control-plane --agent agent1 --task "task1"');
const result2 = await exec('npx agent-control-plane --agent agent2 --task "task2"');
const result3 = await exec('npx agent-control-plane --agent agent3 --task "task3"');
```
...
// CORRECT: Parallel execution
const results = await Promise.all([
  exec('npx agent-control-plane --agent agent1 --task "task1"'),
  exec('npx agent-control-plane --agent agent2 --task "task2"'),
  exec('npx agent-control-plane --agent agent3 --task "task3"'),
]);
```
...
await Promise.all(
  subprocesses.map((_, i) =>
    exec(
      `npx agent-control-plane --agent worker --task "task-${i}" --output reasoningbank:swarm/task-id/worker-${i}`
    )
  )
);
```
...
const config = await autoSelectSwarmConfig(reasoningBank, 'Multi-domain research task', {
  taskComplexity: 'high',
...
async function intelligentSwarmExecution(taskDescription: string) {
  ...
  const results = await Promise.all(
    Array.from({ length: config.recommendedBatchSize }).map((_, i) =>
      execAsync(
        `npx agent-control-plane --agent researcher --task "subtask-${i}" --output reasoningbank:swarm/task/agent-${i}`
      )
    )
  );
