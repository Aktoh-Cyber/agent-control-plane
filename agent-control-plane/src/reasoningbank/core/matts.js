...
export async function mattsParallel(taskFn, query, options = {}) {
  ...
  const successRate =
    trajectories.filter((t) => t.verdict.label === 'Success').length / trajectories.length;
  const duration = Date.now() - startTime;
  console.log(
    `[INFO] MaTTS parallel complete: ${trajectories.length} trajectories, ${successRate * 100}% success in ${duration}ms`
  );
...
export async function mattsSequential(taskFn, query, options = {}) {
  ...
  const duration = Date.now() - startTime;
  console.log(
    `[INFO] MaTTS sequential complete: ${trajectories.length} iterations, ${successRate * 100}% success in ${duration}ms`
  );
