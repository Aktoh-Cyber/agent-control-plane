...
export async function consolidate(): Promise<ConsolidationResult> {
  ...
  contradictionsFound = await detectContradictions(
    memories,
    config.consolidate.contradiction_threshold
  );
  ...
  console.log(
    `[INFO] Consolidation complete: ${duplicatesFound} dupes, ${contradictionsFound} contradictions, ${itemsPruned} pruned in ${durationMs}ms`
  );
 ...
 */
async function deduplicateMemories(memories: any[], threshold: number): Promise<number> {
  let duplicatesFound = 0;
  ...
  for (const mem of memories) {
    const row = dbConn.prepare('SELECT vector FROM pattern_embeddings WHERE id = ?').get(mem.id) as
      | { vector: Buffer }
      | undefined;
    if (row) {
    ...
    for (let j = i + 1; j < memories.length; j++) {
      ...
      if (similarity >= threshold) {
        ...
        if (mem1.usage_count < mem2.usage_count) {
          ...
          console.log(
            `[INFO] Merged duplicate: ${mem1.pattern_data.title} → ${mem2.pattern_data.title}`
          );
        }
      }
    }
 ...
 */
async function detectContradictions(memories: any[], threshold: number): Promise<number> {
  let contradictionsFound = 0;
    ...
    for (let j = i + 1; j < memories.length; j++) {
      ...
      if (similarity >= threshold) {
        ...
        if (outcome1 !== outcome2) {
          ...
          contradictionsFound++;
          console.log(
            `[WARN] Contradiction detected: ${mem1.pattern_data.title} vs ${mem2.pattern_data.title}`
          );
        }
      }
    }
