...
/**
 * Judge a task trajectory using LLM-based evaluation
 *
 * Implements Algorithm 2 from the ReasoningBank paper (arXiv:2509.25140).
 * Uses an LLM as a judge to evaluate whether a task trajectory represents
 * success or failure, with confidence scoring and detailed reasoning.
 *
 * @description Analyzes task execution trajectories to determine success/failure outcomes.
 * Supports multiple LLM providers (OpenRouter, Anthropic, Google Gemini) via ModelRouter.
 * Falls back to heuristic judgment when no API keys are configured.
 *
 * @param {Trajectory} trajectory - The task execution trajectory to evaluate
 * @param {string} query - The original task query/objective
 * @param {Object} options - Optional judgment configuration
 * @param {boolean} [options.useCache=false] - Whether to use cached judgments
 *
 * @returns {Promise<Verdict>} Verdict object with label, confidence, and reasoning
 *
 * @example
 * ```typescript
 * const trajectory = {
 *   taskId: "task-123",
 *   steps: [
 *     { action: "query_database", result: "success", data: {...} },
 *     { action: "process_data", result: "success", data: {...} },
 *     { action: "save_results", result: "complete" }
 *   ]
 * };
 *
 ...
 * );
 *
 * console.log(`Outcome: ${verdict.label}`); // "Success"
 * console.log(`Confidence: ${verdict.confidence}`); // 0.95
 * console.log(`Reasons:`, verdict.reasons); // ["All steps completed", "No errors detected"]
 * ```
 *
 * @example
 * ```typescript
 ...
 * });
 *
 * if (verdict.label === 'Success' && verdict.confidence > 0.8) {
 *   // High-confidence success - create learning memory
 *   await distillMemories(trajectory, verdict, query);
 * }
 * ```
 *
 * @throws {Error} If trajectory format is invalid or LLM API error occurs (falls back to heuristic)
 *
 * @since 1.8.0
 *
 * @remarks
 * The judgment process:
 * 1. Loads the judge prompt template
 * 2. Formats the trajectory for LLM consumption
 * 3. Calls configured LLM via ModelRouter
 * 4. Parses structured verdict (JSON format expected)
 * 5. Falls back to heuristic rules if LLM unavailable
 *
 * Heuristic fallback criteria (when no API key):
 * - Success: No errors + completion markers + steps > 0
 * - Failure: Contains errors OR no completion markers
 * - Confidence: Always 0.5 for heuristic judgments
 *
...
export async function judgeTrajectory(
  ...
  if (!hasApiKey) {
    console.warn(
      '[WARN] No API key set (OPENROUTER_API_KEY, ANTHROPIC_API_KEY, or GOOGLE_GEMINI_API_KEY), using heuristic judgment'
    );
  ...
  try {
    ...
    console.log(
      `[INFO] Judgment complete: ${verdict.label} (${verdict.confidence}) in ${duration}ms`
    );
