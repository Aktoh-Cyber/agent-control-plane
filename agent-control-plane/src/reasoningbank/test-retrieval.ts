...
import { fetchMemoryCandidates, upsertEmbedding, upsertMemory } from './db/queries.js';
...
const testMemories = [
  {
    ...
    content:
      '1) Check for CSRF token in meta tags or form inputs. 2) Include in request. 3) Verify before submit.',
...
for (const test of queries) {
  ...
  candidates.forEach((c, idx) => {
    console.log(
      `  ${idx + 1}. ${c.pattern_data.title} (conf: ${c.confidence}, age: ${c.age_days}d)`
    );
  });
