...
// TODO: Re-enable when ReasoningBank export is available
// import { ReasoningBank } from '../reasoningbank';
type ReasoningBank = any; // Temporary fix until export is available
...
export class SwarmLearningOptimizer {
  ...
  private generateCritique(metrics: SwarmMetrics, success: boolean): string {
    ...
    if (metrics.speedup && metrics.speedup < 1.2) {
      critiques.push(
        `Minimal speedup (${metrics.speedup}x) - consider different topology or larger batches`
      );
    }
  ...
  async getOptimization(
    ...
    const topologyScores = new Map<
      string,
      { totalReward: number; count: number; avgSpeedup: number }
    >();
    ...
    return {
      ...
      recommendedAgentCount: Math.min(
        estimatedAgentCount,
        this.getMaxAgentsForTopology(bestTopology)
      ),
      expectedSpeedup: bestSpeedup,
      confidence: Math.round(bestScore * 100) / 100,
      reasoning:
        `Based on ${similarPatterns.length} similar successful executions. ` +
  ...
  private getDefaultRecommendation(
    ...
    if (estimatedAgentCount <= 5) {
      ...
      expectedSpeedup = 2.5;
      reasoning =
        'Mesh topology optimal for small swarms (≤5 agents) - full peer-to-peer coordination';
    } else if (estimatedAgentCount <= 10) {
      topology = 'hierarchical';
      expectedSpeedup = 3.5;
      reasoning =
        'Hierarchical topology optimal for medium swarms (6-10 agents) - efficient delegation';
    } else {
      topology = 'hierarchical';
      expectedSpeedup = 4.0;
      reasoning =
        'Hierarchical topology required for large swarms (>10 agents) - avoids O(n²) coordination overhead';
    }
  ...
  async getOptimizationStats(): Promise<{
    ...
    for (const [topology, speedups] of Object.entries(stats.speedupByTopology)) {
      if (speedups.length > 0) {
        stats.avgSpeedupByTopology[topology] =
