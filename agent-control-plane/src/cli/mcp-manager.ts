 ...
 * - npx agent-control-plane mcp add weather --npm weather-mcp
 * - npx agent-control-plane mcp add local --local /path/to/server.js
...
const CONFIG_DIR = path.join(os.homedir(), '.agent-control-plane');
...
function addServerFromJson(name: string, configJson: string): void {
  ...
  try {
    ...
    console.log('\n💡 Use it with: npx agent-control-plane --agent coder --task "your task"');
  } catch (error: any) {
...
function addServer(options: {
  ...
  console.log('\n💡 Run tests with: npx agent-control-plane mcp test ' + options.name);
}
...
function listServers(options: { enabled?: boolean; verbose?: boolean }): void {
  ...
  if (servers.length === 0) {
    ...
    console.log('\n💡 Add a server with: npx agent-control-plane mcp add NAME --npm PACKAGE');
    return;
  }
...
program
  ...
  .description('Manage MCP servers for agent-control-plane')
  .version('1.0.0')
  .addHelpText(
    'after',
    `
