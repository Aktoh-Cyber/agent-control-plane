...
export class AgentManager {
  ...
  list(format: 'summary' | 'detailed' | 'json' = 'summary'): void {
    ...
    for (const category of sortedCategories) {
      ...
      for (const agent of categoryAgents) {
        ...
        if (format === 'detailed') {
          ...
          console.log(
            `  ${sourceIcon} ${agent.name.padEnd(30)} ${agent.description.substring(0, 45)}...`
          );
        }
      }
    }
    ...
    console.log(
      `   📝 Local: ${Array.from(agents.values()).filter((a) => a.source === 'local').length}`
    );
    console.log(
      `   📦 Package: ${Array.from(agents.values()).filter((a) => a.source === 'package').length}`
    );
    console.log('');
  }
  ...
  async create(options: {
    ...
    console.log(`\n📝 Usage:`);
    console.log(`   npx agent-control-plane --agent ${kebabName} --task "Your task"`);
    console.log('');
  }
   ...
   */
  private generateAgentMarkdown(metadata: AgentMetadata, systemPrompt: string): string {
    const toolsLine =
      metadata.tools && metadata.tools.length > 0 ? `tools: ${metadata.tools.join(', ')}` : '';
...
\`\`\`bash
npx agent-control-plane --agent ${metadata.name} --task "Your task"
\`\`\`
...
### Example 1
\`\`\`bash
npx agent-control-plane --agent ${metadata.name} --task "Example task description"
\`\`\`
...
`;
  ...
  info(name: string): void {
    ...
    if (!agent) {
      ...
      console.log('\nUse "agent-control-plane agent list" to see all available agents\n');
      return;
    }
...
USAGE:
  npx agent-control-plane agent <command> [options]
...
EXAMPLES:
  ...
  # List with details
  npx agent-control-plane agent list detailed

  # Create agent interactively
  npx agent-control-plane agent create

  # Create agent with CLI
  npx agent-control-plane agent create --name my-agent --description "My custom agent" --prompt "You are a helpful assistant"

  # Get agent info
  npx agent-control-plane agent info coder

  # Check conflicts
  npx agent-control-plane agent conflicts
...
`);
    ...
    case 'info':
      if (!args[1]) {
        ...
        console.log('Usage: npx agent-control-plane agent info <name>\n');
        process.exit(1);
      }
      manager.info(args[1]);
      break;
    ...
    default:
      ...
      console.log('Use "npx agent-control-plane agent help" for usage information\n');
      process.exit(1);
  }
}
