 ...
 *
 * Usage:
 *   npx agent-control-plane claude-code --provider openrouter "Write a function"
 *   npx agent-control-plane claude-code --provider gemini "Create a REST API"
 *   npx agent-control-plane claude-code --provider anthropic "Help me debug"
 *
 * Features:
...
function spawnClaudeCode(config: ProxyConfig, claudeArgs: string[]): ChildProcess {
  ...
  if (config.requiresProxy) {
    ...
    env.ANTHROPIC_API_KEY =
      'sk-ant-api03-proxy-forwarded-to-' +
      config.provider +
...
async function main() {
  ...
  program
    .name('agent-control-plane claude-code')
    .description(
      'Spawn Claude Code with automatic proxy configuration for alternative AI providers'
    )
    .usage('[options] [task]')
    .addHelpText(
      'after',
...
Examples:
  ...
  $ agent-control-plane claude-code --provider openrouter
  $ agent-control-plane claude-code --provider gemini

  # Non-interactive mode - Execute task and exit
  $ agent-control-plane claude-code --provider openrouter "Write a Python hello world function"
  $ agent-control-plane claude-code --provider openrouter --model "mistralai/mistral-small-3.1-24b-instruct" "Create REST API"

  # Using different providers
  $ agent-control-plane claude-code --provider openrouter  # Uses Mistral Small (default, $0.02/M tokens)
  $ agent-control-plane claude-code --provider gemini      # Uses Gemini 2.0 Flash
  $ agent-control-plane claude-code --provider onnx        # Uses local ONNX models (free)

  # With Agent Booster for 57x faster code edits
  $ agent-control-plane claude-code --provider openrouter --agent-booster
...
Documentation:
  https://github.com/Aktoh-Cyber/agent-control-plane#claude-code-mode
  https://ruv.io
`
    )
    .option(
      '--provider <provider>',
      'AI provider (anthropic, openrouter, gemini, onnx)',
      'anthropic'
    )
    .option('--port <port>', 'Proxy server port', '3000')
    .option(
      '--model <model>',
      'Specific model to use (e.g., mistralai/mistral-small-3.1-24b-instruct)'
    )
    .option(
      '--agent-booster',
      'Enable Agent Booster MCP tools for 57x faster code edits (default: true)',
      true
    )
  ...
  if (config.requiresProxy && !config.apiKey) {
    ...
    console.error(
      `   Please set ${config.provider === 'openrouter' ? 'OPENROUTER_API_KEY' : 'GOOGLE_GEMINI_API_KEY'}`
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (options.agentBooster) {
      ...
      logger.info(
        '   Available tools: agent_booster_edit_file, agent_booster_batch_edit, agent_booster_parse_markdown'
      );
      logger.info('   Configure MCP: Add "agent-control-plane" to Claude Desktop config');
