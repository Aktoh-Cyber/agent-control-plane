...
import { FallbackStrategy, ProviderConfig, ProviderManager } from './provider-manager.js';
...
export class LongRunningAgent {
   ...
   */
  public async executeTask<T>(task: {
    name: string;
    ...
    if (this.config.costBudget) {
      ...
      if (currentCost >= this.config.costBudget) {
        throw new Error(
          `Cost budget exceeded: $${currentCost.toFixed(2)} >= $${this.config.costBudget}`
        );
      }
    }
