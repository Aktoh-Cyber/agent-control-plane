...
import { CausalEdge, CausalMemoryGraph } from './CausalMemoryGraph.js';
...
export class NightlyLearner {
  ...
  private async discoverCausalEdges(): Promise<number> {
    ...
    for (const pair of candidatePairs) {
      ...
      if (
        Math.abs(doublyRobustEstimate) >= this.config.upliftThreshold &&
  ...
  private calculatePropensity(episodeId: number): number {
    const episode = this.db
      .prepare('SELECT task, session_id FROM episodes WHERE id = ?')
  ...
  private async completeExperiments(): Promise<number> {
    const runningExperiments = this.db
      .prepare(
        `
  ...
  private async createExperiments(): Promise<number> {
    const currentExperiments = this.db
      .prepare(
        `
      SELECT COUNT(*) as count
      FROM causal_experiments
  ...
  private generateRecommendations(report: LearnerReport): string[] {
    ...
    if (report.avgConfidence < 0.7) {
      recommendations.push(
        'Average confidence is below target. Increase sample sizes or refine hypothesis selection.'
      );
    }

    if (report.experimentsCompleted > 0) {
      recommendations.push(
        `${report.experimentsCompleted} experiments completed. Review results for actionable insights.`
      );
    }

    if (report.edgesPruned > report.edgesDiscovered) {
      recommendations.push(
        'More edges pruned than discovered. Consider adjusting confidence thresholds.'
      );
    }
