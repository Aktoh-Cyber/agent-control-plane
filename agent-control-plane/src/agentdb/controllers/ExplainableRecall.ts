...
export class ExplainableRecall {
  ...
  verifyCertificate(certificateId: string): {
    ...
    const cert = this.db
      .prepare('SELECT * FROM recall_certificates WHERE id = ?')
  ...
  private getContentHash(sourceType: string, sourceId: number): string {
    ...
    switch (sourceType) {
      ...
      case 'skill':
        const skill = this.db
          .prepare('SELECT name, code FROM skills WHERE id = ?')
          .get(sourceId) as any;
      ...
      case 'fact':
        const fact = this.db
          .prepare('SELECT subject, predicate, object FROM facts WHERE id = ?')
          .get(sourceId) as any;
