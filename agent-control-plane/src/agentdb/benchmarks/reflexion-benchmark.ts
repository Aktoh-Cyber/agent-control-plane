...
import { Episode, ReflexionMemory } from '../controllers/ReflexionMemory';
...
export class ReflexionBenchmark {
  ...
  private async testPruning(): Promise<void> {
    ...
    const beforeCount = this.db
      .prepare('SELECT COUNT(*) as count FROM episodes WHERE task = ?')
      .get(task) as any;
    ...
    const remainingQuality = this.db
      .prepare('SELECT AVG(reward) as avg_reward FROM episodes WHERE task = ?')
      .get(task) as any;
