...
import { CausalEdge, CausalMemoryGraph } from '../controllers/CausalMemoryGraph';
...
class FrontierBenchmark {
  ...
  private createResult(operation: string, latencies: number[], target?: number): BenchmarkResult {
  ...
  async benchmarkProvenanceLineage(): Promise<BenchmarkResult> {
    ...
    for (let i = 0; i < 100; i++) {
      ...
      const source = this.db
        .prepare('SELECT content_hash FROM provenance_sources WHERE id = ?')
        .get(sourceId) as any;
  ...
  async benchmarkDatabaseMetrics(): Promise<void> {
    ...
    const certificates = this.db
      .prepare('SELECT COUNT(*) as count FROM recall_certificates')
      .get() as any;
    const provenance = this.db
      .prepare('SELECT COUNT(*) as count FROM provenance_sources')
      .get() as any;
  ...
  async runAll(): Promise<void> {
    ...
    try {
      ...
      results.forEach((result) => {
        ...
        console.log(
          `   p50: ${result.p50Ms.toFixed(2)}ms | p95: ${result.p95Ms.toFixed(2)}ms | p99: ${result.p99Ms.toFixed(2)}ms${target}`
        );
      ...
      if (allPassed) {
        ...
        failed.forEach((r) =>
          console.log(`   - ${r.operation} (p95: ${r.p95Ms.toFixed(2)}ms > target: ${r.target}ms)`)
        );
      }
      console.log('═══════════════════════════════════════════════════════════\n');
    } finally {
