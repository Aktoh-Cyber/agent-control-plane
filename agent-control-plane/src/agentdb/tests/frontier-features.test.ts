...
import { CausalEdge, CausalExperiment, CausalMemoryGraph } from '../controllers/CausalMemoryGraph';
...
describe('Frontier Features - CausalMemoryGraph', () => {
  ...
  test('should record observations and update sample size', () => {
    ...
    const experiment = db
      .prepare('SELECT * FROM causal_experiments WHERE id = ?')
      .get(expId) as any;
    expect(experiment.sample_size).toBe(2);

    // Check observations stored
    const observations = db
      .prepare('SELECT * FROM causal_observations WHERE experiment_id = ?')
      .all(expId);
...
describe('Frontier Features - ExplainableRecall', () => {
  ...
  test('should create recall certificate with minimal hitting set', () => {
    ...
    const requirements = ['authentication', 'security', 'tokens'];
  ...
  test('should detect tampered certificate', () => {
    const chunks = [{ id: '1', type: 'episode', content: 'Original content', relevance: 0.9 }];
