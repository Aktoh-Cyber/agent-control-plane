...
// Cached queries
const episodes = optimizer.query('SELECT * FROM episodes WHERE task = ?', ['my_task']);
...
const needed = db.prepare('SELECT id, task, reward FROM episodes').all(); // ✅ Only what you need

// 3. Unindexed filters
const results = db.prepare('SELECT * FROM episodes WHERE LOWER(output) LIKE ?').all('%keyword%'); // ❌ No index on LOWER(output)

// Better: Use indexed columns or full-text search
const results = db.prepare('SELECT * FROM episodes WHERE task = ?').all(task); // ✅ Uses index
```
