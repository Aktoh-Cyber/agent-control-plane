...
class FrontierValidator {
  ...
  testABExperiment() {
    this.runTest('A/B Experiment and Uplift Calculation', () => {
      ...
      const obs = this.db
        .prepare('SELECT COUNT(*) as count FROM causal_observations WHERE experiment_id = ?')
        .get(expId);
      assert(obs.count === 100, 'Should have 100 observations');

      // Calculate uplift (simplified inline calculation)
      const treatmentObs = this.db
        .prepare(
          `
      ...
      const treatmentMean =
        treatmentObs.reduce((sum, o) => sum + o.outcome_value, 0) / treatmentObs.length;
      const controlMean =
        controlObs.reduce((sum, o) => sum + o.outcome_value, 0) / controlObs.length;
      const uplift = treatmentMean - controlMean;

      assert(uplift > 0.1, 'Uplift should be positive and significant');
      log(
        `    Uplift: ${uplift.toFixed(3)}, Treatment: ${treatmentMean.toFixed(3)}, Control: ${controlMean.toFixed(3)}`,
        'cyan'
      );
    });
  }
  ...
  testMerkleProofGeneration() {
    this.runTest('Merkle Proof Generation and Verification', () => {
      // Create Merkle tree
      const leaves = ['chunk1', 'chunk2', 'chunk3', 'chunk4'].map((content) =>
  ...
  testJustificationPaths() {
    this.runTest('Justification Paths', () => {
      ...
      justStmt.run(
        ...
        'causal_link',
        0.7,
        JSON.stringify(['query', 'chunk1', 'chunk2'])
      );

      // Query justifications
      const justifications = this.db
        .prepare(
          `
  ...
  testCausalRecallIntegration() {
    this.runTest('Causal Recall Integration', () => {
      ...
      log(
        `    Episodes: ${episodes.count}, Embeddings: ${embeddings.count}, Edges: ${edges.count}`,
        'cyan'
      );
    });
  }
