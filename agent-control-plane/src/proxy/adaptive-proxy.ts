  ...
  async start(): Promise<ProxyServer[]> {
    ...
    if (this.config.enableHTTP3) {
      try {
        ...
        console.log(
          `✅ HTTP/3 (QUIC)   → Port ${this.config.http3Port} (fastest, 50-70% improvement)`
        );
      } catch (error) {
    ...
    if (this.config.enableHTTP1) {
      try {
        ...
        console.log(
          `✅ HTTP/1.1        → Port ${this.config.http1Port} (baseline, always available)`
        );
      } catch (error) {
    ...
    if (this.config.enableWebSocket) {
      try {
        ...
        console.log(
          `✅ WebSocket       → Port ${this.config.wsPort} (mobile/unstable connections)`
        );
      } catch (error) {
