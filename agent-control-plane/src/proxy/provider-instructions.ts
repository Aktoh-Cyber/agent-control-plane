...
`,
  emphasis:
    'IMPORTANT: Use these structured commands in your response. The system will automatically execute them.',
};
...
`,
  emphasis:
    'CRITICAL: You must use these exact XML tag formats. Do not just describe the file - actually use the tags.',
};
...
`,
  emphasis:
    'IMPORTANT: Always use the XML tags. Just writing code blocks will NOT create files. You MUST use <file_write> tags.',
};
...
`,
  emphasis:
    'Use these tags to perform actual file operations. Code blocks alone will not create files.',
};
...
`,
  emphasis:
    'CRITICAL: File operations require XML tags. Code blocks alone will not create files on disk.',
};
...
1. SUBPROCESS SPAWNING
   ...
   const results = await Promise.all([
     exec('npx agent-control-plane --agent researcher --task "analyze security"'),
     exec('npx agent-control-plane --agent coder --task "implement features"'),
     exec('npx agent-control-plane --agent tester --task "create tests"')
   ]);
   \`\`\`
...
See /agent-control-plane/src/prompts/parallel-execution-guide.md for detailed examples.
═══════════════════════════════════════════════════════════════
`;
