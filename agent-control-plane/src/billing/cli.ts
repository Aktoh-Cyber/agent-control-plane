...
import { BillingCycle, BillingSystem, CouponType, SubscriptionTier, UsageMetric } from './index.js';
  ...
  private async listCoupons(): Promise<void> {
    try {
      ...
      coupons.forEach((coupon) => {
        ...
        console.log(
          `  Redeemed: ${coupon.timesRedeemed}${coupon.maxRedemptions ? `/${coupon.maxRedemptions}` : ''}`
        );
        console.log('');
      });
    } catch (error) {
