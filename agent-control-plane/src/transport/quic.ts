...
/**
 * QUIC Client - High-performance outbound QUIC connections with stream multiplexing
 *
 * @description Manages QUIC connections to remote servers with connection pooling,
 * stream multiplexing, and HTTP/3 support. Implements QUIC protocol via WebAssembly
 * for cross-platform compatibility and native performance.
 *
 * @example
 * ```typescript
 * // Initialize and connect to QUIC server
 ...
 * await stream.send(new Uint8Array([1, 2, 3, 4]));
 ...
 * - HTTP/3 request/response support
...
export class QuicClient {
  ...
  /**
   * Initialize QUIC client with WebAssembly module
   *
   * @description Loads and initializes the QUIC WASM module. Must be called before
   * establishing any connections. Safe to call multiple times (idempotent).
   *
   * @returns {Promise<void>}
   *
   * @throws {Error} If WASM module fails to load or initialization fails
   *
   * @example
   * ```typescript
   * const client = new QuicClient();
   * await client.initialize(); // Loads WASM module
  ...
  /**
   * Establish connection to QUIC server
   *
   * @description Creates a new QUIC connection or reuses an existing one from the pool.
   * Implements connection pooling for efficiency. Returns connection object for stream creation.
   *
   * @param {string} [host] - Server hostname (defaults to config.serverHost)
   * @param {number} [port] - Server port (defaults to config.serverPort)
   *
   * @returns {Promise<QuicConnection>} Connection object with metadata
   *
   * @throws {Error} If connection fails or max connections limit reached
   *
   * @example
   * ```typescript
   ...
   * const conn2 = await client.connect('backend.example.com', 4433);
   *
  ...
  async connect(host?: string, port?: number): Promise<QuicConnection> {
    ...
    try {
      ...
      logger.error('Failed to establish QUIC connection', {
        error,
        host: targetHost,
        port: targetPort,
      });
  ...
  /**
   * Create bidirectional stream on QUIC connection
   *
   * @description Opens a new bidirectional stream for data exchange. Multiple streams
   * can be multiplexed over a single connection without head-of-line blocking.
   *
   * @param {string} connectionId - Connection identifier (from connect())
   *
   * @returns {Promise<QuicStream>} Stream object with send/receive/close methods
   *
   * @throws {Error} If connection not found or max concurrent streams reached
   *
   * @example
   * ```typescript
   * const conn = await client.connect();
   ...
   *
   * @description Sends an HTTP/3 request using QPACK header compression and QUIC streams.
   * Automatically handles stream creation, request encoding, and response parsing.
   *
   ...
   * HTTP/3 response with status, headers, and body
   *
   * @throws {Error} If request fails or connection is invalid
   *
   * @example
   * ```typescript
   * const conn = await client.connect('api.example.com', 4433);
   *
   * // GET request
   * const response = await client.sendRequest(
   *   conn.id,
  ...
  getStats(): QuicStats {
    return {
      ...
      totalStreams: Array.from(this.connections.values()).reduce(
        (sum, c) => sum + c.streamCount,
        0
      ),
      activeStreams: Array.from(this.connections.values()).reduce(
        (sum, c) => sum + c.streamCount,
        0
      ),
