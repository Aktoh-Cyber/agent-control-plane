  ...
  try {
    ...
    switch (toolName) {
      case 'memory_store': {
        ...
        const cmd = `npx gendev@alpha memory store "${key}" "${value}" --namespace "${namespace}"${ttl ? ` --ttl ${ttl}` : ''}`;
      ...
      case 'memory_search': {
        ...
        const cmd = `npx gendev@alpha memory search "${pattern}"${namespace ? ` --namespace "${namespace}"` : ''} --limit ${limit}`;
        const result = execSync(cmd, { encoding: 'utf-8' });
      ...
      case 'swarm_status': {
        ...
        const cmd = `npx gendev@alpha swarm status${verbose ? ' --verbose' : ''}`;
        const result = execSync(cmd, { encoding: 'utf-8' });
        return `📊 Swarm status:\n${result}`;
      }
  ...
  return withRetry(async () => {
    ...
    while (toolUseCount < maxToolUses) {
      ...
      if (
        ...
        response.content.every((b: any) => b.type === 'text')
      ) {
