...
- Store: mcp__gendev__memory_usage with action="store", namespace="${memoryConfig.namespace}"
- Retrieve: mcp__gendev__memory_usage with action="retrieve", namespace="${memoryConfig.namespace}"
- Search: mcp__gendev__memory_search with pattern="search query"
...
- Init swarm: mcp__gendev__swarm_init with topology="${swarmConfig.topology}"
- Spawn agent: mcp__gendev__agent_spawn with type="researcher|coder|analyst"
- Orchestrate: mcp__gendev__task_orchestrate with task="description"
- Check status: mcp__gendev__swarm_status
  ...
  return withRetry(async () => {
    ...
    for await (const msg of result) {
      if (msg.type === 'assistant') {
        const chunk =
          msg.message.content?.map((c: any) => (c.type === 'text' ? c.text : '')).join('') || '';
        output += chunk;
        if (onStream && chunk) {
          onStream(chunk);
        }
