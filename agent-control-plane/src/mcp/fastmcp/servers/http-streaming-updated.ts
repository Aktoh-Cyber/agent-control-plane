...
server.addTool({
  ...
  execute: async ({ key, namespace }) => {
    try {
      const cmd = `npx gendev@alpha memory retrieve "${key}" --namespace "${namespace}"`;
      const result = execSync(cmd, { encoding: 'utf-8', maxBuffer: 10 * 1024 * 1024 });
...
server.addTool({
  ...
  parameters: z.object({
    ...
    namespace: z
      .string()
      .optional()
      .describe('Memory namespace to search in (searches all if not specified)'),
    limit: z
      .number()
      .positive()
      .optional()
  ...
  execute: async ({ pattern, namespace, limit }) => {
    try {
      const cmd = [
        'npx gendev@alpha memory search',
        `"${pattern}"`,
...
server.addTool({
  ...
  parameters: z.object({
    topology: z
      ...
      .describe(
        'Swarm topology: mesh (peer-to-peer), hierarchical (tree), ring (circular), star (centralized)'
      ),
    maxAgents: z
      .number()
      .positive()
      .optional()
      .default(8)
      .describe('Maximum number of agents in the swarm (1-100)')
    ...
    strategy: z
      ...
      .default('balanced')
      .describe(
        'Agent distribution strategy: balanced (equal), specialized (role-based), adaptive (dynamic)'
      ),
  }),
  execute: async ({ topology, maxAgents, strategy }) => {
    try {
      const cmd = `npx gendev@alpha swarm init --topology ${topology} --max-agents ${maxAgents} --strategy ${strategy}`;
      const result = execSync(cmd, { encoding: 'utf-8', maxBuffer: 10 * 1024 * 1024 });
...
server.addTool({
  ...
  parameters: z.object({
    type: z
      ...
      .describe(
        'Agent type: researcher (data gathering), coder (implementation), analyst (analysis), optimizer (performance), coordinator (orchestration)'
      ),
    capabilities: z
      .array(z.string())
      .optional()
      .describe(
        'Specific capabilities for the agent (e.g., ["python", "testing", "documentation"])'
      ),
  ...
  execute: async ({ type, capabilities, name }) => {
    try {
      ...
      const cmd = `npx gendev@alpha agent spawn --type ${type}${capStr}${nameStr}`;
      const result = execSync(cmd, { encoding: 'utf-8', maxBuffer: 10 * 1024 * 1024 });
...
server.addTool({
  ...
  parameters: z.object({
    ...
    strategy: z
      ...
      .default('adaptive')
      .describe(
        'Execution strategy: parallel (simultaneous), sequential (ordered), adaptive (dynamic based on task)'
      ),
    priority: z
    ...
    maxAgents: z
      ...
      .positive()
      .optional()
      .describe('Maximum number of agents to use for this task (1-10)')
      .refine((val) => !val || (val >= 1 && val <= 10), {
        message: 'maxAgents must be between 1 and 10',
      }),
  }),
  execute: async ({ task, strategy, priority, maxAgents }) => {
    try {
      const maxStr = maxAgents ? ` --max-agents ${maxAgents}` : '';
      const cmd = `npx gendev@alpha task orchestrate "${task}" --strategy ${strategy} --priority ${priority}${maxStr}`;
...
server.addTool({
  ...
  execute: async ({ agent, task, stream }) => {
    try {
      ...
      const cmd =
        `npx agent-control-plane --agent "${agent}" --task "${task}" ${streamFlag}`.trim();
      const result = execSync(cmd, {
        encoding: 'utf-8',
        maxBuffer: 10 * 1024 * 1024,
        timeout: 300000,
...
server.addTool({
  ...
  execute: async ({ topic, diff, dataset, streaming }) => {
    try {
      ...
      const result = execSync('npx agent-control-plane', {
        encoding: 'utf-8',
        maxBuffer: 10 * 1024 * 1024,
        timeout: 300000,
        env,
...
server.addTool({
  ...
  execute: async ({ format }) => {
    try {
      const result = execSync('npx agent-control-plane --list', {
        encoding: 'utf-8',
...
## Usage
\`\`\`bash
npx agent-control-plane --agent ${name} --task "Your task"
\`\`\`
