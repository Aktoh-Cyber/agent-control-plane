#!/usr/bin/env node
// Standalone agent-control-plane MCP server - runs directly via stdio without spawning subprocesses
...
console.error('📦 Local agent-control-plane tools available');
...
// Tool: Run agent-control-plane agent
server.addTool({
  name: 'agentic_flow_agent',
  description:
    'Execute an agent-control-plane agent with a specific task. Supports multiple providers (Anthropic, OpenRouter, ONNX) and comprehensive configuration options.',
  parameters: z.object({
    agent: z
      .string()
      .describe(
        'Agent type (coder, researcher, analyst, etc.) - Use agentic_flow_list_agents to see all 66+ available agents'
      ),
    ...
    model: z
      ...
      .optional()
      .describe(
        'Model to use (e.g., "claude-sonnet-4-5-20250929" for Anthropic, "meta-llama/llama-3.1-8b-instruct" for OpenRouter, or "Xenova/gpt2" for ONNX local models)'
      ),
    provider: z
      .enum(['anthropic', 'openrouter', 'onnx', 'gemini'])
      .optional()
      .describe(
        'LLM provider: "anthropic" (default, highest quality, requires ANTHROPIC_API_KEY), "gemini" (free tier, requires GOOGLE_GEMINI_API_KEY), "openrouter" (99% cost savings, requires OPENROUTER_API_KEY), "onnx" (free local inference, no API key needed)'
      ),
    ...
    anthropicApiKey: z
      ...
      .optional()
      .describe(
        'Anthropic API key (sk-ant-...) - overrides ANTHROPIC_API_KEY environment variable'
      ),
    openrouterApiKey: z
      .string()
      .optional()
      .describe(
        'OpenRouter API key (sk-or-...) - overrides OPENROUTER_API_KEY environment variable'
      ),
    ...
    stream: z
      ...
      .optional()
      .default(false)
      .describe('Enable streaming output (real-time response chunks)'),
    temperature: z
      .number()
      .min(0)
      .max(1)
      .optional()
      .describe(
        'Sampling temperature (0.0-1.0): lower = more focused/deterministic, higher = more creative/random. Default varies by agent.'
      ),
    maxTokens: z
      .number()
      .positive()
      .optional()
      .describe('Maximum tokens in response (default: 4096). Controls output length.'),
    ...
    agentsDir: z
      ...
      .optional()
      .describe(
        'Custom agents directory path (default: .claude/agents) - for using custom agent definitions'
      ),
    ...
    outputFormat: z
      ...
      .optional()
      .describe(
        'Output format: "text" (default, human-readable), "json" (structured data), "markdown" (formatted docs)'
      ),
    ...
    timeout: z
      ...
      .positive()
      .optional()
      .describe('Execution timeout in milliseconds (default: 300000 = 5 minutes)'),
    retryOnError: z
      .boolean()
      .optional()
      .default(false)
      .describe('Automatically retry on transient errors (rate limits, network issues)'),
  }),
  execute: async ({
    agent,
    task,
...
server.addTool({
  ...
  description: 'List all available agent-control-plane agents',
  parameters: z.object({}),
  execute: async () => {
    try {
      // Use npx to run agent-control-plane from npm registry
      const cmd = `npx --yes agent-control-plane --list`;
      const result = execSync(cmd, {
        encoding: 'utf-8',
...
server.addTool({
  ...
  description:
    'Create a new custom agent in .claude/agents directory. Supports conflict detection and will warn if agent already exists in package or local directories.',
  parameters: z.object({
    name: z
      .string()
      .describe('Agent name (will be converted to kebab-case, e.g., my-custom-agent)'),
    description: z.string().describe('Agent description (what this agent does)'),
    systemPrompt: z
      .string()
      .describe('System prompt that defines the agent behavior and personality'),
    category: z
      .string()
      .optional()
      .default('custom')
      .describe('Category/folder to organize the agent (default: custom)'),
    tools: z
      .array(z.string())
      .optional()
      .describe(
        'Optional list of tools this agent can use (e.g., ["web-search", "code-execution"])'
      ),
  }),
  execute: async ({ name, description, systemPrompt, category, tools }) => {
    try {
      let cmd = `npx --yes agent-control-plane agent create --name "${name}" --description "${description}" --prompt "${systemPrompt}"`;
...
server.addTool({
  ...
  description:
    'List all available agents including both package agents and custom local agents. Shows source (package or local) and handles deduplication.',
  parameters: z.object({
    format: z
      ...
      .default('summary')
      .describe(
        'Output format: summary (brief list), detailed (full info), json (structured data)'
      ),
    filterSource: z
      .enum(['all', 'package', 'local'])
      .optional()
      .default('all')
      .describe(
        'Filter by source: all, package (npm distribution), or local (custom .claude/agents)'
      ),
  }),
  execute: async ({ format, filterSource }) => {
    try {
      const cmd = `npx --yes agent-control-plane agent list ${format || 'summary'}`;
      const result = execSync(cmd, {
        encoding: 'utf-8',
        maxBuffer: 10 * 1024 * 1024,
        timeout: 30000,
...
server.addTool({
  ...
  description:
    'Get detailed information about a specific agent including its source, description, category, and system prompt preview.',
  parameters: z.object({
    name: z.string().describe('Agent name to get information about'),
  }),
  execute: async ({ name }) => {
    try {
      const cmd = `npx --yes agent-control-plane agent info "${name}"`;
      const result = execSync(cmd, {
        encoding: 'utf-8',
        maxBuffer: 10 * 1024 * 1024,
        timeout: 30000,
...
server.addTool({
  ...
  description:
    'Check for conflicts between package agents and local custom agents. Identifies agents with the same relative path in both locations.',
  parameters: z.object({}),
  execute: async () => {
    try {
      const cmd = `npx --yes agent-control-plane agent conflicts`;
      const result = execSync(cmd, {
        encoding: 'utf-8',
        maxBuffer: 10 * 1024 * 1024,
        timeout: 30000,
...
server.addTool({
  ...
  description:
    'Automatically select the optimal model for an agent and task based on priorities (quality, cost, speed, privacy). Returns model recommendation with reasoning.',
  parameters: z.object({
    agent: z.string().describe('Agent type (e.g., coder, researcher, reviewer)'),
    task: z.string().describe('Task description'),
    priority: z
      .enum(['quality', 'balanced', 'cost', 'speed', 'privacy'])
      .optional()
      .default('balanced')
      .describe(
        'Optimization priority: quality (best results), balanced (cost/quality), cost (cheapest), speed (fastest), privacy (local only)'
      ),
    max_cost: z
      .number()
      .positive()
      .optional()
      .describe('Maximum cost per task in dollars (optional budget cap)'),
  }),
  execute: async ({ agent, task, priority, max_cost }) => {
    try {
      let cmd = `npx --yes agent-control-plane --agent "${agent}" --task "${task}" --optimize`;
...
server.addTool({
  ...
  parameters: z.object({
    ...
    instructions: z
      .string()
      .describe('First-person instruction (e.g., "I will add error handling")'),
    code_edit: z
      .string()
      .describe(
    ...
    language: z
      .string()
      .optional()
      .describe('Programming language (auto-detected from file extension if not provided)'),
  }),
...
server.addTool({
  ...
  parameters: z.object({
    markdown: z
      .string()
      .describe('Markdown text containing code blocks with filepath= and instruction= metadata'),
  }),
  execute: async ({ markdown }) => {
    try {
      ...
      if (edits.length === 0) {
        throw new Error(
          'No code blocks found in markdown. Expected format: ```language filepath=path instruction=description\\ncode\\n```'
        );
      }
...
server.addTool({
  name: 'agentdb_stats',
  description:
    'Enhanced database statistics including table counts, memory usage, and performance metrics for all AgentDB tables.',
  parameters: z.object({}),
  execute: async () => {
    try {
...
server.addTool({
  ...
  description:
    'Store a reasoning pattern in ReasoningBank for future retrieval and learning. Patterns capture successful problem-solving approaches.',
  parameters: z.object({
  ...
  execute: async ({
    ...
    try {
      const args = [sessionId, `"${task}"`, reward.toString(), success.toString()];
...
server.addTool({
  ...
  description:
    'Search for similar reasoning patterns in ReasoningBank by task description. Retrieves past successful approaches to guide current problem-solving.',
  parameters: z.object({
...
server.addTool({
  ...
  description:
    'Get aggregated statistics and critique summary for patterns related to a specific task. Provides insights from past attempts.',
  parameters: z.object({
    ...
    k: z
      ...
      .optional()
      .default(5)
      .describe('Number of recent patterns to analyze (default: 5)'),
  }),
...
server.addTool({
  ...
  description:
    'Clear the AgentDB query cache to free memory or force fresh queries. Useful after bulk updates or when debugging.',
  parameters: z.object({
    confirm: z
      .boolean()
      .optional()
      .default(false)
      .describe('Confirmation to clear cache (set to true to proceed)'),
  }),
        ...
        return JSON.stringify(
          {
            ...
            warning:
              'Clearing cache will remove all cached query results and may temporarily impact performance.',
          },
          null,
          2
        );
...
console.error('✅ Registered 15 tools (7 agent-control-plane + 3 agent-booster + 5 agentdb):');
