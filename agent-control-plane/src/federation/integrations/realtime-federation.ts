...
export class RealtimeFederationHub {
   ...
   */
  async broadcast(type: CoordinationMessage['type'], payload: any): Promise<void> {
   ...
   * Update agent status
   */
  async updateStatus(status: 'online' | 'busy' | 'offline', task?: string): Promise<void> {
  ...
  return new RealtimeFederationHub(
    {
      ...
      broadcastLatency: (process.env.FEDERATION_BROADCAST_LATENCY as 'low' | 'high') || 'low',
    },
    agentId,
    tenantId
  );
