...
import { createDebugStream, DebugLevel, DebugStream } from '../debug/debug-stream.js';
...
export class SupabaseFederationAdapterDebug {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      const duration = Date.now() - startTime;
      this.debug.logConnection('initialize_error', { duration, error: error as Error });
      throw error;
    }
  }
  ...
  async registerSession(
    ...
    try {
      const { error } = await this.client.from('agent_sessions').insert({
