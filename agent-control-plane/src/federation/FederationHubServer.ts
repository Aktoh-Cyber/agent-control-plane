...
export class FederationHubServer {
   ...
   */
  private broadcastToTenant(tenantId: string, senderAgentId: string, message: SyncMessage): void {
  ...
  getStats(): {
    ...
    const tenants = this.db
      .prepare('SELECT COUNT(DISTINCT tenant_id) as count FROM episodes')
      .get() as any;
  ...
  async queryPatterns(tenantId: string, task: string, k: number = 5): Promise<any[]> {
    try {
      ...
      return results.filter((r: any) => r.sessionId?.startsWith(`${tenantId}/`));
    } catch (error: any) {
