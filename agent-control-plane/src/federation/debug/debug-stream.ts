...
export class DebugStream extends EventEmitter {
  ...
  log(event: Omit<DebugEvent, 'timestamp'>): void {
    ...
    if (
      this.config.filterAgents &&
      event.agentId &&
      !this.config.filterAgents.includes(event.agentId)
    ) {
      return;
    }
  ...
  printMetrics(): void {
    ...
    console.log(
      this.colorize('Operation'.padEnd(40) + 'Count'.padEnd(10) + 'Avg Duration', 'white')
    );
    ...
    for (const [key, value] of sorted) {
      ...
      console.log(
        key.padEnd(40) + this.colorize(countStr, 'yellow') + this.colorize(durationStr, 'green')
      );
    }
