...
import { DebugLevel, DebugStream, createDebugStream } from './debug-stream.js';
...
export class AgentDebugStream extends EventEmitter {
  ...
  printSummary(): void {
    ...
    console.log(
      `Tasks Completed: ${Array.from(this.tasks.values()).filter((t) => t.status === 'completed').length}`
    );
    console.log(
      `Tasks Failed:    ${Array.from(this.tasks.values()).filter((t) => t.status === 'failed').length}`
    );
    ...
    console.log(
      `Messages Recv:   ${this.communications.filter((c) => c.type === 'receive').length}`
    );
    console.log('');
    ...
    if (this.performanceMetrics.size > 0) {
      ...
      for (const [operation, durations] of this.performanceMetrics.entries()) {
        ...
        console.log(
          `  ${operation.padEnd(30)} ${durations.length}x  avg: ${avg.toFixed(1)}ms  min: ${min.toFixed(1)}ms  max: ${max.toFixed(1)}ms`
        );
      }
      console.log('');
    }
  ...
  printTimeline(): void {
    ...
    for (const event of this.timeline) {
      ...
      console.log(
        `${time.padStart(10)} | ${event.type.padEnd(20)} | ${JSON.stringify(event.data || event)}`
      );
    }
 ...
 */
export function createAgentDebugStreamFromEnv(
  agentId: string,
  tenantId?: string
): AgentDebugStream {
  const debugLevel =
    (process.env.DEBUG_LEVEL?.toUpperCase() as keyof typeof DebugLevel) || 'VERBOSE';
