...
async function showStatus() {
  ...
  const stats = {
    totalMemories: dbInstance
      .prepare("SELECT COUNT(*) as count FROM patterns WHERE type = 'reasoning_memory'")
      .get() as { count: number },
    avgConfidence: dbInstance
      .prepare("SELECT AVG(confidence) as avg FROM patterns WHERE type = 'reasoning_memory'")
      .get() as { avg: number },
    totalEmbeddings: dbInstance
      .prepare('SELECT COUNT(*) as count FROM pattern_embeddings')
      .get() as { count: number },
    totalTrajectories: dbInstance
      .prepare('SELECT COUNT(*) as count FROM task_trajectories')
      .get() as { count: number },
  };
...
async function listMemories(args: string[]) {
  ...
  if (memories.length === 0) {
    console.log(
      'No memories found. Run `npx agent-control-plane reasoningbank demo` to create some!\n'
    );
    return;
  }
  ...
  for (let i = 0; i < memories.length; i++) {
    ...
    console.log(
      `   Confidence: ${parseFloat(mem.confidence).toFixed(2)} | Usage: ${mem.usage_count} | Created: ${mem.created_at}`
    );
...
EXAMPLES:
  ...
  npx agent-control-plane reasoningbank consolidate

  # List top 10 memories by confidence
  npx agent-control-plane reasoningbank list --sort confidence --limit 10

  # List top 5 most used memories
  npx agent-control-plane reasoningbank list --sort usage --limit 5

For more information: https://github.com/Aktoh-Cyber/agent-control-plane
  `);
}
