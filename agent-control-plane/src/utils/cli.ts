...
export function parseArgs(): CliOptions {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    switch (arg) {
      ...
      case '--priority':
        options.optimizePriority = args[++i] as
          | 'quality'
          | 'balanced'
          | 'cost'
          | 'speed'
...
REASONINGBANK COMMANDS:
  npx agent-control-plane reasoningbank demo         Run interactive demo comparison
  npx agent-control-plane reasoningbank test         Run validation tests
  npx agent-control-plane reasoningbank init         Initialize database
  npx agent-control-plane reasoningbank benchmark    Run performance benchmarks
  npx agent-control-plane reasoningbank status       Show memory statistics
...
EXAMPLES:
  ...
  npx agent-control-plane reasoningbank demo       # See 0% → 100% success transformation
  npx agent-control-plane reasoningbank test       # Run 27 validation tests
  npx agent-control-plane reasoningbank init       # Setup memory database
  ...
  npx agent-control-plane --agent coder --task "Add types to api.ts" --agent-booster --provider openrouter
  export AGENTIC_FLOW_AGENT_BOOSTER=true  # Enable for all tasks

  # Claude Code with Agent Booster
  npx agent-control-plane claude-code --provider openrouter --agent-booster
  npx agent-control-plane claude-code --provider gemini "Write a REST API"
  npx agent-control-plane claude-code --help     # See all claude-code options

  # Agent Management
  npx agent-control-plane agent list             # List all agents with sources
  npx agent-control-plane agent create           # Interactive agent creator
  npx agent-control-plane agent info coder       # Get agent details
  npx agent-control-plane agent conflicts        # Check for conflicts

  # Configuration
  npx agent-control-plane config                 # Interactive config wizard
  npx agent-control-plane config set PROVIDER openrouter
  npx agent-control-plane config list            # View all settings

  # MCP Server Management
  npx agent-control-plane mcp start              # Start all MCP servers
  npx agent-control-plane mcp list               # List all 209+ MCP tools

  # Agent Execution (Basic)
  npx agent-control-plane --list                 # List all agents
  npx agent-control-plane --agent coder --task "Build REST API"

  # Agent Execution (Advanced)
  npx agent-control-plane --agent coder --task "Build API" \\
    --provider openrouter \\
  ...
  npx agent-control-plane --agent my-custom-agent --task "Your task" \\
    --agents-dir ./my-agents

  # Parallel Mode
  npx agent-control-plane  # Run 3 agents in parallel
  ...
  npx agent-control-plane --agent researcher --task "Analyze data" --optimize --priority quality
...
MCP TOOLS (209+ available):
  • agent-control-plane: 6 tools (agent execution, creation, management)
...
PARALLEL EXECUTION & SWARM OPTIMIZATION (v2.0):
  ...
  Usage Example (CLI subprocess spawning):
    const results = await Promise.all([
      exec('npx agent-control-plane --agent researcher --task "domain1"'),
      exec('npx agent-control-plane --agent researcher --task "domain2"'),
      exec('npx agent-control-plane --agent coder --task "implement"')
    ]);
  ...
  See: /agent-control-plane/src/prompts/parallel-execution-guide.md (v2.0)
       /docs/swarm-optimization-report.md

For more information, visit: https://github.com/Aktoh-Cyber/agent-control-plane
  `);
}
