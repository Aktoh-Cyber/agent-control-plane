...
async function startMCPServers(server: string): Promise<void> {
  ...
  const serversToStart = server === 'all' ? Object.keys(MCP_SERVERS) : [server];
  ...
  console.log(`💡 Tip: Use 'npx agent-control-plane mcp status' to check server status\n`);
}

async function stopMCPServers(server: string): Promise<void> {
  console.log(`\n🛑 Stopping MCP Server(s): ${server}\n`);

  const serversToStop = server === 'all' ? Object.keys(MCP_SERVERS) : [server];
...
async function checkMCPStatus(server: string): Promise<void> {
  ...
  const serversToCheck = server === 'all' ? Object.keys(MCP_SERVERS) : [server];
...
async function listMCPTools(server: string): Promise<void> {
  ...
  const serversToList = server === 'all' ? Object.keys(MCP_SERVERS) : [server];
  ...
  console.log(`   npx agent-control-plane --agent coder --task "Your task"\n`);
}
