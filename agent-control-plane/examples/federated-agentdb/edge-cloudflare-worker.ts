  ...
  async fetch(request: Request, env: any): Promise<Response> {
    ...
    try {
      const result = await agent.execute(async (db, context) => {
        ...
        const regionalMemories = await agent.queryMemories(`user:${userId} region:${region}`, 3);
 ...
 */
async function processEdgeRequest(request: Request, memories: any[], region: string): Promise<any> {
  const url = new URL(request.url);

  // Extract successful patterns from regional memory
  const patterns = memories.filter((m) => m.reward >= 0.7).map((m) => m.output);
