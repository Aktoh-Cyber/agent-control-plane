...
async function multiAgentResearchExample() {
  ...
  await Promise.all([researcher.initialize(), analyst.initialize(), writer.initialize()]);
  ...
  analyst.on('message:share_knowledge', async (message: any) => {
    if (message.from_agent === 'researcher-001') {
      ...
      setTimeout(async () => {
        await analyst.shareKnowledge('Analysis complete: High confidence in safety frameworks', {
  ...
  // Cleanup
  await Promise.all([researcher.shutdown(), analyst.shutdown(), writer.shutdown()]);
}
 ...
 */
async function memorySyncExample() {
  console.log('\n💾 Example 2: Real-Time Memory Synchronization\n');
...
async function collaborativeProblemSolving() {
  ...
  const debuggerAgent = createRealtimeHub('debugger-001', 'dev-team');
  const tester = createRealtimeHub('tester-001', 'dev-team');

  await Promise.all([coder.initialize(), debuggerAgent.initialize(), tester.initialize()]);
  ...
  debuggerAgent.on('message:request_help', async (message: any) => {
    console.log(`🔧 Debugger analyzing: ${message.payload.problem}`);
    await debuggerAgent.updateStatus('busy', 'Debugging type error');

    setTimeout(async () => {
      await debuggerAgent.sendMessage(message.payload.from, 'share_knowledge', {
        solution: 'Change type definition to accept string | number',
        file_fix: 'type ApiResponse = string | number;',
      });
      await debuggerAgent.updateStatus('online');
    }, 1500);
  });
  ...
  await Promise.all([coder.shutdown(), debuggerAgent.shutdown(), tester.shutdown()]);
}
