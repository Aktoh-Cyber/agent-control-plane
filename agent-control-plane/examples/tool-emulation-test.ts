...
import { executeEmulation, Tool, ToolCall, ToolEmulator } from '../src/proxy/tool-emulation.js';
import { getCapabilityReport } from '../src/utils/modelCapabilities.js';
...
// Test function
async function testModel(modelId: string, userQuestion: string, strategy: 'react' | 'prompt') {
  console.log('\n' + '═'.repeat(80));
  ...
  try {
    ...
    if (result.toolCalls.length > 0) {
      result.toolCalls.forEach((call, i) => {
        console.log(`\n   ${i + 1}. ${call.name}`);
        console.log(
          `      Arguments: ${JSON.stringify(call.arguments, null, 2).split('\n').join('\n      ')}`
        );
      });
    }
...
async function runTests() {
  ...
  for (const testCase of testCases) {
    try {
      const result = await testModel(testCase.model, testCase.question, testCase.strategy);
      results.push({
  ...
  console.log(
    `Tests Passed: ${successCount}/${totalCount} (${((successCount / totalCount) * 100).toFixed(1)}%)\n`
  );
