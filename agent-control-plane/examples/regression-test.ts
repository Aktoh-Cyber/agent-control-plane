...
test('Tool emulation files exist', () => {
  return existsSync('src/utils/modelCapabilities.ts') && existsSync('src/proxy/tool-emulation.ts');
});

// Test 2: Tool emulation integrated in CLI (Phase 2)
test('Tool emulation integrated in cli-proxy (Phase 2)', () => {
  const cliProxy = execSync('grep -c "detectModelCapabilities" src/cli-proxy.ts || true', {
    encoding: 'utf-8',
  });
...
test('Tool emulation integrated in anthropic-to-openrouter (Phase 2)', () => {
  const proxy = execSync(
    'grep -c "tool-emulation\\|modelCapabilities" src/proxy/anthropic-to-openrouter.ts || true',
    { encoding: 'utf-8' }
  );
...
test('Tool emulation isolated (not imported in anthropic-to-gemini)', () => {
  const gemini = execSync(
    'grep -c "tool-emulation\\|modelCapabilities" src/proxy/anthropic-to-gemini.ts || true',
    { encoding: 'utf-8' }
  );
...
test('AnthropicToOpenRouterProxy class exists', () => {
  const exports = execSync(
    'grep -c "export class AnthropicToOpenRouterProxy" src/proxy/anthropic-to-openrouter.ts',
    { encoding: 'utf-8' }
  );
...
test('AnthropicToGeminiProxy class exists', () => {
  const exports = execSync(
    'grep -c "export class AnthropicToGeminiProxy" src/proxy/anthropic-to-gemini.ts',
    { encoding: 'utf-8' }
  );
  return parseInt(exports.trim()) === 1;
});
...
test('ToolEmulator class exported', () => {
  const exports = execSync('grep -c "export class ToolEmulator" src/proxy/tool-emulation.ts', {
    encoding: 'utf-8',
  });
  return parseInt(exports.trim()) === 1;
});
...
test('Proxy does NOT rewrite tool names', () => {
  const rewrite = execSync(
    'grep -n "tool.name.*=" src/proxy/anthropic-to-openrouter.ts | grep -v "name: tool.name" || true',
    { encoding: 'utf-8' }
  );
...
test('Proxy passes tool schemas unchanged', () => {
  const schema = execSync(
    'grep -c "parameters: tool.input_schema" src/proxy/anthropic-to-openrouter.ts',
    { encoding: 'utf-8' }
  );
