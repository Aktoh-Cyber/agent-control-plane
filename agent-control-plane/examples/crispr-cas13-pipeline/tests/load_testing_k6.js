...
export default function () {
  ...
  let createResponse = http.post(`${BASE_URL}/api/v1/jobs`, jobPayload, {
    headers,
    tags: { name: 'CreateJob' },
  });
  ...
  let statusResponse = http.get(`${BASE_URL}/api/v1/jobs/${jobId}`, {
    headers,
  ...
  let listResponse = http.get(`${BASE_URL}/api/v1/jobs?page=1&limit=20`, {
    headers,
    tags: { name: 'ListJobs' },
  });
  ...
  if (behavior < 0.3) {
    ...
    let resultsResponse = http.get(`${BASE_URL}/api/v1/jobs/${jobId}/alignments`, {
      headers,
...
export function stressTest() {
  ...
  for (let i = 0; i < 10; i++) {
    const jobPayload = generateJobPayload();
    const response = http.post(`${BASE_URL}/api/v1/jobs`, jobPayload, { headers });
...
export function spikeTest() {
  ...
  const responses = [];
  for (let i = 0; i < 50; i++) {
    responses.push(http.post(`${BASE_URL}/api/v1/jobs`, generateJobPayload(), { headers }));
  }
