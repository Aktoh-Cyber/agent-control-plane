...
Agent Booster is successfully integrated into agent-control-plane@1.4.2's **MCP server** (for Claude Desktop/Cursor), but is **NOT** accessible via the CLI `--agent` mode.
...
```bash
node /tmp/node_modules/agent-control-plane/dist/mcp/standalone-stdio.js
```

**Output:**

```
✅ Registered 10 tools (7 agent-control-plane + 3 agent-booster):
...
```bash
npx agent-control-plane@1.4.2 --agent coder --task "convert var to const in test.js"
```
...
1. **Install agent-control-plane:**
   ...
   ```bash
   npm install -g agent-control-plane@1.4.2
   ```
...
```bash
cd /tmp
npm install agent-control-plane@1.4.2

# Test MCP server directly
node node_modules/agent-control-plane/dist/mcp/standalone-stdio.js
# Should show: ✅ Registered 10 tools (7 agent-control-plane + 3 agent-booster)
```
...
**Agent Booster integration in agent-control-plane@1.4.2 is working correctly** - it's just not available where you might expect it.
