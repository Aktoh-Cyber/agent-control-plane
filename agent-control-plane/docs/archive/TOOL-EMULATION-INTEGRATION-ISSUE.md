...
Enable Claude Code and agent-control-plane to work with **ANY model** (even those without native function calling support) by implementing automatic tool emulation. This will achieve **99%+ cost savings** while maintaining 70-85% functionality.
...
Claude Code and agent-control-plane currently **require models with native tool/function calling support**:
...
const model =
  options.model || process.env.COMPLETION_MODEL || 'mistralai/mistral-small-3.1-24b-instruct';
...
npx agent-control-plane --agent coder --task "test" --provider openrouter --model "deepseek/deepseek-chat"

# Should show emulation message
npx agent-control-plane --agent coder --task "test" --provider openrouter --model "mistralai/mistral-7b-instruct"
```
...
npx agent-control-plane --agent coder --task "What is 2+2?" --provider openrouter --model "deepseek/deepseek-chat"
...
npx agent-control-plane --agent coder --task "What is 5*5?" \
...
3. **Free Model**:
   ...
   ```bash
   npx agent-control-plane --agent researcher --task "Explain AI" \
