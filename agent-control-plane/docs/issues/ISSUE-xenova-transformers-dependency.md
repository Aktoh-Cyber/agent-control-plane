...
**The problem is in `gendev@alpha` package:**

```
User runs: npm i -g gendev@alpha

Dependency chain:
└── gendev@2.7.28 (alpha tag)
    └── agent-control-plane@^1.8.10
...
- `gendev@alpha` (v2.7.28) depends on `agent-control-plane@^1.8.10`
- `agent-control-plane@1.8.10` depends on `agentdb@^1.4.3`
...
- User installs `gendev@alpha` globally
- npm resolves to `agent-control-plane@1.8.10` → `agentdb@1.4.3`
...
- ✅ `agent-control-plane@1.8.10` - Has transformers@^2.17.2 (used by gendev@alpha)
- ✅ `agent-control-plane@1.9.1` - Has transformers@^2.17.2 (LATEST, not published as alpha)
...
2. `agent-control-plane/src/agentdb/controllers/EmbeddingService.ts:32` - Dynamic import
3. `agent-control-plane/src/router/providers/onnx.ts:33` - Dynamic import
4. `agent-control-plane/src/reasoningbank/utils/embeddings.ts:6` - Static import

**Key observation**: Most imports are **dynamic** (`await import()`) except embeddings.ts which uses static import.

## Impact

- **Severity**: HIGH - Blocks all global installations of `gendev@alpha`
- **Affected package**: `gendev@alpha` (v2.7.28)
- **Dependency chain**: `gendev@alpha` → `agent-control-plane@1.8.10` → `agentdb@1.4.3` (missing transformers)
- **User impact**: Cannot install `gendev@alpha` globally
- **Root package**: This repository (`agent-control-plane`) needs to coordinate fix with `gendev`
- **Workaround**: Update `gendev@alpha` to use `agent-control-plane@^1.9.0` which depends on newer `agentdb`

## Proposed Solution

### IMMEDIATE FIX: Update gendev@alpha dependency (REQUIRED)

**The `gendev` package needs to be updated:**

```json
// gendev/package.json
{
  "dependencies": {
    "agent-control-plane": "^1.9.0" // Change from ^1.8.10 to ^1.9.0
  }
}
```
...
1. Update `gendev` repository to bump `agent-control-plane` to `^1.9.0`
2. Publish new `gendev@alpha` version
3. Test global installation: `npm i -g gendev@alpha`
...
**agent-control-plane/src/reasoningbank/utils/embeddings.ts:**
...
rm -f agent-control-plane/package-lock.json
...
npm uninstall -g gendev
npm install -g gendev@alpha

# Test with transformers
npm install @xenova/transformers
npx gendev reasoningbank test
...
```bash
npm i -g gendev@alpha  # FAILS with version error
```

**After fix:**

```bash
npm i -g gendev@alpha  # ✅ Works, uses hash-based embeddings
...
- `agent-control-plane/package.json` - Move to optionalDependencies
- `agent-control-plane/src/reasoningbank/utils/embeddings.ts` - Convert static import to dynamic

### Already Correct (using dynamic imports):

- `packages/agentdb/src/controllers/EmbeddingService.ts`
- `agent-control-plane/src/agentdb/controllers/EmbeddingService.ts`
- `agent-control-plane/src/router/providers/onnx.ts`

## Success Criteria

- ✅ `npm install -g gendev@alpha` completes successfully
