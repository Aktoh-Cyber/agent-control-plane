...
This document provides a step-by-step implementation plan for adding Requesty.ai support to agent-control-plane. The implementation is divided into 5 phases, each with clear deliverables and acceptance criteria.
...
- Create: `/workspaces/agent-control-plane/agent-control-plane/src/proxy/anthropic-to-requesty.ts`
...
if (provider === 'requesty' && process.env.REQUESTY_API_KEY) {
  ...
  envOverrides.ANTHROPIC_BASE_URL =
    process.env.ANTHROPIC_BASE_URL || process.env.REQUESTY_PROXY_URL || 'http://localhost:3000';
...
REQUESTY_API_KEY=invalid npx agent-control-plane --agent coder \
  --task "Test" \
  --provider requesty
```
...
```bash
# OpenAI models
npx agent-control-plane --agent coder --task "Create function" --provider requesty --model "openai/gpt-4o-mini"
npx agent-control-plane --agent coder --task "Create function" --provider requesty --model "openai/gpt-4-turbo"

# Anthropic models
npx agent-control-plane --agent coder --task "Create function" --provider requesty --model "anthropic/claude-3.5-sonnet"
npx agent-control-plane --agent coder --task "Create function" --provider requesty --model "anthropic/claude-3-haiku"

# Google models
npx agent-control-plane --agent coder --task "Create function" --provider requesty --model "google/gemini-2.5-flash"

# DeepSeek models
npx agent-control-plane --agent coder --task "Create function" --provider requesty --model "deepseek/deepseek-chat-v3"

# Meta models
npx agent-control-plane --agent coder --task "Create function" --provider requesty --model "meta-llama/llama-3.3-70b-instruct"
```
