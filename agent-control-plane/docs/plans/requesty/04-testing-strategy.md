...
describe('AnthropicToRequestyProxy - Error Handling', () => {
  it('should handle invalid API key', async () => {
    ...
    await expect(proxy.handleRequest(validRequest, mockRes)).rejects.toThrow('Invalid API key');
  });
  ...
  it('should handle rate limit errors', async () => {
    ...
    await expect(proxy.handleRequest(validRequest, mockRes)).rejects.toThrow('Rate limit exceeded');
  });
  ...
  it('should handle model not found', async () => {
    ...
    await expect(proxy.handleRequest(req, mockRes)).rejects.toThrow('Model not found');
  });
});
```
...
USE_REQUESTY=true npx agent-control-plane --agent coder \
  --task "Say hello" | grep "Requesty"

# Test 3: API key validation (missing key)
echo "Test 3: Missing API key error"
REQUESTY_API_KEY= npx agent-control-plane --agent coder \
  --task "Test" \
...
VERBOSE=true npx agent-control-plane --agent coder \
  --task "Test" \
...
```bash
REQUESTY_API_KEY="requesty-invalid" npx agent-control-plane --agent coder \
  --task "Test" \
  --provider requesty
```
...
REQUESTY_TIMEOUT=1 npx agent-control-plane --agent coder \
  --task "Long task" \
  --provider requesty
```
