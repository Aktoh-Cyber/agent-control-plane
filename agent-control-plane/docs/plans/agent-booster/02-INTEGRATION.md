...
Agent Booster integrates seamlessly with agent-control-plane as an **optional performance accelerator** for code editing operations. When enabled, it replaces slow LLM-based code application with ultra-fast vector-based semantic merging.
...
function getBooster(): AgentBooster | null {
  ...
  if (!booster) {
    booster = new AgentBooster({
      ...
      confidenceThreshold: parseFloat(process.env.AGENT_BOOSTER_CONFIDENCE_THRESHOLD || '0.65'),
...
export const editFileTool = {
  ...
  async execute(params: { file_path: string; edit_description: string; language?: string }) {
...
async function fallbackToLLM(originalCode: string, editDescription: string, filePath: string) {
...
class MetricsCollector {
  ...
  record(metric: EditMetrics) {
    ...
    if (process.env.AGENT_BOOSTER_METRICS_FILE) {
      appendFileSync(process.env.AGENT_BOOSTER_METRICS_FILE, JSON.stringify(metric) + '\n');
    }
  }
...
```typescript
// examples/agent-control-plane-with-agent-booster.ts
