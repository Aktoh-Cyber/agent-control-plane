...
**agent-control-plane v1.1.6** now **correctly uses the Claude Agent SDK** for **all providers** with multi-provider proxy routing as specified in the architecture plans.
...
- **Agentic Cloud**: `agentic-cloud-swarm`, `agentic-cloud-neural`, `agentic-cloud-workflow`
...
npx agent-control-plane --agent coder --task "Create a hello world function" --provider openrouter
...
```bash
npx agent-control-plane --agent sparc-coder --task "Implement TDD workflow"
...
const result = await claudeAgent(coder, 'Create a TypeScript hello world function');
...
- [x] **MCP tools** - 111 tools from gendev MCP server
...
```bash
# Build Docker image
docker build -t agent-control-plane:1.1.6 .

# Test with Anthropic
docker run --rm -e ANTHROPIC_API_KEY agent-control-plane:1.1.6 \
  --agent coder --task "Create hello world"

# Test with OpenRouter
docker run --rm -e OPENROUTER_API_KEY agent-control-plane:1.1.6 \
