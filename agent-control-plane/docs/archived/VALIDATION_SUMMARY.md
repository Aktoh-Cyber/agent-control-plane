...
const result = query({
  ...
  options: {
    ...
    mcpServers: {
      ...
      gendev: { command: 'npx', args: ['gendev@alpha', 'mcp', 'start'] },
    },
  },
});
```
...
npx agent-control-plane --agent coder --task "..." --provider anthropic
...
- 111 tools from `gendev` MCP server
- In-SDK server for basic memory/coordination
- Optional: `agentic-cloud` (96 cloud tools)
...
npx agent-control-plane --agent coder --task "Create hello world" --provider anthropic
