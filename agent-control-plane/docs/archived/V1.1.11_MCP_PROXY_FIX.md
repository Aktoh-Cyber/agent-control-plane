...
node dist/cli-proxy.js --agent coder --task "Use mcp__gendev-sdk__memory_store to save data" --provider anthropic
```
...
_(Rate limit = tools were accepted, just too many requests)_
...
1. `mcp__gendev-sdk__memory_store` - Store persistent memory with TTL
2. `mcp__gendev-sdk__memory_retrieve` - Retrieve from memory
3. `mcp__gendev-sdk__memory_search` - Search memory patterns
4. `mcp__gendev-sdk__swarm_init` - Initialize agent swarm
5. `mcp__gendev-sdk__agent_spawn` - Spawn swarm agents
...
npx agent-control-plane@1.1.11 --agent coder \
  --task "Store user preferences in memory using mcp__gendev-sdk__memory_store" \
  --provider gemini
```
...
npx agent-control-plane@1.1.11 --agent reviewer \
...
npx agent-control-plane@1.1.11 --agent coder --task "..." --provider gemini
```
