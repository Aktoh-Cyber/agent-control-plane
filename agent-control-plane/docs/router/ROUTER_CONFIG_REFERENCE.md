...
The router configuration file (`router.config.json`) controls how the agent-control-plane system selects and routes requests to different LLM providers.

## Configuration File Location

- **Default**: `~/.agent-control-plane/router.config.json`
...
```json
{
  ...
  "providers": {
    ...
    "litellm": {
      "enabled": true,
      "fallbackModels": ["gpt-4-turbo-preview", "claude-3-opus-20240229", "command-r-plus"],
      "timeout": 180000
    }
  },
  "routing": {
...
```bash
# Override provider
npx agent-control-plane --provider openai --task "..."

# Override model
npx agent-control-plane --model gpt-4 --task "..."

# Override routing mode
npx agent-control-plane --router-mode cost-optimized --task "..."

# Use custom config file
npx agent-control-plane --router-config ./custom-router.json --task "..."
```
...
```bash
npx agent-control-plane router validate ./router.config.json
```
...
```bash
npx agent-control-plane router migrate ./old-config.json ./new-config.json
```
