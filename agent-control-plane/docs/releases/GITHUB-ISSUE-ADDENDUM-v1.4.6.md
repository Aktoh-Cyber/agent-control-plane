...
```typescript
import { runTask, retrieveMemories, consolidate } from 'agent-control-plane/reasoningbank';
...
```typescript
import { runTask, mattsSequential } from 'agent-control-plane/reasoningbank';
...
class FederatedReasoningBank {
  async federatedTrain(participants: Organization[]) {
    ...
    const localModels = await Promise.all(participants.map((org) => org.trainLocalModel()));
