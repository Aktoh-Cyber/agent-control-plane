...
Enable **agent-control-plane** to work with alternative LLM providers beyond Anthropic, including OpenAI, OpenRouter, and local models (Ollama, LM Studio). This provides flexibility, cost optimization, and privacy options while maintaining the powerful Agent SDK capabilities.
...
npx agent-control-plane --provider claude --model claude-sonnet-4-5 --task "..."
npx agent-control-plane --provider ollama --model llama3 --task "..."
npx agent-control-plane --provider openrouter --model "anthropic/claude-3-opus" --task "..."

# Router modes
npx agent-control-plane --router-mode fallback --task "..."  # Try providers in order
npx agent-control-plane --router-mode cost-optimized --task "..."  # Cheapest capable model
npx agent-control-plane --router-mode local-first --task "..."  # Prefer local models

# Configuration file
npx agent-control-plane --router-config ./router.config.json --task "..."
```
...
// router.config.json
{
  ...
  "providers": {
    ...
    "openrouter": {
      ...
      "models": ["*"],
      "siteUrl": "https://agent-control-plane.app",
...
export class MCPBridge {
  async executeToolViaProvider(tool: MCPTool, params: any, provider: LLMProvider): Promise<any> {
...
export class CostTracker {
  ...
  async trackRequest(
    ...
    this.costs.set(`${provider}:${model}`, (this.costs.get(`${provider}:${model}`) || 0) + cost);
  }
...
```bash
npx agent-control-plane --task "Build API"
...
```bash
npx agent-control-plane --router-mode cost-optimized --task "Simple analysis"
...
```bash
npx agent-control-plane --provider ollama --model llama3 --task "Analyze sensitive data"
...
```bash
npx agent-control-plane --provider openrouter --model "anthropic/claude-3-opus" --task "Complex reasoning"
...
```bash
npx agent-control-plane --router-config ./router.config.json --task "Multi-step task"
# Intelligent routing:
