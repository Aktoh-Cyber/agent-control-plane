...
```typescript
import { ProviderManager, ProviderConfig } from 'agent-control-plane/core/provider-manager';
...
const { result, provider, attempts } = await manager.executeWithFallback(async (providerName) => {
...
```typescript
import { LongRunningAgent } from 'agent-control-plane/core/long-running-agent';
...
```bash
docker build -f Dockerfile.provider-fallback -t agent-control-plane-provider-fallback .
```
...
docker run --rm \
  ...
  agent-control-plane-provider-fallback

# ONNX only (no API key needed)
docker run --rm agent-control-plane-provider-fallback
```
