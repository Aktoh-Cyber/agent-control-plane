...
[← Back to Main README](https://github.com/Aktoh-Cyber/agent-control-plane/blob/main/README.md) | [Multi-Model Router ←](https://github.com/Aktoh-Cyber/agent-control-plane/blob/main/docs/guides/MULTI-MODEL-ROUTER.md) | [Deployment Options →](https://github.com/Aktoh-Cyber/agent-control-plane/blob/main/docs/guides/DEPLOYMENT.md)
...
| -------------------- | --------- | ------------------------------------ | ----------------------------------------------------------------- |
| **GenDev**           | 101 tools | Agent orchestration, memory, neural  | `claude mcp add gendev npx gendev@alpha mcp start`                |
| **Agentic Cloud**    | 96 tools  | Cloud execution, sandboxes, payments | `claude mcp add agentic-cloud npx agentic-cloud@latest mcp start` |
| **Agentic Payments** | 12 tools  | Payment authorization, mandates      | Built-in with agent-control-plane                                 |
| **GenDev SDK**       | 4 tools   | Low-level SDK integration            | Built-in with agent-control-plane                                 |
...
#### **Sandbox Management (Agentic Cloud)**
...
const swarm = await query({
  mcp: { server: 'gendev', tool: 'swarm_init', params: { topology: 'mesh' } },
});

// Step 2: Spawn agents
await query({ mcp: { server: 'gendev', tool: 'agent_spawn', params: { type: 'coder' } } });
await query({ mcp: { server: 'gendev', tool: 'agent_spawn', params: { type: 'tester' } } });

// Step 3: Execute task
const result = await query({
  mcp: { server: 'gendev', tool: 'task_orchestrate', params: { task: '...' } },
});

// Step 4: Clean up
await query({ mcp: { server: 'gendev', tool: 'swarm_destroy', params: { swarmId: swarm.id } } });
```
...
const memories = await query({
  mcp: { server: 'gendev', tool: 'memory_search', params: { pattern: 'api-design' } },
});
...
const sandbox = await query({
  mcp: { server: 'agentic-cloud', tool: 'sandbox_create', params: { template: 'node' } },
});
...
const logs = await query({
  mcp: { server: 'agentic-cloud', tool: 'sandbox_logs', params: { sandbox_id: sandbox.id } },
});

// Cleanup
await query({
  mcp: { server: 'agentic-cloud', tool: 'sandbox_delete', params: { sandbox_id: sandbox.id } },
});
```
...
- [Deployment Options →](https://github.com/Aktoh-Cyber/agent-control-plane/blob/main/docs/guides/DEPLOYMENT.md)
- [Performance Benchmarks](https://github.com/Aktoh-Cyber/agent-control-plane/blob/main/docs/agent-control-plane/benchmarks/README.md)
- [API Reference](https://docs.rs/agent-control-plane)
...
See [CONTRIBUTING.md](https://github.com/Aktoh-Cyber/agent-control-plane/blob/main/CONTRIBUTING.md) for guidelines.
