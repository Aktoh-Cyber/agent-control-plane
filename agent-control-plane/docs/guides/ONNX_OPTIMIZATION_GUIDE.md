...
```bash
npx agent-control-plane --agent coder --task "Write a function" --provider onnx
```
...
```bash
npx agent-control-plane --agent coder --task "Write a Python function called is_prime(n: int) -> bool that checks if n is prime. Include: 1) Type hints 2) Docstring 3) Handle edge cases (negative, 0, 1) 4) Optimal algorithm. Return ONLY code, no explanation." --provider onnx
```
...
const messages = [
  {
    role: 'user',
    content: 'Write a function to calculate fibonacci. Use memoization for O(n) time.',
  },
];
```
...
for task in task1 task2 task3; do
  npx agent-control-plane --agent coder --task "$task" --provider onnx
done
...
```bash
npx agent-control-plane --agent coder --task "task1" --provider onnx &
npx agent-control-plane --agent coder --task "task2" --provider onnx &
npx agent-control-plane --agent coder --task "task3" --provider onnx &
wait
