...
Use Claude Code with OpenRouter, Gemini, or ONNX providers through agent-control-plane proxy.
...
```bash
# Install script
cp node_modules/agent-control-plane/scripts/claude-code ~/bin/
...
### `npx agent-control-plane claude-code [OPTIONS] [prompt]`
...
npx agent-control-plane claude-code --provider gemini \
  "Create a simple REST API with Flask"

# Direct Anthropic Claude Sonnet 4.5
npx agent-control-plane claude-code --provider anthropic \
...
npx agent-control-plane claude-code \
  --provider openrouter \
...
npx agent-control-plane proxy --provider openrouter --port 8080
...
1. **User runs:** `npx agent-control-plane claude-code --provider openrouter "task"`
...
# Use custom port
npx agent-control-plane claude-code --provider openrouter --port 3001 "task"
```
...
# Or use inline
OPENROUTER_API_KEY=sk-or-v1-xxxxx \
npx agent-control-plane claude-code --provider openrouter "task"
```
...
VERBOSE=true npx agent-control-plane proxy --provider openrouter
...
```bash
# Test OpenRouter
npx agent-control-plane claude-code --provider openrouter "print hello world in python"

# Test Gemini
npx agent-control-plane claude-code --provider gemini "print hello world in python"

# Test Anthropic
npx agent-control-plane claude-code --provider anthropic "print hello world in python"
```
...
import { AnthropicToOpenRouterProxy } from 'agent-control-plane/dist/proxy/anthropic-to-openrouter.js';
...
Create `~/.agent-control-plane-presets`:
...
- Added `npx agent-control-plane claude-code` command
