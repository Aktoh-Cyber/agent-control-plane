...
```bash
# Use ONNX provider
npx agent-control-plane --agent coder --task "Write hello world" --provider onnx

# Use with model optimizer
npx agent-control-plane --agent coder --task "Simple task" --optimize --optimize-priority privacy
```
...
npx agent-control-plane --agent coder --task "test" --provider onnx --verbose
