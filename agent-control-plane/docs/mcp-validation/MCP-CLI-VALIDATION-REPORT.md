...
**Command:** `npx agent-control-plane mcp`
...
```bash
npx agent-control-plane mcp add strange-loops '{"command":"npx","args":["-y","strange-loops","mcp","start"],"description":"Strange Loops MCP server for testing"}'
```
...
```bash
npx agent-control-plane mcp add weather --npm weather-mcp --env "API_KEY=xxx"
```
...
**Location:** `~/.agent-control-plane/mcp-config.json`
...
```bash
npx agent-control-plane --agent researcher --task "List all available MCP tools and their descriptions. Focus on tools from the strange-loops MCP server." --output-format markdown
```
...
| Config File Storage | ✅ PASS | `~/.agent-control-plane/mcp-config.json` created |
...
1. **User adds server via CLI:**
   ...
   ```bash
   npx agent-control-plane mcp add NAME CONFIG
   ```
...
npx agent-control-plane mcp add weather --npm weather-mcp --env "WEATHER_API_KEY=your-key"

# List to verify
npx agent-control-plane mcp list

# Use it
npx agent-control-plane --agent researcher --task "Get weather for San Francisco"
```
...
# Enable/disable
npx agent-control-plane mcp disable my-tools
npx agent-control-plane mcp enable my-tools
```
...
npx agent-control-plane mcp add custom-server '{
  "command": "python3",
...
2. ✅ Stores configuration persistently in `~/.agent-control-plane/mcp-config.json`
...
**Recommendation:** This feature is ready for production use in agent-control-plane v1.1.14.
