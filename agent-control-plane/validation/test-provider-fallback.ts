...
import { ProviderConfig, ProviderManager } from '../src/core/provider-manager.js';
...
async function testProviderManager() {
  ...
  health.forEach((h) => {
    console.log(
      `  ${h.provider}: healthy=${h.isHealthy}, circuitBreaker=${h.circuitBreakerOpen ? 'OPEN' : 'CLOSED'}`
    );
  });
  console.log('');
...
async function testFallbackStrategy() {
  ...
  console.log(
    `✅ Complex task provider: ${complexProvider} (should prefer Anthropic if available)\n`
  );
...
async function testCircuitBreaker() {
  ...
  if (geminiHealth) {
    ...
    console.log(
      `  Gemini circuit breaker: ${geminiHealth.circuitBreakerOpen ? 'OPEN ✅' : 'CLOSED'}`
    );
