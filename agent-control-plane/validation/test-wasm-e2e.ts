...
import {
  ...
  createReasoningBank,
  type PatternInput,
} from '../dist/reasoningbank/wasm-adapter.js';
...
const tests = {
  'WASM Files Exist': test('WASM Files Exist', async () => {
    const wasmPath = resolve(
      import.meta.dirname,
      '../wasm/reasoningbank/reasoningbank_wasm_bg.wasm'
    );
  ...
  'Find Similar Patterns': test('Find Similar Patterns', async () => {
    ...
    // Search for similar
    const similar = await rb.findSimilar('Speed up database operations', 'performance', 5);
    if (similar.length === 0) {
      throw new Error('No similar patterns found');
    }
    console.log(
      `   Found ${similar.length} similar, top score: ${similar[0].similarity_score.toFixed(3)}`
    );
  }),
