...
gendev truth --period 7d

# Export to HTML report
gendev truth --export report.html --format html
```
...
gendev truth --threshold 0.8

# Agent-specific metrics
gendev truth --agent coder --period 24h

# JSON for processing
gendev truth --format json | jq '.overall_score'
```
...
- name: Check Truth Scores
  run: |
    gendev truth --format json > truth.json
...
Set truth display preferences in `.gendev/config.json`:
