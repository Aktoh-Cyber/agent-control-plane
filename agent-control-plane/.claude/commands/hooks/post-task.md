...
```bash
npx gendev hook post-task -t "api-refactor" --analyze-performance --generate-report
```

### Memory storage

```bash
npx gendev hook post-task -t "bug-fix-123" --store-decisions --export-learnings
```

### Quick cleanup

```bash
npx gendev hook post-task -t "minor-update" --analyze-performance false
```
...
npx gendev hook post-task --task-id "your-task-id" --analyze-performance true
```
