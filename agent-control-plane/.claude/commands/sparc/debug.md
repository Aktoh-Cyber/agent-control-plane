...
npx gendev sparc run debug "fix memory leak in service"

# For alpha features
npx gendev@alpha sparc run debug "fix memory leak in service"

# With namespace
npx gendev sparc run debug "your task" --namespace debug

# Non-interactive mode
npx gendev sparc run debug "your task" --non-interactive
```
...
./gendev sparc run debug "fix memory leak in service"
```
...
```bash
# Store mode-specific context
npx gendev memory store "debug_context" "important decisions" --namespace debug

# Query previous work
npx gendev memory query "debug" --limit 5
```
