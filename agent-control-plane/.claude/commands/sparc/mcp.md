...
npx gendev sparc run mcp "integrate with external API"

# For alpha features
npx gendev@alpha sparc run mcp "integrate with external API"

# With namespace
npx gendev sparc run mcp "your task" --namespace mcp

# Non-interactive mode
npx gendev sparc run mcp "your task" --non-interactive
```
...
./gendev sparc run mcp "integrate with external API"
```
...
```bash
# Store mode-specific context
npx gendev memory store "mcp_context" "important decisions" --namespace mcp

# Query previous work
npx gendev memory query "mcp" --limit 5
```
