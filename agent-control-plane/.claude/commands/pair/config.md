...
Main configuration file: `.gendev/pair-config.json`
...
gendev pair config reset verification
```
...
`.gendev/pair-config.json` in project root

### User-Specific

`~/.gendev/pair-config.json`

### Global

`/etc/gendev/pair-config.json`
...
gendev pair config validate

# Test configuration
gendev pair config test
```
...
```bash
gendev pair config migrate --from 1.x
```
...
gendev pair config export > my-config.json

# Import config
gendev pair config import my-config.json
```
