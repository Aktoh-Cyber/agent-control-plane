...
```bash
# Run SPARC orchestrator (default)
npx gendev sparc "build complete authentication system"

# Run a specific mode
npx gendev sparc run architect "design API structure"
npx gendev sparc run tdd "implement user service"

# Execute full TDD workflow
npx gendev sparc tdd "implement user authentication"

# List all modes with details
npx gendev sparc modes --verbose

# For alpha features
npx gendev@alpha sparc run <mode> "your task"
```
...
./gendev sparc "build complete authentication system"
./gendev sparc run architect "design API structure"
```
...
```bash
# Store specifications
npx gendev memory store "spec_auth" "OAuth2 + JWT requirements" --namespace spec

# Store architectural decisions
./gendev memory store "arch_api" "RESTful microservices design" --namespace arch

# Query previous work
./gendev memory query "authentication" --limit 10

# Export project memory
./gendev memory export sparc-project-backup.json
```
...
./gendev swarm "Build e-commerce platform" --strategy development --monitor --review

# Background optimization swarm
./gendev swarm "Optimize system performance" --strategy optimization --background

# Distributed research swarm
./gendev swarm "Analyze market trends" --strategy research --distributed --ui
```
...
```bash
./gendev sparc run code "implement API" --non-interactive
./gendev sparc tdd "user tests" --non-interactive --enable-permissions
```
...
See `/gendev-help` for all available commands.
