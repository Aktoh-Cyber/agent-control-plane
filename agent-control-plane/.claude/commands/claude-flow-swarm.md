...
```bash
./gendev swarm "your complex task" --strategy <type> [options]
```
...
```bash
./gendev swarm "Build e-commerce REST API" \
...
```bash
./gendev swarm "Analyze AI market trends 2024-2025" \
  --strategy research \
  --background \
  --distributed \
  --max-agents 8
...
```bash
./gendev swarm "Optimize database queries and API performance" \
...
```bash
./gendev swarm "Implement secure payment processing system" \
  --strategy development \
  --mode distributed \
  --max-agents 10 \
  --parallel \
...
```bash
./gendev swarm "Comprehensive security audit and testing" \
  --strategy testing \
  --review \
  --verbose \
  --max-agents 6
...
./gendev memory store "swarm_objective" "Build scalable API" --namespace swarm

# Query swarm progress
./gendev memory query "swarm_progress" --namespace swarm

# Export swarm memory
./gendev memory export swarm-results.json --namespace swarm
```
...
```bash
./gendev swarm "Test task" --dry-run --strategy development
```

### Custom quality thresholds:

```bash
./gendev swarm "High quality API" \
  --strategy development \
