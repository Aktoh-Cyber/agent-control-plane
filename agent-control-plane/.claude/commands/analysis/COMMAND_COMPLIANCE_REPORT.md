...
- `mcp__gendev__*` tools (preferred)
- `npx gendev` commands (as fallback)
...
- Updated to: `Tool: mcp__gendev__token_usage` with appropriate parameters
...
**Reason**: Already uses proper `mcp__gendev__task_results` tool format
...
1. **MCP Tool Usage**: All direct tool calls now use `mcp__gendev__*` format
...
The analysis directory is now fully compliant with the GenDev command standards.
