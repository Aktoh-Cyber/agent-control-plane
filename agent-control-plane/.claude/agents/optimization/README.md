...
- **MCP Tools**: Extensive use of gendev MCP performance tools
...
npx gendev swarm-monitor --swarm-id production --interval 30

# 2. Analyze current performance
npx gendev performance-report --format detailed --timeframe 24h

# 3. Optimize topology if needed
npx gendev topology-optimize --swarm-id production --strategy adaptive

# 4. Load balance based on current metrics
npx gendev load-balance --swarm-id production --strategy work-stealing

# 5. Scale resources predictively
npx gendev swarm-scale --swarm-id production --target-size auto
```
...
npx gendev benchmark-run --suite comprehensive --duration 300

# Validate against SLA requirements
npx gendev quality-assess --target swarm-performance --criteria throughput,latency,reliability

# Detect performance regressions
npx gendev detect-regression --current latest-results.json --historical baseline.json
```
...
npx gendev metrics-collect --components ["cpu", "memory", "network", "agents"]

# Optimize resource allocation
npx gendev daa-resource-alloc --resources optimal-config.json

# Profile system performance
npx gendev profile-performance --duration 60000 --components all
```
