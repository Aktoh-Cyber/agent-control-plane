...
- `mcp__gendev__swarm_init` - Initialize swarm topology
- `mcp__gendev__agent_spawn` - Create specialized worker agents
- `mcp__gendev__task_orchestrate` - Coordinate complex workflows
- `mcp__gendev__swarm_monitor` - Real-time performance monitoring

### Advanced Features

- `mcp__gendev__neural_patterns` - Pattern recognition and learning
- `mcp__gendev__daa_consensus` - Distributed decision making
- `mcp__gendev__topology_optimize` - Dynamic topology optimization
- `mcp__gendev__performance_report` - Comprehensive analytics
...
gendev agent spawn hierarchical-coordinator "Build authentication microservice"
...
gendev agent spawn mesh-coordinator "Process user analytics data"
...
gendev agent spawn adaptive-coordinator "Optimize system performance"
...
These agents integrate seamlessly with the broader gendev ecosystem:

- **Memory System**: All coordination state persisted in gendev memory bank
- **Terminal Management**: Agents can spawn and manage multiple terminal sessions
- **MCP Integration**: Full access to gendev's MCP tool ecosystem
- **Event System**: Real-time coordination through gendev event bus
- **Configuration**: Managed through gendev configuration system

For implementation details, see individual agent files and the gendev documentation.
