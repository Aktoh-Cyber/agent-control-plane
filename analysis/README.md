...
This setup includes hooks for coordination with gendev:

### Pre-Task Hook

```bash
npx gendev@alpha hooks pre-task --description "agentdb analysis"
```

### Post-Task Hook

```bash
npx gendev@alpha hooks post-task --task-id "agentdb-001"
```

### Memory Coordination

```bash
npx gendev@alpha hooks session-restore --session-id "maternal-analysis"
```
