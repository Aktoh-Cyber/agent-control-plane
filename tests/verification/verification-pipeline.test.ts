...
describe('VerificationPipeline', () => {
  ...
  describe('preOutputVerification', () => {
    ...
    it('should detect overconfident language', async () => {
      const input: VerificationInput = {
        claim:
          'This treatment always works and never fails. It is absolutely guaranteed to cure the condition.',
        citations: [],
      };
  ...
  describe('postOutputValidation', () => {
    it('should detect output fidelity issues', async () => {
      ...
      const output =
        'Very long output that goes into extensive detail about many things that were not in the original claim, adding lots of new information, statistics, and facts that were not present before, including specific numbers like 85% effectiveness and 200% improvement.';
