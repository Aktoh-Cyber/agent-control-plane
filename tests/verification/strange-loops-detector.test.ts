...
describe('StrangeLoopsDetector', () => {
  ...
  describe('detectCircularReasoning', () => {
    ...
    it('should not flag valid causal chains', async () => {
      ...
      const criticalCircular = patterns.find(
        (p) => p.type === 'circular' && p.severity === 'critical'
      );
  ...
  describe('detectContradictions', () => {
    ...
    it('should detect affirmation vs negation', async () => {
      const text =
        'The medication is effective for treating pain. The medication is not effective for treating pain.';
    ...
    it('should not flag non-contradictory statements', async () => {
      const text =
        'Treatment A is effective. Treatment B is effective. Both have different mechanisms.';
  ...
  describe('validateCausalChain', () => {
    ...
    it('should detect weak causal links', async () => {
      const chain: CausalChain = {
        ...
        edges: [{ from: 'a', to: 'b', type: 'causes', strength: 0.2 }],
        valid: true,
        cycles: [],
        contradictions: [],
      };
  ...
  describe('detectRecursivePatterns', () => {
    ...
    it('should detect nested parenthetical', async () => {
      const text =
        'The treatment (which includes medication (which is FDA approved)) is effective.';
    ...
    it('should flag excessive recursion', async () => {
      const text =
        'A recursive definition that references itself which in turn references itself again.';
