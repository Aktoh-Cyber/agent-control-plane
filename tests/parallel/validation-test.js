...
test('Parallel Execution Guide Exists', () => {
  const guidePath = path.join(
    __dirname,
    '../../agent-control-plane/src/prompts/parallel-execution-guide.md'
  );
  ...
  const hasSubprocessPattern = content.includes('npx agent-control-plane --agent');
...
test('Provider Instructions Enhanced', () => {
  const providerPath = path.join(
    __dirname,
    '../../agent-control-plane/src/proxy/provider-instructions.ts'
  );
...
test('Validation Hooks Implemented', () => {
  const hooksPath = path.join(
    __dirname,
    '../../agent-control-plane/src/hooks/parallel-validation.ts'
  );
...
test('Test Suites Complete', () => {
  ...
  const missingTests = requiredTests.filter((test) => !fs.existsSync(path.join(testDir, test)));
...
test('Parallel Execution Patterns Valid', () => {
  ...
  const bestPractices = {
    ...
    'CLI subprocess': content.includes('npx agent-control-plane --agent'),
