...
async function compareTopologies() {
  ...
  try {
    ...
    topologies.forEach((topology) => {
      ...
      console.log(
        `${topology.padEnd(15)}: ${speedup.toFixed(2)}x speedup (${faster}% ${faster >= 0 ? 'faster' : 'slower'})`
      );
    });
...
function generateRecommendations(benchmarkResults) {
  ...
  recommendations.push(
    `Best topology for this workload: ${bestTopology} (${tests[bestTopology].statistics.avgTimeMs}ms avg)`
  );
  ...
  topologies.forEach((topology) => {
    ...
    if (successRate < 80) {
      recommendations.push(
        `${topology} has low success rate (${successRate.toFixed(1)}%) - investigate failures`
      );
    }
  });
  ...
  if (hierarchicalTime < meshTime * 0.7) {
    recommendations.push(
      `Hierarchical shows strong parallel benefits (${((1 - hierarchicalTime / meshTime) * 100).toFixed(1)}% faster) - good for delegated tasks`
    );
  }
...
${Object.keys(benchmarkResults.tests)
  .map((topology) => {
    const avgTime = benchmarkResults.tests[topology].statistics.avgTimeMs;
    const baselineTime = benchmarkResults.tests.mesh.statistics.avgTimeMs;
