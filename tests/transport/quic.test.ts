...
describe('QUIC Transport Layer', () => {
  ...
  describe('Error Handling', () => {
    ...
    it('should throw error when writing to finished stream', async () => {
      ...
      await expect(send.write(new Uint8Array([1, 2, 3]))).rejects.toThrow(
  ...
  describe('Performance Characteristics', () => {
    ...
    it('should scale to 1000+ streams per connection', async () => {
      ...
      const streamPromises = Array(1000)
        .fill(null)
        .map(() => conn.openBi());
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle connection pool exhaustion', async () => {
      ...
      const connections = await Promise.all(
        Array(100)
          .fill(null)
          .map((_, i) => connectionPool.getConnection(`localhost:${8443 + i}`))
      );
