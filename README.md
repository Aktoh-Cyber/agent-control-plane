...
[![npm version](https://img.shields.io/npm/v/agentopia.svg)](https://www.npmjs.com/package/agentopia)
[![npm downloads](https://img.shields.io/npm/dm/agentopia.svg)](https://www.npmjs.com/package/agentopia)
[![License: MIT](https://img.shields.io/badge/License-MIT-blue.svg)](https://opensource.org/licenses/MIT)
[![Node.js Version](https://img.shields.io/badge/node-%3E%3D18.0.0-brightgreen)](https://nodejs.org/)
[![tafy](https://img.shields.io/badge/by-tafy-purple.svg)](https://github.com/tafyai/)
[![Agentic Engineering](https://img.shields.io/badge/Agentic-Engineering-orange.svg)](https://github.com/tafyai/agentopia#-agent-types)

[![CI Pipeline](https://github.com/tafyai/agentopia/actions/workflows/ci.yml/badge.svg)](https://github.com/tafyai/agentopia/actions/workflows/ci.yml)
[![PR Validation](https://github.com/tafyai/agentopia/actions/workflows/pr-validation.yml/badge.svg)](https://github.com/tafyai/agentopia/actions/workflows/pr-validation.yml)
[![Release](https://github.com/tafyai/agentopia/actions/workflows/release.yml/badge.svg)](https://github.com/tafyai/agentopia/actions/workflows/release.yml)
[![Docker](https://github.com/tafyai/agentopia/actions/workflows/docker.yml/badge.svg)](https://github.com/tafyai/agentopia/actions/workflows/docker.yml)
...
| [Model Optimization](#-model-optimization) | [Multi-Model Router](#-using-the-multi-model-router) | [Deployment Patterns](#-deployment-patterns)             | [Complete Docs](https://github.com/tafyai/agentopia/tree/main/docs) |
...
| ------------------------- | ----------------------------------------------------------------------------------------------- | --------------------------------------- | ------------------------------------------------------------------------------------------- |
| **Agent Booster**         | Ultra-fast local code transformations via Rust/WASM (auto-detects edits)                        | 352x faster, $0 cost                    | [Docs](https://github.com/tafyai/agentopia/tree/main/agent-booster)                         |
...
| **ReasoningBank**         | Persistent learning memory system with semantic search                                          | 46% faster, 100% success                | [Docs](https://github.com/tafyai/agentopia/tree/main/agent-control-plane/src/reasoningbank) |
| **Multi-Model Router**    | Intelligent cost optimization across 100+ LLMs                                                  | 85-99% cost savings                     | [Docs](https://github.com/tafyai/agentopia/tree/main/agent-control-plane/src/router)        |
| **QUIC Transport**        | Ultra-low latency agent communication via Rust/WASM QUIC protocol                               | 50-70% faster than TCP, 0-RTT           | [Docs](https://github.com/tafyai/agentopia/tree/main/crates/agent-control-plane-quic)       |
...
- **Federation Hub**: `npx agentopia federation start` 🆕
...
npx agentopia --agent coder --task "Build a REST API" --optimize

# CLI: Federation Hub (ephemeral agents with persistent memory)
npx agentopia federation start       # Start hub server
npx agentopia federation spawn       # Spawn ephemeral agent
npx agentopia federation stats       # View statistics
...
import { ReflexionMemory, SkillLibrary, CausalMemoryGraph } from 'agentopia/agentdb';
...
import { SwarmLearningOptimizer, autoSelectSwarmConfig } from 'agentopia/hooks/swarm-learning-optimizer';
```

Built on **[Claude Agent SDK](https://docs.claude.com/en/api/agent-sdk)** by Anthropic, powered by **[GenDev](https://github.com/tafyai/gendev)** (101 MCP tools), **[Agentic Cloud](https://github.com/tafyai/agentic-cloud)** (96 cloud tools), **[OpenRouter](https://openrouter.ai)** (100+ LLM models), **[Google Gemini](https://ai.google.dev)** (fast, cost-effective inference), **[Agentic Payments](https://github.com/tafyai/agentopia/tree/main/agentic-payments)** (payment authorization), and **[ONNX Runtime](https://onnxruntime.ai)** (free local CPU or GPU inference).
...
**Documentation:** [Kubernetes Controller Guide](https://github.com/tafyai/agentopia/tree/main/packages/k8s-controller)
...
# Programmatic API
import { BillingSystem } from 'agentopia/billing';
...
**Documentation:** [Economic System Guide](https://github.com/tafyai/agentopia/tree/main/docs/ECONOMIC-SYSTEM-GUIDE.md)
...
**Documentation:** [Deployment Patterns Guide](https://github.com/tafyai/agentopia/tree/main/docs/DEPLOYMENT-PATTERNS-GUIDE.md)
...
**Documentation:** [agentic-jujutsu Package](https://github.com/tafyai/agentopia/tree/main/packages/agentic-jujutsu)
...
**Documentation:** [Healthcare AI Components](https://github.com/tafyai/agentopia/tree/main/src/consent)
...
**Documentation:** [Maternal Health Platform](https://github.com/tafyai/agentopia/tree/main/src/verification)
...
```bash
# Build container
docker build -f deployment/Dockerfile -t agentopia .
...
_Use `npx agentopia --list` to see all 150+ agents_
...
npx agentopia --agent coder --task "Build REST API" --optimize

# Optimize for lowest cost
npx agentopia --agent coder --task "Simple function" --optimize --priority cost

# Optimize for highest quality
npx agentopia --agent reviewer --task "Security audit" --optimize --priority quality

# Set maximum budget ($0.001 per task)
npx agentopia --agent coder --task "Code cleanup" --optimize --max-cost 0.001
```
...
- See [Model Capabilities Guide](https://github.com/tafyai/agentopia/blob/main/docs/agent-control-plane/benchmarks/MODEL_CAPABILITIES.md) for detailed analysis
...
npx agentopia --agent coder --task "Build REST API" --optimize
npx agentopia --agent coder --task "Fix bug" --provider openrouter --priority cost
...
npx agentopia mcp start   # Start MCP server
npx agentopia mcp list    # List 7 agentopia tools
npx agentopia mcp status  # Check server status

# Agent management
npx agentopia --list              # List all 79 agents
npx agentopia agent info coder    # Get agent details
npx agentopia agent create        # Create custom agent
```

**Built-in CLIs:**

- **agentopia**: Main agent execution and MCP server (7 tools)
...
npx agentopia quic --port 5000 --cert ./certs/cert.pem --key ./certs/key.pem
...
```javascript
import { QuicTransport } from 'agentopia/transport/quic';
import { getQuicConfig } from 'agentopia/dist/config/quic.js';
...
**Learn More:** [QUIC Documentation](https://github.com/tafyai/agentopia/tree/main/crates/agent-control-plane-quic)
...
**Manual**: `import { AgentBooster } from 'agentopia/agent-booster'` for direct control
...
| ----------------- | ------------------------- | --------------------------------------------------------------------------------------------------------- |
| **NPM Package**   | Install and usage         | [npmjs.com/package/agentopia](https://www.npmjs.com/package/agentopia)                                    |
| **Agent Booster** | Local code editing engine | [Agent Booster Docs](https://github.com/tafyai/agentopia/tree/main/agent-booster)                         |
| **ReasoningBank** | Learning memory system    | [ReasoningBank Docs](https://github.com/tafyai/agentopia/tree/main/agent-control-plane/src/reasoningbank) |
| **Model Router**  | Cost optimization system  | [Router Docs](https://github.com/tafyai/agentopia/tree/main/agent-control-plane/src/router)               |
| **MCP Tools**     | Complete tool reference   | [MCP Documentation](https://github.com/tafyai/agentopia/tree/main/docs/mcp)                               |
...
| **Agentic Payments** | Payment authorization  | [Payments Docs](https://github.com/tafyai/agentopia/tree/main/agentic-payments) |
...
We welcome contributions! Please see [CONTRIBUTING.md](https://github.com/tafyai/agentopia/blob/main/CONTRIBUTING.md) for guidelines.
...
MIT License - see [LICENSE](https://github.com/tafyai/agentopia/blob/main/LICENSE) for details.
...
- **Documentation**: See [docs/](https://github.com/tafyai/agentopia/tree/main/docs) folder
- **Issues**: [GitHub Issues](https://github.com/tafyai/agentopia/issues)
- **Discussions**: [GitHub Discussions](https://github.com/tafyai/agentopia/discussions)
...
```bash
npx agentopia --agent researcher --task "Your task here"
```
