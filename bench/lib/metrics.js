...
export class MetricsCollector {
  ...
  calculateImprovement(baseline, reasoningbank) {
    ...
    const successRatePercent =
      baseline.successRate > 0
        ...
        : reasoningbank.successRate > 0
          ? 100
          : 0;
    const tokenDelta = baseline.avgTokens - reasoningbank.avgTokens;
    const tokenSavings = baseline.avgTokens > 0 ? (tokenDelta / baseline.avgTokens) * 100 : 0;
    const latencyDelta = reasoningbank.avgLatency - baseline.avgLatency;
    const latencyOverhead =
      baseline.avgLatency > 0 ? (latencyDelta / baseline.avgLatency) * 100 : 0;
  ...
  generateInsights(results) {
    ...
    if (Math.abs(improvement.latencyDelta) < 500) {
      insights.push(
        `⚡ Minimal latency overhead: ${Math.abs(improvement.latencyDelta).toFixed(0)}ms`
      );
