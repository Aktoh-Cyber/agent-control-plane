...
**Generated with agent-control-plane v1.4.11**
...
function generateLearningInsights(learningCurve: any[]): string {
  ...
  const successImprovement =
    lastIteration.reasoningbankSuccess - firstIteration.reasoningbankSuccess;
  if (successImprovement > 0.1) {
    insights.push(
      `- ReasoningBank success rate improved by ${(successImprovement * 100).toFixed(1)}% across iterations (learning effect)`
    );
  }

  // Check baseline consistency
  const baselineVariance =
    learningCurve.reduce(
      (sum, point) => sum + Math.abs(point.baselineSuccess - firstIteration.baselineSuccess),
      0
  ...
  if (lastIteration.memoriesAvailable > 0) {
    insights.push(
      `- Memory bank grew to ${lastIteration.memoriesAvailable} memories by final iteration`
    );
  }

  // Check token efficiency over time
  const tokenReduction =
    (firstIteration.reasoningbankTokens - lastIteration.reasoningbankTokens) /
    firstIteration.reasoningbankTokens;
  if (tokenReduction > 0.1) {
    insights.push(
      `- Token usage improved by ${(tokenReduction * 100).toFixed(1)}% as memory bank matured`
    );
  }
...
export function generateCSVReport(report: BenchmarkReport): string {
  let csv =
    'Scenario,Agent,Success Rate,Avg Tokens,Avg Latency,Total Tasks,Successful Tasks,Memories Created,Memories Used\n';
