...
const tasks = [
  {
    ...
    description:
      'Find and fix the bug in this array iteration function that causes an index out of bounds error.',
    domain: 'debugging',
  ...
  },
  {
    ...
    description:
      'Fix the race condition in this async function that sometimes returns incorrect results.',
    domain: 'debugging',
    input: `let counter = 0;
...
// Bug: isEqual(0, "") returns true when it should return false`,
  ...
  },
  {
    ...
    description:
      'Fix the closure bug that causes all event handlers to reference the same variable.',
    domain: 'debugging',
  ...
  return user.name.toUpperCase();
    ...
    successCriteria: (result) => {
      const code = result.output;
      return (
        code.includes('?') || code.includes('||') || code.includes('if') || code.includes('??')
      );
    },
    difficulty: 'easy',
  },
  ...
  },
  {
    ...
    description:
      'Fix the bug where the function mutates the input instead of returning a new object.',
    domain: 'debugging',
...
export const debuggingTasksScenario = {
  ...
  domain: 'debugging',
  tasks,
  metrics: ['success_rate', 'token_efficiency', 'latency', 'memory_usage', 'accuracy'],
};
