...
import { db } from '../agent-control-plane/src/reasoningbank/index.js';
...
class BenchmarkOrchestrator {
  ...
  private async runScenario(scenario: BenchmarkScenario): Promise<ScenarioResults> {
    ...
    for (let iteration = 0; iteration < this.config.iterations; iteration++) {
      ...
      console.log(
        `    └─ ReasoningBank: ${reasoningbankSuccesses}/${totalTasks} success (${reasoningbankTokens.toLocaleString()} tokens, ${memoryStats.total} memories)`
      );
    }
    ...
    console.log('\n  📊 Scenario Results:');
    console.log(
      `    Success Rate: ${(results.baseline.successRate * 100).toFixed(1)}% → ${(results.reasoningbank.successRate * 100).toFixed(1)}% (${results.improvement.successRateDelta})`
    );
    ...
    if (results.improvement.learningVelocity) {
      console.log(
        `    Learning Velocity: ${results.improvement.learningVelocity.toFixed(1)}x faster`
      );
    }
