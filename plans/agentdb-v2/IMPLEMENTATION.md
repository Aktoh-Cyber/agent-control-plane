  ...
  async load(path: string): Promise<void> {
    ...
    try {
      ...
      this.labelToId = new Map(
        Object.entries(data.labelToId).map(([k, v]) => [Number(k), v as string])
      );
...
export class RuVectorLearning {
  ...
  async initialize(): Promise<void> {
    ...
    this.gnnLayer = new GNNLayer(this.config.inputDim, this.config.outputDim, this.config.heads);
  }
...
export class ReasoningBankV2 {
  ...
  private hydratePatterns(results: any[]): ReasoningPattern[] {
    return results.map((r) => {
      const row = this.db.prepare('SELECT * FROM reasoning_patterns WHERE id = ?').get(r.id);
