...
Deploy agent-control-plane to Google Cloud Platform with Docker containers, Cloud Run, and Secret Manager integration.
...
```bash
# Get service URL
SERVICE_URL=$(gcloud run services describe agent-control-plane \
  --region=us-central1 \
...
# Scale services
docker-compose up -d --scale agent-control-plane-openrouter=3
```
...
gcloud run services update agent-control-plane \
...
```bash
# View logs
gcloud logging read "resource.type=cloud_run_revision AND resource.labels.service_name=agent-control-plane" \
  --limit=50 \
  --format=json
...
```bash
# Increase memory
gcloud run services update agent-control-plane --memory=4Gi

# Or reduce concurrency
gcloud run services update agent-control-plane --concurrency=40
```
...
gcloud run services describe agent-control-plane --format="value(spec.template.spec.serviceAccountName)"
```
...
for region in us-central1 europe-west1 asia-east1; do
  gcloud run deploy agent-control-plane-$region \
    --image=gcr.io/$PROJECT_ID/agent-control-plane \
    --region=$region \
    --platform=managed
done

# Setup global load balancer
gcloud compute backend-services create agent-control-plane-backend \
  --global
```
