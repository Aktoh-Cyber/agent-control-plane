...
if [ -d "agent-control-plane/dist" ]; then
    print_pass "agent-control-plane/dist/ exists"

    if [ -f "agent-control-plane/dist/index.js" ]; then
        print_pass "Main entry point exists"
    else
        print_fail "Main entry point (index.js) missing"
    fi

    if [ -f "agent-control-plane/dist/index.d.ts" ]; then
        print_pass "TypeScript declarations exist"
    else
        print_warn "TypeScript declarations missing"
    fi
else
    print_fail "agent-control-plane/dist/ not found - run 'npm run build'"
fi

# 5. Check bin files
print_check "CLI executables"
if [ -f "agent-control-plane/dist/cli-proxy.js" ]; then
    if head -1 "agent-control-plane/dist/cli-proxy.js" | grep -q "^#!/usr/bin/env node"; then
        print_pass "agent-control-plane CLI has shebang"
    else
        print_warn "agent-control-plane CLI missing shebang"
    fi
else
    print_fail "agent-control-plane CLI not found"
fi

if [ -f "agent-control-plane/dist/agentdb/cli/agentdb-cli.js" ]; then
