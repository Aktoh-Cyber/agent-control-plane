 ...
 * Tests different swarm configurations and sizes for agent-control-plane
 */
...
console.log(
  `✓ E2B API Key: ${API_KEY.substring(0, 10)}...${API_KEY.substring(API_KEY.length - 4)}\n`
);

/**
 * Test mesh topology swarm coordination
 */
async function benchmarkMeshSwarm(
...
!npx gendev@alpha agent spawn \\
  ...
  --agent-id "mesh-agent-${i}"
    ...
    await sandbox.notebook.execCell(`
!npx gendev@alpha task orchestrate \\
  --swarm-id "e2b-mesh-${agentCount}" \\
 ...
 */
async function benchmarkHierarchicalSwarm(
  sandbox: CodeInterpreter,
  agentCount: number
): Promise<BenchmarkResult> {
...
!npx gendev@alpha agent spawn \\
  ...
  --agent-id "worker-${i}"
    ...
    console.log(
      `   ✓ Agents spawned: ${successfulSpawns}/${agentCount} (1 queen + ${workerCount} workers)`
    );
 ...
 */
async function benchmarkAdaptiveSwarm(
  sandbox: CodeInterpreter,
  agentCount: number
): Promise<BenchmarkResult> {
...
async function runBenchmarkSuite(): Promise<void> {
  ...
  try {
    ...
    // Install agent-control-plane in sandbox
    console.log('📥 Installing agent-control-plane...');
    await sandbox.notebook.execCell('!npm install -g claude-flow@alpha');
    console.log('✓ agent-control-plane installed\n');
    ...
    for (const [topology, topologyResults] of Object.entries(byTopology)) {
      ...
      // Calculate averages
      const avgDuration =
        topologyResults.reduce((sum, r) => sum + r.duration, 0) / topologyResults.length;
      const avgLatency =
        topologyResults.reduce((sum, r) => sum + r.coordinationLatency, 0) / topologyResults.length;
      const avgMemory =
        topologyResults.reduce((sum, r) => sum + r.memoryUsage, 0) / topologyResults.length;
      const avgCompletion =
        topologyResults.reduce((sum, r) => sum + r.taskCompletionRate, 0) / topologyResults.length;
    ...
    for (const topology of ['mesh', 'hierarchical', 'adaptive']) {
      ...
      if (topologyData && topologyData.length >= 2) {
        ...
        console.log(
          `  Scaling: ${first.agentCount} → ${last.agentCount} agents (${scalingFactor}x)`
        );
    ...
    const report = {
      ...
      summary: {
        ...
        avgCoordinationLatency:
          results.reduce((sum, r) => sum + r.coordinationLatency, 0) / results.length,
      },
      results,
    };
    ...
    const fs = require('fs');
    const localPath =
      '/home/user/agent-control-plane/test-reports/swarm-scalability-benchmark.json';
