...
function runTestSuite(): void {
  ...
  const sortedByLatency = [...results].sort(
    (a, b) => (a.metrics.networkLatency || 9999) - (b.metrics.networkLatency || 9999)
  );
  ...
  const sortedByRecovery = [...results].sort(
    (a, b) => (a.metrics.recoveryTime || 9999) - (b.metrics.recoveryTime || 9999)
  );
  ...
  const report = {
    ...
    rankings: {
      ...
      byLatency: sortedByLatency.map((r) => ({
        pattern: r.pattern,
        latency: r.metrics.networkLatency,
      })),
      byRecovery: sortedByRecovery.map((r) => ({
        pattern: r.pattern,
        recovery: r.metrics.recoveryTime,
      })),
    },
  };
