...
function runBenchmarkSuite(): void {
  ...
  for (const [topology, topologyResults] of Object.entries(byTopology)) {
    ...
    const avgThroughput =
      topologyResults.reduce((sum, r) => sum + r.throughput, 0) / topologyResults.length;
    const avgCompletion =
  ...
  const avgByTopology = Object.entries(byTopology)
    .map(([topology, results]) => ({
      topology,
  ...
  avgByTopology
    ...
    .forEach((item, index) => {
      const medal = index === 0 ? '🥇' : index === 1 ? '🥈' : '🥉';
      console.log(
        `  ${medal} ${item.topology.padEnd(15)}: ${item.avgThroughput.toFixed(2)} tasks/s`
      );
    });
  ...
  // Write report
  const reportDir = '/home/user/agent-control-plane/test-reports';
